...
public class OrderedPairlist<C extends RingElem<C>> {
  ...
  public OrderedPairlist(int m, MultiVarPowerSeriesRing<C> r) {
    ...
    pairlist =
        new TreeMap<ExpVector, LinkedList<Pair<C>>>(ring.polyRing().tord.getAscendComparator());
    // pairlist = new TreeMap<ExpVector,
    // LinkedList<Pair<C>>>(ring.polyRing().tord.getDescendComparator());
  ...
  /**
   * Remove the next required pair from the pairlist and reduction matrix. Apply the criterions 3
   * and 4 to see if the S-power-series is required.
   *
  ...
  public synchronized Pair<C> removeNext() {
    ...
    while (!c && ip.hasNext()) {
      ...
      if (logger.isInfoEnabled()) logger.info("g  = " + g);
      pair = null;
      while (!c && xl.size() > 0) {
        pair = xl.removeFirst();
