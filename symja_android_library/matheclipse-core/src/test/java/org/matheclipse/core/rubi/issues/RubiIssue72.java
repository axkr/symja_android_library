...
public class RubiIssue72 extends AbstractRubiTestCase {
	...
	public void test0009() {
		check("Rubi`AbsurdNumberGCDList[Rubi`FactorAbsurdNumber[First[{1, 1}]], Rubi`FactorAbsurdNumber[Rubi`AbsurdNumberGCD @@ Rest[{1, 1}]]]",
				"1");
	}

	public void test0010() {
		check("Rubi`AbsurdNumberGCDList[Rubi`FactorAbsurdNumber[First[{2, 2}]], Rubi`FactorAbsurdNumber[Rubi`AbsurdNumberGCD @@ Rest[{2, 2}]]]",
				"2");
	}
	...
	public void test0164() {
		check("Rubi`Simp[Plus @@ (Rubi`Coeff[(2 + 2*x)/(2*Sqrt[2]), x, #1]*x^#1 & ) /@ Rubi`Expon[(2 + 2*x)/(2*Sqrt[2]), x, List], x]", //
				"1/Sqrt[2] + x/Sqrt[2]");
	}
