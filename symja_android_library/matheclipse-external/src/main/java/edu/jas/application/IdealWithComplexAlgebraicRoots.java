...
public class IdealWithComplexAlgebraicRoots<D extends GcdRingElem<D> & Rational>
   ...
   */
  public IdealWithComplexAlgebraicRoots(
      Ideal<D> id, List<GenPolynomial<D>> up, List<List<Complex<RealAlgebraicNumber<D>>>> cr) {
    super(id, up);
    can = cr;
  }
  ...
  public synchronized boolean isDecimalApproximation() {
    ...
    Complex<BigDecimal> c =
        new Complex<BigDecimal>(dr, new BigDecimal("0.15").power(BigDecimal.DEFAULT_PRECISION / 2));
    c = c.norm();
    // System.out.println("eps: c = " + c);
    Complex<BigDecimal> cc =
        new Complex<BigDecimal>(dr, new BigDecimal("0.1").power(BigDecimal.DEFAULT_PRECISION / 3));
    cc = cc.norm();

    ComplexRing<D> cr = new ComplexRing<D>(ideal.list.ring.coFac);
    List<GenPolynomial<Complex<BigDecimal>>> upds =
        new ArrayList<GenPolynomial<Complex<BigDecimal>>>(upolys.size());
    for (GenPolynomial<D> up : upolys) {
      ...
      GenPolynomialRing<Complex<BigDecimal>> dpfac =
          new GenPolynomialRing<Complex<BigDecimal>>(dr, cpfac);
    ...
    c =
        new Complex<BigDecimal>(
            dr, new BigDecimal("0.15").power(BigDecimal.DEFAULT_PRECISION / 2 - 1));
