...
public class GroebnerBaseFGLM<C extends GcdRingElem<C>> extends GroebnerBaseAbstract<C> {
  ...
  public List<GenPolynomial<C>> GB(int modv, List<GenPolynomial<C>> F) {
    ...
    GenPolynomialRing<C> gfac =
        new GenPolynomialRing<C>(pfac.coFac, pfac.nvar, tord, pfac.getVars());
  ...
  /**
   * Algorithm CONVGROEBNER: Converts Groebner bases w.r.t. total degree termorder into Groebner
   * base w.r.t to inverse lexicographical term order
   *
  ...
  public List<GenPolynomial<C>> convGroebnerToLex(List<GenPolynomial<C>> groebnerBasis) {
    ...
    String[] ArrayOfVariables =
        ring.getVars(); // Variables of given polynomial ring w.r.t. to input G
    ...
    GenPolynomialRing<C> ufac =
        new GenPolynomialRing<C>(cfac, numberOfVariables, invlex, ArrayOfVariables);

    // Local Lists
    List<GenPolynomial<C>> newGB =
        new ArrayList<GenPolynomial<C>>(); // Instantiate the return list of polynomials
    List<GenPolynomial<C>> H =
        new ArrayList<GenPolynomial<C>>(); // Instantiate a help list of polynomials
    List<GenPolynomial<C>> redTerms =
        new ArrayList<GenPolynomial<C>>(); // Instantiate the return list of reduced terms
   ...
   * @return Term that specifies condition (D) or null (Condition (D) in "A computational approach
   *     to commutative algebra", Becker, Weispfenning, Kredel 1993, p. 427)
   */
  public GenPolynomial<C> lMinterm(List<GenPolynomial<C>> G, GenPolynomial<C> t) {
    ...
    GenPolynomial<C> u =
        new GenPolynomial<C>(
            ring,
            t.leadingBaseCoefficient(),
            t.leadingExpVector()); // HeadTerm of of input polynomial
    ReductionSeq<C> redHelp =
        new ReductionSeq<C>(); // Create instance of ReductionSeq to use method isReducible
    // not ok: if ( redHelp.isTopReducible(G,u) ) ...
    for (int i = numberOfVariables - 1;
        i >= 0;
        i--) { // Walk through all variables, starting with least w.r.t to lex-order
      ...
      GenPolynomial<C> s =
          ring.univariate(
              i, u.degree(numberOfVariables - (i + 1))); // if not, eliminate variable x_i
      u = u.divide(s);
    }
    return null;
  }
  ...
  public List<GenPolynomial<C>> redTerms(List<GenPolynomial<C>> groebnerBasis) {
    ...
    int numberOfVariables = ring.nvar; // Number of Variables of the given Polynomial Ring
    long[] degrees =
        new long[numberOfVariables]; // Array for the degree-limits for the reduced terms

    List<GenPolynomial<C>> terms =
        new ArrayList<GenPolynomial<C>>(); // Instantiate the return object
    ...
    for (int i = 0; i < numberOfVariables; i++) {
      ...
      List<GenPolynomial<C>> S =
          new ArrayList<GenPolynomial<C>>(
              terms); // Copy all entries of return list "terms" into list "S"
  ...
  /**
   * Internal method to create a polynomial ring in i indeterminates. Create new ring over
   * coefficients of ring with i variables Y1,...,Yi (indeterminate)
   *
  ...
  GenPolynomialRing<C> createRingOfIndeterminates(GenPolynomialRing<C> ring, int i) {
    ...
    TermOrder invlex = new TermOrder(TermOrder.INVLEX);
    GenPolynomialRing<C> cpfac =
        new GenPolynomialRing<C>(cfac, indeterminates, invlex, stringIndeterminates);
    return cpfac;
  }

  /**
   * Internal method to add new indeterminates. Add another variabe (indeterminate) Y_{i+1} to
   * existing ring
   *
   * @return polynomial ring with variables Y1,..,Yi,Yi+1 and coefficients of ring.
   */
