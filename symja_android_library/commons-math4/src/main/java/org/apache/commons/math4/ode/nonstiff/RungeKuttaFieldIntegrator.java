...
public abstract class RungeKuttaFieldIntegrator<T extends RealFieldElement<T>>
    ...
    public FieldODEStateAndDerivative<T> integrate(final FieldExpandableODE<T> equations,
        ...
        if (forward) {
            if (getStepStart().getTime().sum(step).subtract(finalTime).getReal() >= 0) {
        ...
        do {
            ...
            for (int k = 1; k < stages; ++k) {
                ...
                for (int j = 0; j < y0.length; ++j) {
                    ...
                    for (int l = 1; l < k; ++l) {
                        sum = sum.sum(yDotK[l][j].multiply(a[k-1][l]));
                    }
                    yTmp[j] = y[j].sum(getStepSize().multiply(sum));
                }

                yDotK[k] = computeDerivatives(getStepStart().getTime().sum(getStepSize().multiply(c[k-1])), yTmp);
            ...
            for (int j = 0; j < y0.length; ++j) {
                ...
                for (int l = 1; l < stages; ++l) {
                    sum = sum.sum(yDotK[l][j].multiply(b[l]));
                }
                yTmp[j] = y[j].sum(getStepSize().multiply(sum));
            }
            final T stepEnd   = getStepStart().getTime().sum(getStepSize());
            ...
            if (!isLastStep()) {
                ...
                final T  nextT      = getStepStart().getTime().sum(getStepSize());
    ...
    public T[] singleStep(final FirstOrderFieldDifferentialEquations<T> equations,
        ...
        for (int k = 1; k < stages; ++k) {
            ...
            for (int j = 0; j < y0.length; ++j) {
                ...
                yTmp[j] = y[j].sum(h.multiply(sum));
            }

            yDotK[k] = equations.computeDerivatives(t0.sum(h.multiply(c[k-1])), yTmp);
