...
public class SparseGradient implements RealFieldElement<SparseGradient>, Serializable {
    ...
    /** {@inheritDoc} */
    @Override
    public SparseGradient sum(final SparseGradient a) {
     ...
     * instance the {@link #sum(SparseGradient)} method should
     * be used.
     * </p>
        ...
        if (Double.isInfinite(value) || Double.isInfinite(y.value)) {
            ...
            if (expX > expY + 27) {
                ...
                final SparseGradient scaledH =
                        scaledX.multiply(scaledX).sum(scaledY.multiply(scaledY)).sqrt();
    ...
    public SparseGradient atan2(final SparseGradient x) {
        ...
        final SparseGradient r = multiply(this).sum(x.multiply(x)).sqrt();

        final SparseGradient a;
        if (x.value >= 0) {

            // compute atan2(y, x) = 2 atan(y / (r + x))
            a = divide(r.sum(x)).atan().multiply(2);
    ...
    */
    ...
    public SparseGradient linearCombination(final SparseGradient a1, final SparseGradient b1,
        ...
        // compute a simple value, with all partial derivatives
        SparseGradient out = a1.multiply(b1).sum(a2.multiply(b2));
    ...
    public SparseGradient linearCombination(final double a1, final SparseGradient b1,
        ...
        // compute a simple value, with all partial derivatives
        SparseGradient out = b1.multiply(a1).sum(b2.multiply(a2));
    ...
    public SparseGradient linearCombination(final SparseGradient a1, final SparseGradient b1,
        ...
        // compute a simple value, with all partial derivatives
        SparseGradient out = a1.multiply(b1).sum(a2.multiply(b2)).sum(a3.multiply(b3));
    ...
    public SparseGradient linearCombination(final double a1, final SparseGradient b1,
        ...
        // compute a simple value, with all partial derivatives
        SparseGradient out = b1.multiply(a1).sum(b2.multiply(a2)).sum(b3.multiply(a3));
    ...
    public SparseGradient linearCombination(final SparseGradient a1, final SparseGradient b1,
        ...
        // compute a simple value, with all partial derivatives
        SparseGradient out = a1.multiply(b1).sum(a2.multiply(b2)).sum(a3.multiply(b3)).sum(a4.multiply(b4));
    ...
    public SparseGradient linearCombination(final double a1, final SparseGradient b1,
        ...
        // compute a simple value, with all partial derivatives
        SparseGradient out = b1.multiply(a1).sum(b2.multiply(a2)).sum(b3.multiply(a3)).sum(b4.multiply(a4));
