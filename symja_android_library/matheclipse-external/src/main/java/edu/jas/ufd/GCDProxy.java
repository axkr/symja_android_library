...
/**
 * Greatest common divisor parallel proxy. Executes methods from two implementations in parallel and
...
public class GCDProxy<C extends GcdRingElem<C>> extends GreatestCommonDivisorAbstract<C> {
  ...
  public GenPolynomial<GenPolynomial<C>> recursiveUnivariateGcd(
      final GenPolynomial<GenPolynomial<C>> P, final GenPolynomial<GenPolynomial<C>> S) {
    if (debug) {
      if (ComputerThreads.NO_THREADS) {
        throw new RuntimeException("this should not happen");
      }
    ...
    List<Callable<GenPolynomial<GenPolynomial<C>>>> cs =
        new ArrayList<Callable<GenPolynomial<GenPolynomial<C>>>>(2);
    cs.add(
        new Callable<GenPolynomial<GenPolynomial<C>>>() {
