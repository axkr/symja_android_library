...
public class Hypergeometric2F1RegularizedRules {
  final public static IAST RULES = List(
      ...
      ISetDelayed(Hypergeometric2F1Regularized(a_, b_, c_, C0), Power(Gamma(c), CN1)),
      ...
      ISetDelayed(Hypergeometric2F1Regularized(a_, b_, c_, z_), Module(list(Set(n, Subtract(b, c))),
          Condition(Times(Power(Subtract(C1, z), Subtract(Negate(a), n)),
              Sum(Times(Power(z, k), Power(Times(Gamma(Plus(b, Negate(n), k)), Factorial(k)), CN1),
                  Pochhammer(Plus(Negate(a), b, Negate(n)), k), Pochhammer(Negate(n), k)),
                  list(k, C0, n))),
              And(GreaterEqual(n, C0), Element(n, Integers))))));
}
