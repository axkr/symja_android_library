...
/**
 * RelationTable for solvable polynomials. This class maintains the non-commutative multiplication
 * relations of solvable polynomial rings. The table entries are initialized with relations of the
 * form x<sub>j</sub> * x<sub>i</sub> = p<sub>ij</sub>. During multiplication the relations are
 * updated by relations of the form x<sub>j</sub><sup>k</sup> * x<sub>i</sub><sup>l</sup> =
 * p<sub>ijkl</sub>. If no relation for x<sub>j</sub> * x<sub>i</sub> is found in the table, this
 * multiplication is assumed to be commutative x<sub>i</sub> x<sub>j</sub>. Can also be used for
...
public class RelationTable<C extends RingElem<C>> implements Serializable {
  ...
  /**
   * Constructor for RelationTable requires ring factory. Note: This constructor is called within
   * the constructor of the ring factory, so methods of this class can only be used after the other
   * constructor has terminated.
   *
  ...
  @SuppressWarnings("unchecked")
  boolean equalMaps(
      Map<ExpVectorPair, GenPolynomial<C>> m1, Map<ExpVectorPair, GenPolynomial<C>> m2) {
  ...
  public synchronized void update(ExpVector e, ExpVector f, GenSolvablePolynomial<C> p) {
    ...
    if (!coeffTable) { // old case
      ...
      if (!e.equals(lp)) { // check for suitable term order
        logger.error("relation term order = " + ring.tord);
        throw new IllegalArgumentException(
            "Coefficient RelationTable update e != lt(p): "
                + sring.toScript(e)
                + " != "
                + sring.toScript(lp));
      }
      if (p.leadingBaseCoefficient() instanceof GenPolynomial) {
        ...
        if (!f.equals(lp)) { // check for suitable term order
          logger.error("relation term order = " + ring.tord);
          logger.error(
              "Coefficient RelationTable update f != lt(lfcd(p)): "
                  + sring.toScript(e)
                  + ", f = "
                  + f
                  + ", p = "
        ...
        lp =
            ((GenWordPolynomial<C>) (Object) p.leadingBaseCoefficient())
                .leadingWord()
                .leadingExpVector();
  ...
  /**
   * Lookup RelationTable for existing relation. Find p with e * f = p. If no relation for e * f is
   * contained in the table then return the symmetric product p = 1 e f.
   *
   * @param e first term.
   * @param f second term.
   * @return t table relation container, contains e' and f' with e f = e' lt(p) f'.
   */
  @SuppressWarnings({"unchecked", "cast"})
        ...
        if (evp.isMultiple(look)) {
          ...
          if (debug) {
            if (p != null && p.ring.vars != null) {
              logger.info(
                  "found relation = "
                      + e.toString(p.ring.vars)
                      + " .*. "
                      + f.toString(p.ring.vars)
                      + " = "
                      + p);
            } else {
  ...
  /**
   * Extend variables. Used e.g. in module embedding. Extend all ExpVectors and polynomials of the
   * given relation table by i elements and put the relations into this table, i.e. this should be
   * empty.
   *
  ...
  /**
   * Contract variables. Used e.g. in module embedding. Contract all ExpVectors and polynomials of
   * the given relation table by i elements and put the relations into this table, i.e. this should
   * be empty.
   *
  ...
  /**
   * Recursive representation. Split all ExpVectors and polynomials of the given relation table to
   * lower elements and upper elements and put the relations into this table or this as coefficient
   * table, i.e. this should be empty.
   *
   * @param tab a relation table to be contracted and inserted into this.
   */
  @SuppressWarnings({"unchecked", "cast"})
    ...
    for (Object okey : tab.table.keySet()) {
      ...
      for (Iterator jt = val.iterator(); jt.hasNext(); ) {
        ...
        if (mc.size() == 1) { // < 1 only for p == 0
          ...
          if (coeffTable) {
            String qrs =
                ring.toScript(ec) + " * " + qr.leadingBaseCoefficient() + " = " + qr.toScript();
  ...
  /**
   * Reverse variables and relations. Used e.g. in opposite rings. Reverse all ExpVectors and
   * polynomials of the given relation table and put the modified relations into this table, i.e.
   ...
   *
   * @param rel = (e1,f1,p1, ...) where ei * fi = pi are solvable relations. <b>Note:</b> Only
   *     because of type erasure, aequivalent to addRelations().
   */
    ...
    while (relit.hasNext()) {
      ...
      if (coeffTable) {
        ...
        if (ring.coFac instanceof GenPolynomialRing) {
          ...
          f =
              ((GenWordPolynomial<C>) (Object) F.leadingBaseCoefficient())
                  .leadingWord()
                  .leadingExpVector();
        }
      } else {
