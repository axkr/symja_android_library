...
/**
 * GenMatrix implements a generic matrix algebra over RingElem entries. Matrix has n columns and m
 * rows over C.
 *
 * @author Heinz Kredel
 */
public class GenMatrix<C extends RingElem<C>> implements AlgebraElem<GenMatrix<C>, C> {
  ...
  /**
   * Test if this is a unit. I.e. there exists x with this.multiply(x).isONE() == true. Tests if all
   * diagonal elements are units and all other elements are zero.
   *
