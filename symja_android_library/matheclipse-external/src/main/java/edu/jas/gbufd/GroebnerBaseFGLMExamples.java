...
public class GroebnerBaseFGLMExamples {
  ...
  // field Q
  String all =
      "Zahlbereich | Ordnung    | Elements G | Elements L | bitHeight G | bitHeight L | Deg G | Deg L | Time G | Time FGLM | Time L";
  ...
  String fglm =
      "Zahlbereich | Ordnung      | Elements G | Elements L  | bitHeight G | bitHeight L |  Deg G | Deg L | Time G | Time FGLM";

  // MOD 1831
  String modAll =
      "Zahlbereich | Ordnung    | Elements G | Elements L | Deg G | Deg L | Time G | Time FGLM | Time L";

  // String modGrad = "Zahlbereich | Ordnung      | Elements G | Deg G | Time G";

  String modfglm =
      "Zahlbereich | Ordnung      | Elements G | Elements L | Deg G | Deg L | Time G | Time FGLM";
  ...
  */
  ...
  // ===================================================================
  // Examples taken from "Efficient Computation of Zero-Dimensional Gröbner Bases by Change of
  // Ordering",
  ...
  public void xtestModC5() {
    String polynomials =
        "( "
            ...
            + "(a*b + b*c + c*d + a*e + d*e),"
            + "(a*b*c + b*c*d + a*b*e + a*d*e + c*d*e),"
            + "(b*c*d + a*b*c*e + a*b*d*e + a*c*d*e + b*c*d*e),"
            + "(a*b*c*d*e -1)"
            + ") ";
  ...
  // ===================================================================
  // Examples taken from "Der FGLM-Algorithmus: verallgemeinert und implementiert in SINGULAR",
  // 1997, Wichmann
  // ===================================================================
  ...
  public void ytestWalkS7() {
    String polynomials =
        "( "
            + " (2*g*b + 2*f*c + 2*e*d + a^2 + a),"
            + "(2*g*c + 2*f*d + e^2 + 2*b*a + b),"
            + "(2*g*d + 2*f*e + 2*c*a + c + b^2),"
            + "(2*g*e + f^2 + 2*d*a + d + 2*c*b),"
            + "(2*g*f + 2*e*a + e + 2*d*b + c^2),"
            + "(g^2 + 2*f*a + f + 2*e*b + 2*d*c),"
            + "(2*g*a + g + 2*f*b + 2*e*c + d^2)"
            + ") ";
  ...
  public void ytestOmdi1() {
    String polynomials =
        "( "
            + " (a + c + v + 2*x - 1),"
            + "(a*b + c*u + 2*v*w + 2*x*y + 2*x*z -2/3),"
            + "(a*b^2 + c*u^2 + 2*v*w^2 + 2*x*y^2 + 2*x*z^2 - 2/5),"
            + "(a*b^3 + c*u^3 + 2*v*w^3 + 2*x*y^3 + 2*x*z^3 - 2/7),"
            + "(a*b^4 + c*u^4 + 2*v*w^4 + 2*x*y^4 + 2*x*z^4 - 2/9),"
            + "(v*w^2 + 2*x*y*z - 1/9),"
            + "(v*w^4 + 2*x*y^2*z^2 - 1/25),"
            + "(v*w^3 + 2*x*y*z^2 + x*y^2*z - 1/15),"
  ...
  // ===================================================================
  // Examples taken from "Some Examples for Solving Systems of Algebraic Equations by Calculating
  // Gröbner Bases", 1984, Boege, Gebauer, Kredel
  // ===================================================================
  ...
  public void xtestHairerRungeKutta_1() {
    String polynomials =
        "( " + " (a-f),(b-h-g),(e+d+c-1),(d*a+c*b-1/2),(d*a^2+c*b^2-1/3),(c*g*a-1/6)" + ") ";
  ...
  public String all(String order, String polynomials) {
    ...
    OrderedPolynomialList<BigRational> o1 =
        new OrderedPolynomialList<BigRational>(GG.get(0).ring, GG);
    OrderedPolynomialList<BigRational> o2 =
        new OrderedPolynomialList<BigRational>(resultFGLM.get(0).ring, resultFGLM);
    // List<GenPolynomial<BigRational>> resultBuchberger = GL;
    OrderedPolynomialList<BigRational> o3 =
        new OrderedPolynomialList<BigRational>(GL.get(0).ring, GL);

    int grad_numberOfElements = GG.size();
    int lex_numberOfElements = resultFGLM.size();
    long grad_maxPolyGrad =
        PolyUtil.<BigRational>totalDegreeLeadingTerm(GG); // IdealObjectFGLM.maxDegreeOfGB();
    long lex_maxPolyGrad =
        PolyUtil.<BigRational>totalDegreeLeadingTerm(GL); // IdealObjectLex.maxDegreeOfGB();
    ...
    String erg =
        "BigRational |"
            + order
            + " |"
            + grad_numberOfElements
            + "          |"
            + lex_numberOfElements
            + "          |"
            + grad_height
            + "   |"
            + lex_height
  ...
  public String fglm(String order, String polynomials) {
    ...
    int lex_numberOfElements = resultFGLM.size();
    long grad_maxPolyGrad =
        PolyUtil.<BigRational>totalDegreeLeadingTerm(GG); // IdealObjectGrad.maxDegreeOfGB();
    long lex_maxPolyGrad =
        PolyUtil.<BigRational>totalDegreeLeadingTerm(resultFGLM); // IdealObjectLex.maxDegreeOfGB();
  ...
  public String grad(String order, String polynomials) {
    ...
    OrderedPolynomialList<BigRational> o4 =
        new OrderedPolynomialList<BigRational>(reducedTerms.get(0).ring, reducedTerms);
  ...
  public String modAll(String order, String polynomials, Integer m) {
    ...
    OrderedPolynomialList<ModInteger> o1 =
        new OrderedPolynomialList<ModInteger>(GG.get(0).ring, GG);
    OrderedPolynomialList<ModInteger> o2 =
        new OrderedPolynomialList<ModInteger>(resultFGLM.get(0).ring, resultFGLM);
    List<GenPolynomial<ModInteger>> resultBuchberger = GL;
    OrderedPolynomialList<ModInteger> o3 =
        new OrderedPolynomialList<ModInteger>(resultBuchberger.get(0).ring, resultBuchberger);

    int grad_numberOfElements = GG.size();
    int lex_numberOfElements = resultFGLM.size();
    long grad_maxPolyGrad =
        PolyUtil.<ModInteger>totalDegreeLeadingTerm(GG); // IdealObjectFGLM.maxDegreeOfGB();
    long lex_maxPolyGrad =
        PolyUtil.<ModInteger>totalDegreeLeadingTerm(GL); // IdealObjectLex.maxDegreeOfGB();
  ...
  public String modfglm(String order, String polynomials, Integer m) {
    ...
    long grad_maxPolyGrad =
        PolyUtil.<ModInteger>totalDegreeLeadingTerm(GG); // IdealObjectFGLM.maxDegreeOfGB();
    long lex_maxPolyGrad =
        PolyUtil.<ModInteger>totalDegreeLeadingTerm(resultFGLM); // IdealObjectLex.maxDegreeOfGB();
  ...
  /**
   * Method bitHeight returns the bitlength of the greatest number occurring during the computation
   * of a Groebner base.
   */
  public int bitHeight(List<GenPolynomial<BigRational>> list) {
    ...
    for (GenPolynomial<BigRational> g : list) {
      for (Monomial<BigRational> m : g) {
        ...
        if (i.compareTo(denom) > 0) denom = i;
        if (j.compareTo(num) > 0) num = j;
      }
    }
    int erg;
