...
/**
 * Multivariate power series ring implementation. Uses lazy evaluated generating function for
 * coefficients.
 *
...
public class MultiVarPowerSeriesRing<C extends RingElem<C>>
    ...
    if (vars == null) {
      ...
      if (vars.length != nvar) {
        throw new IllegalArgumentException(
            "incompatible variable size " + vars.length + ", " + nvar);
      }
      // GenPolynomialRing.addVars(vars);
    }
    EVZERO = ExpVector.create(nvar);
    ONE =
        new MultiVarPowerSeries<C>(
            this,
            new MultiVarCoefficients<C>(this) {
  ...
  public MultiVarPowerSeries<C> fromPolynomial(GenPolynomial<C> a) {
    ...
    return new MultiVarPowerSeries<C>(
        this,
        new MultiVarCoefficients<C>(pfac, cache, check) {
  ...
  public List<MultiVarPowerSeries<C>> fromPolynomial(List<GenPolynomial<C>> A) {
    return ListUtil.<GenPolynomial<C>, MultiVarPowerSeries<C>>map(
        A,
