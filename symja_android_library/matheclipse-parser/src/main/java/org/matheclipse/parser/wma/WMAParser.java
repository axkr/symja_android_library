...
/**
 * Parser implementation specific to Wolfram Mathematical Assistant (WMA) syntax. This extends the
 * base Parser functionality with WMA-specific operator handling and syntax rules.
 */
public class WMAParser extends WMAScanner {
  ...
  /**
   * Create a WMA parser with default settings
   */
  public WMAParser() {
   ...
   *        ASTNodes
   */
  public WMAParser(boolean packageMode) {
    this(ASTNodeFactory.MMA_STYLE_FACTORY, packageMode);
  }

  /**
   * Create a WMA parser with specified settings
   * 
   * @param factory parser factory for creating nodes
   * @param packageMode parse in &quot;package mode&quot;
   */
  public WMAParser(INodeParserFactory factory, boolean packageMode) {
    super(packageMode, false);
    fFactory = factory;
  }
   ...
   */
  protected WMAInfixOperator determineBinaryOperator() {
    for (Operator oper : fOperList) {
      if (oper instanceof WMAInfixOperator) {
        return (WMAInfixOperator) oper;
      }
    }

    // Check if we can find an operator in WMAOperatorTables
    String operatorName = WMAOperatorTables.getOperatorForCharacter(fOperatorString);
    if (operatorName != null && WMAOperatorTables.LEFT_BINARY_OPERATORS.containsKey(operatorName)
        || WMAOperatorTables.RIGHT_BINARY_OPERATORS.containsKey(operatorName)
        || WMAOperatorTables.FLAT_BINARY_OPERATORS.containsKey(operatorName)
        || WMAOperatorTables.NON_ASSOC_BINARY_OPERATORS.containsKey(operatorName)) {
      ...
      if (precedence != null) {
        ...
        return new WMAInfixOperator(fOperatorString, operatorName, precedence, associativity);
      }
    }
   ...
   */
  protected WMAPostfixOperator determinePostfixOperator() {
    for (Operator oper : fOperList) {
      if (oper instanceof WMAPostfixOperator) {
        return (WMAPostfixOperator) oper;
      }
    }

    // Check if we can find a postfix operator in WMAOperatorTables
    String operatorName = WMAOperatorTables.getOperatorForCharacter(fOperatorString);
    if (operatorName != null && WMAOperatorTables.POSTFIX_OPERATORS.containsKey(operatorName)) {
      Integer precedence = WMAOperatorTables.getOperatorPrecedence(operatorName);
      if (precedence != null) {
        return new WMAPostfixOperator(fOperatorString, operatorName, precedence);
      }
    }
   ...
   */
  protected WMAPrefixOperator determinePrefixOperator() {
    for (Operator oper : fOperList) {
      if (oper instanceof WMAPrefixOperator) {
        return (WMAPrefixOperator) oper;
      }
    }

    // Check if we can find a prefix operator in WMAOperatorTables
    String operatorName = WMAOperatorTables.getOperatorForCharacter(fOperatorString);
    if (operatorName != null && WMAOperatorTables.PREFIX_OPERATORS.containsKey(operatorName)) {
      Integer precedence = WMAOperatorTables.getOperatorPrecedence(operatorName);
      if (precedence != null) {
        return new WMAPrefixOperator(fOperatorString, operatorName, precedence);
      }
    }
  ...
  private ASTNode parseCompoundExpressionNull(WMAInfixOperator infixOperator, ASTNode lhs) {
  ...
  private ASTNode parseExpression() {
    if (fToken == TT_SPAN) {
      ...
      if (fToken == TT_SPAN) {
        ...
        WMAInfixOperator infixOperator = determineBinaryOperator();
    ...
    while (true) {
      ...
      if ((fToken == TT_LIST_OPEN) || (fToken == TT_PRECEDENCE_OPEN) || (fToken == TT_IDENTIFIER)
        ...
        if (infixOperator != null) {
          ...
          WMAPostfixOperator postfixOperator = determinePostfixOperator();
   ...
   * @return
   */
  private ASTNode parseInequality(final FunctionNode ast, final WMAInfixOperator infixOperator) {
    ...
    WMAInfixOperator compareOperator = determineBinaryOperator();
  ...
  private final ASTNode parseInfixOperator(ASTNode lhs, WMAInfixOperator infixOperator) {
    ASTNode rhs;
  ...
  private final ASTNode parsePostfixOperator(ASTNode lhs, WMAPostfixOperator postfixOperator) {
    getNextToken();
  ...
  private final ASTNode parsePrefixOperator(final WMAPrefixOperator prefixOperator) {
    getNextToken();
  ...
  private ASTNode parsePrimary(final int min_precedence) {
    if (fToken == TT_OPERATOR) {
      ...
      final WMAPrefixOperator prefixOperator = determinePrefixOperator();
