...
/**
 * GenWordPolynomialRing generic polynomial factory implementing RingFactory; Factory for
 * non-commutative string polynomials over C.
 *
...
/*, Iterable<GenWordPolynomial<C>>*/ {
  ...
  /**
   * Get the generating elements <b>excluding</b> the generators for the coefficient ring.
   *
  ...
  public List<GenWordPolynomial<C>> generators() {
    ...
    List<GenWordPolynomial<C>> univs = univariateList();
    List<GenWordPolynomial<C>> gens =
        new ArrayList<GenWordPolynomial<C>>(univs.size() + cogens.size());
    for (C c : cogens) {
