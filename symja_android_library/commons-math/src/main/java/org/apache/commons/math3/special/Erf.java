...
public class Erf {
	...
	/**
	 * The number {@code X_CRIT} is used by {@link #erf(double, double)} internally. This number solves {@code erf(x)=0.5} within
	 * 1ulp. More precisely, the current implementations of {@link #erf(double)} and {@link #erfc(double)} satisfy:<br/>
	 ...
	 * 
	 * <p>
	 * This implementation computes erf(x) using the {@link Gamma#regularizedGammaP(double, double, double, int) regularized gamma
	 * function}, following <a href="http://mathworld.wolfram.com/Erf.html"> Erf</a>, equation (3)
	 * </p>
	 * 
	 * <p>
	 * The value returned is always between -1 and 1 (inclusive). If {@code abs(x) > 40}, then {@code erf(x)} is indistinguishable
	 * from either 1 or -1 as a double, so the appropriate extreme value is returned.
	 * </p>
	 * 
	 * @param x
	 *            the value.
	 ...
	 * 
	 * <p>
	 * This implementation computes erfc(x) using the {@link Gamma#regularizedGammaQ(double, double, double, int) regularized gamma
	 * function}, following <a href="http://mathworld.wolfram.com/Erf.html"> Erf</a>, equation (3).
	 * </p>
	 * 
	 * <p>
	 * The value returned is always between 0 and 2 (inclusive). If {@code abs(x) > 40}, then {@code erf(x)} is indistinguishable
	 * from either 0 or 2 as a double, so the appropriate extreme value is returned.
	 * </p>
	 * 
	 * @param x
	 *            the value
	...
	public static double erf(double x1, double x2) {
		...
		return x1 < -X_CRIT ? x2 < 0.0 ? erfc(-x2) - erfc(-x1) : erf(x2) - erf(x1) : x2 > X_CRIT && x1 > 0.0 ? erfc(x1) - erfc(x2)
				: erf(x2) - erf(x1);
	}
	 ...
	 * href="http://people.maths.ox.ac.uk/gilesm/files/gems_erfinv.pdf">Approximating the erfinv function</a> by Mike Giles,
	 * Oxford-Man Institute of Quantitative Finance, which was published in GPU Computing Gems, volume 2, 2010. The source code is
	 * available <a href="http://gpucomputing.net/?q=node/1828">here</a>.
	 * </p>
	 * 
	 * @param x
	 *            the value
