...
public class FactorTest01 implements FactorTest {
   ...
   *
   * <p>Reducing both A and sqrt(Q) (mod N) means a great speed gain. Otherwise these products can
   * become huge, like a 1.000.000 bit number for N having 250 bit.
   *
   * @param aqPairs
  ...
  public void testForFactor(Set<AQPair> aqPairs) throws FactorException {
    ...
    // BlockLanczos returns non-square "solutions", too, so we test on the fly if Q is really a
    // square.
    ...
    for (long factor : totalQ_factors.keySet()) {
      ...
      totalQSqrt =
          totalQSqrt
              .multiply(BigInteger.valueOf(factor).pow(halfExp))
              .mod(N); // reduction (mod N) is much faster
    }
    ...
    //			LOG.debug("N = " + N + ": testForFactor(): A=" + getAString(aqPairs) + "=" + AProd + ", Q="
    // + getQString(aqPairs) + ", sqrt(Q)=" + totalQSqrt);
    ...
    if (minusGcd.compareTo(I_1) > 0 && minusGcd.compareTo(N) < 0)
      throw new FactorException(minusGcd); // factor!
    // test A+sqrt(Q)
    BigInteger plusGcd = AProd.add(totalQSqrt).gcd(N);
    if (DEBUG) LOG.debug("plusGcd = " + plusGcd);
    if (plusGcd.compareTo(I_1) > 0 && plusGcd.compareTo(N) < 0)
      throw new FactorException(plusGcd); // factor!
