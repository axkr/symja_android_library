...
public class SerializableDataSetTest {
  ...
  /** A vector dataset is stored as a one column table, so its shape has to travel with it. */
  @Test
  public void testVectorDatasetKeepsItsShape() throws Exception {
    IExpr vector = ASTDataset.newVector(F.List(F.C1, F.C2, F.C3));
    assertEquals(ASTDataset.Shape.VECTOR, ((ASTDataset) vector).getShape());
    ...
    assertEquals(ASTDataset.Shape.VECTOR, ((ASTDataset) restored).getShape());
    assertEquals(vector.toString(), restored.toString());
  }
