...
public abstract class AbstractFieldMatrix<T extends FieldElement<T>>
    ...
    public FieldMatrix<T> add(FieldMatrix<T> m)
        ...
        for (int row = 0; row < rowCount; ++row) {
            for (int col = 0; col < columnCount; ++col) {
                out.setEntry(row, col, getEntry(row, col).sum(m.getEntry(row, col)));
            }
        }
    ...
    public FieldMatrix<T> scalarAdd(final T d) {
        ...
        for (int row = 0; row < rowCount; ++row) {
            for (int col = 0; col < columnCount; ++col) {
                out.setEntry(row, col, getEntry(row, col).sum(d));
            }
        }
        ...
        for (int row = 0; row < nRows; ++row) {
            for (int col = 0; col < nCols; ++col) {
                ...
                for (int i = 0; i < nSum; ++i) {
                    sum = sum.sum(getEntry(row, i).multiply(m.getEntry(i, col)));
                }
    ...
    public T[] operate(final T[] v) throws DimensionMismatchException {
        ...
        for (int row = 0; row < nRows; ++row) {
            T sum = field.getZero();
            for (int i = 0; i < nCols; ++i) {
                sum = sum.sum(getEntry(row, i).multiply(v[i]));
            }
            out[row] = sum;
        }
    ...
    public FieldVector<T> operate(final FieldVector<T> v)
        ...
        try {
            ...
            for (int row = 0; row < nRows; ++row) {
                T sum = field.getZero();
                for (int i = 0; i < nCols; ++i) {
                    sum = sum.sum(getEntry(row, i).multiply(v.getEntry(i)));
                }
                out[row] = sum;
            }
    ...
    public T[] preMultiply(final T[] v) throws DimensionMismatchException {
        ...
        for (int col = 0; col < nCols; ++col) {
            T sum = field.getZero();
            for (int i = 0; i < nRows; ++i) {
                sum = sum.sum(getEntry(i, col).multiply(v[i]));
            }
            out[col] = sum;
        }
    ...
    public FieldVector<T> preMultiply(final FieldVector<T> v)
        ...
        try {
            ...
            for (int col = 0; col < nCols; ++col) {
                T sum = field.getZero();
                for (int i = 0; i < nRows; ++i) {
                    sum = sum.sum(getEntry(i, col).multiply(v.getEntry(i)));
                }
                out[col] = sum;
            }
