...
public class WXFTestCase extends AbstractTestCase {
	...
	public void testBinaryDeserialize() {
		...
		check("BinaryDeserialize(ByteArray({56, 58, 82, 64, 51, 46, 49, 52, 49, 53, 57, 50, 54, 53, 51, 53, 56, 57, 55, \r\n"
		...
		check("BinaryDeserialize(ByteArray({56, 58, 102, 6, 115, 10, 83, 101, 114, 105, 101, 115, 68, 97, 116, 97, 115, \n"
		...
		// Quantity(12, "Hours")
		check("BinaryDeserialize(ByteArray({56,58,102,2,115,8,81,117,97,110,116,105,116,121,67,12,83,5,72,111,117,114,115}))", //
				//
				"12[Hours]");

		checkNumeric("BinaryDeserialize(ByteArray({56,58,193,35,2,2,3,154,153,153,153,153,153,241,63,184,30,133,235,81,184,2,64,"
				+ "113,61,10,215,163,112,16,64,111,18,131,192,202,33,9,64,144,187,173,249,1,106,248,"
				+ "64,82,39,160,137,7,106,248,192}))", //
				//
				"{{1.1,2.34,4.11},\n" + //
		...
		check("BinaryDeserialize(ByteArray({56, 58, 193, 35, 1, 3, 154, 153, 153, 153, 153, 153, 241, 63, 184, 30, 133, 235, 81, 184, 2, 64, 113, 61, 10, 215, 163, 112, 16, 64}))", //
				"{1.1,2.34,4.11}");

		check("BinaryDeserialize(ByteArray({56,58,102,2,115,4,82,117,108,101,115,8,71,108,111,98,97,108,96,120,115,8,71,108,111,98,97,108,96,121}))", //
				"x->y");

		// BlankNullSequence(x);
		check("BinaryDeserialize(ByteArray({56, 58, 102, 2, 115, 7, 80, 97, 116, 116, 101, 114, 110, 115, 8, 71, 108, 111, 98, 97, 108, 96, 120, 102, 0, 115, 17, 66, 108, 97, 110, 107, 78, 117, 108, 108, 83, 101, 113, 117, 101, 110, 99, 101}))", //
				"x___");
		// BlankSequence(x);
		check("BinaryDeserialize(ByteArray({56, 58, 102, 2, 115, 7, 80, 97, 116, 116, 101, 114, 110, 115, 8, 71, 108, 111, 98, 97, 108, 96, 120, 102, 0, 115, 13, 66, 108, 97, 110, 107, 83, 101, 113, 117, 101, 110, 99, 101}))", //
				"x__");
		// BlankNullSequence();
		check("BinaryDeserialize(ByteArray({56, 58, 102, 0, 115, 17, 66, 108, 97, 110, 107, 78, 117, 108, 108, 83, 101, 113, 117, 101, 110, 99, 101}))", //
				"___");
		// BlankSequence();
		check("BinaryDeserialize(ByteArray({56, 58, 102, 0, 115, 13, 66, 108, 97, 110, 107, 83, 101, 113, 117, 101, 110, 99, 101}))", //
				"__");
		// -25!
		check("BinaryDeserialize(ByteArray({56,58,73,27,45,49,53,53,49,49,50,49,48,48,52,51,51,51,48,57,56,53,57,56,52,48,48,48,48,48,48}))", //
				"-15511210043330985984000000");
		// 25!
		check("BinaryDeserialize(ByteArray({56,58,73,26,49,53,53,49,49,50,49,48,48,52,51,51,51,48,57,56,53,57,56,52,48,48,48,48,48,48}))", //
				"15511210043330985984000000");
		...
		check("BinaryDeserialize(ByteArray({56,58,102,1,115,8,79,112,116,105,111,110,97,108,102,2,115,7,80,97,116,116,101,114,110,115,1,120,102,0,115,5,66,108,97,110,107}))", //
				"x_.");
		check("BinaryDeserialize(ByteArray({56,58,102,2,115,7,80,97,116,116,101,114,110,115,8,71,108,111,98,97,108,96,120,102,1,115,5,66,108,97,110,107,115,7,73,110,116,101,103,101,114}))", //
				"Pattern(x,Blank(Integer))");

		check("BinaryDeserialize(ByteArray({56,58,102,2,115,7,80,97,116,116,101,114,110,115,8,71,108,111,98,97,108,96,120,102,0,115,5,66,108,97,110,107}))", //
				"x_");

		check("BinaryDeserialize(ByteArray({56,58,102,0,115,5,66,108,97,110,107}))", //
				"_");

		check("BinaryDeserialize(ByteArray({56, 58, 102, 1, 115, 5, 66, 108, 97, 110, 107, 115, 7, 73, 110, 116, 101, 103, 101, 114}))", //
				"_Integer");
		...
		check("BinaryDeserialize(ByteArray({56,58,102,2,115,7,67,111,109,112,108,101,120,102,2,115,8,82,97,116,105,111,110,\n"
				+ "97,108,67,2,67,3,102,2,115,8,82,97,116,105,111,110,97,108,67,7,67,4}))", //
				"2/3+I*7/4");

		check("BinaryDeserialize(ByteArray({56,58,102,2,115,8,82,97,116,105,111,110,97,108,67,2,67,3}))", //
				"2/3");
		...
		check("BinaryDeserialize(ByteArray({56,58,102,1,115,8,71,108,111,98,97,108,96,102,115,8,71,108,111,98,97,108,96,103}))", //
				"f(g)");

		check("BinaryDeserialize(ByteArray({56,58,102,1,115,8,71,108,111,98,97,108,96,102,102,2,115,8,71,108,111,98,97,108,96,103,115,8,71,108,111,98,97,108,96,120,115,8,71,108,111,98,97,108,96,121}))", //
				"f(g(x,y))");

		check("BinaryDeserialize(ByteArray({56,58,102,2,115,8,71,108,111,98,97,108,96,102,115,8,71,108,111,98,97,108,96,103,67,2}))", //
				"f(g,2)");
