  ...
  public List<Integer> sieve() {
    ...
    for (int b = 0;
      ...
      for (long x = nextBlockAddress; x > sieveBlockAddress; ) {
        if ((((y0 = UNSAFE.getLong(x -= 8)) | (y1 = UNSAFE.getLong(x -= 8))) & 0x8080808080808080L)
            != 0) {
  ...
  private void sievePositiveXBlock(
    ...
    for (r = r_max - 1; r >= r_medium; r--) {
      ...
      if ((d1 = dPosArray[r]) != 0) { // two x-solutions
        ...
        // For some reasons it is faster to do the comparison with sieveBlockAddress+B instead of
        // with
