...
/**
 * Squarefree decomposition for algebraic extensions of infinite coefficient fields of
 * characteristic p &gt; 0.
 *
 * @author Heinz Kredel
 */
public class SquarefreeInfiniteAlgebraicFieldCharP<C extends GcdRingElem<C>>
  ...
  private static final Logger logger =
      LogManager.getLogger(SquarefreeInfiniteAlgebraicFieldCharP.class);
  ...
  public SortedMap<AlgebraicNumber<C>, Long> rootCharacteristic(AlgebraicNumber<C> P) {
    ...
    GenPolynomialRing<AlgebraicNumber<C>> pfac =
        new GenPolynomialRing<AlgebraicNumber<C>>(afac, d, vn);
    List<GenPolynomial<AlgebraicNumber<C>>> uv =
        (List<GenPolynomial<AlgebraicNumber<C>>>) pfac.univariateList();
    ...
    GenPolynomial<AlgebraicNumber<C>> cpp =
        Power.<GenPolynomial<AlgebraicNumber<C>>>positivePower(cp, c);
        ...
        for (Monomial<C> ma : ap) {
          ...
          if (cc instanceof AlgebraicNumber && pc instanceof AlgebraicNumber) {
            ...
            if (dcp.isConstant()) { // not possible: dc.isConstant()
              // System.out.println("dcp = " + dcp + " : " + cc.toScriptFactory()); //  + ", dc = "
              // + dc);
  ...
  public GenPolynomial<AlgebraicNumber<C>> rootCharacteristic(GenPolynomial<AlgebraicNumber<C>> P) {
    ...
    if (pfac.nvar > 1) {
      ...
      GenPolynomial<GenPolynomial<AlgebraicNumber<C>>> Pr =
          PolyUtil.<AlgebraicNumber<C>>recursive(rfac, P);
      GenPolynomial<GenPolynomial<AlgebraicNumber<C>>> Prc =
          recursiveUnivariateRootCharacteristic(Pr);
      if (Prc == null) {
        return null;
      }
   ...
   */
  @Override
  public GenPolynomial<AlgebraicNumber<C>> baseRootCharacteristic(
    ...
    if (pfac.nvar > 1) {
      ...
      throw new IllegalArgumentException(
          P.getClass().getName() + " only for univariate polynomials");
    }
    RingFactory<AlgebraicNumber<C>> rf = pfac.coFac;
    if (rf.characteristic().signum() != 1) {
      // basePthRoot not possible
