...
/**
 * Complex roots implemented by Sturm sequences. Algorithms use exact method derived from Wilf's
 * numeric Routh-Hurwitz method.
 *
...
public class ComplexRootsSturm<C extends RingElem<C> & Rational> extends ComplexRootsAbstract<C> {
   ...
   */
  @Override
  public Rectangle<C> invariantRectangle(
      Rectangle<C> rect, GenPolynomial<Complex<C>> f, GenPolynomial<Complex<C>> g)
      throws InvalidBoundaryException {
    Rectangle<C> v = rect;
    if (g == null || g.isZERO()) {
      return v;
  ...
  /**
   * Exclude zero. If an axis intersects with the rectangle, it is shrinked to exclude the axis. Not
   * used.
   *
   * @param rect root isolating rectangle for f which contains exactly one root.
   * @return a new rectangle r such that re(r) &lt; 0 or (re)r &gt; 0 and im(r) &lt; 0 or (im)r &gt;
   *     0.
   */
  public Rectangle<C> excludeZero(Rectangle<C> rect, GenPolynomial<Complex<C>> f)
    ...
    // System.out.println("contains: ir = " + ir.contains(zero.getRe()) + ", ii = " +
    // ii.contains(zero.getIm()) );
    Rectangle<C> rn = rect;
    // shrink real part
