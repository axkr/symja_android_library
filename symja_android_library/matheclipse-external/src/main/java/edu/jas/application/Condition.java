...
/**
 * Condition. Container for an ideal of polynomials considered to be zero and a multiplicative set
 * of polynomials considered to be non-zero.
 *
...
public class Condition<C extends GcdRingElem<C>> implements Serializable {
  ...
  public String toString() {
    return "Condition[ 0 == "
        + zero.getList().toString()
        + ", 0 != "
        + nonZero.mset.toString()
        + " ]";
  }
  ...
  /**
   * Determine polynomial. If this condition does not determine the polynomial, then a run-time
   * exception is thrown.
   *
   * @param A polynomial.
  ...
  /**
   * Determine list of polynomials. If this condition does not determine all polynomials, then a
   * run-time exception is thrown. The returned list does not contain polynomials with all green
   * terms.
   *
