...
/**
 * Comprehensive Groebner Base sequential algorithm. Implements faithful comprehensive Groebner
 * bases via Groebner systems and CGB test.
 *
...
/* extends GroebnerBaseAbstract<GenPolynomial<C>> */ {
  ...
  public GroebnerSystem<C> GBsys(List<GenPolynomial<GenPolynomial<C>>> F) {
    ...
    while (CSs.size() > 0) {
      ...
      logger.info(
          "poped GBsys (remaining " + (CSs.size() - 1) + ") with pairlist  = " + cs.pairlist);
      if (!cs.isDetermined()) {
        cs = cs.reDetermine();
      }
