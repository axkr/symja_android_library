...
/**
 * Integrate a real function of one variable using a globally adaptive Gauss-Kronrod quadrature
 * rule. This algorithm is a translation of subroutines from the QUADPACK library and
 * well-documented in [1].
 * 
 * <p>
 * References:
 * <ul>
 * <li>[1] Piessens, Robert, et al. QUADPACK: A subroutine package for automatic integration. Vol.
 * 1. Springer Science & Business Media, 2012.</li>
 * </ul>
 * </p>
 */
public final class GaussKronrod extends Quadrature {
  ...
  private static final void dqagse(final DoubleUnaryOperator f, final double a, final double b,
      ...
      final double[] abserr, final int[] neval, final int[] ier, final double[] alist,
      final double[] blist, final double[] rlist, final double[] elist, final int[] iord,
      final int[] last) {
    double area, area12, a1, a2, b1, b2, correc = 0.0, dres, epmach, erlarg = 0.0, erlast, errbnd,
        erro12, errsum, ertest = 0.0, oflow, small = 0.0, uflow;
    ...
    final double[] abseps = new double[1], area1 = new double[1], area2 = new double[1],
        defabs = new double[1], defab1 = new double[1], defab2 = new double[1],
        errmax = new double[1], error1 = new double[1], error2 = new double[1],
        resabs = new double[1], reseps = new double[1];
    ...
    for (last[0] = 2; last[0] <= limit; ++last[0]) {
      ...
      if (Math.max(Math.abs(a1), Math.abs(b2)) <= (1.0 + 100.0 * epmach)
          * (Math.abs(a2) + 1000.0 * uflow)) {
        ier[0] = 4;
      }
  ...
  private static final void dqelg(final int[] n, final double[] epstab, final double[] result,
      final double[] abserr, final double[] res3la, final int[] nres) {
  ...
  private static final void dqpsrt(final int limit, final int last, final int[] maxerr,
  ...
  private static void dqk15i(final DoubleUnaryOperator f, final double boun, final int inf,
      final double a, final double b, final double[] result, final double[] abserr,
      final double[] resabs, final double[] resasc) {

    double absc, absc1, absc2, centr, dinf, epmach, fc, fsum, fval1, fval2, hlgth, resg, resk,
  ...
  private static void dqagie(final DoubleUnaryOperator f, final double bound, final int inf,
    ...
    double area, area12, a1, a2, boun, b1, b2, correc = 0.0, dres, epmach, erlarg = 0.0, erlast,
        errbnd, erro12, errsum, ertest = 0.0, oflow, small = 0.0, uflow;
    final double[] res3la = new double[4], rlist2 = new double[53], defabs = new double[1],
        resabs = new double[1], reseps = new double[1], area1 = new double[1],
        area2 = new double[1], defab1 = new double[1], defab2 = new double[1],
        error1 = new double[1], error2 = new double[1], errmax = new double[1],
        abseps = new double[1];
    int id, ierro, iroff1, iroff2, iroff3, jupbnd, k, ksgn, ktmin;
    final int[] maxerr = new int[1], nrmax = new int[1], numrl2 = new int[1], nres = new int[1];
    boolean extrap, noext;
