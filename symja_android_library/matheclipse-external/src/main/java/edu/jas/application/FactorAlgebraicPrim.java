...
/**
 * Algebraic number coefficients factorization algorithms. This class implements factorization
 * methods for polynomials over algebraic numbers over rational numbers or over (prime) modular
 * integers. The algorithm uses zero dimensional ideal prime decomposition.
 *
 * @author Heinz Kredel
...
public class FactorAlgebraicPrim<C extends GcdRingElem<C>>
  ...
  public List<GenPolynomial<AlgebraicNumber<C>>> baseFactorsSquarefree(
    ...
    if (logger.isDebugEnabled()) {
      Squarefree<AlgebraicNumber<C>> sqengine =
          SquarefreeFactory.<AlgebraicNumber<C>>getImplementation(afac);
      ...
      // GreatestCommonDivisor<AlgebraicNumber<C>> aengine //= GCDFactory.<AlgebraicNumber<C>>
      // getProxy(afac);
    ...
    for (IdealWithUniv<C> Iu : Iul) {
      ...
      if (ag.equals(Ad)) {
        ...
        GenPolynomial<AlgebraicNumber<C>> pga =
            PolyUtil.<C>convertRecursiveToAlgebraicCoefficients(pfac, pgr);
