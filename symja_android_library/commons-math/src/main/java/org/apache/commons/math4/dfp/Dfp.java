...
public class Dfp implements RealFieldElement<Dfp> {
    ...
    protected Dfp(final DfpField field, double x) {
        ...
        xdfp = xdfp.divide(new Dfp(field, 4503599627370496l)).sum(field.getOne());  // Divide by 2^52, then add one
            ...
            switch (rmode) {
                case ROUND_FLOOR:
                    if (result.sign == -1) {
                        ...
                        result = result.sum(newInstance(-1));
                    }
                    break;
    ...
    public Dfp hypot(final Dfp y) {
        return multiply(this).sum(y.multiply(y)).sqrt();
    }
    ...
    @Override
    public Dfp log1p() {
        return DfpMath.log(this.sum(getOne()));
    }
    ...
    public Dfp atan2(final Dfp x)
        ...
        final Dfp r = x.multiply(x).sum(multiply(this)).sqrt();

        if (x.sign >= 0) {

            // compute atan2(y, x) = 2 atan(y / (r + x))
            return getTwo().multiply(divide(r.sum(x)).atan());
    ...
    @Override
    public Dfp cosh() {
        return DfpMath.exp(this).sum(DfpMath.exp(negate())).divide(2);
    }
    ...
    public Dfp tanh() {
        ...
        return ePlus.subtract(eMinus).divide(ePlus.sum(eMinus));
    }
    ...
    @Override
    public Dfp acosh() {
        return multiply(this).subtract(getOne()).sqrt().sum(this).log();
    }
    ...
    @Override
    public Dfp asinh() {
        return multiply(this).sum(getOne()).sqrt().sum(this).log();
    }
    ...
    @Override
    public Dfp atanh() {
        return getOne().sum(this).divide(getOne().subtract(this)).log().divide(2);
    }
    ...
    public Dfp linearCombination(final Dfp a1, final Dfp b1, final Dfp a2, final Dfp b2) {
        return a1.multiply(b1).sum(a2.multiply(b2));
    }
    ...
    public Dfp linearCombination(final double a1, final Dfp b1, final double a2, final Dfp b2) {
        return b1.multiply(a1).sum(b2.multiply(a2));
    }
    ...
    public Dfp linearCombination(final Dfp a1, final Dfp b1,
        ...
        return a1.multiply(b1).sum(a2.multiply(b2)).sum(a3.multiply(b3));
    }
    ...
    public Dfp linearCombination(final double a1, final Dfp b1,
        ...
        return b1.multiply(a1).sum(b2.multiply(a2)).sum(b3.multiply(a3));
    }
    ...
    public Dfp linearCombination(final Dfp a1, final Dfp b1, final Dfp a2, final Dfp b2,
        ...
        return a1.multiply(b1).sum(a2.multiply(b2)).sum(a3.multiply(b3)).sum(a4.multiply(b4));
    }
    ...
    public Dfp linearCombination(final double a1, final Dfp b1, final double a2, final Dfp b2,
        ...
        return b1.multiply(a1).sum(b2.multiply(a2)).sum(b3.multiply(a3)).sum(b4.multiply(a4));
    }
