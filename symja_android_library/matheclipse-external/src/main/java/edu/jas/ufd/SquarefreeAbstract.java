...
public abstract class SquarefreeAbstract<C extends GcdRingElem<C>> implements Squarefree<C> {
   ...
   *     in B with b|a and each b in B is squarefree. B does not contain zero or constant
   *     polynomials.
   */
   ...
   * @return true, if for all b != c in B gcd(b,c) = 1 and each b in B is squarefree, else false.
   */
   ...
   */
  public SortedMap<GenPolynomial<C>, Long> normalizeFactorization(
      SortedMap<GenPolynomial<C>, Long> F) {
    if (F == null || F.size() <= 1) {
      return F;
    }
   ...
   * @return [ [Ai0, Ai1,..., Aie_i], i=0,...,k ] with A/prod(D) = A0 + sum( sum ( Aij/di^j ) ) with
   *     deg(Aij) < deg(di).
   */
  public List<List<GenPolynomial<C>>> basePartialFraction(
      GenPolynomial<C> A, SortedMap<GenPolynomial<C>, Long> D) {
    if (D == null || A == null) {
  ...
  public boolean isBasePartialFraction(
      GenPolynomial<C> A, SortedMap<GenPolynomial<C>, Long> D, List<List<GenPolynomial<C>>> F) {
    ...
    for (Map.Entry<GenPolynomial<C>, Long> me : D.entrySet()) { // assume fixed sequence order
      ...
      //             if ( ! t ) {
      //                 System.out.println("not isPartFrac exp = " + pi + ", d = " + d + ", e = " +
      // ei);
