...
/**
 * Polynomial parametric ring reduction sequential use algorithm. Implements normalform, condition
 * construction and polynomial determination.
 *
...
/* implements CReduction<C> */ {
   ...
   */
  @SuppressWarnings("unchecked")
  public ColorPolynomial<C> normalform(
      Condition<C> cond, List<ColorPolynomial<C>> Pp, ColorPolynomial<C> Ap) {
    if (Pp == null || Pp.isEmpty()) {
      return Ap;
    }
    if (Ap == null || Ap.isZERO()) {
  ...
  /**
   * Case distinction conditions of parametric polynomial list. The returned condition determines
   * the polynomial list.
   *
   ...
   */
  public List<Condition<C>> caseDistinction(
      List<Condition<C>> cd, GenPolynomial<GenPolynomial<C>> A) {
    if (A == null || A.isZERO()) {
      return cd;
    }
    if (cd == null) {
   ...
   * @return new determined list of colored systems.
   */
  public List<ColoredSystem<C>> determine(
      List<Condition<C>> cd, List<GenPolynomial<GenPolynomial<C>>> H) {
    List<ColoredSystem<C>> CS = new ArrayList<ColoredSystem<C>>();
    if (H == null || H.size() == 0) {
      return CS;
    }
