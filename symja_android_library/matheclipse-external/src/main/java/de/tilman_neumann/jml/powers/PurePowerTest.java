 ...
 *
 * <p>Thanks to Graeme Willoughby for countless suggestions, which dramatically improved performance
 * (something like factor 200 compared to PSIQS 01)
 *
 * @author Tilman Neumann
 */
// TODO: A p-adic implementation like in gmp would be much faster for large numbers (with thousands
// of digits)
public class PurePowerTest {
   ...
   * the binary representation <somenumber><pm zeros>. So there is a fast bit pattern test for even
   * N: If the number of trailing zeros in N is not equal to zero (mod p), then N is not a p.th
   * power</br> 2. All even powers will have been detected as squares, all powers that are multiples
   * of three will have been detected as cubes and so on. In short, we only need to test prime
   * exponents up to log3 N, not all exponents.</br> 3. A square test is of course faster than an
   * i.th power test with argument 2.</br>
   *
   * @param N
  ...
  public Result test_v01(BigInteger N) {
    ...
    if (lsb > 0) {
      ...
      for (int b = 3; b < log3N; b = primesArray.getPrime(++bIndex)) {
        // LOG.debug("test b = " + b);
        // if the number of trailing zeros in N is not equal to 0 (mod b), then N is not a b.th
        // power
        if (lsb % b != 0) continue;
  ...
  public Result test /*_v02*/(BigInteger N) {
    if (N.compareTo(I_4) < 0) return null; // negative or not composite
    if (N.bitCount() == 1)
      return new Result(I_2, N.getLowestSetBit()); // N>=4 and only 1 bit set -> N is a power of 2
    ...
    if (lsb == 0) {
      ...
      // If some a divides N, we determine the exponent which then is the biggest possible exponent,
      // maxExp.
      // If no a<aMax divides N, then we have already excluded exponents b>bMax with N ~ aMax^bMax
      // <=> bMax = ln(N)/ln(aMax)
      ...
      for (a = 3; a < aMax; a = primesArray.getPrime(++aIndex)) {
        ...
        // But experiments showed that it is fastest to keep on searching, with the chance to find
        // maxExp<3 above
      }

      int bMax =
          maxExp > 0 ? maxExp : (int) (1 + lnN / Math.log(a)); // +1 resolves precision issues
      ...
      for (int b = 3; b <= bMax; b = primesArray.getPrime(++bIndex)) {
        ...
        // Sieve out some powers: If (2b+1) is prime, then N = x^b is only possible if N^2 == 1 (mod
        // (2b+1)) or if (2b+1) | N
        final int b2p = (b << 1) + 1;
    ...
    while (true) {
      try {
        ...
        if (purePower == null) {
          ...
          LOG.info(
              "N = " + N + " = " + purePower.base + "^" + purePower.exponent + " is a pure power!");
        }
      } catch (Exception ex) {
