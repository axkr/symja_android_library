...
public class RootFactoryApp {
   ...
   */
  public static <C extends GcdRingElem<C> & Rational> boolean isRootRealCoeff(
      GenPolynomial<C> f, Complex<RealAlgebraicNumber<C>> r) {
  ...
  public static <C extends GcdRingElem<C> & Rational> boolean isRoot(
      GenPolynomial<Complex<C>> f, Complex<RealAlgebraicNumber<C>> r) {
    ...
    // test algebraic part
    Complex<RealAlgebraicNumber<C>> a =
        PolyUtil.<Complex<RealAlgebraicNumber<C>>>evaluateMain(cr, p, r);
    boolean t = a.isZERO();
    if (!t) {
   ...
   */
  public static <C extends GcdRingElem<C> & Rational> boolean isRoot(
      GenPolynomial<Complex<C>> f, List<Complex<RealAlgebraicNumber<C>>> R) {
  ...
  public static <C extends GcdRingElem<C> & Rational>
      List<Complex<RealAlgebraicNumber<C>>> complexAlgebraicNumbersComplex(
    ...
    for (Map.Entry<GenPolynomial<Complex<C>>, Long> me : F.entrySet()) {
      ...
      List<Complex<RealAlgebraicNumber<C>>> ls =
          RootFactoryApp.<C>complexAlgebraicNumbersSquarefree(sp);
      long m = me.getValue();
      for (long i = 0L; i < m; i++) {
        list.addAll(ls);
      }
  ...
  public static <C extends GcdRingElem<C> & Rational>
    ...
    GenPolynomialRing<Complex<C>> tfac =
        new GenPolynomialRing<Complex<C>>(cfac, 2, to); // ,new vars); //tord?
    // System.out.println("tfac = " + tfac);
    GenPolynomial<Complex<C>> t =
        tfac.univariate(1, 1L).sum(tfac.univariate(0, 1L).multiply(cfac.getIMAG()));
  ...
  */
   ...
   * @return container of real and complex algebraic numbers of the primitive element of a and b.
   */
  public static <C extends GcdRingElem<C> & Rational> AlgebraicRootsPrimElem<C> rootReduce(
      AlgebraicRoots<C> a, AlgebraicRoots<C> b) {
   ...
   */
  public static <C extends GcdRingElem<C> & Rational> AlgebraicRootsPrimElem<C> rootsOfUnity(
      AlgebraicRootsPrimElem<C> ar) {
