...
public class EllipticCurveMethod extends FactorAlgorithm {
  ...
  private void addToMapDependingOnPrimeTest(
      ...
      SortedMap<BigInteger, Integer> compositeFactors) {
   ...
   *
   * <ul>
   *   <li>Step 1: Set k<-0, and then repeatedly set k<-k+1, u<-u/2, v<-v/2 zero or more times until
   *       u and v are not both even.
   *   <li>Step 2: If u is odd, set t<-(-v) and go to step 4. Otherwise set t<-u.
   *   <li>Step 3: Set t<-t/2
   *   <li>Step 4: If t is even, go back to step 3.
   *   <li>Step 5: If t>0, set u<-t, otherwise set v<-(-t).
   *   <li>Step 6: Set t<-u-v. If t!=0, go back to step 3.
   *   <li>Step 7: The GCD is u*2^k.
   * </ul>
   *
   * @param Nbr1
   * @param Nbr2
  ...
  public static void main(String[] args) {
    ...
    BigInteger[] testNums =
        new BigInteger[] {
          ...
          // but ECM fails to factor 3108780723099354807613175415185519 = 1210508704285703 *
          // 2568160569265616473
