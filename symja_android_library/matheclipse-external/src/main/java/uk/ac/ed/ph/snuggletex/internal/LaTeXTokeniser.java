...
public final class LaTeXTokeniser {
  ...
  /**
   * Current "working document", taking into account command/environment substitutions. This starts
   * off identical to the content of the {@link SnuggleInput} being read but will change during
   * tokenisation.
   */
   ...
   * monotonically until it hits the length of the {@link #workingDocument}, though we may sometimes
   * move backwards when performing substitutions.
   */
  ...
  /**
   * Represents the "terminator" characters that signify the end of a parsing mode. I have made this
   * an interface is some terminators are static Strings, whereas others are best considered as
   * {@link Pattern}s.
   */
  public static interface Terminator {
    ...
    /**
     * Finds the next match of this Terminator in the given {@link WorkingDocument} starting at the
     * given index.
     *
     * @param workingDocument {@link WorkingDocument} to check
  ...
  /**
   * Enumeration of the various "tokenisation modes" we will be running in. This is only really used
  ...
  public static class ModeState {
    ...
    /**
     * If terminator is not null, then this indicates whether the terminator was found when parsing
     * in this mode ended.
     */
     ...
     * getting at the "useful" content of a parse as it won't include the terminator.
     */
  ...
  private ModeState tokeniseInNewState(final TokenisationMode tokenisationMode,
    ...
    currentModeState =
        modeStack.isEmpty() ? null : modeStack.peek(); /* Set current to previous state */
    return result;
  }
  ...
  /**
   * This reads the next token in {@link LaTeXMode#VERBATIM} Mode, which is "simply" a case of
   * pulling in everything until the next match of the required terminator. This always returns
   * exactly one {@link Token}.
   */
  ...
  /**
   * Reads either the next text lump or "new paragraph" tokens, until the start of something else.
   */
  private SimpleToken readNextSimpleTextParaMode() {
    ...
    for (index = position; index < workingDocument.length(); index++) {
      ...
      if (c == '\\' || c == '$' || c == '{' || c == '%' || c == '&' || c == '#' || c == '^'
          || c == '_') {
        break;
      } else if (currentModeState.terminator != null
          && currentModeState.terminator.matchesAt(workingDocument, index) != -1) {
        break;
      } else if (Character.isWhitespace(c)) {
  ...
  private FlowToken readDollarMath() throws SnuggleParseException {
    ...
    ArgumentContainerToken contentToken =
        new ArgumentContainerToken(contentSlice, LaTeXMode.MATH, contentResult.tokens);
    FrozenSlice environmentSlice = workingDocument.freezeSlice(openDollarPosition, position);
    BuiltinEnvironment environment =
        isDisplayMath ? CorePackageDefinitions.ENV_DISPLAYMATH : CorePackageDefinitions.ENV_MATH;
    return new EnvironmentToken(environmentSlice, startLatexMode, environment, contentToken);
  }

  /**
   * Reads the content of a region explicitly delimited inside <tt>{....}</tt>. This is the simplest
   * type of "mode change" in the tokenisation process so is a useful template in understanding more
   * complicated changes.
   */
  private BraceContainerToken readBraceRegion() throws SnuggleParseException {
    ...
    FrozenSlice braceOuterSlice =
        workingDocument.freezeSlice(openBraceIndex, position); /* Includes {...} */
    FrozenSlice braceInnerSlice =
        workingDocument.freezeSlice(openBraceIndex + 1, endInnerIndex); /* Without {...} */
    ArgumentContainerToken braceContents =
        new ArgumentContainerToken(braceInnerSlice, openLaTeXMode, result.tokens);
  ...
  private FlowToken readSlashToken() throws SnuggleParseException {
    ...
    if (c == -1) {
      /* Nothing following \\ */
      result =
          createError(CoreErrorCode.TTEG01, position, afterSlashIndex, currentModeState.latexMode);
    } else if (c == '(' || c == '[') {
      ...
      if (currentModeState.latexMode == LaTeXMode.MATH) {
        ...
        ArgumentContainerToken contentToken =
            new ArgumentContainerToken(contentSlice, LaTeXMode.MATH, contentResult.tokens);
        BuiltinEnvironment environment =
            (c == '(') ? CorePackageDefinitions.ENV_MATH : CorePackageDefinitions.ENV_DISPLAYMATH;
        result =
  ...
  private FlowToken readCommandOrEnvironmentOrVerb() throws SnuggleParseException {
    ...
    if (commandName == null) {
      ...
      throw new SnuggleLogicException(
          "Expected caller to have picked the commandName==null case up");
    }
    ...
    for (FlowToken resultToken : contentTokens) {
      if (resultToken.getType() == TokenType.VERBATIM_MODE_TEXT && verbatimContentToken == null) {
        ...
        if (((ErrorToken) resultToken).getError().getErrorCode() == CoreErrorCode.TTEG00) {
          ...
          throw new SnuggleLogicException(
              "Unexpected error when parsing \\verb content: " + resultToken);
        }
      } else {
        throw new SnuggleLogicException(
            "Unexpected token when examining \\verb content: " + resultToken);
      }
    }
   ...
   * @throws SnuggleParseException
   */
  private FlowToken finishCombiningCommand(final BuiltinCommand command)
      throws SnuggleParseException {
  ...
  private FlowToken finishComplexCommand(final BuiltinCommand command)
    ...
    BuiltinCommandOrEnvironmentArgumentSearchResult argumentSearchResult =
        new BuiltinCommandOrEnvironmentArgumentSearchResult();
    ErrorToken errorToken =
        advanceOverBuiltinCommandOrEnvironmentArguments(command, argumentSearchResult);
    if (errorToken != null) {
      return errorToken;
    }
  ...
  static class BuiltinCommandOrEnvironmentArgumentSearchResult {
    ...
    /**
     * Tokenised versions of required arguments, will include null entries for arguments where
     * tokenisation was not asked for and did not occur.
     */
  ...
  private ErrorToken advanceOverBuiltinCommandOrEnvironmentArguments(
    ...
    if (commandOrEnvironment.isAllowingOptionalArgument()) {
      ...
      if (c == '[') {
        ...
        optionalArgument =
            new ArgumentContainerToken(optionalArgumentSlice, argumentMode, argumentResult.tokens);
      }
    }
    ...
    for (int i = 0; i < argCount; i++) {
      ...
      if (c == '{') {
        ...
        if (nextToken != null) {
          requiredArguments[i] =
              ArgumentContainerToken.createFromSingleToken(argumentMode, nextToken);
  ...
  private FlowToken finishUserDefinedCommand(final UserDefinedCommand command)
    ...
    ErrorToken errorToken =
        advanceOverUserDefinedCommandOrEnvironmentArguments(command, argumentSearchResult);
    if (errorToken != null) {
      return errorToken;
    }
  ...
  /**
   * This helper substitutes the arguments provided for a user-defined command/environment into the
  ...
  private FlowToken finishBeginBuiltinEnvironment(final BuiltinEnvironment environment)
    ...
    ErrorToken argumentErrorToken =
        advanceOverBuiltinCommandOrEnvironmentArguments(environment, argumentSearchResult);
    ...
    if (contentMode == LaTeXMode.VERBATIM) {
      ...
      ModeState contentResult =
          tokeniseInNewState(TokenisationMode.BUILTIN_ENVIRONMENT_CONTENT, null, contentMode);
      int endContentIndex = contentResult.computeLastTokenEndIndex();
      FrozenSlice contentSlice = workingDocument.freezeSlice(startContentIndex, endContentIndex);
      contentToken = new ArgumentContainerToken(contentSlice, contentMode, contentResult.tokens);
  ...
  private FlowToken finishBeginUserDefinedEnvironment(final UserDefinedEnvironment environment)
    ...
    UserDefinedCommandOrEnvironmentArgumentSearchResult argumentSearchResult =
        new UserDefinedCommandOrEnvironmentArgumentSearchResult();
    ErrorToken errorToken =
        advanceOverUserDefinedCommandOrEnvironmentArguments(environment, argumentSearchResult);
    if (errorToken != null) {
      return errorToken;
    }
    ...
    int argumentCount =
        argumentDefinitionResult != null
            ? argumentDefinitionResult.requiredArgumentCount
                + (argumentDefinitionResult.optionalArgument != null ? 1 : 0)
            : 0;
    boolean inEscape = false; /* Whether we are in the middle of a character escape */
    boolean inArgument = false; /* Whether we are inside an argument */
    int index;
   ...
   * @throws SnuggleParseException
   */
  private ErrorToken advanceOverUserDefinedCommandOrEnvironmentArgumentDefinition(
      final String commandOrEnvironmentName, final ArgumentDefinitionResult result)
      throws SnuggleParseException {
    ...
    if (c == '[') {
      ...
      String rawArgCount =
          workingDocument.extract(afterOpenSquare, closeSquareIndex).toString().trim();
      try {
  ...
  /**
   * Returns the index of the next ']', handling any balanced braces appropriately. Returns -1 if no
   * corresponding ']' was found.
   */
  ...
  private void skipOverComment() {
    ...
    if (workingDocument.charAt(index) == '\n') {
      ...
      int c;
      while (searchIndex < workingDocument.length()
          && (c = workingDocument.charAt(searchIndex)) != '\n') {
