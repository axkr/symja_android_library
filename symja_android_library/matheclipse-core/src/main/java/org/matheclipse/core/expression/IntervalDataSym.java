...
public class IntervalDataSym {
   ...
   */
  private static final Flag INTERVAL_DATA_NORMALIZED = Flag.BUILT_IN_EVALED;
  ...
  private static IAST normalize(final IAST intervalList, EvalEngine engine)
    ...
    if (evaled) {
      result.addFlag(INTERVAL_DATA_NORMALIZED);
      // result.builtinEvaled();
      return result;
    }
    if (intervalList instanceof IASTMutable) {
      intervalList.addFlag(INTERVAL_DATA_NORMALIZED);
