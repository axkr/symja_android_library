...
public class HypergeometricPFQRules {
  ...
  final public static IAST RULES = List(IInit(HypergeometricPFQ, SIZES),
      // HypergeometricPFQ({a_},{},z_):=(1-z)^(-a)
      ISetDelayed(HypergeometricPFQ(list(a_), List(), z_), Power(Subtract(C1, z), Negate(a))),
      // HypergeometricPFQ({},{b_},z_):=z^(1/2-b/2)*BesselI(-1+b,2*Sqrt(z))*Gamma(b)
      ISetDelayed(HypergeometricPFQ(List(), list(b_), z_),
          Times(Power(z, Plus(C1D2, Times(CN1D2, b))), BesselI(Plus(CN1, b), Times(C2, Sqrt(z))),
              Gamma(b))),
      ...
      ISetDelayed(HypergeometricPFQ(list(C1D2, b_), list(QQ(3L, 2L), c_), z_),
          Condition(
              Times(b, Power(Plus(CN1, Times(C2, b)), CN1),
                  Plus(Times(Sqrt(Times(Pi, Power(z, CN1))), Erfi(Sqrt(z))),
                      Times(CN1, Power(Power(Negate(z), b), CN1),
                          Subtract(Gamma(b), Gamma(b, Negate(z)))))),
              PossibleZeroQ(Plus(C1, b, Negate(c))))),
      ...
      ISetDelayed(HypergeometricPFQ(list(C1, C1), list(C2, C2, QQ(3L, 2L)), z_),
          Times(Power(z, CN1),
              Plus(Negate(EulerGamma), CoshIntegral(Times(C2, Sqrt(z))),
                  Negate(Log(Times(C2, Sqrt(z))))))),
      ...
      ISetDelayed(HypergeometricPFQ(list(C1), list(C1D2, c_), z_),
          Plus(C1,
              Times(CSqrtPi, Power(z, Times(C1D4, Plus(C3, Times(CN2, c)))), Gamma(c),
                  StruveL(Plus(C1, Times(C1D2, Plus(CN3, Times(C2, c)))), Times(C2, Sqrt(z)))))),
      ...
      ISetDelayed(HypergeometricPFQ(list(C1), list(QQ(3L, 2L), c_), z_),
          Times(C1D2, CSqrtPi, Power(z, Plus(C1D4, Times(CN1D2, c))), Gamma(c),
              StruveL(Plus(QQ(-3L, 2L), c), Times(C2, Sqrt(z))))));
}
