...
public abstract class AbstractRubiTestCase {
    ...
    if (result.isFree(F.Integrate)) {
      ...
      if (expectedResult.length() > 0) {
        ...
        if (expected.isTrue()) {
          ...
          // stage produce a different-looking (but correct) antiderivative than Rubi.
          //
  ...
  /**
   * Tests if in the evaluation of the integral the same rule is used as is used by Rubi
   *
