...
/**
 * Abstract factorization algorithms class. This class contains implementations of all methods of
 * the <code>Factorization</code> interface, except the method for factorization of a squarefree
 * polynomial. The methods to obtain squarefree polynomials delegate the computation to the <code>
...
public abstract class FactorAbstract<C extends GcdRingElem<C>> implements Factorization<C> {
  ...
  /**
   * GenPolynomial factorization of a multivariate squarefree polynomial, using Kronecker
   * substitution and variable order optimization.
   *
  ...
  public List<GenPolynomial<C>> factorsSquarefreeOptimize(GenPolynomial<C> P) {
    ...
    if (degv.length() == donv.length) { // all variables appear
      ...
      // GenPolynomial<C> pl = PolyUtil.<C> removeUnusedLowerVariables(pu); // not useful after
      // optimize
  ...
  public List<GenPolynomial<C>> factorsSquarefree(GenPolynomial<C> P) {
    if (P != null && P.ring.nvar > 1) {
      logger.warn(
          "no multivariate factorization for "
              + P.toScript()
              + ": falling back to Kronecker algorithm in "
              + P.ring.toScript());
  ...
  public List<GenPolynomial<C>> factorsSquarefreeKronecker(GenPolynomial<C> P) {
    ...
    logger.info(
        "deg(subs(P,d=" + d + ")) = " + kr.degree(0) + ", original degrees: " + P.degreeVector());
    if (debug) {
    ...
    // wrong: List<GenPolynomial<C>> klist = PolyUfdUtil.<C> backSubstituteKronecker(pfac, ulist,
    // d);
    // System.out.println("back(klist) = " + PolyUfdUtil.<C> backSubstituteKronecker(pfac, ulist,
    // d));
    if (logger.isInfoEnabled()) {
   ...
   * @return remove every element of b from a, but only one occurrence. <b>Note:</b> not available
   *     in java.util.
   */
   ...
   * @return [p_1, ..., p_k] with p = prod_{i=1,...,k} p_i**{e_i} for some e_i for all p in L.
   */
  ...
  public SortedMap<GenPolynomial<C>, Long> factors(GenPolynomial<C> P) {
    ...
    if (!pfac.tord.equals(TermOrderByName.INVLEX)) {
      logger.warn(
          "wrong term order "
              + pfac.tord
              + ", factorization may not be correct, better use "
  ...
  /**
   * GenPolynomial greatest squarefree divisor. Delegates computation to a GreatestCommonDivisor
   * class.
   *
   * @param P GenPolynomial.
   * @return squarefree(P).
   ...
   * @return true if P = prod_{i=1,...,k} p_i**e_i , else false.
   */
  public boolean isRecursiveFactorization(
      GenPolynomial<GenPolynomial<C>> P, SortedMap<GenPolynomial<GenPolynomial<C>>, Long> F) {
   ...
   * @return [p_1,...,p_k] with P = prod_{i=1, ..., k} p_i.
   */
  public List<GenPolynomial<GenPolynomial<C>>> recursiveFactorsSquarefree(
   ...
   * @return [p_1 -&gt; e_1, ..., p_k -&gt; e_k] with P = prod_{i=1,...,k} p_i**e_i.
   */
  public SortedMap<GenPolynomial<GenPolynomial<C>>, Long> recursiveFactors(
      GenPolynomial<GenPolynomial<C>> P) {
    if (P == null) {
      throw new IllegalArgumentException(this.getClass().getName() + " P != null");
    }
