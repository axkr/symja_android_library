...
IIntegrate(8,Integrate(Times(u_DEFAULT,Power(x_,m_DEFAULT),Power(Times(a_DEFAULT,x_),p_)),x_Symbol),
    Condition(Simp(Dist(Power(Power(a,m),CN1),Integrate(Times(u,Power(Times(a,x),Plus(m,p))),x),x),x),And(FreeQ(list(a,m,p),x),IntegerQ(m)))),
IIntegrate(9,Integrate(Times(u_DEFAULT,Power($p("§px"),p_DEFAULT),Power(Times(e_DEFAULT,x_),m_DEFAULT)),x_Symbol),
    Condition(With(list(Set(r,Expon($s("§px"),x,Min))),Condition(Simp(Dist(Power(Power(e,Times(p,r)),CN1),Integrate(Times(u,Power(Times(e,x),Plus(m,Times(p,r))),Power(ExpandToSum(Times($s("§px"),Power(Power(x,r),CN1)),x),p)),x),x),x),IGtQ(r,C0))),And(FreeQ(list(e,m),x),PolyQ($s("§px"),x),IntegerQ(p),Not(MonomialQ($s("§px"),x))))),
IIntegrate(10,Integrate(Times(u_DEFAULT,Power(Times(e_DEFAULT,x_),m_DEFAULT),Power(Plus(Times(a_DEFAULT,Power(x_,r_DEFAULT)),Times(b_DEFAULT,Power(x_,s_DEFAULT))),p_DEFAULT)),x_Symbol),
    Condition(Simp(Dist(Power(Power(e,Times(p,r)),CN1),Integrate(Times(u,Power(Times(e,x),Plus(m,Times(p,r))),Power(Plus(a,Times(b,Power(x,Subtract(s,r)))),p)),x),x),x),And(FreeQ(List(a,b,e,m,r,s),x),IntegerQ(p),Or(IntegerQ(Times(p,r)),GtQ(e,C0)),PosQ(Subtract(s,r))))),
IIntegrate(11,Integrate(Times(u_DEFAULT,Power(Times(e_DEFAULT,x_),m_DEFAULT),Power(Plus(Times(a_DEFAULT,Power(x_,r_DEFAULT)),Times(b_DEFAULT,Power(x_,s_DEFAULT)),Times(c_DEFAULT,Power(x_,t_DEFAULT))),p_DEFAULT)),x_Symbol),
    Condition(Simp(Dist(Power(Power(e,Times(p,r)),CN1),Integrate(Times(u,Power(Times(e,x),Plus(m,Times(p,r))),Power(Plus(a,Times(b,Power(x,Subtract(s,r))),Times(c,Power(x,Subtract(t,r)))),p)),x),x),x),And(FreeQ(List(a,b,c,e,m,r,s,t),x),IntegerQ(p),Or(IntegerQ(Times(p,r)),GtQ(e,C0)),PosQ(Subtract(s,r)),PosQ(Subtract(t,r))))),
...
IIntegrate(18,Integrate(Times(c_DEFAULT,Power(Plus(a_DEFAULT,Times(b_DEFAULT,u_)),m_)),x_Symbol),
    Condition(Simp(Dist(Power(D(u,x),CN1),Subst(Integrate(Times(c,Power(Plus(a,Times(b,x)),m)),x),x,u),x),x),And(FreeQ(List(a,b,c,m),x),LinearQ(u,x),NeQ(u,x)))),
IIntegrate(19,Integrate(Power(Times(a_DEFAULT,Power(x_,CN1)),p_),x_Symbol),
    Condition(Simp(Times(CN1,a,Power(Times(a,Power(x,CN1)),Plus(p,CN1)),Power(Plus(p,CN1),CN1)),x),And(FreeQ(list(a,p),x),Not(IntegerQ(p))))),
IIntegrate(20,Integrate(Power(Times(a_DEFAULT,Power(x_,n_)),p_),x_Symbol),
    Condition(Simp(Dist(Times(Power(Times(a,Power(x,n)),p),Power(Power(x,Times(n,p)),CN1)),Integrate(Power(x,Times(n,p)),x),x),x),And(FreeQ(list(a,n,p),x),Not(IntegerQ(p)))))
  );
}
