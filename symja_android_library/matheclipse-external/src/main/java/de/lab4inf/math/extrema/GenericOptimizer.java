...
public abstract class GenericOptimizer extends L4MObject implements Optimizer {
	...
	/**
	 * Return the Gradient for this function. This implementation uses an approximation, derived implementations should
	...
	/**
	 * Return the Hessian for this function. This implementation uses an approximation, derived implementations should
	 * provide an analytical Hessian if possible.
	 *
	 * @param fct
	 *            to use
	 ...
	 *            of iteration
	 */
	protected final void informIterationIsFinished(final int iteration, final double... actualguess) {
		synchronized (listeners) {
	 ...
	 *            final counter value
	 * @param bestguess
	 *            of iteration after finish
	 */
	protected final void informOptimizationIsFinished(final int iteration, final double... bestguess) {
		synchronized (listeners) {
			for (OptimizerListener l : listeners) {
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.lab4inf.extrema.Optimizer#minimize(de.lab4inf.extrema.Computable, double[])
	 */
	 ...
	 * @param guess
	 *            the initial starting guess for the solution
	 ...
	 * @return boolean flag if minimization already finished, which is false for an asynchron run.
	 */
	protected final boolean startMinimize(final Function fct, final double... guess) {
		Thread t = new WorkerThread(true, format("Minimizer-%d", incrementThreadCounter()), fct, guess);
		t.start();
		return false;
	}
	 ...
	 * @param guess
	 *            initial guess of the solution
	 * @return boolean flag if maximization already finished, which is false for an asynchron run
	 */
	protected final boolean startMaximize(final Function fct, final double... guess) {
		Thread t = new WorkerThread(false, format("Maximizer-%d", incrementThreadCounter()), fct, guess);
		t.start();
		return false;
	}
	...
	private class WorkerThread extends Thread {
		...
		WorkerThread(final boolean minimize, final String name, final Function fct, final double... guess) {
			super(name);
