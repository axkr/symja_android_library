...
/**
 * GenPolynomial generic polynomials implementing RingElem. n-variate ordered polynomials over
 * coefficients C. The variables commute with each other and with the coefficients. For
 * non-commutative coefficients some care is taken to respect the multiplication order.
 *
 * <p>Objects of this class are intended to be immutable. The implementation is based on TreeMap
 * respectively SortedMap from exponents to coefficients. Only the coefficients are modeled with
 * generic types, the exponents are fixed to ExpVector with long entries (this will eventually be
 * changed in the future). C can also be a non integral domain, e.g. a ModInteger, i.e. it may
 * contain zero divisors, since multiply() does check for zeros. <b>Note:</b> multiply() now checks
 * for wrong method dispatch for GenSolvablePolynomial.
 *
 * @param <C> coefficient type
 * @author Heinz Kredel
 */
public class GenPolynomial<C extends RingElem<C>>
    implements RingElem<GenPolynomial<C>>, /* not yet Polynomial<C> */ Iterable<Monomial<C>> {
  ...
  /**
   * Put an ExpVector to coefficient entry into the internal map of this GenPolynomial. <b>Note:</b>
   * Do not use this method unless you are constructing a new polynomial. this is modified and
   * breaks the immutability promise of this class.
   *
   * @param c coefficient.
   * @param e exponent.
   */
  ...
  public String toScript() {
    ...
    for (Map.Entry<ExpVector, C> m : val.entrySet()) {
      ...
      boolean parenthesis =
          (cs.indexOf("-") >= 0 || cs.indexOf("+") >= 0) && !klammern.matcher(cs).matches();
      if (!c.isONE() || e.isZERO()) {
        if (parenthesis) {
          s.append("( ");
        }
   ...
   */
  public GenPolynomial<C> scaleSubtractMultiple(
      C b, ExpVector g, C a, ExpVector e, GenPolynomial<C> S) {
    if (a == null || S == null) {
  ...
  /**
   * GenPolynomial right division. Right division by coefficient ring element. Fails, if exact
  ...
  /**
   * GenPolynomial left division. Left division by coefficient ring element. Fails, if exact
   * division is not possible.
   *
   * @param s coefficient.
   * @return s**(-1) * this.
  ...
  /**
   * Extend variables. Used e.g. in module embedding. Extend all ExpVectors by i elements and
   * multiply by x_j^k.
   *
  ...
  /**
   * Extend lower variables. Used e.g. in module embedding. Extend all ExpVectors by i lower
  ...
  /**
   * Contract variables to coefficient polynomial. Remove i elements of each ExpVector, removed
   ...
   *
   * @return number of bits in the representation of this polynomial, including sign bits.
   */
