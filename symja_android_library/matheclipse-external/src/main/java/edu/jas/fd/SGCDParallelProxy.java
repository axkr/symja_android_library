...
/**
 * Solvable greatest common divisor parallel proxy. Executes methods from two implementations in
 * parallel and returns the result from the fastest run. Uses timeout on <code>invokeAny()</code>
 * and return fake common divisor <it>1</it> in case of timeout.
 *
 * @author Heinz Kredel
 */
public class SGCDParallelProxy<C extends GcdRingElem<C>> extends GreatestCommonDivisorAbstract<C> {
   ...
   */
  public SGCDParallelProxy(
      RingFactory<C> cf, GreatestCommonDivisorAbstract<C> e1, GreatestCommonDivisorAbstract<C> e2) {
    super(cf);
  ...
  public GenSolvablePolynomial<C> leftBaseGcd(
    ...
    List<Callable<GenSolvablePolynomial<C>>> cs =
        new ArrayList<Callable<GenSolvablePolynomial<C>>>(2);
    cs.add(
        new Callable<GenSolvablePolynomial<C>>() {
  ...
  public GenSolvablePolynomial<GenPolynomial<C>> leftRecursiveUnivariateGcd(
    ...
    List<Callable<GenSolvablePolynomial<GenPolynomial<C>>>> cs =
        new ArrayList<Callable<GenSolvablePolynomial<GenPolynomial<C>>>>(2);
    cs.add(
        new Callable<GenSolvablePolynomial<GenPolynomial<C>>>() {
   ...
   */
  @Override
  public GenSolvablePolynomial<C> leftGcd(
      final GenSolvablePolynomial<C> P, final GenSolvablePolynomial<C> S) {
    if (debug) {
      if (ComputerThreads.NO_THREADS) {
        throw new RuntimeException("this should not happen");
   ...
   */
  @Override
  public GenSolvablePolynomial<C> rightGcd(
      final GenSolvablePolynomial<C> P, final GenSolvablePolynomial<C> S) {
    if (debug) {
      if (ComputerThreads.NO_THREADS) {
        throw new RuntimeException("this should not happen");
