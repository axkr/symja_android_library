...
public class Rectangle<C extends RingElem<C> & Rational> implements Serializable {
  ...
  public Rectangle(Complex<C> sw, Complex<C> ne) {
    this(
        new Complex<C>(sw.ring, sw.getRe(), ne.getIm()),
        sw,
        new Complex<C>(sw.ring, ne.getRe(), sw.getIm()),
        ne);
  }
  ...
  public String toString() {
    ...
    // return centerApprox() + " = [" + corners[0] + ", " + corners[1] + ", " + corners[2] + ", " +
    // corners[3] + "]";
  }
  ...
  public boolean contains(Complex<C> c) {
    ...
    C cre = c.getRe();
    C cim = c.getIm();
    return cre.compareTo(ll.getRe()) >= 0
        && cim.compareTo(ll.getIm()) >= 0
        && cre.compareTo(ur.getRe()) <= 0
        && cim.compareTo(ur.getIm()) <= 0;
  }
  ...
  public boolean contains(Rectangle<C> r) {
    return contains(r.getSW())
        && contains(r.getNE()); // && contains(r.getSE()) && contains(r.getNW())
  }
