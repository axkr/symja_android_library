...
/**
 * Basic linear algebra methods based on double precision arrays. It provides basic methods
 * comparable to the BLAS I,II and III but without multi-processor parallel computing support.
 *
...
public final class LinearAlgebra extends L4MObject {
  ...
  /**
   * String representation of a n-dimensional array vector, using a default format assignment of 4
   * decimal digits.
   *
  ...
  /**
   * Helper method to calculate sqrt(x_0*x_0 + ... + x_n*x_n) without over- or underflow.
   *
  ...
  /**
   * Euclidean norm of a vector or parameter list with given start to end range to be included into
   * the calculation.
   *
  ...
  public static double norm2(final double[][] x) {
    ...
    final int m = x[0].length;
    double aij, norm = 0;
    for (int i = 0; i < n; i++) for (int j = 0; j < m; aij = x[i][j], norm += aij * aij, j++) ;
  ...
  /**
   * Norm of a NxN matrix, the default is the L2 norm according to the euclidean vector norm.
   *
   * @param x double[][] the matrix
   * @return double the norm
   */
  ...
  /**
   * The maximum matrix norm of the difference of two matrixes x,y, e.g. the greatest difference
   * |x_ij - y_ij|.
   *
  ...
  private static <T extends Numeric<T>> T[] op(
      final T[] x, final BinaryOperator<?> o, final T[] y) {
    dimensionCheck(x, y);
    final int n = x.length;
  ...
  public static float[] crossProduct(final float[] u, final float[] v) {
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < n; j++)
        for (int k = 0; k < n; k++) w[i] += leviCivita(i, j, k) * u[j] * v[k];
    return w;
  }
  ...
  /**
   * Cross- or vector product of two generic vectors u,v of type T resulting in a vector
  ...
  public static double[][] multTrans(final double[] x, final double[] y) {
    ...
    for (int i = 0; i < n; i++) for (int j = 0; j < n; result[i][j] = x[i] * y[j], j++) ;
    return result;
  }
  ...
  public static float[][] transpose(final float[][] a) {
    ...
    for (int i = 0; i < n; i++) for (int j = 0; j < m; b[j][i] = a[i][j], j++) ;
    return b;
  }
  ...
  public static Complex[][] conjugate(final Complex[][] a) {
    ...
    for (int i = 0; i < n; i++) for (int j = 0; j < m; b[j][i] = a[i][j].conj(), j++) ;
    return b;
  }
  ...
  public static double[] mult(final double[] x, final double[][] a) {
    ...
    for (i = 0; i < m; y[i] = tmp, i++) for (tmp = 0, j = 0; j < n; tmp += a[j][i] * x[j], j++) ;
    return y;
  }
  ...
  public static double[][] mult(final double[][] a, final double scale) {
    ...
    for (i = 0; i < n; i++) for (j = 0; j < m; c[i][j] = a[i][j] * scale, j++) ;
    return c;
  }
  ...
  public static <T extends Numeric<T>> T[][] identity(final T type, final int n) {
    ...
    for (int i = 0; i < n; e[i][i] = one, i++) for (int j = 0; j < n; e[i][j] = zero, j++) ;
  ...
  public static <T extends Numeric<T>> T[][] rndMatrix(final T o, final int n, final int m) {
    ...
    for (int i = 0; i < n; i++) for (int j = 0; j < m; r[i][j] = o.rnd(), j++) ;
    return r;
  }
  ...
  public static double[][] rndSymmetricMatrix(final int n) {
    ...
    for (int j = 0; j < n; j++) for (int k = 0; k < j; k++) a[j][k] = a[k][j];
    return a;
  }
  ...
  public static double[][] rndOrthonormalMatrix(final int n) {
    ...
    w = rndVector(n);
    normalize(w);
    for (int j = 0; j < n; j++) for (int k = 0; k < n; k++) p[j][k] -= 2 * w[j] * w[k];
   ...
   * @return the matrix
   */
  public static double[][] createTridiagonalToeplitz(
      final int dim, final double a, final double b, final double c) {
  ...
  public static double[] eigenvaluesTridiagonalToeplitzMatrix(
      final int dim, final double a, final double b, final double c) {
  ...
  /**
   * create the N,N dimensional nonesymmetric Pseudo Hilbert matrix. The greatest eigenvalue of the
   * matrix converges to pi. This matrix seems to be due to: E. E. Tyrtyshnikov...
   *
   * <p>H_i,j = 1/(i-j+1/2)
   *
   * @param dim the matrix dimension
  ...
  /**
   * create the N,N dimensional symmetric Hilbert matrix. A matrix with a very bad condition.
   *
   * <p>H_i,j = 1/(i+j-1)
   *
   * @param dim the matrix dimension
  ...
  /**
   * Create a default N,N dimensional Vandermonde matrix V, with the seed values x[j] = 1/(j+1).
   *
  ...
  /**
   * Create the N,N dimensional inverse Vandermonde matrix V**(-1). The algorithmn uses an explizit
   * formular for the inverse of the LU decomposition of V.
   *
   * @param x the matrix column seeds
  ...
  public static double[][] createInverseVandermondeMatrix(final double[] x) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      // if(i>0) invU[i][0]=0;
      for (int j = i + 1; j < n; j++) {
        if (i == 0) invU[i][j] = -invU[i][j - 1] * x[j - 1];
        else invU[i][j] = invU[i - 1][j - 1] - invU[i][j - 1] * x[j - 1];
      }
    }
  ...
  /**
   * create the N,N dimensional inverse symmetric Hilbert matrix. A matrix with a very bad
   * condition.
   *
   * <pre>
   *
  ...
  public static double[][] createInverseHilbertMatrix(final int dim) {
    ...
    for (int i = 1; i <= dim; i++) {
      for (int j = 1; j <= i; j++) {
        ...
        hij *= hij;
        hij *=
            ((double) binomial(dim + i - 1, dim - j)) * ((double) binomial(dim + j - 1, dim - i));
        // sign = (i+j)%2==0 ? 1 : -1;
        sign = 1 - 2 * ((i + j) & 1);
  ...
  /**
   * Return the eigenvalues of the first 10 hilbert matrix. The values are adapted from:
   *
   * <pre>
  ...
  public static double[] eigenvaluesHilbertMatrix(final int dim) {
    ...
    final double[][] evs = {
      ...
      /* 5 */ {
        1.567050691098231,
        2.08534218611013E-1,
        1.140749162341981E-2,
        3.05898040151191E-4,
        3.287928772171863E-6
      },
      /* 6 */ {
        1.618899858924339,
        2.423608705752096E-1,
        1.632152131987582E-2,
        6.157483541826577E-4,
        1.257075712262519E-5,
        1.082799484565550E-7
      },
      /* 7 */ {
        1.660885338926931,
        2.719201981493452E-1,
        2.128975490832795E-2,
        1.008587610770142E-3,
        2.938636814592969E-5,
        4.856763361574250E-7,
        3.493898605991218E-9
      },
      /* 8 */ {
        1.695938996921949,
        2.981252113169307E-1,
        2.621284357811905E-2,
        1.467688117741867E-3,
        5.436943369749942E-5,
        1.294332091872811E-6,
        1.798873745817577E-8,
        1.111538966372442E-10
      },
      /* 9 */ {
        1.725882660901847,
        3.216331222992068E-1,
        3.103892578126833E-2,
        1.978933860215924E-3,
        8.758085051459757E-5,
        2.673013410599414E-6,
        5.385613348522494E-8,
        6.460905422638582E-10,
        3.499676402911493E-12
      },
      /* 10 */ {
        1.75191967026517,
        3.429295484835091E-1,
        3.574181627163924E-2,
        2.530890768670038E-3,
        1.287496142763771E-4,
        4.729689293182348E-6,
        1.228967738751175E-7,
        2.147438817350479E-9,
        2.266746747762926E-11,
        1.093153819379666E-13
      }
    };
    if (dim < 1 || dim > 10) throw new IllegalArgumentException("wrong dimension:" + dim);
    return evs[dim - 1];
  }
  ...
  /**
   * Calculate the maximal {|x_0|, |x_1|, .... |x_n-1|} element, using the absolute magnitude.
   *
  ...
  /**
   * Calculate the maximal {|x_0|, |x_1|, .... |x_n-1|} element, using the absolute magnitude,
   * beginning from the given start index.
   *
   * @param start index
   * @param x array or parameter list
   * @return the absolute maximal element
  ...
  /**
   * Check if the given matrix is symmetric, that is A[i][j] == A[j][i].
   *
  ...
  /**
   * Check if the given matrix is self-adjoint, that is A[i][j] == A*[j][i].
   *
   * @param a generic matrix to check
   * @param <T> type of matrix elements
  ...
  /**
   * Check if the given matrix is hermitian (self-adjoint), that is A[i][j] == A*[j][i].
   *
  ...
  public static boolean isHermitian(final Complex[][] a) {
    ...
    if (ret) {
      ...
      for (int i = 0; ret && i < n; i++) {
        ...
        for (int j = 0; j < i; j++) {
          if (!isSimilar(a[i][j].real(), a[j][i].real())
              || !isSimilar(a[i][j].imag(), -a[j][i].imag())) {
            ret = false;
            break;
          }
        }
  ...
  public static boolean isReal(final Complex[] v) {
    ...
    for (int i = 0; i < n; i++) if (!v[i].isReal()) return false;
  ...
  public static boolean isReal(final Complex[][] a) {
    final int n = a.length;
    for (int i = 0; i < n; i++) if (!isReal(a[i])) return false;
  ...
  public static boolean isImaginary(final Complex[] v) {
    final int n = v.length;
    for (int i = 0; i < n; i++) if (v[i].real() != 0) return false;
  ...
  /**
   * Minimal check if matrix a can be positive definite. Necessary but not sufficient are:
   *
   * <pre>
   *
   * 1)  a[i][i] &gt; 0
  ...
  /**
   * Check if the given matrix is diagonal dominant, that is abs(A[i][i]) greater than sum
   * abs(A[i][j]).
   *
   * @param a float[][] the matrix to check
  ...
  /**
   * Check if the given matrix is diagonal dominant using the given (pivot) reordering p, that is
   * abs(A[p[i]][i]) is greater than sum abs(A[p[i]][j]).
   *
  ...
  /**
   * Quick check, if a matrix is regular or might be singular. If all eigenvalues are either
   * positive or all are negative the matrix cannot be singular, but is definite regular, otherwise
   * it depends on the exact location of the eigenvalues not checked by this method.
   *
   * @param a double[][] the matrix to check
  ...
  /**
   * Using the Gerschgorin circles find the real range [a,b] which holds all eigenvalues of matrix
   * a.
   *
   * @param a double[][] the matrix to check
  ...
  public static double[] characteristic(final double[][] a) {
    ...
    nbk = norm(bk);
    if (abs(c[0]) > 1) nbk /= ((double) n) * ((double) n) * abs(c[0]);
    if (nbk > Accuracy.FEPS) {
      String msg =
          format(
              "characteristic polynomial p%s numerical instable %.3g",
              Strings.toLowerScript(n), nbk);
      getLogger().error(msg);
  ...
  public static double[][] real(final Complex[][] c) {
    ...
    for (int j = 0; j < n; j++) for (int k = 0; k < m; k++) b[j][k] = c[j][k].real();
  ...
  public static double[][] imag(final Complex[][] c) {
    ...
    final int m = c[0].length;
    final double[][] b = new double[n][m];
    for (int j = 0; j < n; j++) for (int k = 0; k < m; k++) b[j][k] = c[j][k].imag();
    return b;
  }
  ...
  public static double[] real(final Complex[] a) {
    ...
    for (int j = 0; j < n; j++) b[j] = a[j].real();
  ...
  public static double[] imag(final Complex[] a) {
    final int n = a.length;
    final double[] b = new double[n];
    for (int j = 0; j < n; j++) b[j] = a[j].imag();
    return b;
  }
   ...
   *
   * <p>C = (A + jB), z=(x+jy) and b=(c + jd) using reals.
   *
   * <p>This method returns the corresponding real matrix for the system:
   *
   * <pre>
   *        ( A  | -B ) ( x )   ( c )
   *        (----+----) (   ) = (   )
  ...
  /**
   * Transform the complex vector z into a real one twice as long with the first half the real and
   * the second half the imaginary part.
   *
  ...
  /**
   * Create a complex vector from the real input vector, where the first half are the real and the
   * second half are the and imaginary parts.
   *
   * @param v input vector
   * @return complex vector.
   */
  public static Complex[] createComplexVector(final double[] v) {
    if (v.length % 2 != 0) throw new IllegalArgumentException("not a real/imag input vector");
  ...
  /**
   * Create a complex matrix from the real input matrix, where the first half are the real and the
   * second half are the and imaginary parts.
   *
   * @param a input matrix
  ...
  /**
   * Return the sign vector of x. For all x[j] &ge; 0 sign is +1 other wise -1.
   *
  ...
  /**
   * Estimate the L1 condition of a square matrix A. This algorithm gives a lower bound and is based
