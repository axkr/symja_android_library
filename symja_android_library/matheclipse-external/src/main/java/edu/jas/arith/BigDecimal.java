...
/**
 * BigDecimal class to make java.math.BigDecimal available with RingElem interface. Objects of this
 * class are immutable. Experimental, use with care, compareTo is hacked.
 *
 * @author Heinz Kredel
...
public final class BigDecimal implements GcdRingElem<BigDecimal>, RingFactory<BigDecimal> {
   ...
   * @see edu.jas.structure.ElemFactory#isFinite() <b>Note: </b> is actually finite but returns
   *     false.
   */
  ...
  /**
   * Query if this ring is associative. Floating point number addition is not associative, but
   * multiplication is.
   *
   * @return true.
   */
   ...
   *
   * @param b BigDecimal.
   * @return 0 if abs(this-b)/max(this,b) &lt; epsilon, 1 if this &gt; b, -1 if this &lt; b.
   */
   ...
   * BigDecimal random.
   *
   * @param n such that 0 &le; val(r) &le; (2<sup>n</sup>-1). 0 &le; exp(r) &le; (10-1).
  ...
  /**
   * Returns the number of bits in the representation of this BigDecimal, including a sign bit. For
   * positive BigDecimal, this is equivalent to {@code val.unscaledValue().bitLength()}.)
   *
   * @return number of bits in the representation of this BigDecimal, including a sign bit.
   */
