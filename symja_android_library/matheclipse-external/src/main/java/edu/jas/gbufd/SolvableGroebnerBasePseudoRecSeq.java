 ...
 * fraction free Groebner bases. Coefficients can for example be (commutative) multivariate
 * polynomials.
 *
...
public class SolvableGroebnerBasePseudoRecSeq<C extends GcdRingElem<C>>
   ...
   */
  public SolvableGroebnerBasePseudoRecSeq(
   ...
   */
  public SolvableGroebnerBasePseudoRecSeq(
      RingFactory<GenPolynomial<C>> rf, SolvablePseudoReduction<C> red) {
   ...
   */
  @Override
  public List<GenSolvablePolynomial<GenPolynomial<C>>> leftGB(
      int modv, List<GenSolvablePolynomial<GenPolynomial<C>>> F) {
    List<GenSolvablePolynomial<GenPolynomial<C>>> G = normalizeZerosOnes(F);
    G =
        PolynomialList.<GenPolynomial<C>>castToSolvableList(
            PolyUtil.<C>monicRec(
                engine.recursivePrimitivePart(PolynomialList.<GenPolynomial<C>>castToList(G))));
    if (G.size() <= 1) {
      return G;
    }
  ...
  public List<GenSolvablePolynomial<GenPolynomial<C>>> leftMinimalGB(
    ...
    List<GenSolvablePolynomial<GenPolynomial<C>>> F =
        new ArrayList<GenSolvablePolynomial<GenPolynomial<C>>>(G.size());
    while (G.size() > 0) {
      a = G.remove(0);
    ...
    while (i < len) {
      ...
      a =
          (GenSolvablePolynomial<GenPolynomial<C>>)
              engine.recursivePrimitivePart(a); // a.monic(); not possible
   ...
   */
  @Override
  public List<GenSolvablePolynomial<GenPolynomial<C>>> twosidedGB(
      int modv, List<GenSolvablePolynomial<GenPolynomial<C>>> Fp) {
    ...
    List<GenSolvablePolynomial<GenPolynomial<C>>> F =
        new ArrayList<GenSolvablePolynomial<GenPolynomial<C>>>(G.size() * (1 + X.size()));
    F.addAll(G);
  ...
  public boolean isTwosidedGB(int modv, List<GenSolvablePolynomial<GenPolynomial<C>>> Fp) {
    ...
    logger.info("right multipliers = " + X);
    List<GenSolvablePolynomial<GenPolynomial<C>>> F =
        new ArrayList<GenSolvablePolynomial<GenPolynomial<C>>>(Fp.size() * (1 + X.size()));
    F.addAll(Fp);
  ...
  @Override
  public SolvableExtendedGB<GenPolynomial<C>> extLeftGB(
      int modv, List<GenSolvablePolynomial<GenPolynomial<C>>> F) {
