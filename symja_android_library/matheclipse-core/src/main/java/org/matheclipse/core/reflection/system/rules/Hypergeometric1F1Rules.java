...
public class Hypergeometric1F1Rules {
  ...
  final public static IAST RULES = List(IInit(Hypergeometric1F1, SIZES),
      ...
      ISetDelayed(Hypergeometric1F1($p(a, Integer), b_, z_),
          Module(list(Set(n, Negate(a))),
              Condition(
                  Sum(Times(FactorialPower(Negate(n), k, CN1),
                      Power(FunctionExpand(FactorialPower(b, k, CN1)), CN1), Power(z, k),
                      Power(Factorial(k), CN1)), list(k, C0, n)),
                  And(Less(a, C0),
                      Not(TrueQ(And(Element(b, Integers), LessEqual(b, C0), Greater(b, a)))))))));
}
