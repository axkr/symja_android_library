    ...
    private class ComplexSolver {
        ...
        public Complex[] solveAll(Complex coefficients[], Complex initial)
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = n - i - 1; j >= 0; j--) {
                    oldc = c[j];
                    c[j] = newc;
                    newc = oldc.sum(newc.multiply(root[i]));
                }
            }
        ...
        public Complex solve(Complex coefficients[], Complex initial)
            ...
            while (true) {
                ...
                for (int j = n-1; j >= 0; j--) {
                    d2v = dv.sum(z.multiply(d2v));
                    dv = pv.sum(z.multiply(dv));
                    pv = coefficients[j].sum(z.multiply(pv));
                }
