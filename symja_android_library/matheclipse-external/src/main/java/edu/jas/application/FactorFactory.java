...
/**
 * Factorization algorithms factory. Select appropriate factorization engine based on the
 * coefficient types.
 *
 * <p><b>Usage:</b> To create objects that implement the <code>Factorization</code> interface use
 * the <code>FactorFactory</code>. It will select an appropriate implementation based on the types
 * of polynomial coefficients C. To obtain an implementation use <code>getImplementation()</code>,
 * it returns an object of a class which extends the <code>FactorAbstract</code> class which
 * implements the <code>Factorization</code> interface.
 *
 * <pre>
...
public class FactorFactory extends edu.jas.ufd.FactorFactory {
  ...
  /**
   * Determine suitable implementation of factorization algorithms, case recursive
   * GenPolynomial&lt;C&gt;. Use <code>recursiveFactors()</code>.
   *
  ...
  public static <C extends GcdRingElem<C> & Rational>
      FactorAbstract<RealAlgebraicNumber<C>> getImplementation(RealAlgebraicRing<C> fac) {
    return new FactorRealAlgebraic<C>(
        fac, FactorFactory.<AlgebraicNumber<C>>getImplementation(fac.algebraic));
  }
  ...
  public static <C extends GcdRingElem<C> & Rational>
      FactorAbstract<edu.jas.application.RealAlgebraicNumber<C>> getImplementation(
          edu.jas.application.RealAlgebraicRing<C> fac) {
    edu.jas.root.RealAlgebraicRing<C> rar =
        (edu.jas.root.RealAlgebraicRing<C>) (Object) fac.realRing;
    return new FactorRealReal<C>(
        fac, FactorFactory.<edu.jas.root.RealAlgebraicNumber<C>>getImplementation(rar));
  }
  ...
  public static <C extends GcdRingElem<C>> FactorAbstract<C> getImplementation(RingFactory<C> fac) {
    ...
    if (ofac instanceof edu.jas.application.RealAlgebraicRing) {
      ...
      // ofac = rfac.algebraic;
      ufd =
          new FactorRealAlgebraic /*raw <C>*/(
              rfac, FactorFactory.<AlgebraicNumber<C>>getImplementation(rfac.algebraic));
      ...
      ufd =
          new FactorAlgebraic /*raw <C>*/(
              afac, FactorFactory.<C>getImplementation(afac.ring.coFac));
    } else if (ofac instanceof QuotientRing) {
      // System.out.println("qfac_o = " + ofac);
      qfac = (QuotientRing) ofac;
      ufd =
          new FactorQuotient /*raw <C>*/(qfac, FactorFactory.<C>getImplementation(qfac.ring.coFac));
