 ...
 * </pre>
 * <p>
 * Unlike an ordinary polynomial this class offers the algebraic properties of a mathematical ring and with the
 * exception of series truncation the division is the inverse of the multiplication operation.
 * <p>
 * Further there are special methods like the sin, cosine or exponentiation or root of a power series defined via:
 * 
 * <pre>
 *
 ...
 * </pre>
 * 
 * To derive further none straightforward formulae for a power function f use the derivative chain rule:
 * 
 * <pre>
 *  q(x) := f(p(x)) then  q'(x) = df/dp*p'(x)
 * </pre>
 * 
 * and do a recursive power by power comparison. E.g. for the exp power series:
 * 
 * <pre>
 *   q(x) := exp(p(x))
 *
...
public class PowerSeries extends L4MFunction implements Differentiable, Integrable, Field<PowerSeries> {
	...
	/**
	 * Distance between two power series, building block for a metric space. Let k be the index of the first different
	 * coefficient than the distance is 2**(-k). If all coefficients are the same the distance is zero.
	 *
	 * @param x
	 *            first series
	 * @param y
	...
	/**
	 * Calculate sin(p) and cos(p) for the given series p(x) at once. The arrays c and s have to be allocated
	 * appropiate.
	 *
	 * @param p
	 *            input series
	 * @param c
	 *            coefficients for the cosine series
	 * @param s
	 *            coefficients for the sine series
	 */
	...
	/**
	 * Division of two power series, truncated at the lowest degree of both series.
	 *
	 * @param x
	 *            the series to divide with
	 * @return this / x
	 */
	@Operand(symbol = "/")
	...
	/**
	 * Calculate the anti-derivative/primitive of this series. This is unique up to the constant p_0 set to zero.
	 *
	 * @return P(x)
	 */
