...
ISetDelayed(Int(Power(Plus(Times(b_DEFAULT,Power(x_,n_DEFAULT)),Times(a_DEFAULT,Power(x_,q_DEFAULT)),Times(c_DEFAULT,Power(x_,r_DEFAULT))),p_),x_Symbol),
    Condition($(AbortRubi($s("Int")),Power(Plus(Times(a,Power(x,q)),Times(b,Power(x,n)),Times(c,Power(x,Plus(Times(C2,n),Negate(q))))),p),x),And(FreeQ(List(a,b,c,n,p,q),x),ZeroQ(Plus(r,Negate(Plus(Times(C2,n),Negate(q)))))))),
