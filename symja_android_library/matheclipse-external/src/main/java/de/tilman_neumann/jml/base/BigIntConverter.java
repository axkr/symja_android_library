 ...
 *
 * @see <a
 *     href="https://de.wikipedia.org/wiki/IEEE_754#Allgemeines">https://de.wikipedia.org/wiki/IEEE_754#Allgemeines</a>
...
public class BigIntConverter {
  ...
  public static BigInteger fromDouble(double d) {
    ...
    // Bits 62-52 (the bits that are selected by the mask 0x7ff0000000000000L) represent the
    // exponent.
    // We have to subtract the fixed bias value (1023 for doubles) to get the real exponent e,
    // and use the unsigned shift operation >>> to forget the sign bit and shift in "0"s from the
    // left.
  ...
  /**
   * Compute BigInteger from double multiplied with a power of 2, i.e. result ~ d * 2^e2, with
  ...
  public static BigInteger fromDoubleMulPow2(double d, int e2) {
    ...
    // Now we can reduce the loss of precision we'ld get converting M/2^p to integer !!
    // Note that shiftLeft(n) will do a right shift for negative n.
    // Exponent e2 can simply be added, but will only have beneficial effects on the result
    // precision if it is positive.
