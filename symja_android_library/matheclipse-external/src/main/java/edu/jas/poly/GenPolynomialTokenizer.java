...
/**
 * GenPolynomial Tokenizer. Used to read rational polynomials and lists of polynomials from input
 * streams. Arbitrary polynomial rings and coefficient rings can be read with RingFactoryTokenizer.
 * <b>Note:</b> Can no more read QuotientRing since end of 2010, revision 3441. Quotient
 * coefficients and others can still be read if the respective factory is provided via the
 * constructor.
 *
...
public class GenPolynomialTokenizer {
  ...
  /**
   * Parsing method for GenPolynomial. Syntax depends also on the syntax of the coefficients, as the
   * respective parser is used. Basic term/monomial syntax:
   *
   * <pre>
   * ... coefficient variable**exponent ... variable^exponent + ... - ....
   * </pre>
   *
   * Juxtaposition means multiplication <code>*</code>. Then terms/monomials can be added or
   * subtracted <code>+, -</code> and grouped by parenthesis <code>()</code>. There are some
   * heuristics to detect when a coefficient should be parsed. To force parsing of a coefficient
   * enclose it in braces <code>{}</code>.
   *
        ...
        case StreamTokenizer.TT_WORD:
          ...
          if (digit(first) || first == '/' || first == '.' || first == '~') {
            ...
            if (tok.sval.charAt(tok.sval.length() - 1) == 'i') { // complex number
              tt = tok.nextToken();
              if (debug) logger.debug("tt,im = " + tok);
              if (tok.sval != null || tt == '-') {
                if (tok.sval != null) {
                  df.append(tok.sval);
                } else {
  ...
  public RingFactory nextCoefficientRing() throws IOException {
    ...
    if (tok.sval != null) {
      if (tok.sval.equalsIgnoreCase("Q")) {
        ...
        logger.warn(
            "parse of octonion coefficients may fail for negative components (use ~ for -)");
        ...
        // logger.error("RatFunc and ModFunc can no more be read, see
  ...
  /**
   * Parsing method for solvable polynomial. Syntax: same as for polynomial. If the relation table
   * is set-up, then multiplication will mean solvable-multiplication.
   *
   * @return the next polynomial.
   * @throws IOException
   */
