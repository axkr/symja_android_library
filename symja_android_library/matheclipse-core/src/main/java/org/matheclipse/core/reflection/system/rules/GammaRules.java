...
public class GammaRules {
  ...
  final public static IAST RULES = List(IInit(Gamma, SIZES),
      ...
      ISet(Gamma(Undefined), Undefined, true),
      // Gamma(-5/2)=(-1)*1/15*8*Sqrt(Pi)
      ISet(Gamma(QQ(-5L, 2L)), Times(QQ(-8L, 15L), CSqrtPi), true),
      // Gamma(-3/2)=1/3*4*Sqrt(Pi)
      ISet(Gamma(QQ(-3L, 2L)), Times(QQ(4L, 3L), CSqrtPi), true),
      // Gamma(-1/2)=-2*Sqrt(Pi)
      ISet(Gamma(CN1D2), Times(CN2, CSqrtPi), true),
      // Gamma(1/2)=Sqrt(Pi)
      ISet(Gamma(C1D2), CSqrtPi, true),
      // Gamma(3/2)=Sqrt(Pi)/2
      ISet(Gamma(QQ(3L, 2L)), Times(C1D2, CSqrtPi), true),
      // Gamma(5/2)=1/4*3*Sqrt(Pi)
      ISet(Gamma(QQ(5L, 2L)), Times(QQ(3L, 4L), CSqrtPi), true),
      // Gamma(Infinity)=Infinity
      ISet(Gamma(oo), oo, true),
      // Gamma(-Infinity)=Indeterminate
      ISet(Gamma(Noo), Indeterminate, true),
      // Gamma(I*Infinity)=0
      ISet(Gamma(DirectedInfinity(CI)), C0, true),
      // Gamma(-I*Infinity)=0
      ISet(Gamma(DirectedInfinity(CNI)), C0, true),
      // Gamma(ComplexInfinity)=Indeterminate
      ISet(Gamma(CComplexInfinity), Indeterminate, true),
      ...
      ISetDelayed(Gamma(C1D2, PatternTest(z_, NumericQ)), Times(CSqrtPi, Erfc(Sqrt(z)))),
