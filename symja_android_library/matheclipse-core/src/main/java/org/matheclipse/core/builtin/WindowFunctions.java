...
public class WindowFunctions {
  ...
  private static class Initializer {
    ...
    private static void init() {
      ...
      S.BlackmanHarrisWindow.setEvaluator(
          new WindowFunction(WindowFunctions::blackmanHarrisWindow));
      S.BlackmanNuttallWindow.setEvaluator(
          new WindowFunction(WindowFunctions::blackmanNuttallWindow));
  ...
  public static IExpr bartlettWindow(IExpr x) {
    ...
    F.Piecewise(
        F.List(
            F.List(F.Plus(F.C1, F.Times(F.CN2, x)), F.LessEqual(F.C0, x, F.C1D2)),
            F.List(
                F.Plus(F.C1, F.Times(F.C2, x)), F.And(F.LessEqual(F.CN1D2, x), F.Less(x, F.C0)))),
        F.C0); // $$;
  }
  ...
  public static IExpr gaussianWindow(IExpr x) {
    ...
    F.Piecewise(
        F.List(
            F.List(
                F.Exp(F.Times(F.CN1, F.QQ(1L, 9L), F.ZZ(50L), F.Sqr(x))),
                F.LessEqual(F.CN1D2, x, F.C1D2))),
        F.C0); // $$;
  }
  ...
  public static IExpr parzenWindow(IExpr x) {
    ...
    F.Piecewise(
        F.List(
            F.List(
                F.Times(F.CN2, F.Power(F.Plus(F.CN1, F.Times(F.C2, x)), F.C3)),
                F.And(F.Less(F.C1D4, x), F.LessEqual(x, F.C1D2))),
            F.List(
                F.Times(F.C2, F.Power(F.Plus(F.C1, F.Times(F.C2, x)), F.C3)),
                F.And(F.LessEqual(F.CN1D2, x), F.Less(x, F.CN1D4))),
            F.List(
                F.Plus(F.C1, F.Times(F.ZZ(-24L), F.Sqr(x)), F.Times(F.ZZ(-48L), F.Power(x, F.C3))),
