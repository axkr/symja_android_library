...
public class GaussSolver extends LASolver {
	...
	/**
	 * Internal helper method to perform the L*R decomposition. It has a runtime complexity of order 2/3n**3
	 *
	 * @param a
	 ...
	 *
	 * @param m
	 *            integer the column/row index to start with
	 * @param p
	 *            int[] array with the pivot indexes
	 * @param a
	 *            double[][] the matrix
	 */
	...
	private static <T extends Numeric<T>> void pivot(final int m, final int[] p, final T[][] a) {
		...
		// calculate the scaling to avoid fake pivot elements
		for (k = m; k < n; s[k] = sk, k++)
			for (sk = a[p[k]][m].abs().doubleValue(), j = m + 1; j < n; sk = max(sk,
					a[p[k]][j].abs().doubleValue()), j++)
				;
		// compare the scaled pivot elements
	 ...
	 *            the original matrix
	 * @param lr
	 *            the LR decomposition of A
	 * @param y
	 *            the right side vector
	 * @param x
	 *            the left side solution vector
	 * @param p
	 *            the pivot vector
	 */
	 ...
	 * @param x
	 *            the left side solution matrix
	 */
	private static void residualCorrection(final float[][] a, final float[][] lr, final float[][] b,
			final float[][] x) {
		int m = b[0].length;
		for (int k = 0; k < m; k++)
	...
	/**
	 * Internal check if pivot search for matrix A should be used. This is true if the matrix is not diagonal dominant.
	 *
	 * @param a
	 ...
	 *
	 * @param a
	 *            float[][] real NxN matrix.
	 * @param y
	 *            float[] real N dimensional right side vector.
	 ...
	 *
	 * @param a
	 *            double[][] real NxN matrix.
	 * @param y
	 *            double[] real N dimensional right side vector.
	 ...
	 *            type T NxN matrix.
	 * @param y
	 *            N dimensional right side vector.
	 * @param <T>
	 *            the elements type
	 ...
	 *            Complex NxN matrix.
	 * @param y
	 *            Complex NxN dimensional right side vectors as matrix.
	 * @param <T>
	 *            the elements type
	 ...
	 *            float[][] real NxN matrix
	 * @param b
	 *            float[][] real NxN right side vectors as matrix
	 ...
	 *            double[][] real NxN matrix
	 * @param b
	 *            double[][] real NxN right side vectors as matrix
	...
	private <T extends Numeric<T>> T[][] solveWithPivot(final T[][] a, final T[][] b) {
		...
		if (resErr > getResidualError()) {
			residualCorrection(a, ma, b, x, p);
			double corrected = LinearAlgebra.norm1(residual(a, b, x));
			getLogger().warn(String.format("Global Residual Error %e changed to %e ratio=%f", resErr, corrected,
					corrected / resErr));
		}
		return x;
	}
