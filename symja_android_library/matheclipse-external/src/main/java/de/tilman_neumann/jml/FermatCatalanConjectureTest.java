 ...
 *
 * <p>The latter condition exludes (m,n,k) = (2,2,k), (2,3,3), (2,3,4), (2,3,5) with infinitely many
 * solutions; (2,3,6); and (2,4,4) and (3,3,3) not having any solutions.
 *
 * <p>If we find a solution with m,n,k > 2 then we have a counterexample to Beal's conjecture and
...
public class FermatCatalanConjectureTest {
  ...
  private static void test(int m, int n) {
    // Catalan's conjecture means that the only solution with x=1 or y=1 is 2^3 + 1^n = 3^2 -> no
    // need to test that
    for (int a = 2; ; a++) {
      LOG.info("Test a=" + a);
  ...
  /**
   * If Beal's conjecture is correct, one of the powers has to be a square. This test is about 7
   * times faster than the general test.
   *
   * @param m
   * @param n
   */
    ...
    for (int a = 2; ; a++) {
      ...
      for (long b = bMax; b >= bMin; b--) {
        if (gcdEngine.gcd(a, b) == 1) {
          ...
          if (sqrt != null) {
            LOG.info(
                "Case 1 found solution " + a + "^" + m + " + " + b + "^" + n + " = " + sqrt + "^2");
            LOG.debug(n + ".th root(a^m) = " + nthRoot + ", b - nthRoot = " + (b - nthRoot));
          }
        }
      }
      ...
      for (long b = nthRoot; b >= bMin; b--) {
        if (gcdEngine.gcd(a, b) == 1) {
          ...
          if (sqrt != null) {
            LOG.info(
                "Case 2 found solution " + a + "^" + m + " = " + b + "^" + n + " + " + sqrt + "^2");
            LOG.debug(n + ".th root(a^m) = " + nthRoot + ", b - nthRoot = " + (b - nthRoot));
          }
        }
      }
      ...
      for (long b = nthRoot + 1; b <= bMax; b++) {
        if (gcdEngine.gcd(a, b) == 1) {
          ...
          if (sqrt != null) {
            LOG.info(
                "Case 3 found solution " + b + "^" + n + " = " + a + "^" + m + " + " + sqrt + "^2");
            LOG.debug(n + ".th root(a^m) = " + nthRoot + ", b - nthRoot = " + (b - nthRoot));
          }
        }
      }
  ...
  public static void main(String[] args) {
    ...
    // Case 1 found solution 17^7 + 76271^3 = 21063928^2      nthRoot(a^m) = 743, b - nthRoot =
    // 75528      (Beukers, Zagier)
    // Case 2 found solution 65^7 = 1414^3 + 2213459^2        nthRoot(a^m) = 16987, b - nthRoot =
    // -15573   (Beukers, Zagier)
    // Case 2 found solution 113^7 = 9262^3 + 15312283^2      nthRoot(a^m) = 61732, b - nthRoot =
    // -52470   (Beukers, Zagier)
    //		test(8, 3);
    // Case 3 found solution 15613^3 = 33^8 + 1549034^2       nthRoot(a^m) = 11203, b - nthRoot =
    // 4410     (Beukers, Zagier)
    // Case 1 found solution 43^8 + 96222^3 = 30042907^2      nthRoot(a^m) = 22694, b - nthRoot =
    // 73528    (Beukers, Zagier)
    // test(9, 3);
