    ...
    public T[] value(T x) throws NoDataException, NullArgumentException {
        ...
        for (int i = 0; i < topDiagonal.size(); ++i) {
            ...
            for (int k = 0; k < value.length; ++k) {
                value[k] = value[k].sum(dividedDifference[k].multiply(valueCoeff));
            }
        ...
        for (int i = 0; i < topDiagonal.size(); ++i) {
            ...
            for (int j = order; j >= 0; --j) {
                for (int k = 0; k < derivatives[j].length; ++k) {
                    derivatives[j][k] =
                            derivatives[j][k].sum(dividedDifference[k].multiply(valueCoeff[j]));
                }
                valueCoeff[j] = valueCoeff[j].multiply(deltaX);
                if (j > 0) {
                    valueCoeff[j] = valueCoeff[j].sum(tj[j].multiply(valueCoeff[j - 1]));
                }
            }
        }
