...
/**
 * Container for the partial fraction decomposition of a squarefree denominator. num/den = sum( a_i
 * / d_i )
 *
 * @author Heinz Kredel
...
public class PartialFraction<C extends GcdRingElem<C>> implements Serializable {
   ...
   * @param ad list of linear (irreducible) factors d_i of d with algebraic coefficients. n/d = sum(
   *     a_i / d_i )
   */
  ...
  public boolean isPartialFraction() {
    ...
    for (AlgebraicNumberRing<C> ar : fields) {
      ...
      GenPolynomialRing<GenPolynomial<C>> prfac =
          new GenPolynomialRing<GenPolynomial<C>>(cfac, apfac);
