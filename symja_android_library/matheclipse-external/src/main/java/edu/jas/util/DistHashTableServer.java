...
/**
 * Server for the distributed version of a list. TODO: redistribute list for late coming clients,
 * removal of elements.
 *
 * @author Heinz Kredel
 */
public class DistHashTableServer<K> extends Thread {
  ...
  public static final int DEFAULT_PORT = 9009; // ChannelFactory.DEFAULT_PORT + 99;
...
class DHTBroadcaster<K> extends Thread /*implements Runnable*/ {
  ...
  public DHTBroadcaster(
      SocketChannel s, List<DHTBroadcaster<K>> bc, SortedMap<K, DHTTransport> sm) {
    channel = s;
    bcaster = bc;
    theList = sm;
  }
