...
public class PSIQS_SB extends PSIQSBase {
	 ...
	 */
	public PSIQS_SB(float Cmult, float Mmult, Integer wantedQCount, int numberOfThreads, PowerFinder powerFinder, MatrixSolver matrixSolver) {
	...
	public String getName() {
		return "PSIQS_SB(Cmult=" + Cmult + ", Mmult=" + Mmult + ", qCount=" + apg.getQCount() + ", " + powerFinder.getName() + ", " + matrixSolver.getName() + ", " + numberOfThreads + " threads)";
	}
	...
	protected PSIQSThreadBase createThread(
		...
		return new PSIQSThread_SB(k, N, kN, d, sieveParams, baseArrays, apg, cc, threadIndex);
	}
}
