...
public interface TanRules {
  final public static IAST RULES = List(
    ...
    ISetDelayed(Tan(ArcSin(x_)),
      Times(x,Power(Plus(C1,Negate(Sqr(x))),Power(C1D2,CN1)))),
    ISetDelayed(Tan(Times(Pi,$p(x,NumberQ))),
      Condition(If(Less(x,C1),Negate(Tan(Times(Plus(C1,Negate(x)),Pi))),If(Less(x,C2),Tan(Times(Plus(x,Negate(C1)),Pi)),Tan(Times(Plus(x,Negate(Times(C2,Quotient(IntegerPart(x),C2)))),Pi)))),Greater(x,C1D2))),
    ISetDelayed(Tan(ArcTan(x_)),
      x),
    ISetDelayed(Tan(ArcCos(x_)),
      Times(Sqrt(Plus(C1,Negate(Sqr(x)))),Power(x,CN1))),
