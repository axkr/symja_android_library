...
/**
 * Polynomial pseudo reduction sequential use algorithm. Coefficients of polynomials must not be
 * from a field, i.e. the fraction free reduction is implemented. Implements normalform.
 *
 * @param <C> coefficient type
 * @author Heinz Kredel
 */
public class SolvablePseudoReductionSeq<C extends GcdRingElem<C>>
    extends SolvableReductionAbstract<C> implements SolvablePseudoReduction<C> {
  ...
  /**
   * Left normalform with recording. <b>Note:</b> Only meaningful if all divisions are exact.
   * Compute first the multiplication factor <code>m</code> with <code>normalform(Pp,Ap,m)</code>,
   ...
   * @param Pp polynomial list.
   * @param Ap polynomial.
   * @return ( nf(Ap), mf ) with respect to Pp and mf as multiplication factor for Ap.
   */
  @SuppressWarnings("unchecked")
  public PseudoReductionEntry<C> leftNormalformFactor(
      List<GenSolvablePolynomial<C>> Pp, GenSolvablePolynomial<C> Ap) {
    if (Ap == null) {
      return null;
    }
   ...
   * @param Pp a polynomial list for reduction.
   * @param Ap a polynomial.
   * @return nf(Pp,Ap), the normal form of Ap wrt. Pp. <b>Note: </b> not implemented;
   */
  public GenSolvablePolynomial<C> rightNormalform(
      List<GenSolvablePolynomial<C>> row,
      List<GenSolvablePolynomial<C>> Pp,
   ...
   * @param Pp polynomial list.
   * @param Ap polynomial.
   * @return ( nf(Ap), mf ) with respect to Pp and mf as multiplication factor for Ap. <b>Note: </b>
   *     not implemented;
   */
  public PseudoReductionEntry<C> rightNormalformFactor(
      List<GenSolvablePolynomial<C>> Pp, GenSolvablePolynomial<C> Ap) {
    if (Pp == null || Ap == null) {
      throw new IllegalArgumentException("Pp or Ap == null not supported");
    }
