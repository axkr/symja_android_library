 ...
 *
 * <p><b>Usage:</b> To create objects that implement the <code>GroebnerBase</code> interface one can
 * use the <code>GBFactory</code> or this <code>GBAlgorithmBuilder</code>. This class will select
 * and compose an appropriate implementation based on the types of polynomial coefficients C and the
 * desired properties. To build an implementation start with the static method <code>
 * polynomialRing()</code> to define the polynomial ring. Then continue to construct the algorithm
 * with the methods
 *
 * <ul>
 ...
 *   <li><code>normalPairlist()</code> (default), <code>syzygyPairlist()</code> or <code>
 *       simplePairlist()</code> for pair-list selection strategies,
 *   <li><code>fractionFree()</code> for clearing denominators and computing with pseudo reduction,
 *   <li><code>graded()</code> for using the FGLM algorithm to first compute a Gr&ouml;bner base
 *       with respect to a graded term order and then constructing a Gr&ouml;bner base wrt. a
 *       lexicographical term order,
 *   <li><code>walk()</code> for using the Gr&ouml;bner walk algorithm to first compute a
 *       Gr&ouml;bner base with respect to a graded term order and then constructing a Gr&ouml;bner
 *       base wrt. a lexicographical term order,
 *   <li><code>iterated()</code> for using the iterative GB algorithm to compute a Gr&ouml;bner base
 *       adding one polynomial after another,
 *   <li><code>F5()</code>, <code>GGV()</code> and <code>Arri()</code> for using the respective
 *       iterative signature based GB algorithm (over field coefficients) to compute a Gr&ouml;bner
 *       base adding one polynomial after another,
 *   <li><code>parallel()</code> additionaly compute a Gr&ouml;bner base over a field or integral
 *       domain in parallel,
 ...
 * </ul>
 *
 * <p>Finally call the method <code>build()</code> to obtain an implementaton of class <code>
 * GroebnerBaseAbstract</code>. For example
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>For example, if the coefficient type is BigRational, the usage looks like
 *
 * <pre>
...
public class GBAlgorithmBuilder<C extends GcdRingElem<C>> implements Serializable {
  ...
  public GBAlgorithmBuilder(
      GenPolynomialRing<C> ring, GroebnerBaseAbstract<C> algo, PairList<C> strategy) {
    if (ring == null) {
   ...
   */
  public static <C extends GcdRingElem<C>> GBAlgorithmBuilder<C> polynomialRing(
      GenPolynomialRing<C> fac) {
    return new GBAlgorithmBuilder<C>(fac);
  }

  /**
   * Select syzygy critical pair-list strategy. Gebauer and M&ouml;ller algorithm.
   *
   * @return GBAlgorithmBuilder object.
   */
  ...
  /**
   * Request fraction free algorithm. For BigRational and Quotient coefficients denominators are
   * cleared and pseudo reduction is used.
   *
   * @return GBAlgorithmBuilder object.
   */
      ...
      if (algo instanceof GroebnerBaseQuotient) { // fraction free requested
        ...
        bb =
            (GroebnerBaseAbstract)
                new GroebnerBaseQuotient<C>(threads, fac, pli); // pl not possible
      } else {
        bb = (GroebnerBaseAbstract) new GroebnerBaseParallel<C>(threads, strategy);
      }
      GroebnerBaseAbstract<C> pbb = new GBProxy<C>(algo, bb);
