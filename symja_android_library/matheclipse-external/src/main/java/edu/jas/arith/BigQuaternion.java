...
/**
 * BigQuaternion class based on BigRational implementing the RingElem interface and with the
...
public /*final*/ class BigQuaternion
   ...
   * Since quaternion numbers are unordered, we use lexicographical order of re, im, jm and km.
   *
   * @return 0 if this is equal to 0; 1 if re > 0, or re == 0 and im > 0, or ...; -1 if re < 0, or
   *     re == 0 and im < 0, or ...
  ...
  public BigQuaternion subtract(BigQuaternion B) {
    return new BigQuaternion(
        ring, re.subtract(B.re), im.subtract(B.im), jm.subtract(B.jm), km.subtract(B.km));
  }
  ...
  public BigQuaternion inverse() {
    ...
    return new BigQuaternion(
        ring,
        re.multiply(a),
        im.negate().multiply(a),
        jm.negate().multiply(a),
  ...
  public BigQuaternion divide(BigRational b) {
    ...
    return new BigQuaternion(
        ring, re.multiply(bi), im.multiply(bi), jm.multiply(bi), km.multiply(bi));
  }
   ...
   * It is equivalent to {@code re.bitLength()+im.bitLength()+jm.bitLength()+km.bitLength()}.)
   *
   * @return number of bits in the representation of this BigQuaternion, including a sign bit.
   */
  ...
  /**
   * BigQuaternion round to next Lipschitz integer. BigQuaternion with all integer components.
   *
  ...
  /**
   * BigQuaternion round to next Hurwitz integer. BigQuaternion with all integer or all 1/2 times
   * integer components.
   *
