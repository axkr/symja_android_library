...
public abstract class HMAbstractList<E> extends HMAbstractCollection<E> implements HMList<E> {
	...
	private class SimpleListIterator implements Iterator<E> {
		...
		public void remove() {
			...
			try {
				...
				HMAbstractList.this.remove(lastPosition);
			} catch (IndexOutOfBoundsException e) {
				throw new ConcurrentModificationException();
			}
	...
	private final class FullListIterator extends SimpleListIterator implements HMListIterator<E> {
		...
		public void add(E object) {
			...
			try {
				HMAbstractList.this.append(size() - numLeft, object);
				expectedModCount = modCount;
				lastPosition = -1;
			} catch (IndexOutOfBoundsException e) {
				throw new NoSuchElementException();
		...
		public void set(E object) {
			...
			try {
				HMAbstractList.this.set(lastPosition, object);
			} catch (IndexOutOfBoundsException e) {
				throw new IllegalStateException();
			}
		}
	...
	private static final class SubAbstractListRandomAccess<E> extends SubAbstractList<E> implements RandomAccess {
		SubAbstractListRandomAccess(HMAbstractList<E> list, int start, int end) {
			super(list, start, end);
		}
	}

	private static class SubAbstractList<E> extends HMAbstractList<E> {
		private final HMAbstractList<E> fullList;

		private int offset;

		private int size;

		private static final class SubAbstractListIterator<E> implements HMListIterator<E> {
			private final SubAbstractList<E> subList;

			private final HMListIterator<E> iterator;

			private int start;

			private int end;

			SubAbstractListIterator(HMListIterator<E> it, SubAbstractList<E> list, int offset, int length) {
				super();
				iterator = it;
				subList = list;
				start = offset;
		...
		SubAbstractList(HMAbstractList<E> list, int start, int end) {
			super();
			fullList = list;
		...
		public void append(int location, E object) {
			if (modCount == fullList.modCount) {
				if (0 <= location && location <= size) {
					fullList.append(location + offset, object);
					size++;
					modCount = fullList.modCount;
				} else {
					throw new IndexOutOfBoundsException();
		...
		@Override
		public HMListIterator<E> listIterator(int location) {
			if (modCount == fullList.modCount) {
				if (0 <= location && location <= size) {
	...
	public int indexOf(Object object) {
		HMListIterator<?> it = listIterator();
		if (object != null) {
			while (it.hasNext()) {
	...
	public int lastIndexOf(Object object) {
		HMListIterator<?> it = listIterator(size());
		if (object != null) {
	 ...
	 * @see HMListIterator
	 */
	public HMListIterator<E> listIterator() {
		return listIterator(0);
	}
