...
public abstract class RealRootsAbstract<C extends RingElem<C> & Rational> implements RealRoots<C> {
   ...
   */
  public List<Interval<C>> refineIntervals(
   ...
   */
  public abstract Interval<C> invariantSignInterval(
      Interval<C> iv, GenPolynomial<C> f, GenPolynomial<C> g);
   ...
   * @param f univariate polynomial, non-zero.
   * @param eps requested interval length.
   * @return a list of decimal approximations d such that |d-v| &lt; eps for all real v with f(v) =
   *     0.
   */
   ...
   * @param f univariate polynomial, non-zero.
   * @param eps requested interval length.
   * @return true if each x in R is a decimal approximation of a real v with f(v) = 0 with |d-v|
   *     &lt; eps, else false.
   */
