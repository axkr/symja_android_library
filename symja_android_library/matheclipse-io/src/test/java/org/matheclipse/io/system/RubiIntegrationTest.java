...
public class RubiIntegrationTest extends AbstractTestCase {
  ...
  public void testRubi003() {
    ...
    ast =
        MatchQ(Times(2, x), Condition(Plus(a_DEFAULT, Times(b_DEFAULT, x)), FreeQ(List(a, b), x)));
    check(ast, "True");
  }
  ...
  public void testRationalFunction003() {
    check(
        "Integrate((b+2*c*x)*(a+b*x+c*x^2)^m, x)", //
        "(a+b*x+c*x^2)^(1+m)/(1+m)");
    check(
        "D((a+b*x+c*x^2)^13/13,x)", //
        "(b+2*c*x)*(a+b*x+c*x^2)^12");
    check(
        "Integrate((b+2*c*x)*(a+b*x+c*x^2)^12, x)", //
        "(a+b*x+c*x^2)^13/13");
  }
