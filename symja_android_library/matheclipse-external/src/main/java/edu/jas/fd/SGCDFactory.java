...
/**
 * Solvable greatest common divisor algorithms factory. Select appropriate SGCD engine based on the
 * coefficient types.
 *
 * <p><b>Usage:</b> To create objects that implement the <code>GreatestCommonDivisor</code>
 * interface use the <code>SGCDFactory</code>. It will select an appropriate implementation based on
 ...
 * GreatestCommonDivisor</code> interface. The proxy will run two implementations in parallel,
 * return the first computed result and cancel the second running task. On systems with one CPU the
 * computing time will be two times the time of the fastest algorithm implementation. On systems
 * with more than two CPUs the computing time will be the time of the fastest algorithm
 * implementation.
 *
 * <pre>
    ...
    if (ofac instanceof BigInteger) {
      ...
      if (fac.isField()) {
        ...
        ufd =
            new SGCDParallelProxy<C>(
                fac,
                new GreatestCommonDivisorSyzygy<C>(fac),
                new GreatestCommonDivisorPrimitive<C>(fac));
      }
    }
