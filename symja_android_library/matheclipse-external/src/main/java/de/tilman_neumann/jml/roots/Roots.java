...
public class Roots {
   ...
   * <code>i</code>) or a Heron iteration procedure (for rather small roots <code>i</code>). The
   * decision point between the two algorithms is heuristic and may not be very accurate.
   *
   * @param N argument
   * @param i root
  ...
  /**
   * Computes integer values around (i.th root(N)), using a bitwise correction approach. This
   * algorithm is fast for very small <code>ld(N)/i</code>.
   *
   * @param N argument
   * @param i root
   ...
   *
   * <p>This algorithm is much faster than the bitwise correction except for <code>ld(N)/i <= 2
   * </code>.</br></br>
   *
   * <p>Notes:</br> 1. We use a good initial guess based on double computations.</br> 2. The main
   * loop could have convergence problems for small <code>ld(N)/i</code>; but in these cases the
   * initial guess is very good! Testing the initial guess before the main loop solves the
   * problem.</br> 3. Doing an additional iteration step before the main loop does not pay
   * out.</br></br>
   *
   * <p>Version 1 is sure to give the correct result because of the final step doing a loop.
   *
   * @param N
   * @param i
   * @return [lower, upper] int values of i.th root(N)
  ...
  public static BigInteger[] ithRoot_Heron1(BigInteger N, int i) {
    ...
    if (cmp < 0) {
      ...
      // if (linStepCount>0) LOG.info(i + ".th root(" + N + ") required " + linStepCount + " linear
      // steps at the end.");
      return cmp == 0
          ? new BigInteger[] {guess, guess}
          : new BigInteger[] {guess.subtract(I_1), guess};
    }
    if (cmp > 0) {
  ...
  private static BigInteger computeInitialGuess(BigInteger N, int i) {
    ...
    if (bits >= 1024) {
      // right shift: the shift result should have at least the double mantissa precision which is
      // 52 bit.
      ...
      double root =
          Math.pow(N.shiftRight(shiftsRight).doubleValue(), 1.0D / i)
              * Math.pow(2, shiftsLeft_frac);
    ...
    for (int bits = 100; ; bits += 100) {
      ...
      for (int root = 3; ((float) bits) / root > 4; root += 1 + RNG.nextInt(10)) {
        ...
        if (TEST_BITWISE) {
          ...
          LOG.info(
              "   Bitwise ith root test with " + nCount + " numbers took " + (t1 - t0) + " ms");
        }
