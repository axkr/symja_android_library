...
/**
 * Rational number coefficients factorization algorithms. This class implements factorization
 * methods for polynomials over rational numbers.
 *
 * @author Heinz Kredel
 */
public class FactorRational extends FactorAbsolute<BigRational> {
  ...
  public SortedMap<GenPolynomial<BigRational>, Long> factors(GenPolynomial<BigRational> P) {
    ...
    GenPolynomialRing<BigRational> pfac = P.ring;
    SortedMap<GenPolynomial<BigRational>, Long> factors =
        new TreeMap<GenPolynomial<BigRational>, Long>(pfac.getComparator());
    if (P.isZERO()) {
      return factors;
    }
    if (P.isONE()) {
