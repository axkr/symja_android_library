...
/**
 * ResidueSolvableWordPolynomialRing solvable polynomial with word residue coefficients factory. It
 * implements RingFactory and extends GenSolvablePolynomialRing factory. Factory for n-variate
 * ordered solvable polynomials over non-commutative word residue coefficients. The non-commutative
 * multiplication relations are maintained in a relation table and the non-commutative
 * multiplication relations between the coefficients and the main variables are maintained in a
 * coefficient relation table. Almost immutable object, except variable names and relation table
 * contents.
 *
...
public class ResidueSolvableWordPolynomialRing<C extends GcdRingElem<C>>
  ...
  private static final Logger logger =
      LogManager.getLogger(ResidueSolvableWordPolynomialRing.class);

  private static final boolean debug = logger.isDebugEnabled();

  /**
   * The constructor creates a solvable polynomial factory object with the default term order and
   ...
   */
  public ResidueSolvableWordPolynomialRing(
      RingFactory<WordResidue<C>> cf, int n, RelationTable<WordResidue<C>> rt) {
    this(cf, n, new TermOrder(), null, rt);
  }

  /**
   * The constructor creates a solvable polynomial factory object with the given term order and
   * commutative relations.
   *
   * @param cf factory for coefficients of type C.
   * @param n number of variables.
   ...
   */
  public ResidueSolvableWordPolynomialRing(
      RingFactory<WordResidue<C>> cf, int n, TermOrder t, RelationTable<WordResidue<C>> rt) {
    this(cf, n, t, null, rt);
  }
   ...
   */
  public ResidueSolvableWordPolynomialRing(
      RingFactory<WordResidue<C>> cf, int n, TermOrder t, String[] v) {
    this(cf, n, t, v, null);
  }
   ...
   */
  public ResidueSolvableWordPolynomialRing(
      RingFactory<WordResidue<C>> cf, TermOrder t, String[] v) {
  ...
  /**
   * The constructor creates a solvable polynomial factory object with the the same term order,
   * number of variables and variable names as the given polynomial factory, only the coefficient
   * factories differ and the solvable multiplication relations are <b>empty</b>.
   *
   * @param cf factory for coefficients of type C.
   * @param o other solvable polynomial ring.
   */
  public ResidueSolvableWordPolynomialRing(
      RingFactory<WordResidue<C>> cf, ResidueSolvableWordPolynomialRing o) {
  ...
  /**
   * Query if this ring is associative. Test if the relations between the mian variables and the
   * coefficient generators define an associative solvable ring.
   *
  ...
  /**
   * Random solvable polynomial. Generates a random solvable polynomial with k = 5, l = n, d = (nvar
   * == 1) ? n : 3, q = (nvar == 1) ? 0.7 : 0.3.
   *
  ...
  public List<ResidueSolvableWordPolynomial<C>> univariateList(int modv, long e) {
    List<ResidueSolvableWordPolynomial<C>> pols =
        new ArrayList<ResidueSolvableWordPolynomial<C>>(nvar);
    int nm = nvar - modv;
    for (int i = 0; i < nm; i++) {
  ...
  public ResidueSolvableWordPolynomialRing<C> extend(int i, boolean top) {
    ...
    ResidueSolvableWordPolynomialRing<C> spfac =
        new ResidueSolvableWordPolynomialRing<C>(pfac.coFac, pfac.nvar, pfac.tord, pfac.getVars());
  ...
  /**
   * Extend variables. Used e.g. in module embedding. Extend number of variables by length(vn). New
   * variables commute with the exiting variables.
   *
  ...
  public ResidueSolvableWordPolynomialRing<C> reverse(boolean partial) {
    ...
    ResidueSolvableWordPolynomialRing<C> spfac =
        new ResidueSolvableWordPolynomialRing<C>(
            pfac.coFac, pfac.nvar, pfac.tord, pfac.getVars()); // , pfac.table);
  ...
  public GenSolvablePolynomialRing<WordResidue<C>> permutation(List<Integer> P) {
    ...
    GenSolvablePolynomialRing<WordResidue<C>> pfac =
        (GenSolvablePolynomialRing<WordResidue<C>>) super.permutation(P);
    return pfac;
  }

  /**
   * Word residue coefficients from integral word polynomial coefficients. Represent as polynomial
   * with type WordResidue<C> coefficients.
   *
   * @param A polynomial with integral word polynomial coefficients to be converted.
   * @return polynomial with type WordResidue<C> coefficients.
   */
  public ResidueSolvableWordPolynomial<C> fromPolyCoefficients(
      GenSolvablePolynomial<GenWordPolynomial<C>> A) {
  ...
  /**
   * Integral word function from word residue coefficients. Represent as polynomial with type
   * GenWordPolynomial<C> coefficients.
   *
