...
public class CompileAnalyzer {
  ...
  /**
   * The type of the numbers in each lifted constant table, keyed by the symbol which stands for it.
   *
   * <p>
   * Without this a read of such a table is just <code>Part</code> of an unknown symbol, which
   * infers as symbolic and takes everything computed from it with it - so a lookup into a table of
   * integers would not be recognized as producing an integer.
   */
  private final Map<ISymbol, VarType> constantArrays = new HashMap<>();

  /** Record that <code>symbol</code> stands for a lifted table of numbers of type <code>type</code>. */
  public void declareConstantArray(ISymbol symbol, VarType type) {
    constantArrays.put(symbol, type);
  }
  ...
  private VarType analyzeAST(IAST ast) {
    ...
    if (head.isBuiltInSymbol()) {
      switch (((IBuiltInSymbol) head).ordinal()) {
        ...
        case ID.Part:
          VarType partType = analyzePart(ast);
          if (partType != VarType.UNKNOWN) {
            return partType;
          }
          break;
        default:
  ...
  /**
   * The type of a read of a lifted constant table, or {@link VarType#UNKNOWN} if this is a
   * <code>Part</code> of something else.
   *
   * <p>
   * Both <code>t[[i, j]]</code> and <code>t[[i]][[j]]</code> reach here; the indices are analyzed
   * either way, so that a variable used only as an index is still typed.
   */
  private VarType analyzePart(IAST ast) {
    IExpr base = ast;
    while (base.isAST(S.Part) && ((IAST) base).argSize() >= 2) {
      IAST part = (IAST) base;
    ...
    VarType type = constantArrays.get(base);
    return type == null ? VarType.UNKNOWN : type;
  }
