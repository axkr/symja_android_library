 ...
 *
 * <p>Appears to be faster than the Gaussian solver starting at N>200 bit.
 *
...
public class MatrixSolver02_BlockLanczos extends MatrixSolver {
  ...
  @Override
  protected void solve(List<Smooth> congruences, Map<Integer, Integer> factors_2_columnIndices)
    ...
    // LOG.debug("constructed matrix with " + matrixBlength + " rows and " +
    // factors_2_columnIndices.size() + " columns");
    ...
    for (int mask = 1; mask != 0; mask *= 2) {
      HashSet<AQPair> totalAQPairs =
          new HashSet<AQPair>(); // Set required for the "xor"-operation below
      for (int row = matrixBlength - 1; row >= 0; row--) {
        if ((matrixV[row] & mask) != 0) {
          // the current row belongs to the solution encoded in matrixV by the bit addressed by
          // mask.
      ...
      if (!totalAQPairs.isEmpty()) {
        // Sometimes the BlockLanczos() method returns non-null-vectors (having q-factors with odd
        // exponent),
