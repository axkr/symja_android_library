 ...
 * matrix. (See {@link QRDecomposition} for details on the decomposition algorithm.) The
 * <code>X</code> matrix, also known as the <i>design matrix,</i> has rows corresponding to sample
 ...
 *
 */
public class ExprOLSLinearRegression extends ExprAbstractLinearRegression {

  /** Cached QR decomposition of X matrix */
  private FieldQRDecomposition<IExpr> qr;

  /** Singularity threshold for QR decomposition */
  private final IExpr threshold;

  /**
   * Create an empty OLSMultipleLinearRegression instance.
   */
  public ExprOLSLinearRegression() {
    this(F.C0);
  }
   ...
   */
  public ExprOLSLinearRegression(final IExpr threshold) {
   ...
   *         regression
   */
  public void newSampleData(FieldVector<IExpr> y, FieldMatrix<IExpr> x)
  ...
  public void newSampleData(FieldMatrix<IExpr> data, int nobs, int nvars) {
    ...
    qr = new FieldQRDecomposition<IExpr>(getX(), threshold);
  }
   ...
   * </p>
   * <p>
   * The implementation here uses the QR decomposition to compute the hat matrix as Q
   ...
   * </p>
   * <p>
   * Data for the model must have been successfully loaded using one of the {@code newSampleData}
   ...
   */
  public FieldMatrix<IExpr> calculateHat() {
    // Create augmented identity matrix
    FieldMatrix<IExpr> Q = qr.getQ();
    final int p = qr.getR().getColumnDimension();
    final int n = Q.getColumnDimension();
    // No try-catch or advertised MathIllegalArgumentException - NPE above if n < 3
    FieldMatrix<IExpr> augI = new Array2DRowFieldMatrix<IExpr>(F.EXPR_FIELD, n, n);
    IExpr[][] augIData = augI.getData();
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i == j && i < p) {
          augIData[i][j] = F.C1;
   ...
   *
   * <p>
   * If the model has no intercept term, <code>0</code> is used for the mean of Y - i.e., what is
   * returned is the sum of the squared Y values.
   * </p>
   *
   * <p>
   * The value returned by this method is the SSTO value used in the {@link #calculateRSquared()
   ...
   */
  public IExpr calculateResidualSumOfSquares() {
    final FieldVector<IExpr> residuals = calculateResiduals();
    // No advertised DME, args are valid
    return residuals.dotProduct(residuals);
  }

  /**
   * Returns the R-Squared statistic, defined by the formula \(R^2 = 1 -
   * \frac{\mathrm{SSR}}{\mathrm{SSTO}}\) where SSR is the {@link #calculateResidualSumOfSquares()
   * sum of squared residuals} and SSTO is the {@link #calculateTotalSumOfSquares() total sum of
   * squares}
   *
   * <p>
  ...
  /**
   * <p>
   * Returns the adjusted R-squared statistic, defined by the formula \(R_\mathrm{adj}^2 = 1 -
   * \frac{\mathrm{SSR} (n - 1)}{\mathrm{SSTO} (n - p)}\) where SSR is the
   * {@link #calculateResidualSumOfSquares() sum of squared residuals}, SSTO is the
   * {@link #calculateTotalSumOfSquares() total sum of squares}, n is the number of observations and
   ...
   * {@inheritDoc}
   * <p>
   * This implementation computes and caches the QR decomposition of the X matrix once it is
   * successfully loaded.
   * </p>
   */
  @Override
   ...
   */
  @Override
  protected FieldVector<IExpr> calculateBeta() {
   ...
   * </p>
   * <p>
   * Uses QR decomposition to reduce (X<sup>T</sup>X)<sup>-1</sup> to (R<sup>T</sup>R)<sup>-1</sup>,
   * with only the top p rows of R included, where p = the length of the beta vector.
   * </p>
   *
   * <p>
   * Data for the model must have been successfully loaded using one of the {@code newSampleData}
   ...
   */
  @Override
  protected FieldMatrix<IExpr> calculateBetaVariance() {
    int p = getX().getColumnDimension();
    FieldMatrix<IExpr> Raug = qr.getR().getSubMatrix(0, p - 1, 0, p - 1);
    FieldMatrix<IExpr> Rinv = new FieldLUDecomposition<IExpr>(Raug).getSolver().getInverse();
