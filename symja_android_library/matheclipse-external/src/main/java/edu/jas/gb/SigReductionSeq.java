...
/**
 * Polynomial SigReduction class. Implements common S-Polynomial, normalform with respect to
 * signatures.
 *
...
public class SigReductionSeq<C extends RingElem<C>> implements SigReduction<C> {
    ...
    while (!a.isZERO() && reduced) {
      ...
      for (SigPoly<C> p : G) {
        ...
        if (mt) {
          ...
          boolean sigeq =
              (sigup.compareTo(sigma) < 0)
                  || ((sp.compareTo(se) == 0
                      && (sigup.leadingBaseCoefficient().compareTo(sigma.leadingBaseCoefficient())
                          != 0)));
   ...
   *
   * @param F list of signature polynomials.
   * @return [m,p] where m is the list of signature polynomials of F of minimal degree and p
   *     contains the rest of the signature polynomials with non minimal degree.
   */
