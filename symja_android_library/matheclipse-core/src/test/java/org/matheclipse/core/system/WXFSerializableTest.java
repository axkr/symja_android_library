...
public class WXFSerializableTest extends TestCase {
	...
	private void equalsCopy(IExpr original) {
		original = EvalEngine.get().evaluate(original);
		byte[] bArray = WL.serialize(original);
		IExpr copy = EvalEngine.get().evaluate(WL.deserialize(bArray));
