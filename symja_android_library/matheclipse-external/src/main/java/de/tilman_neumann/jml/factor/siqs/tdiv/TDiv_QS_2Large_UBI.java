...
/**
 * A trial division engine where partials can have up to 2 large factors. This is absolutely
...
public class TDiv_QS_2Large_UBI implements TDiv_QS {
    ...
    if (DEBUG)
      LOG.debug(
          "maxQRest = "
              + maxQRest
              + " ("
              + (64 - Long.numberOfLeadingZeros((long) maxQRest))
              + " bits)");
    this.kN = kN;
    // statistics
    if (ANALYZE) testCount = sufficientSmoothCount = 0;
    if (ANALYZE)
      aqDuration = pass1Duration = pass2Duration = primeTestDuration = factorDuration = 0;
    if (ANALYZE_LARGE_FACTOR_SIZES) qRestSizes = new SortedMultiset_BottomUp<>();
  }

  @Override
  public void initializeForAParameter(
      BigInteger da,
      BigInteger b,
  ...
  public List<AQPair> testList(List<Integer> xList) {
    ...
    for (int x : xList) {
      ...
      BigInteger A =
          da.multiply(BigInteger.valueOf(x))
              .add(bParam); // A(x) = d*a*x+b, with d = 1 or 2 depending on kN % 8
  ...
  private AQPair test(BigInteger A, BigInteger Q, int x) {
    ...
    // IMPORTANT: Java gives x % p = x for |x| < p, and we have many p bigger than any sieve array
    // entry.
    ...
    for (int pIndex = baseSize - 1; pIndex > 0; pIndex--) { // p[0]=2 was already tested
      ...
      if (xAbs < p) {
        ...
        // Compute x%p using long-valued Barrett reduction, see
        ...
        //					if (xModP != xModP2) LOG.debug("x=" + x + ", p=" + p + ": xModP=" + xModP + ", but
        // xModP2=" + xModP2);
      ...
      if (xModP == x1Array[pIndex] || xModP == x2Array[pIndex]) {
        ...
        // for some reasons I do not understand it is faster to divide Q by p in pass 2 only, not
        // here
      }
    }
    ...
    if (DEBUG)
      LOG.debug(
          "test(): pMax="
              ...
              + maxQRest
              + " -> resolve all factors");
    // Now we consider Q as sufficiently smooth to want to find all prime factors, as long as we do
    // not find one that is too big to be useful.
    // First we need a prime test, because factor algorithms may not return when called with a prime
    // argument.
    ...
    if (restIsPrime) {
      ...
      return (Q_rest.bitLength() > 31)
          ? null
          : new Partial_1Large(A, smallFactors, Q_rest.longValue());
      ...
      if (DEBUG)
        LOG.debug(
            "test(): pMax^2 = "
                ...
                + " ("
                + Q_rest_bits
                + " bits) not prime -> use pollardRhoR64Mul63");
      ...
      if (DEBUG)
        LOG.debug(
            "test(): pMax^2 = "
                ...
                + " ("
                + Q_rest_bits
                + " bits) not prime -> use pollardRho64");
