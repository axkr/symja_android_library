...
/**
 * SolvableLocalResidue ring factory for SolvableLocalResidue based on GenSolvablePolynomial with
 * GcdRingElem interface. Objects of this class are immutable. It represents the "classical quotient
 * ring modulo an ideal".
 *
 * @author Heinz Kredel
 */
public class SolvableLocalResidueRing<C extends GcdRingElem<C>>
  ...
  public SolvableLocalResidue<C> create(GenPolynomial<C> n, GenPolynomial<C> d) {
    return new SolvableLocalResidue<C>(
        this, (GenSolvablePolynomial<C>) n, (GenSolvablePolynomial<C>) d);
  }
  ...
  public List<SolvableLocalResidue<C>> generators() {
    ...
    List<SolvableLocalResidue<C>> gens =
        new ArrayList<SolvableLocalResidue<C>>(pgens.size() * 2 - 1);
      ...
      for (int j = i + 1; j < ngen; j++) {
        ...
        for (int k = j + 1; k < ngen; k++) {
          ...
          if (p.num.equals(q.num) && p.den.equals(q.den)) { // short cut
            ...
            //         System.out.println("lr assoc: Xk = " + Xk.toScript() + ", Xj = " +
            // Xj.toScript() + ", Xi = " + Xi.toScript());
  ...
  /**
   * Parse SolvableLocalResidue from String. Syntax: "{ polynomial | polynomial }" or "{ polynomial
