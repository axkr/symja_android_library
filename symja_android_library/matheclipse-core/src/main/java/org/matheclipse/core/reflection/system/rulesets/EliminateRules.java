...
public interface EliminateRules {
  ...
  public static Matcher init2() {
    ...
    matcher.caseOf(
        $(elimzeroplus, Plus(c_DEFAULT, Times(Log(Times(b_DEFAULT, x_)), Power(x_, a_DEFAULT))),
            x_),
        Condition(Power(
            Times(CN1, a, c,
                Power(ProductLog(Times(CN1, a, Power(Power(Power(b, CN1), a), CN1), c)), CN1)),
            Power(a, CN1)), FreeQ(list(a, b, c), x)));
    // elimzeroplus(Log(c_.*x_)*b_.+d_.+a_.*x_^n_.,x_):=((b*ProductLog((a*((1/(E^(d/b)*c)))^n*n)/b))/(a*n))^(1/n)/;FreeQ({a,b,c,d,n},x)
    matcher.caseOf(
        $(elimzeroplus,
            Plus(Times(Log(Times(c_DEFAULT, x_)), b_DEFAULT), d_DEFAULT,
                Times(a_DEFAULT, Power(x_, n_DEFAULT))),
            x_),
        Condition(Power(
            Times(b, Power(Times(a, n), CN1),
                ProductLog(Times(a, Power(b, CN1),
                    Power(Power(Times(Exp(Times(Power(b, CN1), d)), c), CN1), n), n))),
            Power(n, CN1)), FreeQ(List(a, b, c, d, n), x)));
