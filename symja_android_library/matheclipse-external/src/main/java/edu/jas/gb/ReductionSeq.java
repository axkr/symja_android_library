  ...
  public GenPolynomial<C> normalformMarked(
    ...
    while (S.length() > 0) {
      ...
      if (!mt) {
        ...
        // System.out.println("i = "+i+", htl[i] = " + Ap.ring.toScript(htl[i]) + ", lbc[i] = " +
        // lbc[i]  + ", p[i] = " + p[i].ring.toScript(p[i].leadingExpVector()));
        f = e.subtract(htl[i]);
        b = a.divide((C) lbc[i]);
        ...
        // System.out.println("NF R = " + R.leadingExpVector() + ", S = " + S.leadingExpVector() +
        // ", e = " + e + ", f = " + f + ", #S = " + S.length());
      }
