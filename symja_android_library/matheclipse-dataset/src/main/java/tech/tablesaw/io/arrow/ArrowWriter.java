...
public class ArrowWriter {
  ...
  private Schema tableSchema(Table table) {
    ...
    for (Column<?> column : table.columns()) {
      ...
      switch (typeName) {
        ...
        case "LONG":
          fields
              .add(new Field(column.name(), FieldType.nullable(new ArrowType.Int(64, true)), null));
          break;
        case "INTEGER":
          fields
              .add(new Field(column.name(), FieldType.nullable(new ArrowType.Int(32, true)), null));
          break;
        case "SHORT":
          fields
              .add(new Field(column.name(), FieldType.nullable(new ArrowType.Int(16, true)), null));
          break;
        case "LOCAL_DATE":
          fields.add(new Field(column.name(),
              FieldType.notNullable(new ArrowType.Date(DateUnit.DAY)), null));
          break;
        case "LOCAL_DATE_TIME":
          fields.add(new Field(column.name(),
              FieldType.notNullable(new ArrowType.Timestamp(TimeUnit.MILLISECOND, null)), null));
          break;
        case "LOCAL_TIME":
          fields.add(new Field(column.name(),
              FieldType.notNullable(new ArrowType.Time(TimeUnit.MILLISECOND, 32)), null));
          break;
        case "INSTANT":
          fields.add(new Field(column.name(),
              FieldType.notNullable(new ArrowType.Timestamp(TimeUnit.MILLISECOND, "UTC")), null));
          break;
        case "BOOLEAN":
          fields.add(
        ...
        case "FLOAT":
          fields.add(new Field(column.name(),
              FieldType.notNullable(new ArrowType.FloatingPoint(SINGLE)), null));
          break;
        case "DOUBLE":
          fields.add(new Field(column.name(),
              FieldType.notNullable(new ArrowType.FloatingPoint(DOUBLE)), null));
          break;
        default:
   ...
   *
   * <p>
   * The arrow format specifies writing tables in record batches, along with any DictionaryProviders
   * that will be used in encoding the data.
   *
   * <p>
  ...
  public void write(Table table, File file) {
    ...
    try (FileOutputStream out = new FileOutputStream(file);
        ArrowStreamWriter writer = new ArrowStreamWriter(schemaRoot, /* DictionaryProvider= */ null,
  ...
  private List<FieldVector> createFieldVectors(Schema schema, BufferAllocator allocator) {
    return schema.getFields().stream().map(field -> field.createVector(allocator))
