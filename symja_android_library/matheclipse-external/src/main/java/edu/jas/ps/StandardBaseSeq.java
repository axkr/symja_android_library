...
/**
 * Standard Base sequential algorithm. Implements Standard bases and GB test. <b>Note: </b>
 * Currently the term order is fixed to the order defined by the iterator over exponent vectors
...
/* extends StandardBaseAbstract<C> */ {
  ...
  public List<MultiVarPowerSeries<C>> STD(int modv, List<MultiVarPowerSeries<C>> F) {
    ...
    OrderedPairlist<C> pairlist =
        new OrderedPairlist<C>(modv, ring); // strategy.create( modv, ring );
    pairlist.put(G);
    ...
    while (pairlist.hasNext()) {
      ...
      if (logger.isInfoEnabled()) {
        ...
        logger.info(
            "ht(H) = "
                + eh.toString(S.ring.vars)
                + ", "
                + eh); // + ", coeff(HT(H)) = " + H.coefficient(eh));
      }
