...
public abstract class AbstractAST implements IASTMutable {
	...
	/**
	 * The class <code>NILPointer</code> implements the constant object <code>F.NIL</code> (not in list), which indicates in the
	 * evaluation process that no evaluation was possible (i.e. no further definition was found to create a new expression from the
	 * existing one).
	 * <p>
	 * Almost every modifying method in this class throws an <tt>UnsupportedOperationException</tt>, almost every predicate returns
	 * <code>false</code>. The main method to check if the object is valid is the <code>isPresent()</code> method. The method is
	 * designed similar to <code>java.util.Optional#isPresent()</code>.
	 * </p>
	 * 
...
//		return temp;
	 ...
	 * elements for which the <code>function</code> returns non <code>F.NIL</code> elements to the <code>filterAST</code>, or otherwise
	 * append the argument to the <code>restAST</code>.
	 * 
	 * @param filterAST the non <code>F.NIL</code> elements which were returned by the <code>function#apply()</code> method
