{
 ...
 Limit(x_^(a_.*(x_^n_.) + b_.), x_Symbol->0):= With({r=ConditionalExpression( 1, Element(a,Reals)&&b>0&&n>0 )}, r /; r=!= Undefined&& FreeQ({a,b,n},x)),
 Limit(x_^(a_.*(x_^n_.) + b_.), x_Symbol->0):= With({r=ConditionalExpression( 0, Element(b,Reals)&&a>0&&n<0&&Cos(-n*Pi)>0&&Sin(-n*Pi)>0 )}, r /; r=!= Undefined && FreeQ({a,b,n},x)),
 Limit(x_^(a_.*(x_^n_.) + b_.), x_Symbol->Infinity):= With({r=ConditionalExpression( Infinity, Element(b,Reals)&&a>0&&n>0 )}, r /; r=!= Undefined && FreeQ({a,b,n},x)),
 Limit(x_^(a_.*(x_^n_.)), x_Symbol->Infinity):= With({r=ConditionalExpression( 1, Element(a,Reals)&&n<0 )}, r /; r=!= Undefined && FreeQ({a,n},x)),
