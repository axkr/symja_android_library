...
public class SieveParams {
  ...
  public SieveParams(
    ...
    // pMinIndex ~ primeBaseSize^0.33 looks clearly better than primeBaseSize^0.3 or
    // primeBaseSize^0.36.
    ...
    // This bound is computed as ln(Q/a) - ln(max rest), where ln(Q/a) is the natural logarithm of
    // average (Q/a)-values,
    // estimated according to the papers of Contini, Pomerance and Silverman as ln(Q/a) = ln(M) +
    // ln(kN)/2 - 1/2.
    double minLnPSum =
        Math.log(sieveArraySize) + Math.log(kN.doubleValue()) / 2 - 0.5 - Math.log(maxQRest);
    // convert the sieve bound from natural logarithm to the actual logBase:
    float lnLogBase =
        (float) (minLnPSum / wantedMinLogPSum); // normalizer to be used as a divisor for p_i values
   ...
   */
  private byte computeInitializerValue(
      int[] primesArray, int pMinIndex, int minLogPSum, double lnLogBase) {
