...
/**
 * Polynomial utilities for applications, for example conversion ExpVector to Product or zero
 * dimensional ideal root computation.
 *
...
public class PolyUtilApp<C extends RingElem<C>> {
  ...
  public static <C extends GcdRingElem<C>> List<GenPolynomial<Product<Residue<C>>>> toProductRes(
    ...
    List<GenPolynomial<Product<Residue<C>>>> list =
        new ArrayList<GenPolynomial<Product<Residue<C>>>>();
    if (L == null || L.size() == 0) {
      return list;
    }
   ...
   */
  public static <C extends GcdRingElem<C>> Product<Residue<C>> toProductRes(
      ProductRing<Residue<C>> pfac, GenPolynomial<C> c) {
   ...
   */
  public static <C extends GcdRingElem<C>> String productToString(
      PolynomialList<Product<Residue<C>>> L) {
   ...
   */
  public static <D extends GcdRingElem<D> & Rational>
      List<List<Complex<BigDecimal>>> complexRootTuples(Ideal<D> I, BigRational eps) {
  ...
  public static boolean isRealRoots(
      List<GenPolynomial<BigDecimal>> L, List<List<BigDecimal>> roots, BigDecimal eps) {
    if (L == null || L.size() == 0) {
      return true;
    }
   ...
   */
  public static boolean isComplexRoots(
      List<GenPolynomial<Complex<BigDecimal>>> L,
   ...
   *
   * @param I zero dimensional ideal with univariate irreducible polynomials and bi-variate
   *     polynomials.
   * @return real algebraic roots for ideal(G)
   */
  public static <D extends GcdRingElem<D> & Rational>
      IdealWithRealAlgebraicRoots<D> realAlgebraicRoots(IdealWithUniv<D> I) {
    ...
    for (int i = 1; i < I.upolys.size(); i++) {
      ...
      if (depi.length == 1) {
        ...
        GenPolynomialRing<GenPolynomial<D>> rfac =
            new GenPolynomialRing<GenPolynomial<D>>(ufac, 1, to); // new vars
   ...
   * Construct real roots for zero dimensional ideal(G).
   *
   * @param I list of zero dimensional ideal with univariate irreducible polynomials and bi-variate
   *     polynomials.
   * @return list of real algebraic roots for all ideal(I_i)
   */
  public static <D extends GcdRingElem<D> & Rational>
      List<IdealWithRealAlgebraicRoots<D>> realAlgebraicRoots(List<IdealWithUniv<D>> I) {
    List<IdealWithRealAlgebraicRoots<D>> lir =
        new ArrayList<IdealWithRealAlgebraicRoots<D>>(I.size());
   ...
   * @param I zero dimensional ideal with univariate irreducible polynomials and bi-variate
   *     polynomials.
   * @return complex algebraic roots for ideal(G) <b>Note:</b> implementation contains errors, do
   *     not use.
   */
  public static <D extends GcdRingElem<D> & Rational>
      IdealWithComplexAlgebraicRoots<D> complexAlgebraicRootsWrong( // Wrong
      IdealWithUniv<D> I) {
    ...
    for (int i = 1; i < I.upolys.size(); i++) {
      ...
      if (depi.length == 1) {
        ...
        GenPolynomial<GenPolynomial<Complex<D>>> pip2cr =
            PolyUtil.<Complex<D>>recursive(rcfac, pip2c);
        ...
        for (Complex<edu.jas.application.RealAlgebraicNumber<D>> cr : cra) {
          ...
          edu.jas.application.RealAlgebraicRing<D> cring =
              (edu.jas.application.RealAlgebraicRing<D>) cr.ring.ring;
          ...
          for (List<Complex<edu.jas.application.RealAlgebraicNumber<D>>> cx : can) {
            ...
            GenPolynomial<Complex<D>> t =
                tfac.univariate(1, 1L).sum(tfac.univariate(0, 1L).multiply(ccfac.getIMAG()));
   ...
   * @param I zero dimensional ideal with univariate irreducible polynomials and bi-variate
   *     polynomials.
   * @return complex algebraic roots for ideal(G) <b>Note:</b> not jet completed oin all cases.
   */
  public static <D extends GcdRingElem<D> & Rational>
      IdealWithComplexAlgebraicRoots<D> complexAlgebraicRoots(IdealWithUniv<D> I) {
    List<List<Complex<edu.jas.application.RealAlgebraicNumber<D>>>> can;
    can = new ArrayList<List<Complex<edu.jas.application.RealAlgebraicNumber<D>>>>();
    if (I == null) {
      throw new IllegalArgumentException("null ideal not permitted");
      ...
      if (depi.length == 1) {
        ...
        for (Complex<edu.jas.application.RealAlgebraicNumber<D>> cr : cra) {
          ...
          for (List<Complex<edu.jas.application.RealAlgebraicNumber<D>>> cx : can) {
            ...
            pcrfac =
                new GenPolynomialRing<Complex<edu.jas.application.RealAlgebraicNumber<D>>>(
                    car, rcfac);
            ...
            if (nr == 1) { // one root
              ...
              logger.error(
                  "to many roots, cxi = " + toString(cx.get(ix)) + ", cr = " + toString(cr));
            } else { // no root
              logger.info("no hit, cxi = " + toString(cx.get(ix)) + ", cr = " + toString(cr));
            }
          }
   ...
   */
  public static <D extends GcdRingElem<D> & Rational>
      List<IdealWithComplexAlgebraicRoots<D>> complexAlgebraicRoots(List<IdealWithUniv<D>> I) {
    List<IdealWithComplexAlgebraicRoots<D>> lic =
        new ArrayList<IdealWithComplexAlgebraicRoots<D>>();
    for (IdealWithUniv<D> iu : I) {
   ...
   */
  public static <D extends GcdRingElem<D> & Rational>
      List<IdealWithComplexAlgebraicRoots<D>> complexAlgebraicRoots(Ideal<D> I) {
   ...
   */
  static <C extends RingElem<C>> GenPolynomial<C> convert(
      GenPolynomialRing<C> fac, GenPolynomial<C> p) {
   ...
   */
  public static <D extends GcdRingElem<D> & Rational>
      List<IdealWithRealAlgebraicRoots<D>> realAlgebraicRoots(Ideal<D> I) {
    List<IdealWithUniv<D>> Ir = I.zeroDimRootDecomposition();
    // System.out.println("Ir = " + Ir);
  ...
  public static <C extends GcdRingElem<C>> AlgebraicNumber<C> convertToPrimitiveElem(
    ...
    GenPolynomialRing<AlgebraicNumber<C>> ar =
        new GenPolynomialRing<AlgebraicNumber<C>>(cfac, aufac);
  ...
  public static <C extends GcdRingElem<C>> GenPolynomial<AlgebraicNumber<C>> convertToPrimitiveElem(
    ...
    GenPolynomialRing<AlgebraicNumber<C>> cr =
        new GenPolynomialRing<AlgebraicNumber<C>>(cfac, a.ring);
    return PolyUtil.<AlgebraicNumber<C>, AlgebraicNumber<C>>map(
        cr, a, new CoeffConvertAlg<C>(cfac, A));
  }
  ...
  public static <C extends GcdRingElem<C>> GenPolynomial<AlgebraicNumber<C>> convertToPrimitiveElem(
    ...
    GenPolynomialRing<AlgebraicNumber<C>> cr =
        new GenPolynomialRing<AlgebraicNumber<C>>(cfac, a.ring);
    return PolyUtil.<AlgebraicNumber<AlgebraicNumber<C>>, AlgebraicNumber<C>>map(
        cr, a, new CoeffRecConvertAlg<C>(cfac, A, B));
  }

  /**
   * Convert to RealAlgebraicNumber coefficients. Represent as polynomial with
   * RealAlgebraicNumber<C> coefficients from package <code>edu.jas.root.</code>
   *
  ...
  public static <C extends GcdRingElem<C> & Rational>
      GenPolynomial<edu.jas.root.RealAlgebraicNumber<C>> realAlgFromRealCoefficients(
    ...
    return PolyUtil
        .<edu.jas.application.RealAlgebraicNumber<C>, edu.jas.root.RealAlgebraicNumber<C>>map(
   ...
   */
  public static <C extends GcdRingElem<C> & Rational>
      GenPolynomial<edu.jas.application.RealAlgebraicNumber<C>> realFromRealAlgCoefficients(
          GenPolynomialRing<edu.jas.application.RealAlgebraicNumber<C>> rfac,
          GenPolynomial<edu.jas.root.RealAlgebraicNumber<C>> A) {
    edu.jas.application.RealAlgebraicRing<C> cfac =
        (edu.jas.application.RealAlgebraicRing<C>) rfac.coFac;
    return PolyUtil
        .<edu.jas.root.RealAlgebraicNumber<C>, edu.jas.application.RealAlgebraicNumber<C>>map(
   ...
   */
  public static <C extends GcdRingElem<C> & Rational>
      GenPolynomial<Complex<edu.jas.application.RealAlgebraicNumber<C>>>
    ...
    return PolyUtil.<Complex<C>, Complex<edu.jas.application.RealAlgebraicNumber<C>>>map(
        pfac, A, new CoeffToComplexReal<C>(afac));
  }

  /**
   * Evaluate to Complex&lt;RealAlgebraicNumber&gt; coefficients. Represent as polynomial with
   * Complex&lt;RealAlgebraicNumber&gt; coefficients, C is e.g. BigRational.
   *
   * @param pfac result polynomial factory.
   * @param A = A(x,Y) a recursive polynomial with GenPolynomial&lt;Complex&gt; coefficients to be
   *     converted.
  ...
  public static <C extends GcdRingElem<C> & Rational>
    ...
    return PolyUtil
        .<GenPolynomial<Complex<C>>, Complex<edu.jas.application.RealAlgebraicNumber<C>>>map(
...
class CoeffRecConvertAlg<C extends GcdRingElem<C>>
  ...
  public CoeffRecConvertAlg(
      AlgebraicNumberRing<C> fac, AlgebraicNumber<C> a, AlgebraicNumber<C> b) {
...
class ReAlgFromRealCoeff<C extends GcdRingElem<C> & Rational>
    implements UnaryFunctor<
        edu.jas.application.RealAlgebraicNumber<C>, edu.jas.root.RealAlgebraicNumber<C>> {
...
class RealFromReAlgCoeff<C extends GcdRingElem<C> & Rational>
    implements UnaryFunctor<
        edu.jas.root.RealAlgebraicNumber<C>, edu.jas.application.RealAlgebraicNumber<C>> {

  protected final edu.jas.application.RealAlgebraicRing<C> rfac;
  ...
  public edu.jas.application.RealAlgebraicNumber<C> eval(edu.jas.root.RealAlgebraicNumber<C> c) {
    ...
    edu.jas.root.RealAlgebraicNumber<edu.jas.root.RealAlgebraicNumber<C>> rrc =
        (edu.jas.root.RealAlgebraicNumber<edu.jas.root.RealAlgebraicNumber<C>>)
            (Object) c; // force resurrect recursion
...
class CoeffToComplexReal<C extends GcdRingElem<C> & Rational>
  ...
  protected final ComplexRing<edu.jas.application.RealAlgebraicNumber<C>> cfac;
...
class EvaluateToComplexReal<C extends GcdRingElem<C> & Rational>
    implements UnaryFunctor<
        GenPolynomial<Complex<C>>, Complex<edu.jas.application.RealAlgebraicNumber<C>>> {

  protected final GenPolynomialRing<Complex<edu.jas.application.RealAlgebraicNumber<C>>> pfac;

  protected final ComplexRing<edu.jas.application.RealAlgebraicNumber<C>> cfac;

  protected final Complex<edu.jas.application.RealAlgebraicNumber<C>> root;

  public EvaluateToComplexReal(
      GenPolynomialRing<Complex<edu.jas.application.RealAlgebraicNumber<C>>> fac,
      Complex<edu.jas.application.RealAlgebraicNumber<C>> r) {
    if (fac == null) {
      throw new IllegalArgumentException("fac must not be null");
    }
