...
/**
 * Algebraic number factory. Based on GenPolynomial factory with RingElem interface. Objects of this
 * class are immutable.
 *
 * @author Heinz Kredel
 */
public class AlgebraicNumberRing<C extends RingElem<C>>
   ...
   *
   * @param field true, if this ring is determined to be a field, false, if it is determined that it
   *     is not a field.
   */
  ...
  /**
   * AlgebraicNumber chinese remainder algorithm. Assert deg(c.modul) >= deg(a.modul) and c.modul *
   * a.modul = this.modul.
   *
   ...
   */
  public AlgebraicNumber<C> chineseRemainder(
      AlgebraicNumber<C> c, AlgebraicNumber<C> ci, AlgebraicNumber<C> a) {
    if (true) { // debug
  ...
  /**
   * AlgebraicNumber interpolation algorithm. Assert deg(c.modul) >= deg(A.modul) and c.modul *
   * A.modul = this.modul. Special case with deg(A.modul) == 1. Similar algorithm as chinese
   * remainder algortihm.
   *
   * @param c AlgebraicNumber.
  ...
  /**
   * Get a AlgebraicNumber iterator. <b>Note: </b> Only for finite field coefficients or fields
   * which are iterable.
   *
