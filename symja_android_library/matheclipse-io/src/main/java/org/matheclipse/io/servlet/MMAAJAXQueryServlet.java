...
public class MMAAJAXQueryServlet extends HttpServlet {
  ...
  public static String evaluate(
    ...
    try (PrintStream outs = new PrintStream(wouts);
      ...
      if (engine == null) {
        engine = new EvalEngine(session.getId(), 256, 256, outs, errors, false);
  ...
  public void init() throws ServletException {
    super.init();
    if (!INITIALIZED) {
      MMAAJAXQueryServlet.initialization("MMAAJAXQueryServlet");
    }
  }

  public static synchronized void initialization(String servlet) {
    MMAAJAXQueryServlet.INITIALIZED = true;
