...
/**
 * Algorithm to find the first powers of all p<pMin in [pMin, pMax]. These are the only powers that
 * add log(power) to the logPSum.
 *
...
public class PowerOfSmallPrimesFinder extends SomePowerFinder {
   ...
   */
  @Override
  public TreeSet<PowerEntry> findPowers(
    ...
    int sqrtPMaxIndex =
        binarySearch.getInsertPosition(primes, primeBaseSize, (int) Math.sqrt(pMax));
    int maxIndex = Math.min(pMinIndex, sqrtPMaxIndex);
    for (int pIndex = 1; pIndex < maxIndex; pIndex++) { // p[0]==2 never has 2 x-solutions
      // [https://members.loria.fr/EThome/old/MPRI/cours_02.pdf, page 11]: Solutions of Q(x) == 0
      // (mod p^exponent), Q(x) = (a*x+b)^2-kN, exponent>1
      // exist only if Q(x) == 0 (mod p) has two distinct solutions. p must not divide disc(Q).
      if (tArray[pIndex] != 0) {
        int p = primes[pIndex];
        //				if (DEBUG) {
        //					// since we only use odd p with x1!=x2, t!=0, p not dividing k, we strictly have
        // Legendre(kN|p) > 0
        ...
        for (int exponent = 2; ; exponent++) {
          ...
          if (power > pMin) {
            ...
            // XXX In theory, the sieve initializer should be subtracted by the contribution of
            // these powers. In practice, this makes hardly a difference.
            byte logPower = (byte) (Math.log(power) * lnPMultiplier + 0.5F);
            // We need the solution t of t^2 == kN (mod p) to compute the solution u of u^2 == kN
            // (mod p^exponent)
            int u =
                modularSqrtEngine.modularSqrtModPower(
                    kN_UBI.mod(power), power, last_power, tArray[pIndex]);
