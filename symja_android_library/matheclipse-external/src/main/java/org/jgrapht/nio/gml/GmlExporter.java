 ...
 *
 * <p>For a description of the format see <a href=
 ...
 *
 * <p>The behavior of the exporter such as whether to print vertex labels, edge labels, and/or edge
...
public class GmlExporter<V, E> extends BaseExporter<V, E> implements GraphExporter<V, E> {
  ...
  private static final Set<String> FORBIDDEN_VERTEX_CUSTOM_ATTRIBUTE_KEYS = new HashSet<String>();
  //		  Set.of("id");
  private static final Set<String> FORBIDDEN_EDGE_CUSTOM_ATTRIBUTE_KEYS = new HashSet<String>();
  //      Set.of("id", "source", "target");

  static {
    FORBIDDEN_VERTEX_CUSTOM_ATTRIBUTE_KEYS.add("id");
    FORBIDDEN_EDGE_CUSTOM_ATTRIBUTE_KEYS.add("id");
    FORBIDDEN_EDGE_CUSTOM_ATTRIBUTE_KEYS.add("source");
    FORBIDDEN_EDGE_CUSTOM_ATTRIBUTE_KEYS.add("target");
  }
  ...
  public enum Parameter {
    ...
    EXPORT_EDGE_WEIGHTS,
    /**
     * If set the exporter outputs all custom edge attributes. The attributes are located from the
     * edge attribute provider of the importer. Note that these attributes have lowest priority
     * compared to special handled ones like "label", or "weight" and cannot contain special keys
     * like "id", "source" and "target".
     */
    ...
    /**
     * If set the exporter outputs all custom vertex attributes. The attributes are located from the
     * vertex attribute provider of the importer. Note that these attributes have lowest priority
     * compared to special handled ones like "label" and cannot contain special keys like "id".
     */
  ...
  private void exportVertices(PrintWriter out, Graph<V, E> g) {
    ...
    for (V from : g.vertexSet()) {
      ...
      if (exportVertexLabels) {
        String label =
            getVertexAttribute(from, LABEL_ATTRIBUTE_KEY)
                .map(Attribute::getValue)
                .orElse(from.toString());
        out.println(TAB2 + "label" + DELIM + quoted(label));
      }
      if (exportCustomVertexAttributes) {
        getVertexAttributes(from)
            .ifPresent(
                vertexAttributes -> {
                  vertexAttributes
                      .entrySet()
                      ...
                      .forEach(
                          e -> {
                            ...
                            if (FORBIDDEN_VERTEX_CUSTOM_ATTRIBUTE_KEYS.contains(
                                customAttributeKey)) {
                              throw new IllegalArgumentException(
                                  "Key " + customAttributeKey + " is reserved");
                            }

                            if (LABEL_ATTRIBUTE_KEY.equals(customAttributeKey)
                                && exportVertexLabels) {
  ...
  private void exportEdges(PrintWriter out, Graph<V, E> g) {
    ...
    for (E edge : g.edgeSet()) {
      ...
      if (exportEdgeLabels) {
        Attribute label =
            getEdgeAttribute(edge, LABEL_ATTRIBUTE_KEY)
      ...
      if (exportEdgeWeights && g.getType().isWeighted()) {
        exportAttribute(out, "weight", DefaultAttribute.createAttribute(g.getEdgeWeight(edge)));
      }
                  ...
                  edgeAttributes
                      ...
                      .forEach(
                          e -> {
                            ...
                            if (WEIGHT_ATTRIBUTE_KEY.equals(customAttributeKey)
                                && exportEdgeWeights) {
