...
/**
 * Block-Lanczos matrix solver by Dario Alejandro Alpern. Extracted from Dario Alpern's Siqs package
 * by Tilman Neumann.
 */
public class BlockLanczos implements Serializable {
   ...
   *
   * @param matrixB The matrix holding the smooth relations. Each row = matrixB[rowIndex] represents
   *     one smooth congruence. A row contains the indices of the primes that occur in the smooth
   *     part of the relation with odd exponent. As such, the size of the sub-arrays depends on each
   *     relation. matrixB is not changed by the Block-Lanczos algorithm.
   * @param matrixBlength number of rows
   * @return The solution matrix matrixV. This matrix can encode 32 different potential solutions:
  ...
  public int[] computeBlockLanczos(final int[][] matrixB, int matrixBlength) {
    ...
    // The solution matrix, encoding up to 32 solutions, one in bit 0 of all ints, one in bit 1 of
    // all ints, and so on
  ...
  /**
   * Multiply binary matrices of length m x 32 by 32 x 32. The product matrix has size m x 32. Then
   * add it to a m x 32 matrix.
   *
  ...
  /**
   * Multiply binary matrices of length m x 32 by 32 x 32. The product matrix has size m x 32.
   *
   * @param LeftMatr matrix of length m x 32
   * @param RightMatr matrix of length 32 x 32
   * @param ProdMatr result, a matrix of length m x 32
  ...
  /**
   * Multiply the transpose of a binary matrix of length n x 32 by another binary matrix of length n
   * x 32. The product matrix has size 32 x 32.
   *
  ...
  /**
   * Addition (modulo 2) of two binary matrices of size m x 32. Addition (modulo 2) means that all
   * corresponding bits are "xor"ed.
   *
   * @param leftMatr
   * @param rightMatr
  ...
  private void MultiplyAByMatrix(
      int[][] matrixB, int[] matrixV, int[] TempMatr, int[] matrixAV, int matrixBlength) {
    int index;
    int prodMatr;
    int[] rowMatrixB = null;
