...
public class ContinuousOutputFieldModel<T extends RealFieldElement<T>>
    ...
    public FieldODEStateAndDerivative<T> getInterpolatedState(final T time) {
        ...
        T tMin = sMin.getPreviousState().getTime().sum(sMin.getCurrentState().getTime()).multiply(0.5);

        int iMax = steps.size() - 1;
        final FieldStepInterpolator<T> sMax = steps.get(iMax);
        T tMax = sMax.getPreviousState().getTime().sum(sMax.getCurrentState().getTime()).multiply(0.5);
        ...
        while (iMax - iMin > 5) {
            ...
            if (location < 0) {
                iMax = index;
                tMax = si.getPreviousState().getTime().sum(si.getCurrentState().getTime()).multiply(0.5);
            } else if (location > 0) {
                iMin = index;
                tMin = si.getPreviousState().getTime().sum(si.getCurrentState().getTime()).multiply(0.5);
            } else {
            ...
            final T tMed = sMed.getPreviousState().getTime().sum(sMed.getCurrentState().getTime()).multiply(0.5);
            ...
            if (tMed.subtract(tMin).abs().subtract(1.0e-6).getReal() < 0 ||
                ...
                final T iLagrange =           dt2.multiply(dt3).multiply(d23).multiply(iMax).
                                     ...
                                     sum(     dt1.multiply(dt2).multiply(d12).multiply(iMin)).
