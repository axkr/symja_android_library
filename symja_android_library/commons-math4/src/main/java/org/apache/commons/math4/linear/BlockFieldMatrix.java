...
public class BlockFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T> implements Serializable {
                ...
                for (int jBlock = 0; jBlock < out.blockColumns; ++jBlock) {
                    ...
                    for (int p = pStart; p < pEnd; ++p) {
                        for (int q = qStart; q < qEnd; ++q) {
                            outBlock[k] = tBlock[k].sum(m.getEntry(p, q));
                            ++k;
                        }
                    }
    ...
    public BlockFieldMatrix<T> add(final BlockFieldMatrix<T> m)
        ...
        for (int blockIndex = 0; blockIndex < out.blocks.length; ++blockIndex) {
            ...
            for (int k = 0; k < outBlock.length; ++k) {
                outBlock[k] = tBlock[k].sum(mBlock[k]);
            }
        }
                        ...
                        for (int p = pStart; p < pEnd; ++p) {
                            ...
                            for (int q = qStart; q < qEnd; ++q) {
                                ...
                                for (int l = lStart; l < lEnd; ++l) {
                                    sum = sum.sum(tBlock[l].multiply(m.getEntry(r, q)));
                                    ++r;
                                }
                                outBlock[k] = outBlock[k].sum(sum);
                                ++k;
                            }
                        }
                    }
                ...
                for (int kBlock = 0; kBlock < blockColumns; ++kBlock) {
                    ...
                    for (int p = pStart; p < pEnd; ++p) {
                        ...
                        for (int nStart = 0; nStart < jWidth; ++nStart) {
                            ...
                            while (l < lEnd - 3) {
                                sum = sum.
                                      sum(tBlock[l].multiply(mBlock[n])).
                                      sum(tBlock[l + 1].multiply(mBlock[n + jWidth])).
                                      sum(tBlock[l + 2].multiply(mBlock[n + jWidth2])).
                                      sum(tBlock[l + 3].multiply(mBlock[n + jWidth3]));
                                l += 4;
                                n += jWidth4;
                            }
                            while (l < lEnd) {
                                sum = sum.sum(tBlock[l++].multiply(mBlock[n]));
                                n += jWidth;
                            }
                            outBlock[k] = outBlock[k].sum(sum);
                            ++k;
    ...
    public void addToEntry(final int row, final int column, final T increment)
        ...
        blockIJ[k] = blockIJ[k].sum(increment);
    }
        ...
        for (int iBlock = 0; iBlock < blockRows; ++iBlock) {
            ...
            for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
                ...
                for (int p = pStart; p < pEnd; ++p) {
                    ...
                    while (q < qEnd - 3) {
                        sum = sum.
                              sum(block[k].multiply(v[q])).
                              sum(block[k + 1].multiply(v[q + 1])).
                              sum(block[k + 2].multiply(v[q + 2])).
                              sum(block[k + 3].multiply(v[q + 3]));
                        k += 4;
                        q += 4;
                    }
                    while (q < qEnd) {
                        sum = sum.sum(block[k++].multiply(v[q++]));
                    }
                    out[p] = out[p].sum(sum);
                }
            }
        ...
        for (int jBlock = 0; jBlock < blockColumns; ++jBlock) {
            ...
            for (int iBlock = 0; iBlock < blockRows; ++iBlock) {
                ...
                for (int q = qStart; q < qEnd; ++q) {
                    ...
                    while (p < pEnd - 3) {
                        sum = sum.
                              sum(block[k].multiply(v[p])).
                              sum(block[k + jWidth].multiply(v[p + 1])).
                              sum(block[k + jWidth2].multiply(v[p + 2])).
                              sum(block[k + jWidth3].multiply(v[p + 3]));
                        k += jWidth4;
                        p += 4;
                    }
                    while (p < pEnd) {
                        sum = sum.sum(block[k].multiply(v[p++]));
                        k += jWidth;
                    }
                    out[q] = out[q].sum(sum);
                }
