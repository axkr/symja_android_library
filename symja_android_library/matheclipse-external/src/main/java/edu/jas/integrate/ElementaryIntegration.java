...
/**
 * Methods related to elementary integration. In particular there are methods for Hermite reduction
...
public class ElementaryIntegration<C extends GcdRingElem<C>> {
   ...
   * @return Integral container, such that integrate(r) = sum_i(g_i) + sum_j( an_j log(hd_j) )
   */
   ...
   * @param a numerator
   * @param d denominator
   * @return Integral container, such that integrate(a/d) = sum_i(gn_i/gd_i) + integrate(h0) +
   *     sum_j( an_j log(hd_j) )
   */
   ...
   * @return [ [ gn_i, gd_i ], [ h0, hn_j, hd_j ] ] such that integrate(a/d) = sum_i(gn_i/gd_i) +
   *     integrate(h0) + sum_j( integrate(hn_j/hd_j) )
   */
  ...
  /**
   * Univariate GenPolynomial integration of the logaritmic part, eventual preparation for
   * irreducible factorization of P.
   *
  ...
  public boolean isIntegral(LogIntegral<C> rl) {
    ...
    r = r.subtract(i);
    QuotientRing<AlgebraicNumber<C>> aqr =
        new QuotientRing<AlgebraicNumber<C>>(rl.adenom.get(0).ring);
    Quotient<AlgebraicNumber<C>> ai = aqr.getZERO();

    GenPolynomial<AlgebraicNumber<C>> aqn =
        PolyUtil.<C>convertToAlgebraicCoefficients(aqr.ring, r.num);
    GenPolynomial<AlgebraicNumber<C>> aqd =
        PolyUtil.<C>convertToAlgebraicCoefficients(aqr.ring, r.den);
