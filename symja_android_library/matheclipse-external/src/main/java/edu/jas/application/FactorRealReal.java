...
/**
 * Real algebraic number coefficients factorization algorithms. This class implements factorization
 * methods for polynomials over bi-variate real algebraic numbers from package
 *
 * <pre>
 * edu.jas.application
 * </pre>
...
public class FactorRealReal<C extends GcdRingElem<C> & Rational>
  ...
  public FactorRealReal(RealAlgebraicRing<C> fac) {
    ...
    this(
        fac,
        FactorFactory.<edu.jas.root.RealAlgebraicNumber<C>>getImplementation(
  ...
  public List<GenPolynomial<RealAlgebraicNumber<C>>> baseFactorsSquarefree(
    ...
    List<GenPolynomial<RealAlgebraicNumber<C>>> factors =
        new ArrayList<GenPolynomial<RealAlgebraicNumber<C>>>();
    if (P.isZERO()) {
      return factors;
    }
    if (P.isONE()) {
    ...
    edu.jas.root.RealAlgebraicRing<C> rfac =
        (edu.jas.root.RealAlgebraicRing<C>) (Object) rere.realRing;
    ...
    GenPolynomial<edu.jas.root.RealAlgebraicNumber<C>> A =
        PolyUtilApp.<C>realAlgFromRealCoefficients(arfac, P);
    // factor A:
    List<GenPolynomial<edu.jas.root.RealAlgebraicNumber<C>>> afactors =
        factorAlgebraic.baseFactorsSquarefree(A);
