...
/**
 * A trial division engine where partials can have several large factors. Uses standard BigIntegers division.
 * 
...
public class TDiv_QS_nLarge implements TDiv_QS {
	...
	private AQPair test(BigInteger A, BigInteger Q, int x) {
		...
		if (Q_rest.doubleValue() >= maxQRest)
	...
	private boolean factor_recurrent(BigInteger Q_rest) {
		if (Q_rest.compareTo(pMaxSquare) < 0) {
			...
			// }
			if (Q_rest.bitLength() > 31)
				return false;
			bigFactors.add(Q_rest.intValue());
			return true;
		}
		// now we can not do without isProbablePrime(), because calling findSingleFactor() may not return when called
		...
		// if (DEBUG) LOG.debug("factor_recurrent(): Q_rest = " + Q_rest + " (" + Q_rest_bits + " bits) = " + factor1 +
		// " * " + factor2);
