...
public class DoubleBlockHybridSieveU implements Sieve {
  ...
  public void initializeForN(SieveParams sieveParams, int mergedBaseSize) {
    ...
    // * sieveArraySize is near to sieveArraySize0, effectiveB2 is near to B2, effectiveB1 is near
    // to B1
    ...
    //		if (DEBUG) {
    //			LOG.debug("sieveArraySize0=" + sieveArraySize0 + ", B2=" + B2 + ", B1=" + B1 + " ->
    // sieveArraySize=" + sieveArraySize + ", k2=" + k2 + ", effectiveB2=" + effectiveB2 + ", k1=" +
    // k1 + ", effectiveB1=" + effectiveB1);
  ...
  public List<Integer> sieve() {
    ...
    for (int b2 = 0;
      ...
      // Collect block: We collect 16 bytes at once, thus we need 16 | effectiveBlockSize -> see
      // initialize()
      long y0, y1;
   ...
   * @param r_medium index of the first prime > B -> all smaller primes have at least 2 x-solutions
   *     in the block
  ...
  private void sievePositiveXBlock(
    ...
    for (r = r_max - 1; r >= r_medium; r--) {
      ...
      if ((d1 = dPosArray[r]) != 0) { // two x-solutions
        ...
        // For some reasons it is faster to do the comparison with sieveBlockAddress+blockOffset+B
        // instead of with
        // a precomputed nextBlockAddress = sieveBlockAddress+blockOffset+B. Probably a cache-access
        // effect.
  ...
  private void initializeSieveArray(int sieveArraySize) {
    ...
    // 256.
    // XXX We could use setMemory() to initialize the whole sieve array. This is indeed a bit
    // faster,
