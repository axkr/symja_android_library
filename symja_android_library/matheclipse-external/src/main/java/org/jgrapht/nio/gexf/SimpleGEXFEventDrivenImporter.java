 ...
 *
 * <p>The importer notifies lazily and completely out-of-order for any additional vertex, edge or
 * graph attributes in the input file. Users can register consumers for vertex, edge and graph
 * attributes after construction of the importer. Finally, default attribute values and any nested
 * elements are completely ignored.
 *
 * <p>This is a simple implementation with supports only a limited set of features of the GEXF
 * specification, oriented towards parsing speed.
 *
 * <p>For a description of the format see <a href="https://gephi.org/gexf/format/index.html">
 * https://gephi.org/gexf/format/index.html</a> or the <a
 * href="https://gephi.org/gexf/format/primer.html">GEXF Primer</a>.
 *
 * <p>Below is small example of a graph in GEXF format.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The importer by default validates the input using the 1.2draft <a
 * href="https://gephi.org/gexf/1.2draft/gexf.xsd">GEXF Schema</a>. The user can (not recommended)
 * disable the validation by calling {@link #setSchemaValidation(boolean)}. Older schemas are not
 * supported.
 *
...
public class SimpleGEXFEventDrivenImporter
  ...
  private static final List<String> SCHEMA_FILENAMES = new ArrayList();
  // List.of("viz.xsd", "gexf.xsd");

  private boolean schemaValidation;

  static {
    SCHEMA_FILENAMES.add("viz.xsd");
    SCHEMA_FILENAMES.add("gexf.xsd");
  }
  ...
  private class GEXFHandler extends DefaultHandler {
    ...
    private final List<String> GRAPH_ATTRS = new ArrayList();
    //        List.of("defaultedgetype", "timeformat", "mode", "start", "end");
    private static final String NODE = "node";
    private static final String NODE_ID = "id";
    private final List<String> NODE_ATTRS = new ArrayList();
    // List.of("label", "pid");
    ...
    private final List<String> EDGE_ATTRS = new ArrayList();
    // List.of("type", "label");
    ...
    public GEXFHandler() {
      GRAPH_ATTRS.add("defaultedgetype");
      GRAPH_ATTRS.add("timeformat");
      GRAPH_ATTRS.add("mode");
      GRAPH_ATTRS.add("start");
      GRAPH_ATTRS.add("end");
      NODE_ATTRS.add("label");
      NODE_ATTRS.add("pid");
      EDGE_ATTRS.add("type");
      EDGE_ATTRS.add("label");
    }
      ...
      switch (localName) {
        case GRAPH:
          ...
          if (insideGraph == 1) {
            for (String attrName : GRAPH_ATTRS) {
              findAttribute(attrName, attributes)
                  .ifPresent(
                      value -> {
        ...
        case NODE:
          ...
          if (insideNode == 1 ^ insideEdge == 1) {
            String nodeId =
                findAttribute(NODE_ID, attributes)
                    .orElseThrow(
        ...
        case EDGE:
          ...
          if (insideNode == 1 ^ insideEdge == 1) {
            ...
            if (edgeId != null) {
              notifyEdgeAttribute(currentEdge, EDGE_ID, DefaultAttribute.createAttribute(edgeId));
            }
            notifyEdgeAttribute(
            ...
            if (edgeWeightAsDouble != null) {
              notifyEdgeAttribute(
                  currentEdge, "weight", DefaultAttribute.createAttribute(edgeWeightAsDouble));
            }
        ...
        case ATTRIBUTES:
          ...
          if (insideGraph == 1 && insideAttributes == 1) {
            attributesClass =
                findAttribute(ATTRIBUTES_CLASS, attributes)
                    .orElseThrow(() -> new IllegalArgumentException("Attributes class missing"));
          }
          break;
        case ATTRIBUTE:
          insideAttribute++;
        ...
        case ATTVALUE:
          insideAttValue++;
          if (insideAttValues == 1 && insideAttValue == 1 && (insideNode == 1 ^ insideEdge == 1)) {
            String attValueFor =
                findAttribute(ATTVALUE_FOR, attributes)
                    .orElseThrow(() -> new IllegalArgumentException("Attribute for missing"));
            String attValueValue =
                findAttribute(ATTVALUE_VALUE, attributes)
