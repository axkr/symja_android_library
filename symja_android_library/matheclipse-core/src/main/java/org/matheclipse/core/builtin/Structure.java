...
public class Structure {
	...
	private final static class Depth extends AbstractCoreFunctionEvaluator {
		...
		/**
		 * Calculates the depth of an expression. Atomic expressions (no
		 * sublists) have depth <code>1</code> Example: the nested list
		...
		public IExpr evaluate(final IAST ast, EvalEngine engine) {
			...
			if (ast.isAST3()) {
				...
				try {
					...
					if (headDepth < 0) {
						EvalEngine.get()
								.printMessage("Non-negative integer expected at position 3 in Operate()");
						return F.NIL;
					}
