...
public abstract class GroebnerBaseAbstract<C extends RingElem<C>> implements GroebnerBase<C> {
	 ...
	 *            polynomial list.
	 * @param b
	 *            true for simple test, false for GB test.
	 * @return true, if F is a Groebner base, else false.
	 */
	...
	public ExtendedGB<C> minimalExtendedGB(int flen, List<GenPolynomial<C>> Gp, List<List<GenPolynomial<C>>> M) {
		...
		int k = 0;
		// System.out.println("flen, Gp, M = " + flen + ", " + Gp.size() + ", "
		// + M.size() );
		while (G.size() > 0) {
			a = G.remove(0);
		...
		/*
		 * G = F; F = new ArrayList<GenPolynomial<C>>( G.size() );
		 * List<GenPolynomial<C>> temp; k = 0; final int len = G.size(); while (
		 * G.size() > 0 ) { a = G.remove(0); if ( k >= fix ) { // dont touch
		 * copied polys row = Mf.get( k ); //System.out.println("doing k = " + k
		 * + ", " + a); // must keep order, but removed polys missing temp = new
		 * ArrayList<GenPolynomial<C>>( len ); temp.addAll( F ); temp.add(
		 * a.ring.getZERO() ); // ?? temp.addAll( G ); //System.out.println(
		 * "row before = " + row); a = red.normalform( row, temp, a );
		 * //System.out.println("row after  = " + row); } F.add( a ); k++; } //
	...
	public List<Long> univariateDegrees(List<GenPolynomial<C>> A) {
		...
		// int uht = 0;
		Map<Integer, Long> v = new TreeMap<Integer, Long>(); // for non reduced
																// GBs
		for (GenPolynomial<C> p : A) {
			ExpVector e = p.leadingExpVector();
			if (e == null) {
	 ...
	 *            variable index.
	 * @param G
	 *            list of polynomials, a monic reduced Gr&ouml;bner base of a
	...
	public GenPolynomial<C> constructUnivariate(int i, List<GenPolynomial<C>> G) {
		...
		GenPolynomial<C> X;
		GenPolynomial<C> XP;
		// solve system of linear equations for the coefficients of the
