...
/**
 * Solvable Ideal implements some methods for ideal arithmetic, for example sum, intersection,
 * quotient. <b>Note:</b> only left ideals at the moment.
 *
 * @author Heinz Kredel
 */
public class SolvableIdeal<C extends GcdRingElem<C>>
  ...
  public SolvableIdeal(
      GenSolvablePolynomialRing<C> ring, List<GenSolvablePolynomial<C>> F, boolean gb) {
   ...
   */
  public SolvableIdeal(
      GenSolvablePolynomialRing<C> ring, List<GenSolvablePolynomial<C>> F, Side s) {
   ...
   */
  public SolvableIdeal(
      GenSolvablePolynomialRing<C> ring, List<GenSolvablePolynomial<C>> F, boolean gb, Side s) {
   ...
   */
  public SolvableIdeal(
      PolynomialList<C> list, SolvableGroebnerBaseAbstract<C> bb, SolvableReduction<C> red) {
  ...
  public SolvableIdeal(PolynomialList<C> list, boolean gb, boolean topt) {
    ...
    this(
        ...
        gb,
        topt,
        SGBFactory.getImplementation(list.ring.coFac),
        new SolvableReductionSeq<C>());
  }
   ...
   */
  public SolvableIdeal(
      PolynomialList<C> list, boolean gb, boolean topt, SolvableGroebnerBaseAbstract<C> bb) {
  ...
  /**
   * Comparison with any other object. <b>Note:</b> If not both ideals are Groebner Bases, then
  ...
  /**
   * Test if ONE is contained in the ideal. To test for a proper ideal use <code>! id.isONE()</code>
   * .
   *
  ...
  /**
   * Solvable ideal containment. Test if B is contained in this ideal. Note: this is eventually
  ...
  /**
   * Solvable ideal containment. Test if each b in B is contained in this ideal. Note: this is
  ...
  /**
   * Solvable ideal summation. Generators for the sum of ideals. Note: if both ideals are Groebner
   * bases, a Groebner base is returned.
   *
   * @param B solvable ideal
   * @return ideal(this+B)
   */
  ...
  /**
   * Solvable summation. Generators for the sum of ideal and a polynomial. Note: if this ideal is a
  ...
  /**
   * Solvable summation. Generators for the sum of this ideal and a list of polynomials. Note: if
  ...
  /**
   * Product. Generators for the product of ideals. Note: if both ideals are Groebner bases, a
   * Groebner base is returned.
   *
   * @param B solvable ideal
   * @return ideal(this*B)
   */
  ...
  /**
   * Intersection. Generators for the intersection of a ideal with a polynomial ring. The polynomial
   * ring R must be a contraction of this ideal and the TermOrder must be an elimination order.
   *
    ...
    for (int i = 0; i < rvars.length; i++) {
      if (!rvars[i].equals(tvars[i])) {
        throw new IllegalArgumentException(
            "no contraction: " + R.toScript() + " of " + getRing().toScript());
      }
    }
  ...
  /**
   * Eliminate. Generators for the intersection of this ideal with a solvable polynomial ring. The
   * solvable polynomial ring of this ideal must be a contraction of R and the TermOrder must be an
   * elimination order.
   *
   * @param R solvable polynomial ring
   * @return ideal(this \cap R)
  ...
  public SolvableIdeal<C> infiniteQuotientRab(GenSolvablePolynomial<C> h) {
    ...
    if (!getRing().isCommutative()) {
      throw new UnsupportedOperationException(
          "Rabinowich trick only for commutative polynomial rings");
    }
  ...
  /**
   * Power. Generators for the power of this solvable ideal. Note: if this ideal is a Groebner base,
   * a Groebner base is returned.
   *
   * @param d integer
   * @return ideal(this^d)
   */
  ...
  public SolvableIdeal<C> annihilator(GenSolvablePolynomial<C> h) {
    ...
    doGB();
    List<GenSolvablePolynomial<C>> F =
        new ArrayList<GenSolvablePolynomial<C>>(1 + getList().size());
    F.add(h);
    F.addAll(getList());
  ...
  public GenSolvablePolynomial<C> inverse(GenSolvablePolynomial<C> h) {
    ...
    doGB();
    List<GenSolvablePolynomial<C>> F =
        new ArrayList<GenSolvablePolynomial<C>>(1 + list.list.size());
    F.add(h);
    F.addAll(getList());
    // System.out.println("F = " + F);
  ...
  /**
   * Construct univariate polynomials of minimal degree in all variables in zero dimensional
   * ideal(G).
   *
   * @return list of univariate solvable polynomial of minimal degree in each variable in ideal(G)
   */
