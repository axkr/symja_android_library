...
/**
 * Solvable Groebner Base with pseudo reduction sequential algorithm. Implements coefficient
 * fraction free Groebner bases. Coefficients can for example be integers or (commutative)
...
public class SolvableGroebnerBasePseudoSeq<C extends GcdRingElem<C>>
   ...
   * Constructor.
   *
   * @param red pseudo reduction engine. <b>Note:</b> red must be an instance of PseudoReductionSeq.
   * @param rf coefficient ring factory.
   * @param pl pair selection strategy
   */
  public SolvableGroebnerBasePseudoSeq(
      SolvablePseudoReduction<C> red, RingFactory<C> rf, PairList<C> pl) {
    super(red, pl);
    this.sred = red;
    cofac = rf;
  ...
  public List<GenSolvablePolynomial<C>> leftGB(int modv, List<GenSolvablePolynomial<C>> F) {
    ...
    G =
        PolynomialList.<C>castToSolvableList(
            engine.basePrimitivePart(PolynomialList.<C>castToList(G)));
    if (G.size() <= 1) {
      return G;
    }
  ...
  public List<GenSolvablePolynomial<C>> twosidedGB(int modv, List<GenSolvablePolynomial<C>> Fp) {
    ...
    List<GenSolvablePolynomial<C>> F =
        new ArrayList<GenSolvablePolynomial<C>>(G.size() * (1 + X.size()));
    F.addAll(G);
   ...
   * @return a container for an extended left Groebner base of F. <b>Note: </b> not implemented;
   */
