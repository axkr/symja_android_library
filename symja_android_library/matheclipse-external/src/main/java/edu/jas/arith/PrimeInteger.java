 ...
 * 
 * See ALDES/SAC2 or MAS code in SACPRIM. See Symja
 * <code>org/matheclipse/core/expression/Primality.java</code> for Pollard algorithm.
 * 
 * @author Heinz Kredel
 */
...
public final class PrimeInteger {
  ...
  /**
   * Digit prime generator. K and m are positive beta-integers. L is the list (p(1),...,p(r)) of all
   * prime numbers p such that m le p lt m+2*K, with p(1) lt p(2) lt ... lt p(r). See also
   * SACPRIM.DPGEN.
   * 
  ...
  /**
   * Integer small prime divisors. n is a positive integer. F is a list of primes
   * (q(1),q(2),...,q(h)), h non-negative, q(1) le q(2) le ... lt q(h), such that n is equal to m
   * times the product of the q(i) and m is not divisible by any prime in SMPRM. Either m=1 or m gt
   * 1,000,000. <br />
   * In JAS F is a map and m=1 or m &gt; 4.000.000. See also SACPRIM.ISPD.
   * 
   * @param n integer to factor.
   * @param F a map of pairs of prime numbers and multiplicities (p,e) with p**e divides n and e
   *        maximal, F is modified.
  ...
  public static java.math.BigInteger smallPrimeDivisors(java.math.BigInteger n,
      SortedMap<java.math.BigInteger, Integer> F) {
    List<Long> LP;
  ...
  /**
   * Test prime factorization. n is a positive integer. r is true, if n = product_i(pi**ei) and each
   * pi is prime, else false.
   * 
   * @param n integer to test.
  ...
  /**
   * Integer factorization. n is a positive integer. F is a list (q(1), q(2),...,q(h)) of the prime
   * factors of n, q(1) le q(2) le ... le q(h), with n equal to the product of the q(i). <br />
  ...
  public static SortedMap<Long, Integer> factors(long n) {
    if (n > BETA) {
      throw new UnsupportedOperationException(
          "factors(long) only for longs less than BETA: " + BETA);
    }
   ...
   * q(2),...,q(h)) of the prime factors of n, q(1) le q(2) le ... le q(h), with n equal to the
   * product of the q(i). <br />
   * In JAS F is a map. See also SACPRIM.IFACT.
   * 
   * @param n integer to factor.
   * @return a map F of pairs of numbers (p,e) with p**e divides n and p probable prime.
   */
  ...
  /**
   * Integer medium prime divisor search. n, a and b are positive integers such that a le b le n and
   * n has no positive divisors less than a. If n has a prime divisor in the closed interval from a
   * to b then p is the least such prime and q=n/p. Otherwise p=1 and q=n. See also SACPRIM.IMPDS.
   * 
   * @param n integer to factor.
   * @param a lower bound.
   * @param b upper bound.
  ...
  /**
   * Integer selfridge primality test. m is an integer greater than or equal to 3. mp=m-1. F is a
   * list (q(1),q(2),...,q(k)), q(1) le q(2) le ... le q(k), of the prime factors of mp, with mp
   * equal to the product of the q(i). An attempt is made to find a root of unity modulo m of order
   * m-1. If the existence of such a root is discovered then m is prime and s=1. If it is discovered
   * that no such root exists then m is not a prime and s=-1. Otherwise the primality of m remains
   * uncertain and s=0. See also SACPRIM.ISPT.
   * 
   * @param m integer to test.
   * @param mp integer m-1.
   * @param F a map of pairs (p,e), with primes p, multiplicity e and with p**e divides mp and e
   *        maximal.
  ...
  /**
   * Integer large prime divisor search. n is a positive integer with no prime divisors less than
   * 17. 1 le a le b le n. A search is made for a divisor p of the integer n, with a le p le b. If
   * such a p is found then np=n/p, otherwise p=1 and np=n. A modular version of Fermats method is
   * used, and the search goes from a to b. See also SACPRIM.ILPDS.
   * 
   * @param n integer to factor.
   * @param a lower bound.
   * @param b upper bound.
  ...
  /**
   * Fermat residue list, single modulus. m is a positive beta-integer. a belongs to Z(m). L is a
   * list of the distinct b in Z(m) such that b**2-a is a square in Z(m). See also SACPRIM.FRLSM.
   * 
  ...
  /**
   * Fermat residue list. n is a positive integer with no prime divisors less than 17. m is a
   * positive beta-integer and L is an ordered list of the elements of Z(m) such that if x**2-n is a
   * square then x is congruent to a (modulo m) for some a in L. See also SACPRIM.FRESL.
   * 
   * @param n integer to factor.
   ...
   * 
   * @param n integer to factor.
   * @param F a map of pairs of numbers (p,e) with p**e divides n and p is probable prime, F is
   *        modified.
   */
  public static void factorsPollardRho(java.math.BigInteger n,
    ...
    factorsPollardRho(n, F, MAX_RHO_RECURSION_DEPTH);
  }


  /**
   * Integer factorization using Pollards rho algorithm. The algorithm only splits a composite
   * number into two factors, which are not necessarily prime. Every divisor is therefore tested for
   * primality and split again, if it is composite.
   * 
   * @param rest positive integer to factor.
   * @param F a map of pairs of numbers (p,e) with p**e divides rest and p is probable prime, F is
   *        modified.
   * @param recursionDepth number of allowed recursive splittings of a composite divisor.
   */
  static void factorsPollardRho(java.math.BigInteger rest,
    ...
    while (!rest.equals(java.math.BigInteger.ONE)) {
      ...
      java.math.BigInteger divisor =
          (recursionDepth > 0 ? rhoDivisor(rest) : java.math.BigInteger.ONE);
      if (divisor.equals(java.math.BigInteger.ONE)) {
        // no divisor found, store the composite rest as a factor to avoid an endless loop
        addToMap(F, rest, 1);
        return;
      }
      if (divisor.isProbablePrime(32)) {
        ...
        addToMap(F, divisor, exponent);
      } else {
        factorsPollardRho(divisor, F, recursionDepth - 1);
  ...
  /**
   * Search a nontrivial divisor of the composite number n with Pollards rho algorithm. The divisor
   * is not necessarily prime.
   * 
   * @param n composite integer.
   * @return a divisor of n with 1 &lt; divisor &lt; n, or ONE if no divisor could be found.
   */
  static java.math.BigInteger rhoDivisor(java.math.BigInteger n) {
    for (int i = 0; i < MAX_RHO_RESTARTS; i++) {
      java.math.BigInteger divisor = rho(n);
      if (divisor.compareTo(java.math.BigInteger.ONE) > 0 && !divisor.equals(n)) {
        return divisor;
      }
    }
    // Pollards rho algorithm does not work reliably for numbers with only a few small prime
    // factors, fall back to trial division by the small primes
    for (int i = 0; i < SMPRM.size(); i++) {
      java.math.BigInteger p = java.math.BigInteger.valueOf(SMPRM.get(i));
      if (n.compareTo(p) > 0 && n.mod(p).signum() == 0) {
        return p;
      }
    }
  ...
  static void addToMap(SortedMap<java.math.BigInteger, Integer> F, java.math.BigInteger p, int e) {
    Integer count = F.get(p);
    F.put(p, (count == null) ? e : count + e);
  }
  ...
  /**
   * Number of restarts of the Pollards rho cycle search with new random start values, before a
   * composite number is considered as not factorizable.
   */
  final static int MAX_RHO_RESTARTS = 5;


  /**
   * Number of recursive splittings of a composite divisor found by Pollards rho algorithm.
   */
  final static int MAX_RHO_RECURSION_DEPTH = 64;


  /**
   * Search cycle with Pollards rho algorithm x**2 + c mod n. n is a positive integer. <br />
   * 
   * @param n integer test.
   * @return x-y with gcd(x-y, n) = 1.
   */
  static java.math.BigInteger rho(java.math.BigInteger n) {
    if (n.compareTo(java.math.BigInteger.valueOf(4L)) < 0) {
      // 0, 1, 2, 3 cannot be split, for 1 the loop below would not terminate
      return n;
    }
    if (!n.testBit(0)) {
  ...
  static void factorsPollardRho(long rest, SortedMap<Long, Integer> F, int recursionDepth) {
    while (rest != 1L) {
      ...
      long divisor = (recursionDepth > 0 ? rhoDivisor(rest) : 1L);
      if (divisor == 1L) {
        // no divisor found, store the composite rest as a factor to avoid an endless loop
        addToMap(F, rest, 1);
        return;
   ...
   * 
   * @param n composite integer.
   * @return a divisor of n with 1 &lt; divisor &lt; n, or 1 if no divisor could be found.
   */
  static long rhoDivisor(long n) {
    for (int i = 0; i < MAX_RHO_RESTARTS; i++) {
      long divisor = rho(n);
      if (divisor > 1L && divisor != n) {
        return divisor;
      }
    }
    // Pollards rho algorithm does not work reliably for numbers with only a few small prime
  ...
  static void addToMap(SortedMap<Long, Integer> F, long p, int e) {
    Integer count = F.get(p);
    F.put(p, (count == null) ? e : count + e);
  }
  ...
  /**
   * Search cycle with Pollards rho algorithm x**2 + c mod n. n is a positive integer. c is a random
   * constant.
   * 
   * @param n integer test.
   * @return x-y with gcd(x-y, n) == 1.
