...
/**
 * Cartesian product for Long with iterator. Similar to CartesianProduct but returns only tuples of
 * given total degree.
 *
 * @author Heinz Kredel
 */
...
/**
 * Cartesian product iterator for Longs. Similar to CartesianProductIterator but returns only tuples
 * of given total degree.
 *
 * @author Heinz Kredel
 */
    ...
    for (LongIterable ci : comps) {
      ...
      if (it.getUpperBound() < this.upperBound) {
        throw new IllegalArgumentException(
            "each iterator ("
                + it.getUpperBound()
                + ") must be able to reach total upper bound "
                + upperBound);
      }
      if (!it.hasNext()) {
        empty = true;
