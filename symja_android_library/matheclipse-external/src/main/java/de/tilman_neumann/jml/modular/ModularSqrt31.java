...
public class ModularSqrt31 {
        ...
        if (p > 2)
          LOG.warn(
              "Tonelli_Shanks() has been called with even p="
                  + p
                  + " -> brute force search required."); // p=2 is no problem
  ...
  int Tonelli_Shanks(int n, int p) { // not private because used in tests
    ...
    int S =
        Integer.numberOfTrailingZeros(
            pm1); // lowest set bit (0 if pm1 were odd which is impossible because p is odd)
    //		if (DEBUG) {
    //			LOG.debug("n=" + n + ", p=" + p);
    ...
    //		if (DEBUG) assertEquals(BigInteger.valueOf(R).pow(2).mod(BigInteger.valueOf(p)),
    // BigInteger.valueOf(n%p));
  ...
  int Lagrange(int n, int p) { // not private because used in tests
    ...
    //		if (DEBUG) assertEquals(BigInteger.valueOf(t).pow(2).mod(BigInteger.valueOf(p)),
    // BigInteger.valueOf(n%p));
    // return the smaller sqrt
    return t <= (p >> 1) ? t : p - t;
  }
   ...
   * <link>www.point-at-infinity.org/ecc/Algorithm_of_Shanks_&_Tonelli.html</link>. Roughly 3 times
   * faster than Tonelli-Shanks.
   *
   * @param n a positive integer having Jacobi(n|p) = 1
   * @param p
   * @return (the smaller) sqrt of n (mod p)
  ...
  int case5Mod8(int n, int p) { // not private because used in tests
    ...
    int ng = (int) ((n * g) % p);
    int t =
        (int) ((ng * (i - 1)) % p); // TODO Here we may falsely get t=0 if i=1. Can that be cured?
    //		if (DEBUG) {
    //			// We have i = (2a)^(p-1)/4 (mod p). This looks closely related to Euler's criterion
    // a^(p-1)/2 (mod p) == +-1 and the Legendre symbol definition.
    //			long i2 = mpe.modPow(n2, (p-1)/4, p);
    //			assertEquals(i, i2);
    //			LOG.debug("n=" + n + ", p=" + p + ", i=" + i + ", i2=" + i2);
    //			LOG.debug("k=" + k + ", n2=" + n2 + ", g=" + g + ", gSquare=" + gSquare + ", i=" + i + ",
    // ng=" + ng + ", t=" + t);
    //			BigInteger p_big = BigInteger.valueOf(p);
