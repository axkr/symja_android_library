...
public class GreenFunction extends AbstractFunctionEvaluator {
  ...
  public IExpr evaluate(final IAST ast, EvalEngine engine) {
    try {
      ...
      return greenFunction(problem, head, xVar, range.arg2(), range.arg3(), source, engine);
  ...
  private static IExpr greenFunction(IAST problem, IExpr head, IExpr xVar, IExpr xMin, IExpr xMax,
    ...
    if (form == null || form.order != 2 || !form.g.isZero()) {
      // Only the second order case is built here, and the operator has to be the homogeneous one.
      return F.NIL;
    }
    if (problem.argSize() != 3) {
      return F.NIL;
    }

    IExpr[] basis = homogeneousBasis(operator, applied, xVar, engine);
    if (basis == null) {
      return F.NIL;
    }
    // Each boundary condition picks out the solution which meets it.
    ...
    return F.Piecewise(F.List(F.List(below, F.LessEqual(xVar, source))), above);
  }

  /** The two solutions of the homogeneous equation, or <code>null</code> if they are not found. */
  private static IExpr[] homogeneousBasis(IExpr operator, IExpr applied, IExpr xVar,
      EvalEngine engine) {
    ...
    if (!rest.isZero() || basis[0].isZero() || basis[1].isZero()) {
      return null;
    }
    for (int i = 0; i < 2; i++) {
