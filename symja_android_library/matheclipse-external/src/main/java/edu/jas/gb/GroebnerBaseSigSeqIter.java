...
/**
 * Groebner Base signature based sequential iterative algorithm. Implements Groebner bases after the
 * paper "Signature-based Algorithms to Compute Gröbner Bases" by Christian Eder and John Perry,
 * ISSAC 2011. Compare the jython+JAS code in examples/basic_sigbased_gb.py. Originally the
 * Python+Sage code is from http://www.math.usm.edu/perry/Research/basic_sigbased_gb.py
 *
      ...
      while (!Sl.isEmpty()) {
        ...
        if (H.length() > 0) {
          for (SigPoly<C> p : Gs) {
            ...
            // System.out.println("new_pair " + pp.sigma + ", xy = " + xy + ", sigma = " + sigma +
            // ", tau = " + tau + ", mult  = " + Arrays.toString(mult) + ", m0*se = " +
            // mult[0].multiply(se) + ", m1*te = " + mult[1].multiply(te) );
