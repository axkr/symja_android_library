 ...
 * lexicographical, weight aray and block orders. For the definitions see for example the articles
 * <a href="http://doi.acm.org/10.1145/43882.43887">Kredel "Admissible term orderings used in
 * computer algebra systems"</a> and <a href="http://doi.acm.org/10.1145/70936.70941">Sit, "Some
 * comments on term-ordering in Gr&ouml;bner basis computations"</a>. <b>Note: </b> the naming is
 * not quite easy to understand: in case of doubt use the term orders with "I" in the name, like
 * IGRLEX (the default) or INVLEX. Not all algorithms may work with all term orders since not all
 * are well-founded, so watch your step. This class does not implement orders by linear forms over
...
public final class TermOrder implements Serializable {
  ...
  /**
   * Termorders for modules: TOP or POT. POT: position over term (default) TOP: term over position
   * (new)
   */
  public final boolean TOP;

  /** Comparator for ExpVectors. */
  public abstract static class EVComparator implements Comparator<ExpVector>, Serializable {
  ...
  /**
   * Extend variables. Used e.g. in module embedding. Extend TermOrder by k elements. <b>Note:</b>
   * Use POT module term order.
   *
  ...
  /**
   * Extend variables. Used e.g. in module embedding. Extend TermOrder by k elements. <b>Note:</b>
   * Now TOP and POT orders are distinguished.
   *
   * @param r current number of variables.
   * @param k number of variables to extend.
  ...
  /**
   * Extend lower variables. Extend TermOrder by k elements. <b>Note:</b> todo distinguish TOP and
   * POT orders.
   *
   * @param r current number of variables.
   * @param k number of variables to extend.
  ...
  public TermOrder permutation(List<Integer> P) {
    ...
    if (getEvord2() != 0) {
      ...
      logger.warn(
          "split term order '"
              + this
              + "' not permutable, resetting to most base term order "
              + tord);
    }
