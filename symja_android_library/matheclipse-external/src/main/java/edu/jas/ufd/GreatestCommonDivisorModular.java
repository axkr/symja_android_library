...
public class GreatestCommonDivisorModular<MOD extends GcdRingElem<MOD> & Modular>
  ...
  /**
   * Univariate GenPolynomial greatest comon divisor. Delegate to subresultant baseGcd, should not
   * be needed.
   *
   ...
   */
  @Override
  public GenPolynomial<BigInteger> baseGcd(
      GenPolynomial<BigInteger> P, GenPolynomial<BigInteger> S) {
    return iufd.baseGcd(P, S);
  }

  /**
   * Univariate GenPolynomial recursive greatest comon divisor. Delegate to subresultant
   * recursiveGcd, should not be needed.
   *
   ...
   */
  @Override
  public GenPolynomial<BigInteger> baseResultant(
      GenPolynomial<BigInteger> P, GenPolynomial<BigInteger> S) {
  ...
  @Override
  @SuppressWarnings("unchecked")
  public GenPolynomial<BigInteger> resultant(
      GenPolynomial<BigInteger> P, GenPolynomial<BigInteger> S) {
    if (S == null || S.isZERO()) {
      return S;
    }
    ...
    for (java.math.BigInteger p : primes) {
      ...
      if (qm.isZERO() || !qm.leadingExpVector().equals(qdegv)) { // degreeVector()
        ...
        if (debug) {
          logger.info(
              "unlucky prime = " + cofac.getIntegerModul() + ", degv = " + qm.leadingExpVector());
        }
        continue;
      }
      rm = PolyUtil.<MOD>fromIntegerCoefficients(mfac, r);
      if (rm.isZERO() || !rm.leadingExpVector().equals(rdegv)) { // degreeVector()
        ...
        if (debug) {
          logger.info(
              "unlucky prime = " + cofac.getIntegerModul() + ", degv = " + rm.leadingExpVector());
        }
        continue;
      }
