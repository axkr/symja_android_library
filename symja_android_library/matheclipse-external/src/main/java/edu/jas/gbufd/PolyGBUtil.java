...
public class PolyGBUtil {
   ...
   */
  public static <C extends GcdRingElem<C>> boolean isResultant(
      GenPolynomial<C> A, GenPolynomial<C> B, GenPolynomial<C> r) {
    if (r == null || r.isZERO()) {
      return true;
    }
   ...
   */
  public static <C extends RingElem<C>> GenPolynomial<C> topPseudoRemainder(
   ...
   * @param A list of generic polynomials in n variables sorted according to appearing main
   *     variables.
  ...
  public static <C extends RingElem<C>> GenPolynomial<C> topCoefficientPseudoRemainder(
    ...
    GenPolynomial<GenPolynomial<C>> pr = PolyUtil.<C>recursive(rfac, P);
    GenPolynomial<GenPolynomial<GenPolynomial<C>>> pr2 =
        PolyUtil.<GenPolynomial<C>>recursive(rfac2, pr);
   ...
   * @return pseudo remainder of the leading coefficient of P wrt A, with ldcf(A)<sup>m'</sup> P =
   ...
   */
  public static <C extends RingElem<C>> List<GenPolynomial<C>> zeroDegrees(
      List<GenPolynomial<C>> A) {
    if (A == null || A.isEmpty()) {
      return A;
    }
    GenPolynomialRing<C> pfac = A.get(0).ring;
   ...
   */
  public static <C extends GcdRingElem<C>> List<GenPolynomial<C>> intersect(
      GenPolynomialRing<C> pfac, List<GenPolynomial<C>> A, List<GenPolynomial<C>> B) {
  ...
  public static <C extends GcdRingElem<C>> List<GenSolvablePolynomial<C>> intersect(
      ...
      List<GenSolvablePolynomial<C>> A,
  ...
  public static <C extends GcdRingElem<C>> List<GenWordPolynomial<C>> intersect(
      GenWordPolynomialRing<C> pfac, List<GenWordPolynomial<C>> A, List<GenWordPolynomial<C>> B) {
    if (A == null || A.isEmpty()) { // (0)
      return B;
    }
    if (B == null || B.isEmpty()) { // (0)
