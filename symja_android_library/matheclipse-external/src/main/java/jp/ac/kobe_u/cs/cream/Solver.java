...
/**
 * An abstract class for constraint solvers. A solver is constructed with a {@linkplain Network
 * constraint network} which is used by the solver to find solutions. Please note that any network
 * can not be simultaneously shared by two different solvers.
 *
 * <p>Solvers can be used in three typical ways.
 *
 * <ul>
 ...
 *     });
 * </pre>
 *   <li>As a coroutine: {@link #start()}, {@link #waitNext()}, {@link #resume()}, {@link #stop()},
 *       etc.
 *       <pre>
...
public abstract class Solver implements Runnable {
  ...
  /**
   * Constructs a solver for the given network (for invocation by subclass constructors). This
   * constructor is equivalent to <tt>Solver(network, DEFAULT, null)</tt>.
   *
  ...
  /**
   * Constructs a solver for the given network, option, and name (for invocation by subclass
   * constructors). When <tt>option</tt> is <tt>DEFAULT</tt>, <tt>NONE</tt> is used if the network
   * has no objective variable, or else <tt>MINIMIZE</tt> is used. Solvers and subclasses have their
  ...
  public Solver(Network network, int option, String name) {
    ...
    if (option == DEFAULT) {
      if (network.getObjective() == null) option = NONE;
      else option = MINIMIZE;
    }
    this.option = option;
    id = idCounter++;
    if (name == null) {
   ...
   *
   * @return the last solution, or <tt>null</tt> if no solutions have been found
   */
   ...
   *
   * @return the best solution, or <tt>null</tt> if no solutions have been found
   */
  ...
  /**
   * Returns the best objective value this solver has been found. When no solutions have been found,
   * this method returns <tt>IntDomain.MAX_VALUE</tt> if the search strategy is MINIMIZE, or
  ...
  /**
   * Starts the solver in a new thread, and immediately returns to the caller. The {@link
   * #waitNext()} and {@link #waitNext(long timeout)} methods can be used to wait the next solution.
   * When a solution is found, the solver suspends the execution until the {@link #resume()} method
   * is called. You can stop the solver anytime by calling the {@link #stop()} method.
   */
  ...
  /**
   * Starts the solver in a new thread with the timeout, and immediately returns to the caller. When
   * the <tt>timeout</tt> milliseconds have been elapsed since the start of the solver, it stops the
   * execution. The {@link #waitNext()} and {@link #waitNext(long timeout)} methods can be used to
   * wait the next solution, or to detect the timeout. When a solution is found, the solver suspends
   * the execution until the {@link #resume()} method is called. You can stop the solver anytime by
  ...
  /**
   * Starts the solver in a new thread, and immediately returns to the caller. The handler is called
   * for each solution and at the end of the solver execution. You can stop the solver anytime by
   * calling the {@link #stop()} method.
   *
   ...
   * Starts the solver in a new thread with the timeout, and immediately returns to the caller. When
   * the <tt>timeout</tt> milliseconds have been elapsed since the start of the solver, it stops the
   * execution. The handler is called for each solution and at the end of the solver execution. You
  ...
  /**
   * Waits for the next solution, or the end of the solver execution. It returns <tt>true</tt> if
   * the next solution is available, <tt>false</tt> if the solver ends the execution.
   *
  ...
  /**
   * Waits for the next solution, or the end of the solver execution with the timeout. It returns
   * <tt>true</tt> if the next solution is available within the timeout milliseconds, <tt>false</tt>
   * if the solver ends the execution or the <tt>timeout</tt> milliseconds have been elapsed since
  ...
  /**
   * Invokes the handler for each solution. This method is equivalent to {@link
   * #findAll(SolutionHandler, long) findFirst(handler, 0)}.
   *
   * @param handler solution handler
   */
