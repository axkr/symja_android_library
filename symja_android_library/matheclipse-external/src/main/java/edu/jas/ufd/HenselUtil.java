...
public class HenselUtil {
  ...
  /**
   * Modular Hensel lifting algorithm on coefficients. Let p = A.ring.coFac.modul() =
   * B.ring.coFac.modul() and assume C == A*B mod p with gcd(A,B) == 1 mod p and S A + T B == 1 mod
   * p. See Algorithm 6.1. in Geddes et.al.. Linear version, as it does not lift S A + T B == 1 mod
   * p^{e+1}.
   *
  ...
  /**
   * Modular Hensel lifting algorithm on coefficients. Let p = A.ring.coFac.modul() =
   * B.ring.coFac.modul() and assume C == A*B mod p with gcd(A,B) == 1 mod p. See algorithm 6.1. in
   * Geddes et.al. and algorithms 3.5.{5,6} in Cohen.
   *
   * @param C GenPolynomial
   * @param A GenPolynomial
   * @param B other GenPolynomial
  ...
  /**
   * Modular quadratic Hensel lifting algorithm on coefficients. Let p = A.ring.coFac.modul() =
   * B.ring.coFac.modul() and assume C == A*B mod p with gcd(A,B) == 1 mod p and S A + T B == 1 mod
   * p. See algorithm 6.1. in Geddes et.al. and algorithms 3.5.{5,6} in Cohen. Quadratic version, as
   * it also lifts S A + T B == 1 mod p^{e+1}.
   *
   * @param C GenPolynomial
   * @param A GenPolynomial
   * @param B other GenPolynomial
   ...
   * Modular quadratic Hensel lifting algorithm on coefficients. Let p = A.ring.coFac.modul() =
   * B.ring.coFac.modul() and assume C == A*B mod p with gcd(A,B) == 1 mod p. See algorithm 6.1. in
   * Geddes et.al. and algorithms 3.5.{5,6} in Cohen. Quadratic version.
   *
   * @param C GenPolynomial
   * @param A GenPolynomial
   * @param B other GenPolynomial
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> HenselApprox<MOD> liftHenselQuadraticFac(
    ...
    while (Mq.compareTo(M2) < 0) {
      ...
      Ema =
          PolyUtil.<MOD>fromIntegerCoefficients(
              mfac, PolyUtil.integerFromModularCoefficients(fac, A1p));
      Emb =
          PolyUtil.<MOD>fromIntegerCoefficients(
              mfac, PolyUtil.integerFromModularCoefficients(fac, B1p));
  ...
  /**
   * Modular Hensel lifting test. Let p be a prime number and assume C == prod_{0,...,n-1} g_i mod p
   * with gcd(g_i,g_j) == 1 mod p for i != j.
   *
   * @param C GenPolynomial
  ...
  boolean isHenselLift(
      GenPolynomial<BigInteger> C, BigInteger M, BigInteger p, List<GenPolynomial<BigInteger>> G) {
    if (C == null || C.isZERO()) {
      return false;
    }
  ...
  /**
   * Modular Hensel lifting test. Let p be a prime number and assume C == A * B mod p with gcd(A,B)
   * == 1 mod p.
   *
   * @param C GenPolynomial
   * @param A GenPolynomial
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> boolean isHenselLift(
      GenPolynomial<BigInteger> C, BigInteger M, BigInteger p, HenselApprox<MOD> Ha) {
    List<GenPolynomial<BigInteger>> G = new ArrayList<GenPolynomial<BigInteger>>(2);
    G.add(Ha.A);
    G.add(Ha.B);
    return isHenselLift(C, M, p, G);
   ...
   */
  public static <MOD extends GcdRingElem<MOD> & Modular>
      List<GenPolynomial<MOD>> liftExtendedEuclidean(List<GenPolynomial<MOD>> A, long k)
          throws NoLiftingException {
    if (A == null || A.size() <= 1) {
    ...
    for (int j = 0; j < r - 1; j++) {
      ...
      // System.out.println("\nSb = " + S);
      b = S.get(0);
      GenPolynomial<MOD> bb =
          PolyUtil.<MOD>fromIntegerCoefficients(
              fac, PolyUtil.integerFromModularCoefficients(ifac, b));
      B.set(j + 1, bb);
      lift.set(j, S.get(1));
    ...
    // System.out.println("B("+(r-1)+") = " + B.get(r-1) + " : " +  B.get(r-1).ring.coFac + ", b = "
    // +  b + " : " +  b.ring.coFac);
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> List<GenPolynomial<MOD>> liftDiophant(
    ...
    for (Monomial<MOD> m : C) {
      ...
      for (GenPolynomial<MOD> d : S) {
        ...
        d =
            PolyUtil.<MOD>fromIntegerCoefficients(
                fac, PolyUtil.integerFromModularCoefficients(ifac, d));
        d = d.multiply(a);
        d = sol.get(i).sum(d);
        // System.out.println("d = " + d);
        sol.set(i++, d);
    ...
    if (debug) {
      // GenPolynomialRing<BigInteger> ifac = new GenPolynomialRing<BigInteger>(new BigInteger(),
      // fac);
      A =
          PolyUtil.<MOD>fromIntegerCoefficients(
              fac, PolyUtil.integerFromModularCoefficients(ifac, A));
      B =
          PolyUtil.<MOD>fromIntegerCoefficients(
              fac, PolyUtil.integerFromModularCoefficients(ifac, B));
      C =
          PolyUtil.<MOD>fromIntegerCoefficients(
              fac, PolyUtil.integerFromModularCoefficients(ifac, C));
  ...
  /**
   * Modular diophantine equation solution and lifting algorithm. Let p = A_i.ring.coFac.modul() and
   * assume gcd(a,b) == 1 mod p, for a, b in A.
   *
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> boolean isDiophantLift(
    ...
    GenPolynomial<MOD> s1 =
        PolyUtil.<MOD>fromIntegerCoefficients(
            fac, PolyUtil.integerFromModularCoefficients(ifac, S1));
    GenPolynomial<MOD> s2 =
        PolyUtil.<MOD>fromIntegerCoefficients(
            fac, PolyUtil.integerFromModularCoefficients(ifac, S2));
   ...
   * @param C = GenPolynomial
   * @return true if prod_{0,...,n-1} s_i * B_i = C mod p^k, with B_i = prod_{i!=j} A_j, else false.
   */
  public static <MOD extends GcdRingElem<MOD> & Modular> boolean isDiophantLift(
  ...
  /**
   * Modular Hensel lifting algorithm on coefficients. Let p = f_i.ring.coFac.modul() and assume C
   * == prod_{0,...,n-1} f_i mod p with gcd(f_i,f_j) == 1 mod p for i != j
   *
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> List<GenPolynomial<MOD>> liftHenselMonic(
    ...
    //             HenselApprox<MOD> ab = HenselUtil.<MOD> liftHenselQuadratic(C, M, F.get(0),
    // F.get(1));
    //             lift.add(ab.Am);
    //             lift.add(ab.Bm);
    //             return lift;
    ...
    if (debug) {
      ...
      // adjust coefficients
      List<GenPolynomial<MOD>> Sx =
          PolyUtil.fromIntegerCoefficients(pfac, PolyUtil.integerFromModularCoefficients(ifac, S));
      try {
