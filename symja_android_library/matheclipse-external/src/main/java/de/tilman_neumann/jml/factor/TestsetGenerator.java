...
/**
 * Generation of random N that are not too easy to factor. The standard case are semiprimes N where
 * the smaller factor of N is >= cbrt(N).
 *
    ...
    try {
      ...
      switch (mode) {
        case RANDOM_COMPOSITES:
          {
            if (bits < 3)
              throw new IllegalArgumentException("There are no composites with " + bits + " bits.");
        ...
        case RANDOM_ODD_COMPOSITES:
          {
            if (bits < 4)
              throw new IllegalArgumentException(
                  "There are no odd composites with " + bits + " bits.");
            for (int i = 0; i < N_count; ) {
        ...
        case MODERATE_SEMIPRIMES:
          {
            if (bits < 4)
              throw new IllegalArgumentException(
                  "There are no odd semiprimes with " + bits + " bits.");
        ...
        case QUITE_HARD_SEMIPRIMES:
          {
            ...
            for (int i = 0; i < N_count; ) {
              ...
              if (DEBUG)
                LOG.debug(
                    "bits=" + bits + ", N1Bits=" + n1.bitLength() + ", N2Bits=" + n2.bitLength());
        ...
        default:
          throw new IllegalArgumentException("TestsetGeneratorMode " + mode);
      }
    } catch (IOException e) {
