package org.matheclipse.core.reflection.system.rules;

import static org.matheclipse.core.expression.F.*;
import org.matheclipse.core.interfaces.IAST;

/**
 * <p>
 * Generated by <code>org.matheclipse.core.preprocessor.RulePreprocessor</code>.
 * </p>
 * <p>
 * See GIT repository at:
 * <a href="https://github.com/axkr/symja_android_library">github.com/axkr/symja_android_library
 * under the tools directory</a>.
 * </p>
 */
public class CothRules {
  /**
   * <ul>
   * <li>index 0 - number of equal rules in <code>RULES</code></li>
   * </ul>
   */
  final public static int[] SIZES = {9, 7};

  final public static IAST RULES = List(IInit(Coth, SIZES),
      // Coth(Undefined)=Undefined
      ISet(Coth(Undefined), Undefined, true),
      // Coth(0)=ComplexInfinity
      ISet(Coth(C0), CComplexInfinity, true),
      // Coth(0.0)=ComplexInfinity
      ISet(Coth(CD0), CComplexInfinity, true),
      // Coth(I*1/4*Pi)=-I
      ISet(Coth(Times(CC(0L, 1L, 1L, 4L), Pi)), CNI, true),
      // Coth(I*1/2*Pi)=0
      ISet(Coth(Times(CC(0L, 1L, 1L, 2L), Pi)), C0, true),
      // Coth(I*3/4*Pi)=I
      ISet(Coth(Times(CC(0L, 1L, 3L, 4L), Pi)), CI, true),
      // Coth(I*Pi)=ComplexInfinity
      ISet(Coth(Times(CI, Pi)), CComplexInfinity, true),
      // Coth(ArcSinh(x_)):=Sqrt(1+x^2)/x
      ISetDelayed(Coth(ArcSinh(x_)), Times(Power(x, CN1), Sqrt(Plus(C1, Sqr(x))))),
      // Coth(ArcCosh(x_)):=x/(Sqrt((-1+x)/(1+x))*(1+x))
      ISetDelayed(Coth(ArcCosh(x_)),
          Times(x,
              Power(Times(Sqrt(Times(Plus(CN1, x), Power(Plus(C1, x), CN1))), Plus(C1, x)), CN1))),
      // Coth(ArcTanh(x_)):=1/x
      ISetDelayed(Coth(ArcTanh(x_)), Power(x, CN1)),
      // Coth(ArcCoth(x_)):=x
      ISetDelayed(Coth(ArcCoth(x_)), x),
      // Coth(ArcSech(x_)):=1/(Sqrt((1-x)/(1+x))*(1+x))
      ISetDelayed(Coth(ArcSech(x_)),
          Power(Times(Sqrt(Times(Subtract(C1, x), Power(Plus(C1, x), CN1))), Plus(C1, x)), CN1)),
      // Coth(ArcCsch(x_)):=Sqrt(1+1/x^2)*x
      ISetDelayed(Coth(ArcCsch(x_)), Times(Sqrt(Plus(C1, Power(x, CN2))), x)),
      // Coth(Log(x_)):=(1+x^2)/(-1+x^2)
      ISetDelayed(Coth(Log(x_)), Times(Power(Plus(CN1, Sqr(x)), CN1), Plus(C1, Sqr(x)))),
      // Coth(Infinity)=1
      ISet(Coth(oo), C1, true),
      // Coth(ComplexInfinity)=Indeterminate
      ISet(Coth(CComplexInfinity), Indeterminate, true));
}
