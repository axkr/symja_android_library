...
public class SincRules {
  ...
  final public static IAST RULES = List(
    ...
    ISet(Sinc(CPiQuarter),
      Times(C2,CSqrt2,Power(Pi,CN1)), true),
    // Sinc(Pi/3)=3/(2*Pi)*Sqrt(3)
    ISet(Sinc(CPiThird),
      Times(QQ(3L,2L),CSqrt3,Power(Pi,CN1)), true),
    // Sinc(Pi/2)=2/Pi
    ISet(Sinc(CPiHalf),
    ...
    ISet(Sinc(Times(QQ(1L,12L),Pi)),
      Times(C3,CSqrt2,Plus(CN1,CSqrt3),Power(Pi,CN1)), true),
    // Sinc(Pi/10)=(5*(-1+Sqrt(5)))/(2*Pi)
    ISet(Sinc(Times(QQ(1L,10L),Pi)),
      Times(QQ(5L,2L),Plus(CN1,CSqrt5),Power(Pi,CN1)), true),
    // Sinc(2/5*Pi)=(5*Sqrt(5/8+Sqrt(5)/8))/(2*Pi)
    ISet(Sinc(Times(QQ(2L,5L),Pi)),
      Times(QQ(5L,2L),Sqrt(Plus(QQ(5L,8L),Times(QQ(1L,8L),CSqrt5))),Power(Pi,CN1)), true),
    // Sinc(3/10*Pi)=(5*(1+Sqrt(5)))/(6*Pi)
    ISet(Sinc(Times(QQ(3L,10L),Pi)),
      Times(QQ(5L,6L),Plus(C1,CSqrt5),Power(Pi,CN1)), true),
    // Sinc(I)=Sinh(1)
    ISet(Sinc(CI),
      Sinh(C1), true),
    ...
    ISetDelayed(Sinc(ArcCos(x_)),
      Times(Sqrt(Subtract(C1,Sqr(x))),Power(ArcCos(x),CN1))),
