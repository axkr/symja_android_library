package org.matheclipse.core.reflection.system.rules;

import static org.matheclipse.core.expression.F.*;
import org.matheclipse.core.interfaces.IAST;

/**
 * Generated by <code>org.matheclipse.core.preprocessor.RulePreprocessor</code>.<br />
 * See GIT repository at: <a href="https://bitbucket.org/axelclk/symjaunittests">https://bitbucket.org/axelclk/symjaunittests</a>.
 */
public interface ArcCotRules {
  final public static IAST RULES = List(
    ISet(ArcCot(C0),
      Times(C1D2,Pi)),
    ISet(ArcCot(Plus(C2,Times(CN1,Sqrt(C3)))),
      Times(QQ(5L,12L),Pi)),
    ISet(ArcCot(Plus(CN1,Sqrt(C2))),
      Times(QQ(3L,8L),Pi)),
    ISet(ArcCot(Power(C3,CN1D2)),
      Times(C1D3,Pi)),
    ISet(ArcCot(C1),
      Times(C1D4,Pi)),
    ISet(ArcCot(Sqrt(Plus(C1,Times(C2,Power(C5,CN1D2))))),
      Times(QQ(1L,5L),Pi)),
    ISet(ArcCot(Sqrt(C3)),
      Times(QQ(1L,6L),Pi)),
    ISet(ArcCot(Plus(C1,Sqrt(C2))),
      Times(QQ(1L,8L),Pi)),
    ISet(ArcCot(Sqrt(Plus(C5,Times(C2,Sqrt(C5))))),
      Times(QQ(1L,10L),Pi)),
    ISet(ArcCot(Plus(C2,Sqrt(C3))),
      Times(QQ(1L,12L),Pi)),
    ISet(ArcCot(CI),
      Times(CNI,CInfinity)),
    ISet(ArcCot(CInfinity),
      C0),
    ISet(ArcCot(CNInfinity),
      C0),
    ISet(ArcCot(Times(CI,CInfinity)),
      C0),
    ISet(ArcCot(Times(CNI,CInfinity)),
      C0),
    ISet(ArcCot(CComplexInfinity),
      C0)
  );
}