 ...
 *
 * <p>The main problem with this implementation is that we need to know to nature of test numbers.
 * With doTDivFirst=false it is the fastest algorithm for hard semiprimes, but if the test numbers
 * contain factors < cbrt(N), it will fail to find some of them. With doTDivFirst=true it will find
 * all factors of any kind of numbers, but not extraordinarily fast.
 *
 * <p>Hart_TDiv_Race fixes that problem and is faster for test numbers that contain factors <
 * cbrt(N).
 *
...
public class Hart_Fast extends FactorAlgorithm {
  ...
  public long findSingleFactor(long N) {
    ...
    try {
      ...
      // LOG.error("Hart_Fast: Failed to factor N=" + N + ". Either it has factors < cbrt(N) needing
      // trial division, or the arrays are too small.");
      return 1;
    }
  }
