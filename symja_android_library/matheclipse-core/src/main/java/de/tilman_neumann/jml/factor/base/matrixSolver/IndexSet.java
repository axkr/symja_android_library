 ...
 * 
 * The xor() operation has complexity O(n), where n is the sum of the size of the two argument arrays. Since here 64
 * xor's are done at once, this implementation is very fast compared to other algorithms with the same complexity.
 * 
...
public class IndexSet {
	...
	/**
	 * the bit array: bitArray[0] has bits 0..63, bitArray[1] bits 64..127 and so on.
	 */
	...
	public void add(int x) {
		...
		if (x > biggestEntry)
			biggestEntry = x; // update biggest entry
	}
	...
	public boolean contains(Object o) {
		...
		if (x >= numberOfBits)
			return false; // there is no entry as big as x
		int longIndex = x >> 6; // floor(x/64)
	...
	public boolean equals(Object o) {
		...
		if (this.numberOfBits != other.numberOfBits)
			return false;
		if (this.biggestEntry != other.biggestEntry)
			return false;
		for (int i = 0; i < numberOfLongs; i++) {
