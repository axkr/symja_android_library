...
public class HenselMultUtil {
  ...
  /**
   * Modular diophantine equation solution and lifting algorithm. Let p = A_i.ring.coFac.modul() and
   * assume ggt(A,B) == 1 mod p.
   *
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> List<GenPolynomial<MOD>> liftDiophant(
      GenPolynomial<MOD> A, GenPolynomial<MOD> B, GenPolynomial<MOD> C, List<MOD> V, long d, long k)
    ...
    GenPolynomialRing<GenPolynomial<MOD>> qrfac =
        new GenPolynomialRing<GenPolynomial<MOD>>(ckfac, 1, mn);
    ...
    for (int e = 1; e <= d; e++) {
      ...
      UnivPowerSeriesRing<GenPolynomial<MOD>> psfac =
          new UnivPowerSeriesRing<GenPolynomial<MOD>>(qrfac);
  ...
  /**
   * Modular Hensel lifting algorithm on coefficients test. Let p = f_i.ring.coFac.modul() and
   * assume C == prod_{0,...,n-1} f_i mod p with gcd(f_i,f_j) == 1 mod p for i != j
   *
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> boolean isHenselLift(
      GenPolynomial<BigInteger> C,
      GenPolynomial<MOD> Cp,
      List<GenPolynomial<MOD>> F,
      List<GenPolynomial<MOD>> L) {
    boolean t = true;
  ...
  /**
   * Modular Hensel lifting algorithm, monic case. Let p = A_i.ring.coFac.modul() and assume
   * ggt(a,b) == 1 mod p, for a, b in A.
   *
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> List<GenPolynomial<MOD>> liftHenselMonic(
    ...
    GenPolynomialRing<BigInteger> i1fac =
        new GenPolynomialRing<BigInteger>(new BigInteger(), pk1fac);
    ...
    while (Pfac.size() > 0) { // loop through stack of polynomial rings
      ...
      GenPolynomialRing<BigInteger> ifac =
          new GenPolynomialRing<BigInteger>(new BigInteger(), pkfac);
      // System.out.println("ifac = " + ifac.toScript());
      ...
      for (int e = 1; e <= deg && !Ep.isZERO(); e++) {
        ...
        // System.out.println("psfac = " + psfac);
        TaylorFunction<GenPolynomial<MOD>> T =
            new PolynomialTaylorFunction<GenPolynomial<MOD>>(Epr);
  ...
  /**
   * Modular Hensel lifting algorithm. Let p = A_i.ring.coFac.modul() and assume ggt(a,b) == 1 mod
   * p, for a, b in A.
   *
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> List<GenPolynomial<MOD>> liftHensel(
    ...
    while (Pfac.size() > 0) { // loop through stack of polynomial rings
      ...
      logger.info(
          "U with leading coefficient replaced = " + U); // + ", U.ring = " + U.get(0).ring);
      ...
      if (false && !Cp.leadingExpVector().equals(Uf.leadingExpVector())) { // not meanigfull test
        ...
        // System.out.println("Cp.ring = " + Cp.ring.toScript() + ", Uf.ring = " +
        // Uf.ring.toScript() + "\n");
        System.out.println("");
    ...
    if (false && debug) {
      ...
      GreatestCommonDivisorAbstract<BigInteger> ufd =
          GCDFactory.getImplementation(new BigInteger());
  ...
  /**
   * Modular Hensel full lifting algorithm. Let p = A_i.ring.coFac.modul() and assume ggt(a,b) == 1
   * mod p, for a, b in A.
   *
   * @param C GenPolynomial with integer coefficients
  ...
  public static <MOD extends GcdRingElem<MOD> & Modular> List<GenPolynomial<MOD>> liftHenselFull(
    ...
    // lift F to Z_{p^k}[x]
    // System.out.println("Ci = " + Ci + ", F = " + F + ", k = " + k + ", p = " + F.get(0).ring + ",
    // gi0 = " + gi0);
    ...
    // lift U to Z_{p^k}[x,...]
    // System.out.println("C = " + C + ", U1 = " + U1 + ", V = " + V + ", k = " + k + ", q = " +
    // U1.get(0).ring + ", G = " + G);
