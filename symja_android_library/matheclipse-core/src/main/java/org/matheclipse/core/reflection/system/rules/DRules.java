...
public class DRules {
  ...
  final public static IAST RULES = List(IInit(D, SIZES),
      ...
      ISetDelayed(D(Erf(g_, f_), PatternTest(x_, NotListQ)),
          Plus(Times(C2, Power(Times(Exp(Sqr(f)), CSqrtPi), CN1), D(f, x)),
              Times(CN2, Power(Times(Exp(Sqr(g)), CSqrtPi), CN1), D(g, x)))),
      ...
      ISetDelayed(D(InverseErf(g_, f_), PatternTest(x_, NotListQ)),
          Plus(Times(C1D2, Exp(Sqr(InverseErf(g, f))), CSqrtPi, D(f, x)),
              Times(Power(Exp(Subtract(Sqr(g), Sqr(InverseErf(g, f)))), CN1), D(g, x)))),
      ...
      ISetDelayed(D(PolyGamma(C0, x_), list(x_, n_)), Condition(PolyGamma(n, x), Or(And(IntegerQ(n),
          GreaterEqual(n, C0)), SymbolQ(n)))),
      ...
      ISetDelayed(D(Hypergeometric0F1(a_, f_), PatternTest(x_, NotListQ)), Condition(Times(Power(a,
          CN1), D(f, x), Hypergeometric0F1(Plus(C1, a), f)), FreeQ(a, x))),
      ...
      ISetDelayed(D(Hypergeometric1F1(a_, b_, f_), PatternTest(x_, NotListQ)),
          Condition(Times(
              a, Power(b, CN1), D(f, x), Hypergeometric1F1(Plus(C1, a), Plus(C1, b), f)),
              FreeQ(list(a, b), x))),
      ...
      ISetDelayed(D(Hypergeometric2F1Regularized(a_, b_, c_, f_), PatternTest(x_, NotListQ)),
          Condition(
              Times(
                  a, b, Hypergeometric2F1Regularized(Plus(C1, a), Plus(C1, b), Plus(C1, c), f),
                  D(f, x)),
              FreeQ(list(a, b, c), x))),
      ...
      ISetDelayed(D(Times(Exp(y_), Power(x_, m_)), list(x_, n_)),
          Condition(
              Times(Exp(x), Power(x, Subtract(m, n)), Binomial(m, n), Factorial(n),
                  Hypergeometric1F1(Negate(n), Plus(C1, m, Negate(n)), Negate(x))),
              And(FreeQ(list(m, n), x), UnsameQ(Negative(n), True), Equal(y, x)))),
      // D(E^y_*x_^m_,{x_,n_}):=(x^(m-n)*Binomial(m,n)*n!*Hypergeometric1F1(-n,1+m-n,x))/E^x/;FreeQ({m,n},x)&&Negative(n)=!=True&&-y==x
      ISetDelayed(D(Times(Exp(y_), Power(x_, m_)), list(x_, n_)),
          Condition(
              Times(Power(Exp(x), CN1), Power(x, Subtract(m, n)), Binomial(m, n), Factorial(n),
                  Hypergeometric1F1(Negate(n), Plus(C1, m, Negate(n)), x)),
              And(FreeQ(list(m, n), x), UnsameQ(Negative(n), True), Equal(Negate(y), x)))),
      ...
      ISetDelayed(D(ArcCsc(x_), list(x_, C2)),
          Times(Power(Times(Sqrt(Subtract(C1, Power(x, CN2))), Power(x, C3), Plus(CN1, Sqr(x))),
              CN1), Plus(CN1, Times(C2, Sqr(x))))),
      ...
      ISetDelayed(D(Cos(x_), list(x_, n_)),
          Condition(Cos(Plus(Times(C1D2, n, Pi), x)),
      ...
      ISetDelayed(D(Power(x_, a_), list(x_, n_)),
          Condition(
              If(IntegerQ(n), Times(Pochhammer(Plus(a, Negate(n), C1), n),
                  Power(x, Subtract(a, n))), Times(FactorialPower(a, n), Power(x, Subtract(a, n)))),
              And(Or(And(IntegerQ(n), GreaterEqual(n, C0)), SymbolQ(n)), FreeQ(a, x)))),
      ...
      ISetDelayed(D(ArcCos(x_), list(x_, $p(n, Integer))),
          Condition(
              Plus(Times(KroneckerDelta(n), ArcCos(x)),
                  Times(CN1, Power(-1, Plus(CN1, n)), Power(Power(Subtract(C1, Sqr(x)),
      ...
      ISetDelayed(D(ArcTan(x_), list(x_, $p(n, Integer))),
          Condition(
              Plus(Times(KroneckerDelta(n), ArcTan(x)), Sum(
                  Times(Power(-1, k),
                      Power(
      ...
      ISetDelayed(D(Cot(x_), list(x_, n_)),
          Condition(
              Plus(
                  ...
                  Times(CN1, n,
                      Sum(Times(Power(-1, j), Power(Plus(k, C1), CN1), Binomial(Plus(CN1, n), k),
              ...
              Plus(Times(Tan(x), KroneckerDelta(n)),
                  Times(Sqr(Sec(
                      x)), KroneckerDelta(
                          Plus(CN1, n))),
                  Times(n,
                      Sum(Times(Power(-1, k), Power(Plus(k, C1), CN1), Binomial(Plus(CN1, n), k),
      ...
      ISetDelayed(D(Log(x_), list(x_, $p(n, Integer))), Condition(Times(Power(-1, Plus(CN1, n)),
      ...
      ISetDelayed(D(HarmonicNumber(x_), list(x_, $p(n, Integer))),
          Condition(
              Plus(Times(Power(-1, n), Power(x, Subtract(CN1, n)), Factorial(n)),
                  Times(EulerGamma, KroneckerDelta(n)), PolyGamma(n, x)),
              Or(And(IntegerQ(n), GreaterEqual(n, C1)), FreeQ(n, PatternTest($b(), NumberQ))))),
      ...
      ISetDelayed(D(PolyLog(f_, g_, h_), PatternTest(x_, NotListQ)),
          Plus(Times(Power(h, CN1), D(h, x), PolyLog(Plus(CN1, f), g, h)),
              Times(D(g, x), $($(Derivative(C0, C1, C0), PolyLog), f, g, h)),
              Times(D(f, x), $($(Derivative(C1, C0, C0), PolyLog), f, g, h)))),
      ...
      ISetDelayed(D(StruveH(f_, g_), PatternTest(x_, NotListQ)),
          Plus(
              Times(C1D2,
                  Plus(
                      Times(Power(g, f),
                          Power(Times(Power(C2, f), CSqrtPi, Gamma(Plus(QQ(3L, 2L), f))), CN1)),
      ...
      ISetDelayed(D(AppellF1(a_, b_, c_, d_, f_, g_), PatternTest(x_, NotListQ)),
          Condition(
              Plus(
                  Times(a, b, Power(d, CN1),
                      AppellF1(Plus(C1, a), Plus(C1, b), c, Plus(C1, d), f, g), D(f, x)),
                  Times(a, c, Power(d, CN1),
                      AppellF1(Plus(C1, a), b, Plus(C1, c), Plus(C1, d), f, g), D(g, x))),
      ...
      ISetDelayed(D(InverseLaplaceTransform(f_, g_, h_), PatternTest(x_, NotListQ)),
          Condition(Plus(Times(DiracDelta(h), D(f, x)),
              Times(f, $($(Derivative(C1), DiracDelta), h), D(h, x))), FreeQ(list(f), g))),
      ...
      ISetDelayed(D(LaplaceTransform(f_, g_, h_), PatternTest(x_, NotListQ)),
          Condition(Plus(Times(Power(h, CN1), D(f, x)), Times(CN1, f, Power(h, CN2), D(h, x))),
              FreeQ(list(f), g))),
      ...
      ISetDelayed(D(ZTransform(f_, g_, h_), PatternTest(x_, NotListQ)),
          Condition(Plus(Times(Power(Plus(CN1, h), CN1), h, D(f, x)),
              Times(f, Power(Plus(CN1, h), CN1), D(h, x)),
              Times(CN1, f, Power(Plus(CN1, h), CN2), h, D(h, x))), FreeQ(list(f), g))));
}
