...
public class GreatestCommonDivisorSubres<C extends GcdRingElem<C>>
   ...
   * @see edu.jas.poly.GenPolynomial#remainder(edu.jas.poly.GenPolynomial).
   * @deprecated Use {@link
   *     edu.jas.poly.PolyUtil#recursiveDensePseudoRemainder(edu.jas.poly.GenPolynomial,edu.jas.poly.GenPolynomial)}
   *     instead
   */
  @Deprecated
  public GenPolynomial<GenPolynomial<C>> recursivePseudoRemainder(
  ...
  public GenPolynomial<GenPolynomial<C>> recursiveUnivariateGcd(
    ...
    while (!r.isZERO()) {
      ...
      if (!x.isZERO()) {
        ...
        z = g.power(delta); // power(P.ring.coFac, g, delta);
        h =
            PolyUtil.<C>basePseudoDivide(
                z, h.power(delta - 1)); // power(P.ring.coFac, h, delta - 1)
      } else {
        r = x;
      }
    }
  ...
  public GenPolynomial<C> baseResultant(GenPolynomial<C> P, GenPolynomial<C> S) {
    ...
    z =
        r.leadingBaseCoefficient()
            .power(q.degree(0)); // power(cofac, r.leadingBaseCoefficient(), q.degree(0));
  ...
  public GenPolynomial<GenPolynomial<C>> recursiveUnivariateResultant(
    ...
    h =
        PolyUtil.<C>basePseudoDivide(
            z, h.power(q.degree() - 1)); // power(cofac, h, q.degree(0) - 1));
    z = h.multiply(t);
    if (s < 0) {
      z = z.negate();
    }
  ...
  /**
   * Univariate GenPolynomial recursive Subresultant list. Uses pseudoRemainder for remainder.
