...
public class RulePreprocessor {
  ...
  /**
   * The body of the generated <code>AutomaticRules</code> class which precedes the generated
   * <code>defer()</code> calls. The rules of a symbol are installed on the first access to them, so
   * that a session only pays for the rules it really uses.
   */
  static final String AUTOMATIC_RULES_PROLOG = //
  ...
  /**
   * Determine the symbols a rule file defines rules or own values for.
   *
   * <p>
   * Most rule files only define the symbol named after the file, but <code>PowerRules.m</code> also
   * defines rules for <code>Exp</code> and <code>RGBColorRules.m</code> defines the own values of
   * all named colors. The generated <code>*Rules</code> class has to be registered at every one of
   * them, otherwise the rules of a symbol would never be installed.
   *
   * @param rules the parsed content of a rule (*.m) file
   * @param symbolName the symbol named after the rule file, always part of the result
   * @return the names of the defined symbols, in a stable order
   */
  static java.util.Set<String> ruleTargetSymbols(IExpr rules, String symbolName) {
    java.util.Set<String> symbols = new java.util.TreeSet<String>();
    symbols.add(symbolName);
    if (rules != null && rules.isList()) {
      IAST list = (IAST) rules;
      for (int i = 1; i < list.size(); i++) {
        IExpr rule = list.get(i);
        if (rule.isAST(S.Set, 3) || rule.isAST(S.SetDelayed, 3)) {
          IExpr leftHandSide = rule.first();
          IExpr head = leftHandSide.isAST() ? leftHandSide.head() : leftHandSide;
          if (head.isSymbol()) {
            symbols.add(head.toString());
          }
        }
      }
    }
    ...
    if (sourceLocation.exists()) {
      ...
      if (!createMatcher) {
        try {
          ...
          automaticRules.println(AUTOMATIC_RULES_PROLOG);
        } catch (IOException e) {
      ...
      if (files != null) {
        // sort, so that the generated sources do not depend on the order in which the file system
        // returns the rule files
              ...
              try {
                ...
                if (!createMatcher) {
                  if (!isSpecialRuleList(className)) {
                    for (String target : ruleTargetSymbols(expr, symbolName)) {
                      automaticRules.println("    defer(S." + target + ", \"" + className + "\");");
                    }
                  }
                }
  ...
  private static boolean isSpecialRuleList(String className) {
    return className.equals("IntegratePowerTimesFunctionRules")
        || className.equals("FunctionExpandRules") || className.equals("PodDefaultsRules");
  }
