        ...
        if (data.elements) {
            data.elements.forEach(function(el) {
				...
				if (el.type === 'Polygon') {
				    ...
				    if (el.vertexColors) geom.setAttribute('color', new THREE.Float32BufferAttribute(el.vertexColors, 3));
				    
				    if (el.indices) {
				        geom.setIndex(el.indices);
				    }
				    
				    geom.computeVertexNormals();
				    mesh = new THREE.Mesh(geom, getMat(el, false));

				    if (el.showMesh) {
				        var edgeGeom = new THREE.EdgesGeometry(geom, 2); 
				        var edgeMat = new THREE.LineBasicMaterial({
				            color: 0x111111, linewidth: 1, transparent: true, opacity: 0.35
				        });
