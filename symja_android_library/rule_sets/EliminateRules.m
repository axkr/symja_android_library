...
elimzero(x_+Sqrt(a_.+b_.*x_+c_.*x_^2)+z_., x_) := If(SameQ(c,1)||SameQ(c,1.0), (-a+z^2)/(b-2*z), {(-b+2*z-Sqrt(4*a+b^2-4*a*c-4*b*z+4*c*z^2))/(2*(-1 + c)), (-b + 2*z + Sqrt(4*a+b^2-4*a*c-4*b*z+4*c*z^2))/(2*(-1+c))})
  /; FreeQ({a,b,c,z},x)
} 
}