...
public class FieldVector3D<T extends RealFieldElement<T>> implements Serializable {
     ...
     */
    public T getNorm1() {
        return x.abs().sum(y.abs()).sum(z.abs());
    }
    ...
    public T getNorm() {
        ...
        return x.multiply(x).sum(y.multiply(y)).sum(z.multiply(z)).sqrt();
    }
    ...
    public T getNormSq() {
        // there are no cancellation problems here, so we use the straightforward formula
        return x.multiply(x).sum(y.multiply(y)).sum(z.multiply(z));
    }
    ...
    public FieldVector3D<T> add(final FieldVector3D<T> v) {
        return new FieldVector3D<T>(x.sum(v.x), y.sum(v.y), z.sum(v.z));
    }
    ...
    public FieldVector3D<T> add(final T factor, final Vector3D v) {
        return new FieldVector3D<T>(x.sum(factor.multiply(v.getX())),
                                    y.sum(factor.multiply(v.getY())),
                                    z.sum(factor.multiply(v.getZ())));
    }
    ...
    public FieldVector3D<T> orthogonal() throws MathArithmeticException {
        ...
        if (FastMath.abs(x.getReal()) <= threshold) {
            final T inverse  = y.multiply(y).sum(z.multiply(z)).sqrt().reciprocal();
            ...
            final T inverse  = x.multiply(x).sum(z.multiply(z)).sqrt().reciprocal();
            return new FieldVector3D<T>(inverse.multiply(z).negate(), inverse.getField().getZero(), inverse.multiply(x));
        } else {
            final T inverse  = x.multiply(x).sum(y.multiply(y)).sqrt().reciprocal();
    ...
    public T distance(final FieldVector3D<T> v) {
        ...
        return dx.multiply(dx).sum(dy.multiply(dy)).sum(dz.multiply(dz)).sqrt();
    }
    ...
    public T distanceSq(final FieldVector3D<T> v) {
        ...
        final T dy = v.y.subtract(y);
        final T dz = v.z.subtract(z);
        return dx.multiply(dx).sum(dy.multiply(dy)).sum(dz.multiply(dz));
    }
