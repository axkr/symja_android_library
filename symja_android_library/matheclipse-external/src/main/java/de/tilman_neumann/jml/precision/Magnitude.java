...
public class Magnitude {
   ...
   *
   * <p>The result is not exact for two reasons: First: We try to convert the bit length, but this
   * is not enough information. Second: The used multiplier has double precision; so if a number has
   * a bit length with more than 15 digits (which is quite a big number), then the result will be
   * erroneous due to lack of precision.
   *
   * @param x Argument
