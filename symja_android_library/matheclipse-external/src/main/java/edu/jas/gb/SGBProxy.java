...
public class SGBProxy<C extends GcdRingElem<C>> extends SolvableGroebnerBaseAbstract<C> {
   ...
   */
  @Override
  public List<GenSolvablePolynomial<C>> leftGB(
      final int modv, final List<GenSolvablePolynomial<C>> F) {
    if (F == null || F.isEmpty()) {
      return F;
    }
    // parallel case
    List<GenSolvablePolynomial<C>> G = null;
    List<Callable<List<GenSolvablePolynomial<C>>>> cs =
        new ArrayList<Callable<List<GenSolvablePolynomial<C>>>>(2);
    cs.add(
        new Callable<List<GenSolvablePolynomial<C>>>() {
   ...
   */
  @Override
  public List<GenSolvablePolynomial<C>> rightGB(
      final int modv, final List<GenSolvablePolynomial<C>> F) {
    if (F == null || F.isEmpty()) {
      return F;
    }
   ...
   */
  @Override
  public List<GenSolvablePolynomial<C>> twosidedGB(
      final int modv, final List<GenSolvablePolynomial<C>> F) {
    if (F == null || F.isEmpty()) {
      return F;
    }
