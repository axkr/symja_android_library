...
public interface IAST extends IExpr, Iterable<IExpr>, ITensorAccess, AnyMatrix {
  ...
  /**
   * Iterate all elements from index <code>start</code> inclusive to <code>end</code> exclusive and
   * call the method {@link ObjIntConsumer#accept(Object, int)} for these elements.
   * 
   * @param start start index (inclusive)
   * @param end end index (exclusive)
