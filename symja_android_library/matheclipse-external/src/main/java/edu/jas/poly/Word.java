...
public final class Word implements MonoidElem<Word> {
  ...
  public Word divideLeft(Word V) {
    ...
    if (!ret[1].isONE()) {
      throw new IllegalArgumentException(
          "not simple left dividable: left = "
              + ret[0]
              + ", right = "
              + ret[1]
              + ", use divideWord");
  ...
  public Word divideRight(Word V) {
    ...
    if (!ret[0].isONE()) {
      throw new IllegalArgumentException(
          "not simple right dividable: left = "
              + ret[0]
              + ", right = "
              + ret[1]
              + ", use divideWord");
   ...
   *
   * @param V other word.
   * @return list of overlaps [l1,r1,l2,r2] with l1 * this * r1 = l2 * V * r2. If no such overlaps
   *     exist the empty overlap list is returned.
   */
   ...
   *
   * @param V other word.
   * @return w = l1*this*r1, with l1*this*r1 == l2*V*r2, if l1, r1, l2, r2 exist, else null is
   *     returned.
   */
