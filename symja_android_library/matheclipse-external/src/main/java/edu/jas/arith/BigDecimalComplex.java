...
/**
 * BigComplex class based on BigDecimal implementing the RingElem respectively the StarRingElem
...
public final class BigDecimalComplex
  ...
  /**
   * The constructor creates a BigDecimalComplex object from two BigDecimal objects real and
   * imaginary part.
   *
   * @param r real part.
  ...
  /**
   * The constructor creates a BigDecimalComplex object from a BigDecimal object as real part, the
  ...
  /**
   * The constructor creates a BigDecimalComplex object from a long element as real part, the
   * imaginary part is set to 0.
   *
   * @param r real part.
   */
   ...
   *
   * @return 0 if this is equal to b; 1 if re &gt; b.re, or re == b.re and im &gt; b.im; -1 if re
   *     &lt; b.re, or re == b.re and im &lt; b.im
   */
  @Override
   ...
   * Since complex numbers are unordered, we use lexicographical order of re and im.
   *
   * @return 0 if this is equal to 0; 1 if re &gt; 0, or re == 0 and im &gt; 0; -1 if re &lt; 0, or
   *     re == 0 and im &lt; 0
   ...
   *
   * @param A is a complex number.
   * @return the absolute value of A, a rational number. Note: The square root is not jet
   *     implemented.
   */
  ...
  public BigDecimalComplex multiply(BigDecimalComplex B) {
    return new BigDecimalComplex(
        re.multiply(B.re).subtract(im.multiply(B.im)), re.multiply(B.im).sum(im.multiply(B.re)));
  }
  ...
  /**
   * Complex number, random. Random rational numbers A and B are generated using random(n). Then R
   * is the complex number with real part A and imaginary part B.
   *
  ...
  /**
   * Returns the number of bits in the representation of this BigDecimalComplex, including a sign
   * bit. It is equivalent to {@code re.bitLength() + im.bitLength()}.)
   *
   * @return number of bits in the representation of this BigDecimalComplex, including a sign bit.
   */
