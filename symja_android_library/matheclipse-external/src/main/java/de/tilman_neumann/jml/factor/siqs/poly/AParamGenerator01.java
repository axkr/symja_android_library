...
/**
 * Generator for the a-parameter (or "hypercube"), which is the leading coefficient of the quadratic
 * polynomial Q(x) = (d*a*x+b)^2 - kN used by SIQS. d is typically 1 or 2.</br></br>
 *
 * <p>The a-parameter in SIQS is chosen as a product of primes from the prime base: <code>
 * a = q1 * ... * q_s</code>. Its value should be roughly a ~ sqrt(2*k*N)/(d*M), where M is the
 * sieve array size, such that |Q(x)| is about the same at x=+M and x=-M, and |Q(x)| <= kN for all
 * x.
 *
 * <p>One consequence of this choice is that about 70% of the Q we get are negative. But trying to
 * correct that means bigger values for positive Q and slows down the QS.
 *
 * <p>As suggested by [Carrier/Wagstaff], it is important to avoid q_l that divide k. Otherwise for
 * many N we would require a big number of solver runs.
 *
 * <p>This class does not implement the standard-procedure due to [Carrier/Wagstaff] for two
 * reasons: 1. I wanted to test which size of the q_l is actually the best. It turned out that the
 * q_l should be rather large and qCount rather small. 2. Splitting the prime base and {q_l} into
 * three sets feels like introducing some unnecessary complexity to the code.
 *
 * <p>The algorithm used here is mostly random, choosing the q_l from a range of about the wanted
 * size. The last q_l is chosen deterministically such that the best a-parameter is matched as close
 * as possible. Therefore the algorithm is not stable for N < 50 bit, but faster for bigger N. The
 * expected best a-value is approximated quite accurately.
 *
 * <p>A very important parameter is <code>qCount
 * </code. Having it configurable via the constructor lets us test
 * which number of factors of <code>
 * a</code> and thus which average factor size is most appropriate for given magnitudes of kN. A
 * minimum value of qCount==4 gives best stability at small N, down to 53 bits.
 *
 * <p>Since 2016-12-15, entries of qArray and qIndexArray are sorted bottom-up. Since 2018-02-13,
 * the d-parameter was introduced.
 *
...
public class AParamGenerator01 implements AParamGenerator {
   ...
   * Full constructor.
   *
   * @param wanted_qCount the wanted number of factors of the a-parameter; null for automatic
   *     selection
   */
  ...
  public void initialize(
    ...
    // compute index of average "best" q, in the sense of realizing the a-parameter with qCount
    // prime base values
    int best_q_index = getBestIndex(best_q);

    // Compute centre and variance for index generator:
    // The variance multiplier plays an important role for the stability of the quadratic sieve at
    // small N.
  ...
  /**
   * Estimate best number of q whose product builds the a-parameter. These are experimental values
   * derived from tests up to 320 bit.
   *
  ...
  /**
   * Given qCount and best_a, compute qIndexArray, qArray and <code>a</code>. The computation should
   * be pretty safe to avoid that QS becomes unstable for bad choices of qCount.
   */
  private void computeAParameter() {
    ...
    for (int i = 0; i < qCount - 1; i++) {
      ...
      int randomOffset =
          rng.nextInt(indexVariance << 1)
              - indexVariance; // randomOffset = -indexVariance..+indexVariance -> centered at 0
    ...
    // LOG.debug("kN=" + kN + " -> best_a = " + best_a + ", qCount = " + qCount + " -> qArray = " +
    // Arrays.toString(qArray) + ", a = " + a);
  }
      ...
      if (DEBUG)
        LOG.warn(
            "New a-parameter #"
                ...
                + ": a="
                + a
                + " has already been used! #(duplicate a in a row) = "
                + duplicateACount);
    }
    if (DEBUG) {
      ...
      // Result: The q's are typically quite disjunct. At 200 bit we get something like [0, 0, 0, 0,
      // 1, 4, 15, 78, 87].
    }
