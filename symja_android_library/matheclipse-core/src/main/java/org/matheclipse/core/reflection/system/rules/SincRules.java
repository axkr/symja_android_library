...
public class SincRules {
  ...
  final public static IAST RULES = List(IInit(Sinc, SIZES),
      ...
      ISet(Sinc(Undefined), Undefined, true),
      // Sinc(0)=1
      ISet(Sinc(C0), C1, true),
      // Sinc(Pi/6)=Pi/3
      ISet(Sinc(Times(QQ(1L, 6L), Pi)), CPiThird, true),
      // Sinc(Pi/4)=2/Pi*Sqrt(2)
      ISet(Sinc(CPiQuarter), Times(C2, CSqrt2, Power(Pi, CN1)), true),
      // Sinc(Pi/3)=3/(2*Pi)*Sqrt(3)
      ISet(Sinc(CPiThird), Times(QQ(3L, 2L), CSqrt3, Power(Pi, CN1)), true),
      // Sinc(Pi/2)=2/Pi
      ISet(Sinc(CPiHalf), Times(C2, Power(Pi, CN1)), true),
      // Sinc(Pi)=0
      ISet(Sinc(Pi), C0, true),
      // Sinc(5/12*Pi)=(3*Sqrt(2)*(1+Sqrt(3)))/(5*Pi)
      ISet(Sinc(Times(QQ(5L, 12L), Pi)),
          Times(QQ(3L, 5L), CSqrt2, Plus(C1, CSqrt3), Power(Pi, CN1)), true),
      // Sinc(Pi/5)=(5*Sqrt(5/8-Sqrt(5)/8))/Pi
      ISet(Sinc(Times(QQ(1L, 5L), Pi)),
          Times(C5, Sqrt(Plus(QQ(5L, 8L), Times(QQ(-1L, 8L), CSqrt5))), Power(Pi, CN1)), true),
      ...
      ISet(Sinc(Times(QQ(3L, 10L), Pi)), Times(QQ(5L, 6L), Plus(C1, CSqrt5), Power(Pi, CN1)), true),
      // Sinc(I)=Sinh(1)
      ISet(Sinc(CI), Sinh(C1), true),
      ...
      // Sinc(-Infinity)=0
      ISet(Sinc(Noo), C0, true),
      // Sinc(Infinity)=0
      ISet(Sinc(oo), C0, true),
      // Sinc(I*Infinity)=Infinity
      ISet(Sinc(DirectedInfinity(CI)), oo, true),
      // Sinc(ComplexInfinity)=Indeterminate
      ISet(Sinc(CComplexInfinity), Indeterminate, true));
}
