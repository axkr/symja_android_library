...
public final class PlusOp {
  ...
  public static org.matheclipse.core.interfaces.IExpr numberLikePlus(
    ...
    if (iExpr.isNumber()) {
      ...
      if (that.isQuantity()) {
        return QuantityOps.plus((org.matheclipse.core.interfaces.IAST) iExpr,
            (org.matheclipse.core.interfaces.IAST) that, EvalEngine.get());
      }
      return F.NIL;
    }
  ...
  public IExpr plus(final IExpr arg) {
    ...
    try {
      ...
      if (arg.isNumber()) {
        ...
        IExpr temp = F.NIL;
        if (numberValue.isQuantity()) {
          // the accumulated (leftmost) quantity's unit wins - WMA behavior
          temp = QuantityOps.plus((IAST) numberValue, (IAST) arg, EvalEngine.get());
          if (temp.isNIL()) {
            Errors.printMessage(S.Quantity, "compat",
                F.list(((IAST) numberValue).arg2(), ((IAST) arg).arg2()), EvalEngine.get());
          }
        }
        if (temp.isPresent()) {
          evaled = true;
