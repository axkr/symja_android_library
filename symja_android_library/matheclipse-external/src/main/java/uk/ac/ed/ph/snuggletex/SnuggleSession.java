...
public final class SnuggleSession implements SessionContext {
  ...
  /**
   * Trivial enumeration which specifies the possible actions to perform once we finish writing with
   * the various {@link SnuggleSession#writeWebPage(WebPageOutputOptions, OutputStream)} methods.
   *
   * @since 1.2.2
   */
  ...
  SnuggleSession(final SnuggleSnapshot snapshot) {
    ...
    this.userEnvironmentMap =
        new HashMap<String, UserDefinedEnvironment>(snapshot.userEnvironmentMap);
  ...
  /**
   * Parses the data contained within the given {@link SnuggleInput}, and performs fixing on the
   * resulting tokens.
   *
  ...
  /**
   * Creates a {@link SnuggleSnapshot} Object holding the current state of this session that can be
  ...
  /**
   * Builds a DOM sub-tree based on the currently parsed tokens, appending the results as children
  ...
  /**
   * Builds a DOM sub-tree based on the currently parsed tokens, appending the results as children
   * of the given target root Element. This uses the default {@link DOMOutputOptions} associated
   * with this engine.
   *
   * @return true if completed successfully, false if the process was terminated by an error in the
   *         input LaTeX and if the session was configured to fail on the first error.
  ...
  /**
   * Convenience method to build a DOM {@link NodeList} representing the converted Tokens. These
