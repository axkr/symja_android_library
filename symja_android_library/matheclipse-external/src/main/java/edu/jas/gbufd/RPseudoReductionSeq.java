...
/**
 * Polynomial regular ring pseudo reduction sequential use algorithm. Implements fraction free
...
public class RPseudoReductionSeq<C extends RegularRingElem<C>> extends RReductionSeq<C>
  ...
  /**
   * Normalform with recording. <b>Note:</b> Only meaningfull if all divisions are exact. Compute
   * first the multiplication factor <code>m</code> with <code>normalform(Pp,Ap,m)</code>, then call
   * this method with <code>normalform(row,Pp,m*Ap)</code>.
   *
   ...
   */
  @SuppressWarnings("unchecked")
  public GenPolynomial<GenPolynomial<C>> normalformRecursive(
      List<GenPolynomial<GenPolynomial<C>>> Pp, GenPolynomial<GenPolynomial<C>> Ap) {
