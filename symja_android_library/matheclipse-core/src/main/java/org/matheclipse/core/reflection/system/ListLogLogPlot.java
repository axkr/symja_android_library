...
public class ListLogLogPlot extends ListPlot {
  ...
  public IExpr evaluateECharts(IAST ast, final int argSize, final IExpr[] options,
    ...
    String graphicsPrimitivesStr = listLogLogPlot(ast, options, graphicsOptions, engine);
  ...
  protected static String listLogLogPlot(IAST plot, IExpr[] options,
    ...
    if (arg1.isNonEmptyList()) {
      ...
      if (pointList.isList()) {
        if (pointList.isListOfPoints(2)) {
          return point2DListLogLogPlot(pointList, graphicsOptions);
        }
      ...
      return yValueListLogLogPlot(pointList, graphicsOptions);
    }
    return null;
  }
   ...
   * @return
   */
  private static String point2DListLogLogPlot(IAST pointList2D, GraphicsOptions graphicsOptions) {
  ...
  private static String yValueListLogLogPlot(IAST pointList, GraphicsOptions graphicsOptions) {
