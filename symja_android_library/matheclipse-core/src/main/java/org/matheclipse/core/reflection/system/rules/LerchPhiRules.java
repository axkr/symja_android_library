...
public class LerchPhiRules {
  ...
  final public static IAST RULES = List(IInit(LerchPhi, SIZES),
      // LerchPhi(-1,1,0)=-Log(2)
      ISet(LerchPhi(CN1, C1, C0), Negate(Log(C2)), true),
      // LerchPhi(-1,2,1/2)=4*Catalan
      ISet(LerchPhi(CN1, C2, C1D2), Times(C4, Catalan), true),
      // LerchPhi(0,1,0)=0
      ISet(LerchPhi(C0, C1, C0), C0, true),
      // LerchPhi(1,1,0)=Infinity
      ISet(LerchPhi(C1, C1, C0), oo, true),
      // LerchPhi(1,2,1)=Pi^2/6
      ISet(LerchPhi(C1, C2, C1), Times(QQ(1L, 6L), Sqr(Pi)), true),
      // LerchPhi(2,1,0)=-I*Pi
      ISet(LerchPhi(C2, C1, C0), Times(CNI, Pi), true),
      ...
      ISetDelayed(LerchPhi(z_, s_, $p(i, Integer)),
          Condition(
              Module(list(Set(n, Negate(i))),
                  Times(Power(z, n),
                      Plus(PolyLog(s, z),
                          Sum(Power(Times(Power(z, k), Power(k, s)), CN1), list(k, C1, n))))),
              Less(i, C0))),
      // LerchPhi(z_,s_,n_Integer):=(PolyLog(s,z)-Sum(z^k/k^s,{k,1,-1+n}))/z^n/;n>0
      ISetDelayed(LerchPhi(z_, s_, $p(n, Integer)),
          Condition(
              Times(Power(Power(z, n), CN1),
                  Subtract(PolyLog(s, z),
                      Sum(Times(Power(Power(k, s), CN1), Power(z, k)), list(k, C1, Plus(CN1, n))))),
              Greater(n, C0))));
}
