 ...
 * methods for polynomials over real algebraic numbers from package
 *
 * <pre>
 * edu.jas.root
 * </pre>
...
public class FactorRealAlgebraic<C extends GcdRingElem<C> & Rational>
   ...
   */
  public FactorRealAlgebraic(
      RealAlgebraicRing<C> fac, FactorAbstract<AlgebraicNumber<C>> factorAlgebraic) {
    super(fac);
  ...
  public List<GenPolynomial<RealAlgebraicNumber<C>>> baseFactorsSquarefree(
    ...
    // System.out.println("\nP = " + P);
    GenPolynomialRing<AlgebraicNumber<C>> afac =
        new GenPolynomialRing<AlgebraicNumber<C>>(rfac.algebraic, pfac);
    ...
    for (GenPolynomial<AlgebraicNumber<C>> a : afactors) {
      GenPolynomial<RealAlgebraicNumber<C>> p =
          PolyUtilRoot.<C>realFromAlgebraicCoefficients(pfac, a);
      factors.add(p);
    }
