...
/**
 * Polynomial Regular ring Reduction sequential use algorithm. Implements normalform and boolean
 * closure stuff.
 *
...
public class RReductionSeq<C extends RegularRingElem<C>> extends ReductionAbstract<C>
  ...
  /**
   * Is top reducible. Condition is a b != 0, for a=ldcf(A) and b=ldcf(B) and lt(B) | lt(A) for some
   * B in F.
   *
   * @param A polynomial.
  ...
  /**
   * Is strong top reducible. Condition is idempotent(a) == idempotent(b), for a=ldcf(A) and
   * b=ldcf(B) and lt(B) | lt(A) for some B in F.
   *
   * @param A polynomial.
   * @param P polynomial list.
  ...
  /**
   * GB criterium 4. Use only for commutative polynomial rings. <b>Note:</b> Experimental version
   * for r-Groebner bases.
   *
   * @param A polynomial.
   * @param B polynomial.
   ...
   */
  @SuppressWarnings("unchecked")
  public GenPolynomial<C> normalform(
      List<GenPolynomial<C>> row, List<GenPolynomial<C>> Pp, GenPolynomial<C> Ap) {
    if (Pp == null || Pp.isEmpty()) {
      return Ap;
    }
    if (Ap == null || Ap.isZERO()) {
   ...
   * @return red(bc(A)).
   */
  public List<GenPolynomial<C>> reducedBooleanClosure(
      List<GenPolynomial<C>> F, GenPolynomial<C> A) {
