 ...
 * and DefaultDict are almost the same, except for the fact that {@link DefaultDict#get(IExpr)}
...
public class DefaultDict<T extends IExpr> {
  ...
  /**
   * {@link DefaultDict#get(IExpr)} never returns a <code>null</code> value. It provides a default
   * value for the key that does not exists. When the {@link IASTAppendable} interface is passed as
   * the generic <code>T</code> argument, then a DefaultDict is created with the values that are
   * appendable lists.
   * 
   * @param key
   * @return
