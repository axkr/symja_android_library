...
/**
 * GenSolvablePolynomial generic solvable polynomials implementing RingElem. n-variate ordered
 * solvable polynomials over C. Objects of this class are intended to be immutable. The
 * implementation is based on TreeMap respectively SortedMap from exponents to coefficients by
 * extension of GenPolybomial. Only the coefficients are modeled with generic types, the exponents
 * are fixed to ExpVector with long entries (this will eventually be changed in the future).
 *
...
public class GenSolvablePolynomial<C extends RingElem<C>> extends GenPolynomial<C> {
  ...
  /**
   * GenSolvablePolynomial left and right multiplication. Product with ring element and exponent
   * vector.
   *
   * @param b coefficient.
   * @param e exponent.
   ...
   */
  public GenSolvablePolynomial<C> scaleSubtractMultiple(
      C b, C a, ExpVector e, GenSolvablePolynomial<C> S) {
    if (a == null || S == null) {
      return this.multiplyLeft(b);
    }
    if (a.isZERO() || S.isZERO()) {
   ...
   */
  public GenSolvablePolynomial<C> scaleSubtractMultiple(
      C b, ExpVector g, C a, ExpVector e, GenSolvablePolynomial<C> S) {
    if (a == null || S == null) {
  ...
  /**
   * GenSolvablePolynomial left monic, i.e. leadingCoefficient == 1. If leadingCoefficient is not
   * invertible returns this abs value.
   *
   * @return monic(this).
   */
  @Override
  ...
  /**
   * GenSolvablePolynomial left division. Fails, if exact division by leading base coefficient is
   ...
   * @return quotient with this = quotient * S + remainder and deg(remainder) &lt; deg(S) or
   *     remiander = 0.
   * @see
  ...
  /**
   * GenSolvablePolynomial remainder by left division. Fails, if exact division by leading base
   * coefficient is not possible. Meaningful only for univariate polynomials over fields, but works
   * in any case.
   *
   * @param S nonzero GenSolvablePolynomial with invertible leading coefficient.
   * @return remainder with this = quotient * S + remainder and deg(remainder) &lt; deg(S) or
   *     remiander = 0.
   * @see
   *     edu.jas.poly.PolyUtil#baseSparsePseudoRemainder(edu.jas.poly.GenPolynomial,edu.jas.poly.GenPolynomial)
   */
  ...
  /**
   * GenSolvablePolynomial left division with remainder. Fails, if exact division by leading base
   * coefficient is not possible. Meaningful only for univariate polynomials over fields, but works
   * in any case.
   *
   * @param S nonzero GenSolvablePolynomial with invertible leading coefficient.
  ...
  /**
   * GenSolvablePolynomial right division. Fails, if exact division by leading base coefficient is
   * not possible. Meaningful only for univariate polynomials over fields, but works in any case.
   *
   * @param S nonzero GenSolvablePolynomial with invertible leading coefficient.
   * @return quotient with this = S * quotient + remainder and deg(remainder) &lt; deg(S) or
   *     remiander = 0.
   * @see
   *     edu.jas.poly.PolyUtil#baseSparsePseudoRemainder(edu.jas.poly.GenPolynomial,edu.jas.poly.GenPolynomial)
   */
  ...
  /**
   * GenSolvablePolynomial remainder by right division. Fails, if exact division by leading base
   * coefficient is not possible. Meaningful only for univariate polynomials over fields, but works
   * in any case.
   *
   * @param S nonzero GenSolvablePolynomial with invertible leading coefficient.
   * @return remainder with this = S * quotient + remainder and deg(remainder) &lt; deg(S) or
   *     remiander = 0.
   * @see
   *     edu.jas.poly.PolyUtil#baseSparsePseudoRemainder(edu.jas.poly.GenPolynomial,edu.jas.poly.GenPolynomial)
   */
  ...
  /**
   * GenSolvablePolynomial right division with remainder. Fails, if exact division by leading base
   * coefficient is not possible. Meaningful only for univariate polynomials over fields, but works
   * in any case.
   *
   * @param S nonzero GenSolvablePolynomial with invertible leading coefficient.
   * @return [ quotient , remainder ] with this = S * quotient + remainder and deg(remainder) &lt;
  ...
  /**
   * RecSolvablePolynomial right coefficients from left coefficients. <b>Note:</b> R is represented
   * as a polynomial with left coefficients, the implementation can at the moment not distinguish
   * between left and right coefficients.
   *
   * @return R = sum( X<sup>i</sup> b<sub>i</sub> ), with P = sum(a<sub>i</sub> X<sup>i</sup> ) and
   *     eval(sum(X<sup>i</sup> b<sub>i</sub>)) == sum(a<sub>i</sub> X<sup>i</sup>)
   */
  @SuppressWarnings({"unchecked"})
  ...
  /**
   * Evaluate RecSolvablePolynomial as right coefficients polynomial. <b>Note:</b> R is represented
   * as a polynomial with left coefficients, the implementation can at the moment not distinguish
   * between left and right coefficients.
   *
   * @return this as evaluated polynomial R. R = sum( X<sup>i</sup> b<sub>i</sub> ), this =
   *     sum(a<sub>i</sub> X<sup>i</sup> ) = eval(sum(X<sup>i</sup> b<sub>i</sub>))
   */
  @SuppressWarnings({"unchecked"})
  ...
  /**
   * Test RecSolvablePolynomial right coefficients polynomial. <b>Note:</b> R is represented as a
   * polynomial with left coefficients, the implementation can at the moment not distinguish between
   * left and right coefficients.
   *
   * @param R GenSolvablePolynomial with right coefficients.
   * @return true, if R is polynomial with right coefficients of this. R = sum( X<sup>i</sup>
   *     b<sub>i</sub> ), with this = sum(a<sub>i</sub> X<sup>i</sup> ) and eval(sum(X<sup>i</sup>
