...
/**
 * LocalSolvablePolynomial generic recursive solvable polynomials implementing RingElem. n-variate
 * ordered solvable polynomials over solvable polynomial coefficients. Objects of this class are
 * intended to be immutable. The implementation is based on TreeMap respectively SortedMap from
 * exponents to coefficients by extension of GenPolynomial. Will be deprecated use
...
public class LocalSolvablePolynomial<C extends GcdRingElem<C>>
  ...
  public LocalSolvablePolynomial(
      LocalSolvablePolynomialRing<C> r, SolvableLocal<C> c, ExpVector e) {
    this(r);
    if (c != null && !c.isZERO()) {
      val.put(e, c);
    }
   ...
   */
  public LocalSolvablePolynomial(
      LocalSolvablePolynomialRing<C> r, GenSolvablePolynomial<SolvableLocal<C>> S) {
    this(r, S.getMap());
  }
   ...
   */
  protected LocalSolvablePolynomial(
      LocalSolvablePolynomialRing<C> r, SortedMap<ExpVector, SolvableLocal<C>> v) {
    this(r);
      ...
      for (Map.Entry<ExpVector, SolvableLocal<C>> x : Bk) {
        ...
        if (ring.polCoeff.coeffTable.isEmpty() || b.isConstant() || e.isZERO()) { // symmetric
          ...
          if (b.den.isONE()) { // recursion base
            ...
            //     Cs = ring.fromPolyCoefficients(crel.p); //LocalSolvablePolynomial<C>(ring,
            // crel.p);
        ...
        if (ring.table.isEmpty() || Cps.isConstant() || f.isZERO()) { // symmetric
          ...
          if (Cps.isConstant()) {
            Ds =
                new LocalSolvablePolynomial<C>(ring, Cps.leadingBaseCoefficient(), g); // symmetric!
          } else {
  ...
  /**
   * LocalSolvablePolynomial left and right multiplication. Product with two polynomials.
   *
   ...
   */
  // not @Override
  public LocalSolvablePolynomial<C> multiply(
      LocalSolvablePolynomial<C> S, LocalSolvablePolynomial<C> T) {
  ...
  /**
   * LocalSolvablePolynomial left and right multiplication. Product with coefficient ring element.
   *
  ...
  /**
   * LocalSolvablePolynomial left and right multiplication. Product with exponent vector.
   *
   * @param e exponent.
   * @param f exponent.
  ...
  /**
   * LocalSolvablePolynomial multiplication. Product with ring element and exponent vector.
   *
   * @param b coefficient polynomial.
   * @param e exponent.
  ...
  /**
   * LocalSolvablePolynomial left and right multiplication. Product with ring element and exponent
   * vector.
   *
   * @param b coefficient polynomial.
   * @param e exponent.
  ...
  @Override
  public LocalSolvablePolynomial<C> multiply(
      SolvableLocal<C> b, ExpVector e, SolvableLocal<C> c, ExpVector f) {
    if (b == null || b.isZERO()) {
      return ring.getZERO();
    }
    if (c == null || c.isZERO()) {
  ...
  /**
   * LocalSolvablePolynomial multiplication. Left product with ring element and exponent vector.
   *
   * @param b coefficient polynomial.
   * @param e exponent.
  ...
  /**
   * LocalSolvablePolynomial multiplication. Left product with exponent vector.
   *
   * @param e exponent.
  ...
  /**
   * LocalSolvablePolynomial multiplication. Left product with coefficient ring element.
   *
   * @param b coefficient polynomial.
