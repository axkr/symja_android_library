...
public class SIQS_Small extends FactorAlgorithm {
  ...
  private KnuthSchroeppel multiplierFinder =
      new KnuthSchroeppel(); // used to compute the multiplier k
  ...
  private BigInteger findSingleFactorInternal(BigInteger N) {
    ...
    // The number of congruences we need to find before we try to solve the smooth congruence
    // equation system:
    ...
    // Compute the d-parameter from kN: We choose d=2 if kN % 8 == 1, otherwise d=1. For the modulus
    // we only need the lowest three bits.
    ...
    try {
      while (true) {
        ...
        // LOG.debug("Sieve found " + smoothXList.size() + " Q(x) smooth enough to be passed to
        // trial division.");
        ...
        if (DEBUG) {
          ...
          LOG.debug(
              "Sieve found "
                  ...
                  + " pass.");
          LOG.debug(
              "-> Now in total we have found "
                  + congruenceCollector.getSmoothCongruenceCount()
                  + " / "
                  + requiredSmoothCongruenceCount
                  + " smooth congruences and "
                  + congruenceCollector.getPartialCongruenceCount()
                  + " partials.");
        }
      } // end poly
  ...
  private void logResults(
    ...
    LOG.info(
        "    Approximate phase timings: powerTest="
            + powerTestDuration
            + "ms, initN="
            + initNDuration
            + "ms, initPoly="
   ...
   *
   * @param args ignored
   *     <p>Some test numbers: 11111111111111111111111111 5679148659138759837165981543
   ...
   *     = 33333 * 33335 * 33337 * 33339 * 33341 * 33343 * 33345 * 33347 * 33349 * 33351 * 33353 *
   *     33355 * 33357 * 33359 * 33361 * 33363 * 33367 * 33369 * 33369 * 33371
   *     <p>15841065490425479923 = 2604221509 * 6082841047
   */
