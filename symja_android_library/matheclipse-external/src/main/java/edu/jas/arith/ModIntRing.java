...
public final class ModIntRing implements ModularRingFactory<ModInt>, Iterable<ModInt> {
  ...
  public static final java.math.BigInteger MAX_INT =
      new java.math.BigInteger(String.valueOf(Short.MAX_VALUE)); // not larger!
  ...
  /**
   * ModInt chinese remainder algorithm. This is a factory method. Assert c.modul >= a.modul and
   * c.modul * a.modul = this.modul.
   *
   * @param c ModInt.
  ...
  /**
   * Modular digit list chinese remainder algorithm. m1 and m2 are positive beta-integers, with
   * GCD(m1,m2)=1 and m=m1*m2 less than beta. L1 and L2 are lists of elements of Z(m1) and Z(m2)
   * respectively. L is a list of all a in Z(m) such that a is congruent to a1 modulo m1 and a is
   * congruent to a2 modulo m2 with a1 in L1 and a2 in L2. This is a factory method. Assert c.modul
   * >= a.modul and c.modul * a.modul = this.modul.
   *
   * @param m1 ModInt.
   * @param m2 other ModInt.
   * @return L list of congruences.
   */
  public static List<ModInt> chineseRemainder(
      ModInt m1, ModInt m2, List<ModInt> L1, List<ModInt> L2) {
