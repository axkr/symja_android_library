...
/**
 * Compute modular sqrts t with t^2 == n (mod p) and u with u^2 == n (mod p^e) using Tonelli-Shanks'
 * algorithm. Implementation for all-BigInteger arguments.
 *
...
public class ModularSqrt_BB {
  ...
  /**
   * Compute the modular sqrt t with t^2 == n (mod p). Uses Tonelli-Shanks algorithm for p==1 (mod
   * 8), Lagrange's formula for p==3, 7 (mod 8) and another special formula for p==5 (mod 8).
   *
    ...
    switch (pMod8) {
      ...
      default:
        throw new IllegalStateException(
            "Tonelli_Shanks() has been called with even p="
                + p
                + " -> brute force search would be hardy a choice for BigInteger arguments.");
    }
  }

  /**
   * Tonelli-Shanks algorithm for modular sqrt R^2 == n (mod p), following
   * <link>en.wikipedia.org/wiki/Tonelli-Shanks_algorithm</link>.
   *
   * <p>This algorithm works for odd primes <code>p</code> and <code>n</code> with <code>
   * Legendre(n|p)=1</code>, but usually it is applied only to p with p==1 (mod 8), because for p ==
   * 3, 5, 7 (mod 8) there are simpler and faster formulas.
   *
   * @param n a positive integer having Jacobi(n|p) = 1
   * @param p odd prime
   * @return the modular sqrt t
  ...
  private BigInteger Tonelli_Shanks(BigInteger n, BigInteger p) {
    ...
    int S =
        pm1.getLowestSetBit(); // lowest set bit (0 if pm1 were odd which is impossible because p
    // is odd)
    //		if (DEBUG) {
    //			LOG.debug("n=" + n + ", p=" + p);
    ...
    BigInteger c =
        mpe.modPow(
            BigInteger.valueOf(z), Q, p); // TODO: implement modPow(int, BigInteger, BigInteger)
    ...
    while (t.compareTo(I_1) != 0) { // if t=1 then R is the result
      ...
      //				// test invariants from
      // <link>https://en.wikipedia.org/wiki/Tonelli%E2%80%93Shanks_algorithm#Proof</link>:
      ...
      if (foundI == false)
        throw new IllegalStateException("Tonelli-Shanks did not find an 'i' < M=" + M);
  ...
  /**
   * The simpler formula for case p == 5 (mod 8), following
   * <link>www.point-at-infinity.org/ecc/Algorithm_of_Shanks_&_Tonelli.html</link>.
   *
   * @param n a positive integer having Jacobi(n|p) = 1
   * @param p
   * @return
  ...
  /**
   * Simplest algorithm to compute solutions u of u^2 == n (mod p^2). This is the first proposition
   * in [Pomerance 1985: The Quadratic Sieve Factoring Algorithm, p. 178]. Works only for p==3 (mod
   * 4).
   *
   * @param n a positive integer having Jacobi(n|p) = 1
  ...
  /**
   * A faster version to compute solutions u of u^2 == n (mod p^2), doing modular arithmetics (mod
   * p) only, which is an application of lifting via Hensels lemma.
   *
   ...
   * [http://mathoverflow.net/questions/52081/is-there-an-efficient-algorithm-for-finding-a-square-root-modulo-a-prime-power,
   * Gottfried Barthel]. Works for any odd p with t^2 == n (mod p) having solution t>0.
   *
   * @param n
   ...
   */
  public BigInteger modularSqrtModPower(
      BigInteger n, BigInteger power, BigInteger last_power, BigInteger t) {
