...
public abstract class SquarefreeFieldCharP<C extends GcdRingElem<C>> extends SquarefreeAbstract<C> {
  ...
  /**
   * Factory for a algebraic extension of a finite field of characteristic p coefficients. If <code>
   * coFac</code> is an algebraic extension, then <code>aCoFac</code> is equal to <code>coFac</code>
   * , else <code>aCoFac</code> is <code>null</code>.
   */
  protected final AlgebraicNumberRing<C> aCoFac;

  /**
   * Factory for a transcendental extension of a finite field of characteristic p coefficients. If
   * <code>coFac</code> is an transcendental extension, then <code>qCoFac</code> is equal to <code>
   * coFac</code>, else <code>qCoFac</code> is <code>null</code>.
   */
  ...
  public GenPolynomial<GenPolynomial<C>> recursiveUnivariateSquarefreePart(
    ...
    SortedMap<GenPolynomial<GenPolynomial<C>>, Long> factors =
        recursiveUnivariateSquarefreeFactors(P);
    if (logger.isWarnEnabled()) {
   ...
   * @return true if P = prod_{i=1,...,k} p_i**(e_i*p), else false.
   */
  public boolean isRecursiveCharRoot(
   ...
   */
  public boolean isRecursiveCharRoot(
      GenPolynomial<GenPolynomial<C>> P, GenPolynomial<GenPolynomial<C>> r) {
    if (P == null || r == null) {
