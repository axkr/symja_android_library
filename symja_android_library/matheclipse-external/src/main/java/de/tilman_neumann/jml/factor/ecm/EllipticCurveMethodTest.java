      ...
      if (bits < 64) {
        ...
        for (int i = 0; i < N_COUNT; i++) {
          ...
          if (!a31Str.equals(b31Str)) {
            ...
            LOG.error(
                "long conversion failure: a31Str="
                    + a31Str
                    + ", b31Str = "
                    + b31Str
                    + ", diff = "
  ...
  private void testSpecialNumbers() {
    SpecialTest[] tests =
        new SpecialTest[] {
          ...
          // The N have in common that their actual bit size is ~20 bit smaller than the generation
          // target was.
          // At negative N > 700 bit such errors seem to occur more often.
          new SpecialTest(
              new BigInteger(
                  "-9047107805356617574821419177374462823988751688943829812044231389676"),
              248),
          new SpecialTest(
              new BigInteger(
                  "-810645591595199570650496051345328669857441632357541306927499776869083777701428362211927875931927"),
              341),
          new SpecialTest(
              new BigInteger(
                  "-1661507549698651677347352443872649283631112234579374113959724008640719132113623333818063620959056420710153"),
              371),
          new SpecialTest(
              new BigInteger(
                  "-3879725014912056720725365884026092090047847467519283535255468693493067092968062030773638553790318583416720"),
              371),
          new SpecialTest(
              new BigInteger(
                  "-81238486828807671926156945485493053897286271079236717483584339941912993848458748988378027450406814055929"),
              371),
        };
