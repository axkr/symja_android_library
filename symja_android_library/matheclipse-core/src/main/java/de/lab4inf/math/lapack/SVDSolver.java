...
public final class SVDSolver extends LASolver {
	 ...
	 *
	 * @param x
	 *            1.st argument to check
	 * @param y
	 *            2.nd argument to compare to
	 ...
	 *            matrix A
	 * @param j
	 *            index one of the rotation
	 * @param k
	 *            index two of the rotation
	 * @param cs
	 *            cosine value of rotation
	 * @param sn
	 *            sine value of rotation
	 */
	private static void givensrotation(final double[][] a, final int j, final int k, final double cs, final double sn) {
		double t;
		final int n = a.length;
		for (int i = 0; i < n; t = cs * a[i][j] + sn * a[i][k], a[i][k] = -sn * a[i][j]
				+ cs * a[i][k], a[i][j] = t, i++)
			;
	}
	...
	/**
	 * Calculate the condition number max(D)/min(D) of matrix A, where D are the eigenvalues.
	 *
	 * @param a
	 *            the matrix
	...
	/**
	 * Calculate the condition number max(D)/min(D). The eigenvalues have to be specified in descending order within the
	 * array d.
	 *
	 * @param d
	 *            the eigenvalues
	 ...
	 *            the NxM matrix U
	 * @param d
	 *            the diagonal elements with the eigenvalues
	 * @param v
	 *            the NxN matrix V
	 */
	 ...
	 * @param u
	 *            matrix U if not null it will be calculated
	 * @param v
	 *            matrix V if not null it will be calculated
	 * @param cols
	 ...
	 *
	 * @param d
	 *            the singular value diagonal elements
	 * @param u
	 *            matrix U if not null it will be generated
	 * @param v
	 *            matrix V if not null it will be generated
	 * @param beg
	 *            minimal index to start the ordering from
	 * @param end
	 *            maximal index to order to
	 */
	...
	private void split(final double[] d, final double[] e, final double[][] u, final int beg, final int end) {
		...
		for (j = beg; j < end; j++) {
			...
			e[j] = cs * e[j];
			if (wantu) {
				for (i = 0; i < m; t = cs * u[i][j] + sn * u[i][beg - 1], u[i][beg - 1] = -sn * u[i][j]
						+ cs * u[i][beg - 1], u[i][j] = t, i++)
					;
			}
		}
	}
	...
	private void deflate(final double[] d, final double[] e, final double[][] v, final int start, final int end) {
		...
		for (j = end - 2; j >= start; j--) {
			...
			if (wantv) {
				for (i = 0; i < n; t = cs * v[i][j] + sn * v[i][end - 1], v[i][end - 1] = -sn * v[i][j]
						+ cs * v[i][end - 1], v[i][j] = t, i++)
					;
			}
		}
	}
