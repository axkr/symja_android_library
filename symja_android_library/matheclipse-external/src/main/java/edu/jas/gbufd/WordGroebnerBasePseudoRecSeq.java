...
/**
 * Non-commutative word Groebner Base sequential algorithm. Implements Groebner bases and GB test.
...
public class WordGroebnerBasePseudoRecSeq<C extends GcdRingElem<C>>
   ...
   */
  public WordGroebnerBasePseudoRecSeq(
      RingFactory<GenPolynomial<C>> rf, WordPseudoReductionSeq<GenPolynomial<C>> red) {
  ...
  public List<GenWordPolynomial<GenPolynomial<C>>> GB(List<GenWordPolynomial<GenPolynomial<C>>> F) {
    ...
    // Collections.sort(G);
    OrderedWordPairlist<GenPolynomial<C>> pairlist =
        (OrderedWordPairlist<GenPolynomial<C>>) strategy.create(ring);
    pairlist.put(G);
   ...
   */
  @Override
  public List<GenWordPolynomial<GenPolynomial<C>>> minimalGB(
      List<GenWordPolynomial<GenPolynomial<C>>> Gp) {
    if (Gp == null || Gp.size() <= 1) {
      return Gp;
    }
    // remove zero polynomials
    List<GenWordPolynomial<GenPolynomial<C>>> G =
        new ArrayList<GenWordPolynomial<GenPolynomial<C>>>(Gp.size());
   ...
   * @return pp(P).
   */
  public GenWordPolynomial<GenPolynomial<C>> recursivePrimitivePart(
      GenWordPolynomial<GenPolynomial<C>> P) {
    if (P == null) {
      throw new IllegalArgumentException(this.getClass().getName() + " P != null");
    }
    if (P.isZERO()) {
  ...
  public List<GenWordPolynomial<GenPolynomial<C>>> recursivePrimitivePart(
    ...
    List<GenWordPolynomial<GenPolynomial<C>>> Pp =
        new ArrayList<GenWordPolynomial<GenPolynomial<C>>>(F.size());
