...
ISetDelayed(Int(Times(u_DEFAULT,Plus($p("A",true),Times($p("B",true),v_),Times($p("C",true),Sqr(v_))),Power(Plus(a_,Times(b_DEFAULT,v_)),m_)),x_Symbol),
    Condition(Times(Power(b,-2),Int(Times(u,Power(Plus(a,Times(b,v)),Plus(m,C1)),Simp(Plus(Times(b,$s("B")),Times(CN1,a,$s("C")),Times(b,$s("C"),v)),x)),x)),And(And(And(FreeQ(List(a,b,$s("A"),$s("B"),$s("C")),x),ZeroQ(Plus(Times($s("A"),Sqr(b)),Times(CN1,a,b,$s("B")),Times(Sqr(a),$s("C"))))),RationalQ(m)),LessEqual(m,CN1)))),
