...
public class GreatestCommonDivisorHensel<MOD extends GcdRingElem<MOD> & Modular>
  ...
  /**
   * Univariate GenPolynomial greatest common divisor. Uses univariate Hensel lifting.
   * 
  ...
  /**
   * Univariate GenPolynomial recursive greatest common divisor. Uses multivariate Hensel list.
   * 
  ...
  public GenPolynomial<GenPolynomial<BigInteger>> recursiveUnivariateGcd(
    ...
    for (int i = 0; i < 11; i++) { // meta loop
      ...
      if (cofac == null) { // no lucky prime found
        ...
        logger.info("no lucky prime, gave up on Hensel: {}= gcd({},{})", T, r, q);
        return T.abs().multiply(c); // .abs();
      }
      // System.out.println("cofac = " + cofac);

      // search evaluation points and evaluate
      List<BigInteger> V = new ArrayList<BigInteger>(dfac.nvar - 1);
      ...
      if (gcr.isONE() && gcq.isONE()) { // both gcds == 1: chose smaller ldcf
        ...
        // break;
        continue; // try next prime/evaluation points
      }
      ...
      logger.info("gcd(ldcf): {}, ldcf cofactor: {}, base cofactor: {}", lui, lh, he);
      ...
      // convert Ci from Z_{p^k}[x,y1,...,yr] to Z[x,y1,...,yr] to Z[x][y1,...,yr] to
      // Z[y1,...,yr][x]
      ...
      if (!Cs.ring.equals(P.ring)) {
        logger.warn("ring mismatch: Cs.ring = {}, P.ring = {}", Cs.ring.toScript(),
            P.ring.toScript());
    ...
    GenPolynomial<GenPolynomial<BigInteger>> T = iufd.recursiveUnivariateGcd(q, r);
    T = T.abs().multiply(c);
    logger.info("no lucky prime or evaluation points, gave up on Hensel: {} = gcd({},{})", T, r, q);
    return T;
  }


  /*
   * move to Ideal ? GenPolynomial<BigInteger> invertPoly(ModularRingFactory<MOD> mfac,
   * GenPolynomial<BigInteger> li, List<BigInteger> V) { if (li == null || li.isZERO()) { throw new
   * RuntimeException("li not invertible: " + li); } if (li.isONE()) { return li; }
   * //System.out.println("mfac = " + mfac + ", V = " + V +", li = " + li);
   * GenPolynomialRing<BigInteger> pfac = li.ring; GenPolynomialRing<MOD> mpfac = new
   * GenPolynomialRing<MOD>(mfac, pfac); GenPolynomial<MOD> lm = PolyUtil.<MOD>
   * fromIntegerCoefficients(mpfac, li); //System.out.println("pfac = " + pfac + ", lm = " + lm);
   * List<GenPolynomial<MOD>> lid = new ArrayList<GenPolynomial<MOD>>(V.size()); int i = 0; for
   * (BigInteger bi : V) { MOD m = mfac.fromInteger(bi.getVal()); GenPolynomial<MOD> mp =
   * mpfac.univariate(i); mp = mp.subtract(m); // X_i - v_i lid.add(mp); i++; }
   * //System.out.println("lid = " + lid); //Ideal<MOD> id = new Ideal<MOD>(mpfac,lid,true); // is a
   * GB //System.out.println("id = " + id); GenPolynomial<MOD> mi = lm; //id.inverse(lm);
   * //System.out.println("mi = " + mi); GenPolynomial<BigInteger> inv =
   * PolyUtil.integerFromModularCoefficients(pfac, mi); return inv; }
   */
