...
/**
 * A map from all "subvalues" s of a multipartite number q with 1<s<=q (with regard to the "partwise
 * comparison") to a TreeMap of all "subvalues" t with 0<t<s (with regard to the "ordering
 * comparison") mapped to their complements s-t.
 *
 * <p>Example: The power map for q=[5] is {2={1=1}, 3={1=2, 2=1}, 4={1=3, 2=2, 3=1}, 5={1=4, 2=3,
 * 3=2, 4=1} }
 *
...
public class MpiPowerMap extends HashMap<Mpi, TreeMap<Mpi, Mpi>> {
  ...
  public static MpiPowerMap create(Mpi q) {
    ...
    // LOG.debug("time to compute power map of " + q + " = " + (System.currentTimeMillis()-start) +
    // "ms");
    return result;
  }
  ...
  /**
   * Delivers all subvalues (piece-wise relation) of x not bigger (ordering relation) than
   * biggestElem.
   *
   * @param x
   * @param biggestElem
