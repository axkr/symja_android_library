	...
	private static class Simplify extends AbstractFunctionEvaluator {
		...
		private static class SimplifyVisitor extends VisitorExpr {
			...
			private IExpr tryTransformations(IExpr expr) {
				...
				if (expr.isTimes()) {
					...
					IExpr[] commonFactors = Algebra.InternalFindCommonFactorPlus.findCommonFactors((IAST) expr, true);
			...
			private IExpr visitTimes(IASTMutable timesAST, SimplifiedResult sResult) {
				...
				while (i < timesAST.size()) {
					...
					if (timesArg.isPowerReciprocal() && timesArg.base().isPlus() && timesArg.base().size() == 3) {
						// example 1/(5+Sqrt(17)) => 1/8*(5-Sqrt(17))
					...
					if ((i + 1 < timesAST.size()) && timesArg.isPower() && ((fFullSimplify && timesArg.base().isAST())
			 ...
			 */
			private final static int SQR_ARG = 1;

			/**
			 * A trigonometric or hyperbolic function <code>-trig(x)^2</code> was found.
			 */
			private final static int NEGATIVE_SQR_ARG = 2;

			/**
			 * Check if <code>plusAST</code> has the form <code>+/- 1 + ... + ... </code>. Try to find a trigonometric
			 * or hyperbolic function <code>+/- trig(x)^2</code> in the rest of the <code>plusAST</code> and simplify if
			 * possible.
			 * 
			 * @param plusAST
			...
			/**
			 * Return a value > 0 at index <code>0</code>, if a power trig- or hyperbolicfunction was found. The
			 * determined type <code>SQR_ARG,NEGATIVE_SQR_ARG</code> of the expression at index <code>1</code>.
			 * 
			 * @param plusAST
			 *            the <code>Plus( ... )</code> expression
			 * @param fromPosition
			 *            start searching at this index inclusive.
			 * @return a value > 0 at index <code>0</code>, if a power trig- or hyperbolicfunction was found. The type
			 *         of expression found at index <code>1</code>.
			 */
			private static int[] plusASTIndexOf(IASTMutable plusAST, int fromPosition) {
				for (int i = fromPosition; i < plusAST.size(); i++) {
					...
					} else if (x.isAST(F.Times, 3) && x.first().isMinusOne() && x.second().isPower() && //
		...
		public IExpr evaluate(final IAST ast, EvalEngine engine) {
			...
			try {
				...
				temp = simplifyStep(arg1, complexityFunction, minCounter, result, engine, isFullSimplifyMode());
		...
		private static IExpr simplifyStep(IExpr arg1, Function<IExpr, Long> complexityFunction, long minCounter,
				IExpr result, EvalEngine engine, boolean fullSimplify) {
			long count;
			IExpr temp;
