...
public class Row implements Iterator<Row> {
  ...
  public Row(TableSlice tableSlice, int rowNumber) {
    ...
    for (Column<?> column : tableSlice.getTable().columns()) {
      if (column instanceof ExprColumn) {
        exprColumnMap.put(column.name(), (ExprColumn) column);
      }
  ...
  public void setExpr(int columnIndex, IExpr value) {
    setExpr(columnNames[columnIndex], value);
  }

  public void setExpr(String columnName, IExpr value) {
    exprColumnMap.get(columnName).set(getIndex(rowNumber), value);
  }
