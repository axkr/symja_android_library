...
public class BairstowRootFinder extends L4MObject {
  ...
  private static final Comparator<Complex> CP =
  ...
  /**
   * Constructor indicating if the roots should be sorted with increasing real and imaginary part.
   *
  ...
  /**
   * Find all roots p(x)=0 for the polynomial p of order n. The n complex roots are coded as
   * (real,imag) parts within a 2n-dimensional array.
   *
  ...
  private double[] splitQuadric(final double[] a) {
    ...
    v = a[n - 2] / a[n];
    do {
      for (b0 = 0, b1 = 0, f0 = 0, f1 = 0, j = n - 2;
          j >= 0;
          b2 = b1, b1 = b0, b0 = a[j + 2] - u * b1 - v * b2, f2 = f1, f1 = f0,
              f0 = b2 - u * f1 - v * f2, j--) ;
