...
public abstract class GreatestCommonDivisorAbstract<C extends GcdRingElem<C>>
  ...
  public GenSolvablePolynomial<C> leftBasePrimitivePart(GenSolvablePolynomial<C> P) {
    ...
    // GenSolvablePolynomial<C> pp = (GenSolvablePolynomial<C>) P.rightDivideCoeff(d); //
    // rightDivide TODO/done
  ...
  public GenSolvablePolynomial<C> rightBasePrimitivePart(GenSolvablePolynomial<C> P) {
    ...
    GenSolvablePolynomial<C> pp =
        (GenSolvablePolynomial<C>) P.leftDivideCoeff(d); // leftDivide TODO/done
    if (debug) {
      GenSolvablePolynomial<C> p = pp.multiplyLeft(d);
      if (!p.equals(P)) {
        throw new ArithmeticException("pp(p)*cont(p) != p: ");
  ...
  /**
   * Univariate GenSolvablePolynomial greatest common divisor. Uses sparse pseudoRemainder for
   * remainder.
   *
   ...
   */
  public abstract GenSolvablePolynomial<C> leftBaseGcd(
      GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S);

  /**
   * Univariate GenSolvablePolynomial right greatest common divisor. Uses sparse pseudoRemainder for
   * remainder.
   *
   * @param P univariate GenSolvablePolynomial.
   * @param S univariate GenSolvablePolynomial.
   ...
   */
  public abstract GenSolvablePolynomial<C> rightBaseGcd(
      GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S);
  ...
  public GenSolvablePolynomial<GenPolynomial<C>> baseRecursivePrimitivePart(
    ...
    GenSolvablePolynomial<GenPolynomial<C>> pp =
        (GenSolvablePolynomial<GenPolynomial<C>>) PolyUtil.<C>baseRecursiveDivide(P, d);
    return pp;
  }

  /**
   * GenSolvablePolynomial left recursive greatest common divisor. Uses pseudoRemainder for
   * remainder.
   *
   * @param P recursive GenSolvablePolynomial.
   * @param S recursive GenSolvablePolynomial.
   * @return gcd(P,S) with P = P'*gcd(P,S)*p and S = S'*gcd(P,S)*s, where deg_main(p) = deg_main(s)
   *     == 0.
   */
  @SuppressWarnings("unchecked")
  public GenSolvablePolynomial<GenPolynomial<C>> leftRecursiveGcd(
  ...
  /**
   * GenSolvablePolynomial right recursive greatest common divisor. Uses pseudoRemainder for
   * remainder.
   *
   * @param P recursive GenSolvablePolynomial.
   * @param S recursive GenSolvablePolynomial.
  ...
  /**
   * Univariate GenSolvablePolynomial recursive greatest common divisor. Uses pseudoRemainder for
   * remainder.
   *
  ...
  /**
   * Univariate GenSolvablePolynomial right recursive greatest common divisor. Uses pseudoRemainder
   * for remainder.
   *
   * @param P univariate recursive GenSolvablePolynomial.
   * @param S univariate recursive GenSolvablePolynomial.
  ...
  public GenSolvablePolynomial<C> leftGcd(GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S) {
    ...
    GenSolvablePolynomialRing<GenPolynomial<C>> rfac =
        pfac.recursive(1); // RecSolvablePolynomialRing<C>
   ...
   * @return B with gcd(b,c) = 1 for all b != c in B and for all non-constant a in A there exists b
   *     in B with b|a. B does not contain zero or constant polynomials.
   */
   ...
   * @return B with gcd(b,c) = 1 for all b != c in B and for non-constant a there exists b in P with
   *     b|a. B does not contain zero or constant polynomials.
   */
  public List<GenSolvablePolynomial<C>> leftCoPrime(
      GenSolvablePolynomial<C> a, List<GenSolvablePolynomial<C>> P) {
  ...
  /**
   * Univariate GenSolvablePolynomial extended greatest common divisor. Uses sparse pseudoRemainder
   * for remainder.
   *
   * @param P univariate GenSolvablePolynomial.
   * @param S univariate GenSolvablePolynomial.
  ...
  public GenSolvablePolynomial<C>[] baseExtendedGcd(
      GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S) {
    // return P.egcd(S);
  ...
  /**
   * Univariate GenSolvablePolynomial half extended greatest comon divisor. Uses sparse
   ...
   */
  @SuppressWarnings({"unchecked", "cast"})
  public GenSolvablePolynomial<C>[] baseHalfExtendedGcd(
      GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S) {
    if (P == null || S == null) {
   ...
   */
  @SuppressWarnings({"unchecked", "cast"})
  public GenSolvablePolynomial<C>[] baseGcdDiophant(
      GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S, GenSolvablePolynomial<C> c) {
    ...
    if (debug) {
      GenSolvablePolynomial<C> y =
          (GenSolvablePolynomial<C>) ret[0].multiply(P).sum(ret[1].multiply(S));
      if (!y.equals(c)) {
  ...
  public C[] leftOreCond(C a, C b) {
    ...
    if (rf.isField()) {
      logger.info(
          "left Ore condition on coefficients, skew field " + rf + " case: " + a + ", " + b);
   ...
   */
  @SuppressWarnings({"unchecked", "cast"})
  public GenSolvablePolynomial<C>[] leftOreCond(
      GenSolvablePolynomial<C> a, GenSolvablePolynomial<C> b) {
    if (a == null || a.isZERO() || b == null || b.isZERO()) {
      throw new IllegalArgumentException("a and b must be non zero");
    }
    ...
    if (pfac.isCommutative()) {
      ...
      edu.jas.ufd.GreatestCommonDivisorAbstract<C> cgcd =
          GCDFactory.<C>getImplementation(pfac.coFac);
  ...
  /**
   * Coefficient rigth Ore condition. Generators for the right Ore condition of two coefficients.
   *
   * @param a coefficient.
   * @param b coefficient.
  ...
  public C[] rightOreCond(C a, C b) {
    ...
    if (rf.isField()) {
      logger.info(
          "right Ore condition on coefficients, skew field " + rf + " case: " + a + ", " + b);
      // C gcd = a.gcd(b); // always one
      // C lcm = rf.getONE();
      oc[0] = a.inverse(); // lcm.divide(a);
      oc[1] = b.inverse(); // lcm.divide(b);
   ...
   */
  @SuppressWarnings({"unchecked", "cast"})
  public GenSolvablePolynomial<C>[] rightOreCond(
      GenSolvablePolynomial<C> a, GenSolvablePolynomial<C> b) {
    if (a == null || a.isZERO() || b == null || b.isZERO()) {
      throw new IllegalArgumentException("a and b must be non zero");
    }
