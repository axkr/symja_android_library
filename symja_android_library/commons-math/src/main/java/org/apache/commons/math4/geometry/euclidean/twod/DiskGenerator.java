...
public class DiskGenerator implements SupportBallGenerator<Euclidean2D, Vector2D> {
            ...
            if (support.size() < 2) {
                ...
                if (support.size() < 3) {
                    ...
                    final BigFraction[] c1 = new BigFraction[] {
                        c2[0].multiply(c2[0]).sum(c3[0].multiply(c3[0])),
                        c2[1].multiply(c2[1]).sum(c3[1].multiply(c3[1])),
                        c2[2].multiply(c2[2]).sum(c3[2].multiply(c3[2]))
                    };
                    ...
                    final BigFraction r2      = dx.multiply(dx).sum(dy.multiply(dy));
    ...
    private BigFraction minor(final BigFraction[] c1, final BigFraction[] c2) {
        return      c2[0].multiply(c1[2].subtract(c1[1])).
                sum(c2[1].multiply(c1[0].subtract(c1[2]))).
                sum(c2[2].multiply(c1[1].subtract(c1[0])));
    }
