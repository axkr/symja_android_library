...
/**
 * Iterative Jacobi Solver for A*x=y. Starting from a random vector x_0 a sequence x_n converging
 * against the solution is generated, if the matrix norm ||A|| is less than one, e.g. A is strict
 * diagonal dominant.
 *
 * @author nwulff
  ...
  protected <T extends Numeric<T>> void updateX(
    ...
    for (k = 0; k < n; x[k] = s.div(ak[k]), k++) {
      for (ak = a[p[k]], s = y[p[k]], j = 0; j < n; j++)
        if (k != j) s = s.minus(ak[j].multiply(xo[j]));
    }
  }
}
