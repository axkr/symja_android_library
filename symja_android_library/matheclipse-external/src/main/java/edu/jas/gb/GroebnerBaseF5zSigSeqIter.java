...
public class GroebnerBaseF5zSigSeqIter<C extends RingElem<C>> extends GroebnerBaseSigSeqIter<C> {
   ...
   * @return updated pair list. <b>Note:<b> stores polynomials not only indices.
   */
  @Override
   ...
   */
  @Override
  List<SigPair<C>> pruneS(
      List<SigPair<C>> S, List<ExpVector> syz, List<SigPoly<C>> done, List<SigPoly<C>> G) {
   ...
   *
   * @param syz list of exponent vectors representing syzygies.
   * @param r polynomial. <b>Note:</b> szy is modified to represent updated list of exponent
   *     vectors.
   */
  @Override
