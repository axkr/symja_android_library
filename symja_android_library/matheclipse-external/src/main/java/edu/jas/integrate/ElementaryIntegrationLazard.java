...
/**
 * Method related to elementary integration. Lazard-Rioboo-Trager integration of the logarithmic
 * part.
 *
...
public class ElementaryIntegrationLazard<C extends GcdRingElem<C>>
  ...
  /**
   * Univariate GenPolynomial integration of the logaritmic part, Lazard - Rioboo - Trager
   *
  ...
  public LogIntegral<C> integrateLogPart(GenPolynomial<C> A, GenPolynomial<C> P) {
    ...
    List<GenPolynomial<GenPolynomial<C>>> RcList =
        engine.recursiveUnivariateSubResultantList(Pc, At); // returning
    GenPolynomial<GenPolynomial<C>> Rc = RcList.get(RcList.size() - 1); // just getting R
    // System.out.println("R = " + Rc);

    // SquareFree(R)
    SortedMap<GenPolynomial<C>, Long> resSquareFree =
        sqf.squarefreeFactors(Rc.leadingBaseCoefficient());
    ...
    for (Entry<GenPolynomial<C>, Long> qi : resSquareFree.entrySet()) {
      ...
      if (qi2.degree() > 0) {
        ...
        for (GenPolynomial<C> ai : sfactors.keySet()) {
          GenPolynomial<AlgebraicNumber<C>> aiC =
              PolyUtil.<C>convertToAlgebraicCoefficients(pafac, ai);
          GenPolynomial<AlgebraicNumber<C>> qiC =
              PolyUtil.<C>convertToAlgebraicCoefficients(pafac, qi2);
          if (aengine == null) {
