...
public class CollatzSequenceTest {
  ...
  private static final int START_PROGRESSION =
      2; // 2 is best for dropping sequences; 2, 3, 9 are good for repeat sequences
  ...
  private static void test3nPlus1DroppingSequence() {
    ...
    TreeMap<Integer, ArrayList<BigInteger>> length2NStartList =
        new TreeMap<Integer, ArrayList<BigInteger>>();

    // Define arithmetic progressions of dropping sequences: progr(mul, add) = {mul*n+add,
    // n=0,1,2,...}
    ...
    // Finally we analyze the counts of unbounded progressions for sets {progr(mul, add),
    // add=0..(mul-1)}
    // The smallest numbers of unbounded proressions ar achieved for mul being powers of 2.
    TreeMap<Integer, TreeMap<Integer, Integer>> progressionToMaxLength =
      ...
      while (true) {
        ...
        if (resolved.contains(n)) {
          ...
          if (length > maxLength) {
            ...
            // sequence of records = A217934 = 0, 1, 6, 11, 96, 132, 171, 220, 267, 269, 282, 287,
            // 298, 365, 401, 468, 476, 486, 502, 613, 644, 649, 706, 729, 892, 897, 988, 1122,
            // 1161, 1177, 1187, 1445, 1471, 1575, 1614, 1639
  ...
  /**
   * Run the 2n+1 sequence until we find some n that was already contained in the sequence of some
   * smaller n.
   */
  ...
  private static void test3nPlus1RepeatSequence() {
    ...
    // Define arithmetic progressions of "repeat" sequences: progr(mul, add) = {mul*n+add,
    // n=0,1,2,...}
    // Then we analyze the maximal length of sequences of progression progr(mul, add).
    // Many such progressions have a small maximal length.
    // Progressions that do not have a small maximal length are called "unbounded".
    // Finally we analyze the counts of unbounded progressions for sets {progr(mul, add),
    // add=0..(mul-1)}
    // The smallest numbers of unbounded progressions are achieved for mul of the form 2^r*3^s, s
    // small.
    TreeMap<Integer, TreeMap<Integer, Integer>> progressionToMaxLength =
        new TreeMap<Integer, TreeMap<Integer, Integer>>();
  ...
  private static void addToLength2NMap(
      TreeMap<Integer, ArrayList<BigInteger>> length2NList, int length, BigInteger nStart) {
  ...
  private static void addToProgressionToMaxLengthMap(
      TreeMap<Integer, TreeMap<Integer, Integer>> progressionToMaxLength,
      BigInteger nStart,
      int length) {
  ...
  private static void analyzeProgressions(
      TreeMap<Integer, TreeMap<Integer, Integer>> progressionToMaxLength) {
    ...
    for (Integer mul : progressionToMaxLength.keySet()) {
      ...
      for (Map.Entry<Integer, Integer> add2MaxLength : progressionToMaxLength.get(mul).entrySet()) {
        ...
        // LOG.info("Progression " + mul + "*n + " + add2MaxLength.getKey() + ": maxLength = " +
        // progrMaxLength);
    ...
    // A076227 = Number of surviving Collatz residues mod 2^n = 1, 1, 1, 2, 3, 4, 8, 13, 19, 38, 64,
    // 128, 226, 367, 734, 1295, 2114, 4228, 7495, 14990, 27328, 46611, 93222, 168807, 286581,
    // 573162, 1037374, 1762293, 3524586, 6385637, 12771274, 23642078, 41347483, 82694966,
    // 151917636, 263841377, 527682754, 967378591, 1934757182, 3611535862
  }
