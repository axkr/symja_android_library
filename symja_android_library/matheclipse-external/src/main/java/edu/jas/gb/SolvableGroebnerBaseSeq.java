...
/**
 * Solvable Groebner bases sequential algorithms. Implements common left, right and twosided
 * Groebner bases and left, right and twosided GB tests.
 *
...
public class SolvableGroebnerBaseSeq<C extends RingElem<C>>
  ...
  public List<GenSolvablePolynomial<C>> leftGB(int modv, List<GenSolvablePolynomial<C>> F) {
    ...
    while (pairlist.hasNext()) {
      ...
      if (debug) {
        ...
        // logger.info("ht(H) = " + H.leadingExpVector() + ", lc(H) = " +
        // H.leadingBaseCoefficient().toScript());
      }
  ...
  public List<GenSolvablePolynomial<C>> twosidedGB(int modv, List<GenSolvablePolynomial<C>> Fp) {
    ...
    List<GenSolvablePolynomial<C>> G =
        new ArrayList<GenSolvablePolynomial<C>>(F.size() * (1 + X.size()));
    G.addAll(F);
   ...
   * @return normalized M.
   */
  public List<List<GenSolvablePolynomial<C>>> normalizeMatrix(
      int flen, List<List<GenSolvablePolynomial<C>>> M) {
    if (M == null) {
      return M;
    }
    if (M.size() == 0) {
   ...
   */
  public SolvableExtendedGB<C> minimalSolvableExtendedGB(
      int flen, List<GenSolvablePolynomial<C>> Gp, List<List<GenSolvablePolynomial<C>>> M) {
    if (Gp == null) {
  ...
  /**
   * Right Groebner base via right reduction using pairlist class. Overides rightGB() via opposite
   * ring.
   *
   * @param modv number of module variables.
   * @param F solvable polynomial list.
