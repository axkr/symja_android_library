...
/**
 * Combination of a monolithic sieve for large primes > sieveArraySize/3, and a single block sieve
 * for p < sieveArraySize/3.
 *
 * <p>Sieving incomplete blocks makes no sense, because we have to run through all primes, but
 * harvest only a small x-range. Thus, we arrange blocks such that block size divides sieve array
 * size.
 *
...
public class SingleBlockHybridSieve implements Sieve {
  ...
  public void initializeForN(SieveParams sieveParams, int mergedBaseSize) {
    ...
    //		if (DEBUG) {
    //			LOG.debug("sieveArraySize0=" + sieveArraySize0 + ", desiredBlockSize=" + desiredBlockSize +
    // " -> blockCount=" + blockCount + ", sieveArraySize=" + sieveArraySize + ",
  ...
  public void initializeForAParameter(SolutionArrays solutionArrays, int filteredBaseSize) {
    ...
    p1Index = binarySearch.getInsertPosition(pArray, primeBaseSize, sieveArraySize);
    p2Index = binarySearch.getInsertPosition(pArray, p1Index, (sieveArraySize + 1) / 2);
    p3Index = binarySearch.getInsertPosition(pArray, p2Index, (sieveArraySize + 2) / 3);
    // find the index of the largest prime < p3 fitting into a block
    r_s = binarySearch.getInsertPosition(pArray, p3Index, effectiveBlockSize);
    if (DEBUG)
      LOG.debug(
          "primeBaseSize="
              + primeBaseSize
  ...
  public List<Integer> sieve() {
    ...
    for (i = primeBaseSize - 1; i >= p1Index; i--) {
      // x1 == x2 happens only if p divides k -> for large primes p > k there are always 2 distinct
      // solutions.
      // x1, x2 may exceed sieveArraySize, but we allocated the arrays somewhat bigger to save the
      // size checks.
    ...
    for (int b = 0;
      ...
      // LOG.debug("sieve pos. block " + b + " (" + effectiveBlockSize + " bytes) with primes " +
      // pMinIndex + " ... " + r_s + " ... " + p3Index);
      sievePositiveXBlock(pArray, logPArray, effectiveBlockSize, pMinIndex, r_s, p3Index);
      if (ANALYZE) sieveDuration += timer.capture();

      // collect block
      // let the sieve entry counter x run down to 0 is much faster because of the simpler exit
      // condition
    ...
    for (int b = 0;
      ...
      // LOG.debug("sieve neg. block " + b + " (" + effectiveBlockSize + " bytes) with primes " +
      // pMinIndex + " ... " + r_s + " ... " + p3Index);
      sieveNegativeXBlock(pArray, logPArray, effectiveBlockSize, pMinIndex, r_s, p3Index);
      if (ANALYZE) sieveDuration += timer.capture();

      // Collect block: We collect 4 bytes at once, thus we need 4 | effectiveBlockSize -> see
      // initialize()
      final int blockOffset1 = blockOffset + 1;
      final int blockOffset2 = blockOffset + 2;
      final int blockOffset3 = blockOffset + 3;
  ...
  private void sievePositiveXBlock(
      final int[] primesArray,
      final byte[] logPArray,
      final int B,
      final int r_start,
      final int r_medium,
      final int r_max) {
    ...
    for (r = r_max - 1; r >= r_medium; r--) {
      ...
      // solution x2: x1 == x2 happens in any of (basic QS, MPQS, SIQS) if p divides k, which
      // implies t=0
  ...
  private void sieveNegativeXBlock(
      final int[] primesArray,
      final byte[] logPArray,
      final int B,
      final int r_start,
  ...
  private void initializeSieveArray(int sieveArraySize) {
    // overwrite existing arrays with initializer. we know that sieve array size is a multiple of
    // 256
