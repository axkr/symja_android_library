...
SetDelayed(NotIntegrableQ(u_,$p("x",$s("Symbol"))),
    Or(MatchQ(u,Condition(Times(Power(x,m_),Power(Log(Plus(a_,Times($p("b",true),x))),n_)),And(And(And(FreeQ(List(a,b),x),IntegerQ(List(m,n))),Less(m,C0)),Less(n,C0)))),MatchQ(u,Condition($(f_,Times(Power(x,$p("m",true)),Log(Plus($p("a",true),Times($p("b",true),x))))),And(And(FreeQ(List(a,b),x),IntegerQ(m)),Or(TrigQ(f),HyperbolicQ(f))))))),
SetDelayed(ZeroQ(u_),
    PossibleZeroQ(u)),
...
SetDelayed(IntegerQ(Plus(u_,Times(m_,Plus(n_,v_)))),
    And(And(And(RationalQ(m),RationalQ(n)),IntegerQ(Times(m,n))),IntegerQ(Plus(u,Times(m,v))))),
SetDelayed(IntegerQ($p("u",$s("List"))),
    MapAnd($s("IntegerQ"),u)),
...
SetDelayed(RationalQ(u_),
    If(ListQ(u),MapAnd($s("Integrate::RationalQ"),u),Or(IntegerQ(u),SameQ(Head(u),$s("Rational"))))),
...
SetDelayed(FractionOrNegativeQ(u_),
    If(ListQ(u),MapAnd($s("Integrate::FractionOrNegativeQ"),u),Or(FractionQ(u),And(IntegerQ(u),Less(u,C0))))),
SetDelayed(SqrtNumberQ(Power(m_,n_)),
    Or(And(IntegerQ(n),SqrtNumberQ(m)),And(HalfIntegerQ(n),RationalQ(m)))),
...
SetDelayed(PolynomialQ($p("u",$s("List")),$p("x",$s("Symbol"))),
    MapAnd($s("PolynomialQ"),u,x)),
SetDelayed(LinearQ(u_,$p("x",$s("Symbol"))),
    And(PolynomialQ(u,x),SameQ(Exponent(u,x),C1)))
  );
}
