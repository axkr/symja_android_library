...
/**
 * Polynomial Reduction abstract class. Implements common S-Polynomial, normalform, criterion 4
...
public abstract class ReductionAbstract<C extends RingElem<C>> implements Reduction<C> {
   ...
   *
   * @param S recording matrix, is modified. <b>Note</b> the negative S-polynomial is recorded as
   ...
   */
  public GenPolynomial<C> SPolynomial(
      List<GenPolynomial<C>> S, int i, GenPolynomial<C> A, int j, GenPolynomial<C> B) {
    if (debug) {
      if (B == null || B.isZERO()) {
  ...
  public boolean criterion4(GenPolynomial<C> A, GenPolynomial<C> B) {
    if (logger.isInfoEnabled()) {
      if (!A.ring.isCommutative()
          || !B.ring.isCommutative()) { // A instanceof GenSolvablePolynomial
        logger.error("GBCriterion4 not applicabable to non-commutative polynomials");
        return true;
      }
    }
   ...
   */
  public GenPolynomial<C> normalformMarked(
      List<Monomial<C>> Mp, List<GenPolynomial<C>> Pp, GenPolynomial<C> Ap) {
   ...
   *
   * @param Pp polynomial list.
   * @return a list P of monic polynomials which are in normalform wrt. P and with ideal(Pp) =
   *     ideal(P).
   */
