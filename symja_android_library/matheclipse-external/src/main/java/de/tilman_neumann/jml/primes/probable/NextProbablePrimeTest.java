...
public class NextProbablePrimeTest {
  ...
  /** Performance test. Each algorithm is test with the same NCOUNT test numbers in random order. */
  private static void testPerformance() {
    for (int nBits = 20; ; nBits += 10) {
      ...
      while (i < NCOUNT) {
        ...
        if (testSet[i].signum() > 0 && testSet[i].bitLength() == nBits)
          i++; // exclude 0 from test set, assure correct bit size
      }
  ...
  private static void addToMap(
      TreeMap<Long, List<String>> duration_2_algLists, Long duration, String algStr) {
