 ...
 * </pre>
 *
 * <p>via the Runge-Kutta Nyström method using
 *
 * <pre>
...
public class RKNSolver extends AbstractOdeSolver implements SecondOrderOdeSolver {
  ...
  public double solve(
    ...
    double y, e = eps / 10;
    if (eps < EPS_MIN) {
      String msg = String.format("epsilon:%.2g less than esp_min=%.2g", eps, EPS_MIN);
      logger.warn(msg);
      e = max(eps, EPS_MIN);
    }
  ...
  /**
   * Evaluate the next y1 at point x1=x0+h, given x0 and y0 for a given differential equation
   * y''=f(x,y,y''), using a Runge-Kutta Nyström approximation.
   *
  ...
  /**
   * Calculate y1 at point x1, given x0, y(x0) and y'(x0) with eps precision for the given 2.nd
   * order differential equation y''=f(x,y,y') using a classical Runge-Kutta Nyström method of
   * fourth order. The step width is adjusted until precision is reached.
   *
   * @param x0 double the x starting point
   * @param y0 double the value y(x0).
   ...
   */
  public double rungeKuttaNystrom(
      final double x0,
      final double y0,
      final double dy0,
      final double x1,
