...
/**
 * Pair list management. The original Buchberger algorithm with criterions following Winkler in
 * SAC-1, Kredel in ALDES/SAC-2, Kredel in MAS. Implemented using GenPolynomial, TreeMap and BitSet.
 *
 * @author Heinz Kredel
 */
public class OrderedPairlist<C extends RingElem<C>> implements PairList<C> {
  ...
  /**
   * Remove the next required pair from the pairlist and reduction matrix. Appy the criterions 3 and
   * 4 to see if the S-polynomial is required.
   *
