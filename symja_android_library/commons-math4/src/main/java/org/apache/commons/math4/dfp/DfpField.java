...
public class DfpField implements Field<Dfp> {
    ...
    private static Dfp computePi(final Dfp one, final Dfp two, final Dfp three) {
        ...
        for (int i = 1; i < 20; i++) {
            ...
            yk = one.subtract(s).divide(one.sum(s));

            two2kp3 = two2kp3.multiply(four);

            final Dfp p = one.sum(yk);
            final Dfp p2 = p.multiply(p);
            ak = ak.multiply(p2.multiply(p2)).subtract(two2kp3.multiply(yk).multiply(one.sum(yk).sum(yk.multiply(yk))));
    ...
    public static Dfp computeLn(final Dfp a, final Dfp one, final Dfp two) {
        ...
        int den = 1;
        Dfp x = a.sum(new Dfp(a.getField(), -1)).divide(a.sum(one));
