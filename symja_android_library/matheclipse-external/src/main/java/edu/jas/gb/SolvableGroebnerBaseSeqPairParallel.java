...
/**
 * Solvable Groebner Base parallel algorithm. Makes some effort to produce the same sequence of
 * critical pairs as in the sequential version. However already reduced pairs are not rereduced if
 * new polynomials appear. Implements a shared memory parallel version of Groebner bases. Threads
 * maintain pairlist.
 *
...
public class SolvableGroebnerBaseSeqPairParallel<C extends RingElem<C>>
  ...
  private static final Logger logger =
      LogManager.getLogger(SolvableGroebnerBaseSeqPairParallel.class);
   ...
   */
  public SolvableGroebnerBaseSeqPairParallel(
      int threads, ExecutorService pool, SolvableReduction<C> sred) {
    super(sred);
  ...
  /**
   * Parallel Groebner base using sequential pair order class. Threads maintain pairlist.
   *
  ...
  public List<GenSolvablePolynomial<C>> leftMinimalGB(List<GenSolvablePolynomial<C>> Fp) {
    ...
    SolvableMiReducerSeqPair<C>[] mirs =
        (SolvableMiReducerSeqPair<C>[]) new SolvableMiReducerSeqPair[G.size()];
    int i = 0;
    F = new ArrayList<GenSolvablePolynomial<C>>(G.size());
    while (G.size() > 0) {
      a = G.remove(0);
      // System.out.println("doing " + a.length());
      List<GenSolvablePolynomial<C>> R =
          new ArrayList<GenSolvablePolynomial<C>>(G.size() + F.size());
      R.addAll(G);
      R.addAll(F);
...
class LeftSolvableReducerSeqPair<C extends RingElem<C>> implements Runnable {
  ...
  LeftSolvableReducerSeqPair(
      Terminator fin, List<GenSolvablePolynomial<C>> G, CriticalPairList<C> L) {
    pool = fin;
    this.G = G;
    pairlist = L;
  ...
  public void run() {
    ...
    while (pairlist.hasNext() || pool.hasJobs()) {
      ...
      S =
          sred.leftSPolynomial(
              (GenSolvablePolynomial<C>) pair.pi, (GenSolvablePolynomial<C>) pair.pj);
      if (S.isZERO()) {
...
class TwosidedSolvableReducerSeqPair<C extends RingElem<C>> implements Runnable {
  ...
  TwosidedSolvableReducerSeqPair(
      Terminator fin,
      List<GenSolvablePolynomial<C>> X,
