...
public class TanhRules {
  ...
  final public static IAST RULES = List(IInit(Tanh, SIZES),
      ...
      ISet(Tanh(Undefined), Undefined, true),
      // Tanh(0)=0
      ISet(Tanh(C0), C0, true),
      // Tanh(I*1/4*Pi)=I
      ISet(Tanh(Times(CC(0L, 1L, 1L, 4L), Pi)), CI, true),
      // Tanh(I*1/3*Pi)=Sqrt(3)*I
      ISet(Tanh(Times(CC(0L, 1L, 1L, 3L), Pi)), Times(CI, CSqrt3), true),
      // Tanh(I*1/2*Pi)=ComplexInfinity
      ISet(Tanh(Times(CC(0L, 1L, 1L, 2L), Pi)), CComplexInfinity, true),
      // Tanh(I*2/3*Pi)=-Sqrt(3)*I
      ISet(Tanh(Times(CC(0L, 1L, 2L, 3L), Pi)), Times(CNI, CSqrt3), true),
      // Tanh(I*3/4*Pi)=-I
      ISet(Tanh(Times(CC(0L, 1L, 3L, 4L), Pi)), CNI, true),
      // Tanh(I*5/6*Pi)=-I/Sqrt(3)
      ISet(Tanh(Times(CC(0L, 1L, 5L, 6L), Pi)), Times(CNI, C1DSqrt3), true),
      // Tanh(I*Pi)=0
      ISet(Tanh(Times(CI, Pi)), C0, true),
      ...
      // Tanh(Infinity)=1
      ISet(Tanh(oo), C1, true),
      // Tanh(ComplexInfinity)=Indeterminate
      ISet(Tanh(CComplexInfinity), Indeterminate, true));
}
