...
/**
 * ResidueSolvablePolynomial generic solvable polynomials with solvable residue coefficients
 * implementing RingElem. n-variate ordered solvable polynomials over solvable residue coefficients.
...
public class ResidueSolvablePolynomial<C extends GcdRingElem<C>>
   ...
   * @param e exponent.
   */
  public ResidueSolvablePolynomial(
      ResidueSolvablePolynomialRing<C> r, SolvableResidue<C> c, ExpVector e) {
    this(r);
    if (c != null && !c.isZERO()) {
      val.put(e, c);
    }
   ...
   */
  public ResidueSolvablePolynomial(
      ResidueSolvablePolynomialRing<C> r, GenSolvablePolynomial<SolvableResidue<C>> S) {
    this(r, S.getMap());
  }
   ...
   */
  protected ResidueSolvablePolynomial(
      ResidueSolvablePolynomialRing<C> r, SortedMap<ExpVector, SolvableResidue<C>> v) {
    this(r);
    ...
    for (Map.Entry<ExpVector, SolvableResidue<C>> y : A.entrySet()) {
      ...
      for (Map.Entry<ExpVector, SolvableResidue<C>> x : Bk) {
        ...
        if (ring.table.isEmpty() || Cps.isConstant() || f.isZERO()) { // symmetric
          ...
          if (Cps.isConstant()) {
            Ds =
                new ResidueSolvablePolynomial<C>(
  ...
  /**
   * ResidueSolvablePolynomial left and right multiplication. Product with two polynomials.
   *
   * @param S ResidueSolvablePolynomial.
   * @param T ResidueSolvablePolynomial.
   * @return S*this*T.
   */
  public ResidueSolvablePolynomial<C> multiply(
      ResidueSolvablePolynomial<C> S, ResidueSolvablePolynomial<C> T) {
  ...
  /**
   * ResidueSolvablePolynomial left and right multiplication. Product with coefficient ring element.
   *
   * @param b coefficient polynomial.
   * @param c coefficient polynomial.
  ...
  /**
   * ResidueSolvablePolynomial left and right multiplication. Product with exponent vector.
   *
   * @param e exponent.
   * @param f exponent.
  ...
  /**
   * ResidueSolvablePolynomial multiplication. Product with ring element and exponent vector.
   *
   * @param b coefficient polynomial.
   * @param e exponent.
  ...
  /**
   * ResidueSolvablePolynomial left and right multiplication. Product with ring element and exponent
   * vector.
   *
   * @param b coefficient polynomial.
   * @param e exponent.
  ...
  @Override
  public ResidueSolvablePolynomial<C> multiply(
      SolvableResidue<C> b, ExpVector e, SolvableResidue<C> c, ExpVector f) {
    if (b == null || b.isZERO()) {
      return ring.getZERO();
    }
    if (c == null || c.isZERO()) {
  ...
  /**
   * ResidueSolvablePolynomial multiplication. Left product with ring element and exponent vector.
   *
   * @param b coefficient polynomial.
   * @param e exponent.
  ...
  /**
   * ResidueSolvablePolynomial multiplication. Left product with exponent vector.
   *
   * @param e exponent.
  ...
  /**
   * ResidueSolvablePolynomial multiplication. Left product with coefficient ring element.
   *
   * @param b coefficient polynomial.
