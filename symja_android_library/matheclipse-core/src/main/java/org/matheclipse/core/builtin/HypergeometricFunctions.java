...
public class HypergeometricFunctions {
  ...
  private static class Hypergeometric1F1 extends AbstractFunctionEvaluator
    ...
    /**
     * Whether <code>Gamma(a, 0)</code> is not the ordinary <code>Gamma(a)</code>.
     *
     * <p>
     * The integral for the incomplete gamma converges at zero only where the real part of
     * <code>a</code> is positive, so an identity written with <code>Gamma(a, 0)</code> says
     * <code>ComplexInfinity</code> for any other number: <code>Hypergeometric1F1(-1/2, 1/2, z)</code>
     * came back as <code>ComplexInfinity</code> for a function whose value at <code>z == 2</code> is
     * <code>-2.069</code>. A symbolic <code>a</code> is left to the identity, which is the one
     * Mathematica gives.
     */
    private static boolean gammaAtZeroDiverges(IExpr a) {
      return a.isNumber() && !a.re().isPositiveResult();
    }
  ...
  private static class Hypergeometric2F1Regularized extends AbstractFunctionEvaluator
 {
  ...
  private static class HypergeometricU extends AbstractFunctionEvaluator
    ...
    /**
     * Fixed z and <code>a=ni+1/2, b=mi+1/2</code>.
     * <p>
     * See <a href=
     * "https://functions.wolfram.com/HypergeometricFunctions/HypergeometricU/03/01/04/07/0005/">HypergeometricFunctions/HypergeometricU/03/01/04/07/0005/</a>
     * 
     * @param ni
     * @param mi
     * @param z
     * @return
     */
    private static IExpr functionExpandHalfHalfZ(final int ni, final int mi, IExpr z) {
      IInteger n = F.ZZ(ni);
      IInteger m = F.ZZ(mi);
      ...
      IExpr sum1 = F.sum(k -> F.Times(F.Power(F.Factorial(k), F.CN1), F.Factorial(F.Plus(k, n)),
