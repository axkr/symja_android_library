...
public class Scale implements Comparable<Scale> {
  ...
  public static Scale of(float x) {
    ...
    return Scale.valueOf(
        Math.min(Precision.FLOAT_PRECISION - Magnitude.of(x), -Magnitude.ZERO_FLOAT_MAGNITUDE));
  }

  public static Scale of(double x) {
    // see comment above
    return Scale.valueOf(
        Math.min(Precision.DOUBLE_PRECISION - Magnitude.of(x), -Magnitude.ZERO_DOUBLE_MAGNITUDE));
  }
  ...
  /**
   * The error a computation with this scale should not exceed. This is just half of an ulp for
   * values with this scale, thus computation results should be correct to this scale after
   * rounding.
   *
