...
/**
 * Critical pair list management. Makes some effort to produce the same sequence of critical pairs
 * as in the sequential case, when used in parallel. However already reduced pairs are not
 * re-reduced if new polynomials appear. Implemented using GenPolynomial, SortedSet / TreeSet and
 * BitSet.
 *
 * @author Heinz Kredel
 */
public class CriticalPairList<C extends RingElem<C>> extends OrderedPairlist<C> {
  ...
  /**
   * Get and remove the next required pair from the pairlist. Appy the criterions 3 and 4 to see if
   * the S-polynomial is required. The pair is not removed from the pair list.
   *
  ...
  /**
   * Get the next required pair from the pairlist. Appy the criterions 3 and 4 to see if the
   * S-polynomial is required. The pair is not removed from the pair list.
   *
   * @return the next pair if one exists, otherwise null.
   */
  ...
  /**
   * Record reduced polynomial and update critical pair list. Note: it is better to use record and
   * uptate separately.
   *
   * @param pair the corresponding critical pair.
   * @param p polynomial.
  ...
  /**
   * Update pairlist, several pairs at once. This version does not preserve the sequential pair
   * sequence. Remove pairs with completed reductions.
   *
   * @return the number of added polynomials.
   */
