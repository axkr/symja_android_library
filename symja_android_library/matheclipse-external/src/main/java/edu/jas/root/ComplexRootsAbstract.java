...
public abstract class ComplexRootsAbstract<C extends RingElem<C> & Rational>
  ...
  public C magnitudeBound(Rectangle<C> rect, GenPolynomial<Complex<C>> f) {
    ...
    GenPolynomial<Complex<C>> fa =
        f.map(
            new UnaryFunctor<Complex<C>, Complex<C>>() {
    ...
    while (work) {
      try {
        while (root.rationalLength().compareTo(length) > 0) {
          // System.out.println("root = " + root + ", len = " + new
          // BigDecimal(root.rationalLength()));
          if (delta == null) {
   ...
   */
  public abstract Rectangle<C> invariantRectangle(
   ...
   */
  public Complex<BigDecimal> approximateRoot(
      Rectangle<C> rt, GenPolynomial<Complex<C>> f, BigRational eps) throws NoConvergenceException {
    if (rt == null) {
    ...
    GenPolynomialRing<Complex<BigDecimal>> dfac =
        new GenPolynomialRing<Complex<BigDecimal>>(cr, f.ring);
    ...
    while (i++ < MITER) {
      ...
      //             if ( false ) { // not useful:
      //                 Complex<BigDecimal> fxx  = PolyUtil.<Complex<BigDecimal>> evaluateMain(cr,
      // df, dx); // f(dx)
      ...
      //                         throw new NoConvergenceException("no convergence after " + i + "
      // steps");
      //                     }
      ...
      //                     System.out.println("dx = " + dx);
      //                     fxx  = PolyUtil.<Complex<BigDecimal>> evaluateMain(cr, df, dx); //
      // f(dx)
      //                     //System.out.println("fxx = " + fxx);
      //                     fsx = fxx.norm().getRe();
      //                     System.out.println("fsx = " + fsx);
   ...
   * @return v with v a new rectangle contained in rect such that |g(a) - g(b)| &lt; eps for a, b in
   *     v in rect.
   */
  public Rectangle<C> invariantMagnitudeRectangle(
      Rectangle<C> rect, GenPolynomial<Complex<C>> f, GenPolynomial<Complex<C>> g, BigRational eps)
      throws InvalidBoundaryException {
    Rectangle<C> v = rect;
    if (g == null || g.isZERO()) {
      return v;
   ...
   *
   * @param rect root isolating rectangle for f which contains exactly one root, with rect such that
   *     |g(a) - g(b)| &lt; eps for a, b in rect.
   * @param f univariate polynomial, non-zero.
   * @param g univariate polynomial, gcd(f,g) == 1.
   * @return g(rect) .
   */
  public Complex<C> complexRectangleMagnitude(
      Rectangle<C> rect, GenPolynomial<Complex<C>> f, GenPolynomial<Complex<C>> g) {
   ...
   * @return g(rect) .
   */
  public Complex<C> complexMagnitude(
      Rectangle<C> rect, GenPolynomial<Complex<C>> f, GenPolynomial<Complex<C>> g, BigRational eps)
      throws InvalidBoundaryException {
    if (g.isZERO() || g.isConstant()) {
      return g.leadingBaseCoefficient();
