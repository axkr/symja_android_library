...
/**
 * Distributed version of a HashTable. Implemented with a SortedMap / TreeMap to keep the sequence
 * order of elements.
 *
 * @author Heinz Kredel
 */
public class DistHashTable<K, V> extends AbstractMap<K, V> /* implements Map<K,V> */ {
  ...
  /**
   * Put object to the distributed hash table. Blocks until the key value pair is send and received
   * from the server.
   *
   * @param key
   * @param value
  ...
  /**
   * Get value under key from DHT. Blocks until the object is send and received from the server
