...
public abstract class L4MObject {
  ...
  protected static <T extends Numeric<T>> Complex asComplex(final T x) {
    if (!isAComplex(x)) throw new IllegalAccessError(format(NOT_COMPLEX_S, x));
   ...
   */
  public static <T extends Field<T> & Factory<T>> T[][] create(
      final T x, final int n, final int m) {
    @SuppressWarnings("unchecked")
  ...
  protected static <T extends Numeric<T>> T[] asT(final T[] o, final Object[] x) {
    ...
    for (int j = 0; j < x.length; j++) ret[j] = asT(o[0], x[j]);
    return ret;
  }
  ...
  protected static <T extends Numeric<T>> T[][] asT(final T[][] o, final Object[][] x) {
    ...
    for (int j = 0; j < x.length; j++) ret[j] = asT(o[j], x[j]);
    return ret;
  }
  ...
  /**
   * Utility method to load an implementation of the interface type T. Internally this method uses
   * the ServiceLoader/L4MLoader mechanism.
   *
