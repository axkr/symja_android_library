...
/**
 * Iterative Gauss-Seidel Solver for A*x=y. Starting from a random vector x_0 a sequence x_n
 * converging against the solution is generated, if the matrix norm ||A|| is less than one, e.g. A
 * is strict diagonal dominant.
 *
 * @author nwulff
...
public final class GaussSeidelSolver extends AbstractIterativeSolver {
  ...
  protected void updateX(
      final double[][] a, final double[] y, final double[] x, final double[] xo) {
    int j, n = a.length;
    double s;
    double[] ak;
    for (int k = 0; k < n; x[k] = s / ak[k], k++)
      for (ak = a[k], s = y[k], j = 0; j < n; j++) if (j != k) s -= ak[j] * x[j];
  }
  ...
  @Override
  protected void updateX(
      final int[] p, final double[][] a, final double[] y, final double[] x, final double[] xo) {
    int j, k, n = a.length;
    double s;
    double[] ak;
   ...
   */
  @Override
  protected <T extends Numeric<T>> void updateX(
      final T[][] a, final T[] y, final T[] x, final T[] xo) {
    int j, n = a.length;
    T s;
    T[] ak;
    for (int k = 0; k < n; x[k] = s.div(ak[k]), k++)
      for (ak = a[k], s = y[k], j = 0; j < n; j++) if (j != k) s = s.minus(ak[j].multiply(x[j]));
  }
  ...
  @Override
  protected <T extends Numeric<T>> void updateX(
      final int[] p, final T[][] a, final T[] y, final T[] x, final T[] xo) {
    int j, k, n = a.length;
    T s;
    T[] ak;
