...
/**
 * ExpVector implements exponent vectors for polynomials. Exponent vectors are implemented as arrays
 * of Java elementary types, like long, int, short and byte. ExpVector provides also the familiar
 * MAS static method names. The implementation is only tested for nonnegative exponents but should
 * work also for negative exponents. Objects of this class are intended to be immutable, but
 * exponents can be set (during construction); also the hash code is only computed once, when
 * needed. The different storage unit implementations are <code>ExpVectorLong</code> <code>
 * ExpVectorInteger</code>, <code>ExpVectorShort</code> and <code>ExpVectorByte</code>. The static
 * factory methods <code>create()</code> of <code>ExpVector</code> select the respective storage
 * unit. The selection of the desired storage unit is internally done via the static variable <code>
 * storunit</code>. This varaible should not be changed dynamically.
 *
 * @author Heinz Kredel
 */
public abstract class ExpVector implements AbelianGroupElem<ExpVector> {
  ...
  /**
   * Used storage representation of exponent arrays. <b>Note:</b> Set this only statically and not
   * dynamically.
   */
  ...
  /**
   * Factory constructor for ExpVector. Converts a String representation to an ExpVector. Accepted
   ...
   *
   * @return number of bits in the representation of this ExpVector, including sign bits.
   */
