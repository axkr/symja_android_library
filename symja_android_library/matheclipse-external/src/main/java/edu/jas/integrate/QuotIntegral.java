...
/**
 * Container for a rational function integral, quotient version . integral(num/den) = pol + sum_rat(
 * rat_i/rat_{i+1} ) + sum_log( a_i log ( d_i ) )
 *
 * @author Heinz Kredel
...
public class QuotIntegral<C extends GcdRingElem<C>> implements Serializable {
   ...
   */
  public QuotIntegral(
      Quotient<C> r, GenPolynomial<C> p, List<GenPolynomial<C>> rat, List<LogIntegral<C>> log) {
    quot = r;
