...
public class Array2DRowFieldMatrix<T extends FieldElement<T>>
    ...
    public Array2DRowFieldMatrix<T> add(final Array2DRowFieldMatrix<T> m)
        ...
        for (int row = 0; row < rowCount; row++) {
            ...
            for (int col = 0; col < columnCount; col++) {
                outDataRow[col] = dataRow[col].sum(mRow[col]);
            }
        }
        ...
        for (int row = 0; row < nRows; row++) {
            ...
            for (int col = 0; col < nCols; col++) {
                ...
                for (int i = 0; i < nSum; i++) {
                    sum = sum.sum(dataRow[i].multiply(m.data[i][col]));
                }
                outDataRow[col] = sum;
            }
        }
    ...
    public void addToEntry(final int row, final int column, final T increment)
        ...
        data[row][column] = data[row][column].sum(increment);
    }
    ...
    public T[] operate(final T[] v) throws DimensionMismatchException {
        ...
        for (int row = 0; row < nRows; row++) {
            ...
            for (int i = 0; i < nCols; i++) {
                sum = sum.sum(dataRow[i].multiply(v[i]));
            }
            out[row] = sum;
        }
        return out;
    ...
    public T[] preMultiply(final T[] v) throws DimensionMismatchException {
        ...
        for (int col = 0; col < nCols; ++col) {
            ...
            for (int i = 0; i < nRows; ++i) {
                sum = sum.sum(data[i][col].multiply(v[i]));
            }
            out[col] = sum;
        }
