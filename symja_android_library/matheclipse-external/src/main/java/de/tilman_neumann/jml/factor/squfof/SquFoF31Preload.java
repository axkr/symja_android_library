...
/**
 * Shanks' SQUFOF algorithm, 31-bit version. This version works without fail for N <= 2^52 and is
 * faster than SquFoF63 for N <= 2^51.
 *
 * <p>This version preloads the square-free sequence, but that makes hardly a difference.
 *
...
public class SquFoF31Preload extends FactorAlgorithm {
  ...
  /**
   * Find a factor of the given composite N.</br></br> <em>Warning:</em> This method will not return
   * when called with a prime argument.
   *
  ...
  public long findSingleFactor(long N) {
    ...
    for (int baseMultiplierIndex = 0;
        baseMultiplierIndex < BASE_MULTIPLIERS.length;
        baseMultiplierIndex++) {
      ...
      while (true) {
        ...
        // The cast may be wrong for some bigger kN, but fixing the cast would mean a small
        // performance
