    ...
    public static <S extends RealFieldElement<S>> FieldODEStateAndDerivative<S> taylor(final FieldODEStateAndDerivative<S> reference,
        ...
        for (int i = nData.length - 1; i >= 0; --i) {
            ...
            for (int j = 0; j < nDataI.length; ++j) {
                ...
                stateVariation[j]          = stateVariation[j].sum(d);
                estimatedDerivatives[j] = estimatedDerivatives[j].sum(d.multiply(order));
            }
        }

        S[] estimatedState = reference.getState();
        for (int j = 0; j < stateVariation.length; ++j) {
            stateVariation[j]    = stateVariation[j].sum(scaled[j].multiply(normalizedAbscissa));
            estimatedState[j] = estimatedState[j].sum(stateVariation[j]);
            estimatedDerivatives[j] =
                estimatedDerivatives[j].sum(scaled[j].multiply(normalizedAbscissa)).divide(x);
        }
