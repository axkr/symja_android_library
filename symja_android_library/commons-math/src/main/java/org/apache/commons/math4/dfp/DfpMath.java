...
public class DfpMath {
    ...
    protected static Dfp[] split(final Dfp a) {
        ...
        result[0] = a.sum(shift).subtract(shift);
    ...
    protected static Dfp[] splitMult(final Dfp[] a, final Dfp[] b) {
        ...
        result[1] = a[0].multiply(b[1]).sum(a[1].multiply(b[0])).sum(a[1].multiply(b[1]));
    ...
    protected static Dfp[] splitDiv(final Dfp[] a, final Dfp[] b) {
        ...
        result[1] = result[1].divide(b[0].multiply(b[0]).sum(b[0].multiply(b[1])));
    ...
    protected static Dfp splitPow(final Dfp[] base, int a) {
        ...
        result[0] = result[0].sum(result[1]);
    ...
    public static Dfp log(Dfp a) {
        ...
        while (spx[0].sum(spx[1]).greaterThan(spy[0])) {
        ...
        spz[0] = spz[0].sum(spy[0]);
        spz[1] = spz[1].sum(spy[1]);
        ...
        return a.newInstance(spz[0].sum(spz[1]));
    ...
    protected static Dfp[] logInternal(final Dfp a[]) {
         ...
         */
        Dfp t = a[0].divide(4).sum(a[1].divide(4));
        Dfp x = t.sum(a[0].newInstance("-0.25")).divide(t.sum(a[0].newInstance("0.25")));
    ...
    public static Dfp atan(final Dfp a) {
        ...
        final Dfp ty = sqr2Split[0].subtract(one).sum(sqr2Split[1]);
        ...
        if (sub) {
            y = y.sum(piSplit[0].divide(8)).sum(piSplit[1].divide(8));
        }

        if (recp) {
            y = piSplit[0].divide(2).subtract(y).sum(piSplit[1].divide(2));
        }
