...
/**
 * RingFactory Tokenizer. Used to read ring factories from input streams. It can also read
 * QuotientRing factory.
 *
...
public class RingFactoryTokenizer {
  ...
  public RingFactory nextCoefficientRing() throws IOException {
    ...
    if (tok.sval != null) {
      if (tok.sval.equalsIgnoreCase("Q")) {
        ...
        logger.warn(
            "parse of quaternion coefficients may fail for negative components (use ~ for -)");
        ...
        if (tt == '[') {
          ...
          if (vs != 1) {
            throw new InvalidExpressionException(
                "AlgebraicNumber only for univariate polynomials " + Arrays.toString(anv));
          }
          String[] ovars = vars;
          vars = anv;
   ...
   * </pre>
   *
   * semantics: <code>p_{n+1} * p_{n+2} = p_{n+3}</code>. The next relation table is stored into the
