...
/**
 * Multivariate power series reduction sequential use algorithm. Implements Mora normal-form
 * algorithm.
 *
...
/* extends ReductionAbstract<C> */ {
   ...
   */
  public MultiVarPowerSeries<C> normalform(
      List<MultiVarPowerSeries<C>> Pp, MultiVarPowerSeries<C> Ap) {
   ...
   */
  public MultiVarPowerSeries<C> totalNormalform(
      List<MultiVarPowerSeries<C>> P, MultiVarPowerSeries<C> A) {
    if (P == null || P.isEmpty()) {
      return A;
    }
    if (A == null) {
    ...
    MultiVarCoefficients<C> Rc =
        new MultiVarCoefficients<C>(A.ring) {
