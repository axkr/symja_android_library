 ...
 *
 * <p>See ALDES/SAC2 or MAS code in SACPRIM. See Symja <code>
 * org/matheclipse/core/expression/Primality.java</code> for Pollard algorithm.
 *
 * @author Heinz Kredel
 */
public final class PrimeInteger {
  ...
  public static final List<Long> SMPRM = smallPrimes(2, (int) (IMPDS_MIN >> 1));
   ...
   * @return n/F a factor of n not divisible by any prime number in SMPRM.
   */
  public static java.math.BigInteger smallPrimeDivisors(
      java.math.BigInteger n, SortedMap<java.math.BigInteger, Integer> F) {
    List<Long> LP;
  ...
  public static SortedMap<Long, Integer> factors(long n) {
    ...
    do {
      ...
      if (PL != 1L) {
        ...
        AL = PL;
        CL =
            Roots.sqrtInt(BigInteger.valueOf(ML)).getVal().longValue(); // SACI.ISQRT( ML, CL, TL );
        // System.out.println("CL = " + CL + ", ML = " + ML + ", CL^2 = " + (CL*CL));
        BL = Math.min(BL, CL);
        if (AL > BL) {
          PL = 1L;
  ...
  public static long largePrimeDivisorSearch(long n, long a, long b) { // return PL, NLP ignored
    ...
    while (XL >= XL1) {
      ...
      YL =
          Roots.sqrtInt(BigInteger.valueOf(YLP)).getVal().longValue(); // SACI.ISQRT( YLP, YL, TL );
   ...
   *     modified.
   */
  public static void factorsPollardRho(
      java.math.BigInteger n, SortedMap<java.math.BigInteger, Integer> F) {
