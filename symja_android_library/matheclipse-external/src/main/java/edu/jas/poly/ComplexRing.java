...
/**
 * Generic Complex ring factory implementing the RingFactory interface. Objects of this class are
 * immutable.
 *
 * @param <C> base type.
 * @author Heinz Kredel
...
public class ComplexRing<C extends RingElem<C>> implements RingFactory<Complex<C>> {
  ...
  public AlgebraicNumberRing<C> algebraicRing() {
    GenPolynomialRing<C> pfac =
        new GenPolynomialRing<C>(ring, TermOrderByName.INVLEX, new String[] {"I"});
    GenPolynomial<C> I = pfac.univariate(0, 2L).sum(pfac.getONE());
    AlgebraicNumberRing<C> afac =
        new AlgebraicNumberRing<C>(I, ring.isField()); // must indicate field
    return afac;
  }
  ...
  /**
   * Complex number random. Random base numbers A and B are generated using random(n). Then R is the
   * complex number with real part A and imaginary part B.
   *
