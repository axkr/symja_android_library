...
IIntegrate(609,Int(Power(Plus(a_,Times(b_DEFAULT,x_),Times(c_DEFAULT,Sqr(x_))),p_),x_Symbol),
    Condition(Simp(Times(Plus(b,Times(C2,c,x)),Power(Plus(a,Times(b,x),Times(c,Sqr(x))),p),Power(Times(C2,c,Plus(Times(C2,p),C1)),-1)),x),And(FreeQ(List(a,b,c,p),x),EqQ(Plus(Sqr(b),Times(CN1,C4,a,c)),C0),NeQ(p,Negate(C1D2)))));
