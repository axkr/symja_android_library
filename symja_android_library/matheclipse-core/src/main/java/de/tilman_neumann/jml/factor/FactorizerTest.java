...
public class FactorizerTest {
	...
	public FactorizerTest() {
		algorithms = new SingleFactorFinder[] {
				...
				new SquFoF63(), new SquFoF31(), new SquFoF31Preload(),
				...
				// SIQS:
				// * TDiv: UBI-variants are better than BigInteger-variants; nLarge is better than 1Large for N >= 200
				// bit
				// * BlockLanczos is better than Gauss for about N>200 bit
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new SimpleSieve(),
				// new TDiv_QS_1Large(), 10, new MatrixSolver01_Gauss(), false),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03g(), new
				// TDiv_QS_1Large_UBI(), 10, new MatrixSolver01_Gauss(), false),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03gU(), new
				// TDiv_QS_1Large_UBI(), 10, new MatrixSolver01_Gauss(), false),

				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03g(), new
				// TDiv_QS_1Large_UBI(), 10, new MatrixSolver02_BlockLanczos(), false),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03g(), new
				// TDiv_QS_2Large_UBI(), 10, new MatrixSolver02_BlockLanczos(), false),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03g(), new
				// TDiv_QS_nLarge(), 10, new MatrixSolver02_BlockLanczos(), true),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03g(), new
				// TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(), true),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new Sieve03gU(), new
				// TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(), true),

				// sieving with prime powers: best sieve for small N!
				// new SIQS(0.32F, 0.37F, null, null, new PowerOfSmallPrimesFinder(), new SIQSPolyGenerator(), new
				// Sieve03gU(), new TDiv_QS_1Large_UBI(), 10, new MatrixSolver01_Gauss(), false),
				// new SIQS(0.32F, 0.37F, null, null, new PowerOfSmallPrimesFinder(), new SIQSPolyGenerator(), new
				// Sieve03gU(), new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(), true),
				// new SIQS(0.32F, 0.37F, null, null, new AllPowerFinder(), new SIQSPolyGenerator(), new Sieve03gU(),
				// new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(), true),
				...
				// SingleBlockSieve(32768), new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(), false),
				// new SIQS(0.32F, 0.385F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new
				// SingleBlockSieveU(32768), new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(), true),
				// new SIQS(0.32F, 0.41F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new
				// DoubleBlockSieve(32768, 131072), new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(),
				// false),
				...
				// SingleBlockHybridSieve01(32768), new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(),
				// true),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new
				// SingleBlockHybridSieveU(32768), new TDiv_QS_nLarge_UBI(), 10, new MatrixSolver02_BlockLanczos(),
				// true),
				// new SIQS(0.32F, 0.37F, null, null, new NoPowerFinder(), new SIQSPolyGenerator(), new
				// DoubleBlockHybridSieve01(32768, 131072), new TDiv_QS_nLarge_UBI(), 10, new
				...
				// Multi-threaded SIQS:
				// * 4/6 threads takes over at N around 100 bit (more exact estimates: 4 threads best for N>=88 bits, 6
				// threads for N>=112 bits)
				// * we need 0.14 < maxQRestExponent < 0.2; everything else is prohibitive; use null for dynamic
				// determination
				...
				// true),
				// new PSIQS_SBH_U(0.32F, 0.37F, null, null, 32768, 6, new PowerOfSmallPrimesFinder(), new
				// MatrixSolver02_BlockLanczos(), true), // best for large N
	...
	private void testRange(int bits) {
		...
		// take REPEATS timings for each algorithm to be quite sure that one timing is not falsified by garbage
		// collection
		...
		for (int i = 0; i < REPEATS; i++) {
			for (SingleFactorFinder algorithm : algorithms) {
				...
				if (bits < 57 && algName.startsWith("PSIQS"))
					continue; // unstable for smaller N
				if (bits > 98 && algName.startsWith("CFrac63"))
					continue; // unstable for N>98 bits
				if (bits > 52 && algName.startsWith("SquFoF31"))
					continue; // int implementation
				if (bits > 60 && algName.startsWith("Lehman"))
					continue;
				if (bits > 31 && algName.startsWith("TDiv31"))
					continue; // int implementation
				if (bits > 31 && algName.startsWith("PollardRho31"))
					continue; // long implementation
				if (bits > 42 && algName.startsWith("TDiv63Inverse"))
					continue; // not enough primes stored
				...
				for (BigInteger N : NSet) {
					...
					if (factor == null || factor.equals(I_0) || factor.equals(I_1) || factor.mod(N).equals(I_0)) {
						// LOG.error("FactorAlgorithm " + algorithm.getName() + " did not find a factor of N=" + N + ",
						// it returned " + factor);
						failExample = N;
						failCount++;
					} else {
						...
						if (!test[1].equals(I_0)) {
							// LOG.error("FactorAlgorithm " + algorithm.getName() + " returned " + factor + ", but this
							// is not a factor of N=" + N);
							failExample = N;
							failCount++;
						}
					}
				...
				if (algList == null)
					algList = new ArrayList<SingleFactorFinder>();
				algList.add(algorithm);
				ms_2_algorithms.put(duration, algList);
				if (failCount > 0) {
					// LOG.error("FactorAlgorithm " + algorithm.getName() + " failed at " + failCount + "/" + N_COUNT
					// + " test numbers, e.g. for N = " + failExample);
				}
			}
		}
