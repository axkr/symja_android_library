...
public class KPermutationsIterator implements Iterator<int[]>, Iterable<int[]> {
	 ...
	 *
	 */
	public KPermutationsIterator(IStepVisitor handler, final int parts) {
		super();
		this.fHandler = handler;
		final int[] data = handler.getMultisetArray();
		n = data.length;
		k = parts;
		// f = fun;
		fPermutationsIndex = new int[n];
		y = new int[n];
		fCopiedResultIndex = new int[k];
		for (int a = 0; a < n; a++) {
	...
	// public <T> KPermutationsIterable(final List<T> fun, final int parts, final
	// int headOffset) {
	// super();
	...
	public int[] next() {
		System.arraycopy(fResultIndex, 0, fCopiedResultIndex, 0, k);
		fResultIndex = nextBeforehand();
