...
/**
 * (Non-unique) factorization domain greatest common divisor common algorithms with monic polynomial
 * remainder sequence. Fake implementation always returns 1 for any gcds.
 *
 * @param <C> coefficient type
 * @author Heinz Kredel
 */
public class GreatestCommonDivisorLR<C extends GcdRingElem<C>>
  ...
  /**
   * Univariate GenSolvablePolynomial greatest common divisor. Uses pseudoRemainder for remainder.
   *
  ...
  public GCDcoFactors<C> leftRightBaseGcd(GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S) {
    ...
    if (debug) {
      // System.out.println("contP = " + contP + ", contS = " + contS + ", leftGcd(contP,contS) = "
      // + contPS);
      C r1 = contP.leftDivide(contPS);
      boolean t = contPS.multiply(r1).equals(contP);
      if (!t) {
        System.out.println(
            "r1: " + r1 + " * " + contPS + " != " + contP + ", r1*cP=" + contPS.multiply(r1));
      }
      C r2 = contS.leftDivide(contPS);
      t = contPS.multiply(r2).equals(contS);
      if (!t) {
        System.out.println(
            "r2: " + r2 + " * " + contPS + " != " + contS + ", r2*cS=" + contPS.multiply(r2));
      }
    ...
    while (!r.isZERO()) {
      ...
      if (field) {
        r = x.monic();
        // System.out.println("baseGCD: lc(q) = " + q.leadingBaseCoefficient() + ", lc(r) = " +
        // r.leadingBaseCoefficient());
      } else {
        r = x;
      }
      // System.out.println("baseGCD: q = " + q + ", r = " + r);
    ...
    if (debug) {
      ...
      if (!t) {
        GenSolvablePolynomial<C> p1q =
            (GenSolvablePolynomial<C>) leftBasePrimitivePart(p1.multiply(q)).abs();
      ...
      if (!t) {
        GenSolvablePolynomial<C> s1q =
            (GenSolvablePolynomial<C>) leftBasePrimitivePart(s1.multiply(q)).abs();
  ...
  public GCDcoFactors<C> rightLeftBaseGcd(GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S) {
    ...
    if (debug) {
      // System.out.println("contP = " + contP + ", contS = " + contS + ", rightGcd(contP,contS) = "
      // + contPS);
      C r1 = contP.rightDivide(contPS);
      boolean t = r1.multiply(contPS).equals(contP);
      if (!t) {
        System.out.println(
            "r1: " + r1 + " * " + contPS + " != " + contP + ", r1*cP=" + r1.multiply(contPS));
      }
      C r2 = contS.rightDivide(contPS);
      t = r2.multiply(contPS).equals(contS);
      if (!t) {
        System.out.println(
            "r2: " + r2 + " * " + contPS + " != " + contS + ", r2*cS=" + r2.multiply(contPS));
      }
   ...
   */
  @Override
  public GenSolvablePolynomial<C> leftBaseGcd(
   ...
   */
  @Override
  public GenSolvablePolynomial<C> rightBaseGcd(
      GenSolvablePolynomial<C> P, GenSolvablePolynomial<C> S) {
    if (S == null || S.isZERO()) {
      return P;
    }
  ...
  /**
   * Univariate GenSolvablePolynomial left recursive greatest common divisor. Uses pseudoRemainder
   * for remainder.
   *
   * @param P univariate recursive GenSolvablePolynomial.
   * @param S univariate recursive GenSolvablePolynomial.
   * @return 1 = gcd(P,S) with P = P'*gcd(P,S)*p and S = S'*gcd(P,S)*s, where deg_main(p) =
   *     deg_main(s) == 0.
   */
  @Override
