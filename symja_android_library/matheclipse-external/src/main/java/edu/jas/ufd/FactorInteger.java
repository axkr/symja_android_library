...
/**
 * Integer coefficients factorization algorithms. This class implements factorization methods for
...
public class FactorInteger<MOD extends GcdRingElem<MOD> & Modular>
  ...
  public FactorInteger(RingFactory<BigInteger> cfac) {
    super(cfac);
    ModularRingFactory<MOD> mcofac =
        (ModularRingFactory<MOD>) (Object) new ModLongRing(13, true); // hack
  ...
  /**
   * Factor search with modular Hensel lifting algorithm. Let p = f_i.ring.coFac.modul() i = 0, ...,
   * n-1 and assume C == prod_{0,...,n-1} f_i mod p with ggt(f_i,f_j) == 1 mod p for i != j
   *
   ...
   * @return [g_0,...,g_{n-1}] = lift(C,F), with C = prod_{0,...,n-1} g_i mod p**e. <b>Note:</b>
   *     does not work in all cases.
   */
  List<GenPolynomial<BigInteger>> searchFactorsMonic(
      GenPolynomial<BigInteger> C, BigInteger M, List<GenPolynomial<MOD>> F, BitSet D) {
  ...
  public List<GenPolynomial<BigInteger>> factorsSquarefree(GenPolynomial<BigInteger> P) {
    ...
    if (degv.length() == donv.length) { // all variables appear, hack for Hensel, check
      ...
      // GenPolynomial<BigInteger> pl = PolyUtil.<BigInteger> removeUnusedLowerVariables(pu); // not
      // useful
      try {
  ...
  public List<GenPolynomial<BigInteger>> factorsSquarefreeHensel(GenPolynomial<BigInteger> P) {
    ...
    GenPolynomial<BigInteger> prrc =
        engine.recursiveContent(prr); // can have content wrt this variable
    ...
    if (!prrc.isONE()) {
      ...
      List<GenPolynomial<BigInteger>> cff =
          new ArrayList<GenPolynomial<BigInteger>>(cfactors.size());
    ...
    List<GenPolynomial<BigInteger>> lfacs =
        new ArrayList<GenPolynomial<BigInteger>>(lfactors.keySet());
    ...
    while (notLucky) { // for Wang's test
      if (Math.abs(evStart) > 371L) {
        ...
        //    logger.warn("some evaluation points found after " + Math.abs(evStart) + " iterations,
        // tParts = " + tParts);
        //    break;
        // }
        logger.warn(
            "no evaluation point for: P = "
                + P
                + ", prr = "
                + prr
                + ", lprr = "
      ...
      for (int j = pfac.nvar; j > 1; j--) {
        ...
        vi =
            Evs.get(j); // evStart + j;//0L; //(long)(pfac.nvar-j); // 1L; 0 not so good for small p
        BigInteger Vi;
        ...
        while (Evset.size() != Evs.size()) {
          ...
          Evs.set(j, vgi);
          Evset.clear();
          Evset.addAll(Evs); // = new HashSet<Long>(Evs);
          evStart = vgi;
      ...
      if (!isMonic) {
        ...
        // System.out.println("lfacs = " + lfacs + ", cei = " + cei + ", ped = " + ped + ", pecw = "
        // + pecw);
        ...
        for (BigInteger ci : cei) {
          ...
          if (q.isONE()) {
            ...
            if (!testSeparate(cei, pecw)) {
              ...
              if (countSeparate > COUNT_MAX) {
                logger.info(
                    "too many inseparable evaluation points: "
                        + countSeparate
                        + ", removing "
                        + pecw);
              }
      ...
      if (!isMonic || !pecw.isONE()) {
        ...
        for (int i = ufactors.size() - 1; i >= 0; i--) {
          ...
          int ii = 0;
          for (BigInteger ci : cei) {
            // System.out.println("ci = " + ci + ", lfp = " + lfp + ", lfacs.get(ii) = " +
            // lfacs.get(ii));
            if (ci.abs().isONE()) {
              System.out.println(
                  "ppl = "
        ...
        if (!pec.isONE()) { // content, was always false by hack
          ...
          if (!pec.isONE()) { // still not 1
            ...
            if (pes.equals(Power.<GenPolynomial<BigInteger>>multiply(pe.ring, un))) {
              ...
              // System.out.println("*pe != prod(un): " + Power.<GenPolynomial<BigInteger>>
              // multiply(pe.ring,un));
            }
          }
        }
        ...
        // logger.warn("V = " + V + ", pe = " + pe + ", cei = " + cei + ", lf = " + lf + ", ln = " +
        // ln);
...
class TrialParts {
  ...
  public TrialParts(
      ...
      List<GenPolynomial<BigInteger>> uf,
      List<BigInteger> le,
