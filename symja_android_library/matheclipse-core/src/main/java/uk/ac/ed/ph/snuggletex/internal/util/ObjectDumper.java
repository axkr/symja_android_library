...
/**
 * Trivial but slightly useful helper for creating deep dumps of Objects and their its properties, which can be useful
 * for displaying status messages and debugging.
 *
...
public final class ObjectDumper {
	...
	/**
	 * Convenience method to do a {@link DumpMode#DEEP} dump of a single Object and return the resulting String.
	 */
	...
	/**
	 * Does a dump of the given Object, using whatever {@link DumpMode} is providied for its class. If the Object is
	 * null, then {@link DumpMode#TO_STRING} is used.
	 */
	 ...
	 * @param object
	 * @param dumpMode
	 *            if {@link DumpMode#DEEP} then the Object is dumped deeply, if {@link DumpMode#TO_STRING} then the
	 *            Object's {@link #toString()} is called, if {@link DumpMode#IGNORE} then nothing happens.
	 */
	...
	private void appendArray(Object[] array, DumpMode dumpMode, int depth) {
		...
		result.append(componentType.getName()).append("[]@")
				// array.hashCode() - changed because of SpotBugs issue:
				.append(Integer.toHexString(java.util.Arrays.hashCode(array))).append("[");
		if (array.length == 0) {
			result.append(EMPTY);
		} else {
			result.append("\n");
	...
	private void appendObjectDeep(Object object, int depth) {
		...
		result.append(objectClass.getName()).append("@").append(Integer.toHexString(object.hashCode())).append("(");
		...
		for (Method method : methods) {
			/*
			 * See if we've got a getter method. If so, extract property name (with first char still capitalised)
			 */
			...
			/*
			 * Make first character after 'get' or 'is' lower case so that it really is the name of the property
			 */
			...
			/*
			 * If no DumpMode was specified for this property, then take the DumpMode of the target's class. If nothing
			 * is specified then we'll use TO_STRING.
			 */
	...
	/**
	 * Works out the most appropriate {@link DumpMode} for the given Object by inspecting its class for the relevant
	 * annotation.
	 * 
	 * @param object
	 *            Object being dumped
	 * @param currentDumpMode
	 *            DumpMode currently in operation, used if nothing is specified for the Object's class or if the Object
	 *            is null.
	 */
