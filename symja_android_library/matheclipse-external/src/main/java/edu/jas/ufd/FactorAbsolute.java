...
/**
 * Absolute factorization algorithms class. This class contains implementations of methods for
 * factorization over algebraically closed fields. The required field extension is computed along
 * with the factors. The methods have been tested for prime fields of characteristic zero, that is
 * for <code>BigRational</code>. It might eventually also be used for prime fields of non-zero
 * characteristic, that is with <code>ModInteger</code>. The field extension may yet not be minimal.
 *
 * @author Heinz Kredel
...
public abstract class FactorAbsolute<C extends GcdRingElem<C>> extends FactorAbstract<C> {
   ...
   * @return factors list container: [p_1,...,p_k] with P = prod_{i=1, ..., k} p_i. <b>Note:</b>
   *     K(alpha) not yet minimal.
   */
  // @Override
  ...
  /**
   * Univariate GenPolynomial algebraic partial fraction decomposition, Absolute factorization for
   * elementary integration algorithm to linear factors.
   *
