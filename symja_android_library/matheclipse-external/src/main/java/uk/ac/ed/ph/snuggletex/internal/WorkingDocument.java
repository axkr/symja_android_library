...
/**
 * Represents the "working view" of the current {@link SnuggleInput} document. This is slightly non-trivial in that
 * user-defined command and environments generally work as simple text substitutions, so the state of the document
 * changes during tokenisation. However, we also need to be able to keep track of where each character in the document
...
public final class WorkingDocument {
	...
	public static final class CharacterSource {
		...
		/**
		 * The block that this block was initially substituted into, or null to indicate that it is "top level" or was
		 * appended rather than substituted.
		 */
		public final CharacterSource substitutedSource;

		/**
		 * The offset in {@link #substitutedSource} where the substitution occurred, or 0 if {@link #substitutedSource}
		 * is null.
		 */
		public final int substitutionOffset;

		/**
		 * The text in the parent component that this data replaced, or null if {@link #substitutedSource} is null.
		 */
		...
		public String toString() {
			if (stringRepresentation == null) {
				StringBuilder resultBuilder = new StringBuilder(getClass().getSimpleName()).append("(context=")
						.append(context);
				if (substitutedSource != null) {
					resultBuilder.append(",substituted=").append(substitutedSource).append(",offset=")
							.append(substitutionOffset).append(",substitutedText=").append(substitutedText);
				}
	...
	/**
	 * Represents a contiguous portion of the document hat comes from the same {@link CharacterSource}. During parsing,
	 * {@link Slice}s will be broken up to create new Slices whenever user-defined commands and environments are
	 * substituted.
	 */
	public static final class Slice {
		...
		public Slice(final int startIndex, final int endIndex, final CharacterSource resolvedComponent,
				final int componentIndexOffset) {
		...
		@Override
		public String toString() {
			return getClass().getSimpleName() + "(span=[" + startIndex + "," + endIndex + ") => " + resolvedComponent
					+ "; offset " + componentIndexOffset + ")";
		}
	}

	/**
	 * Holds information about how a particular index in the document is resolved against the {@link Slice} in which it
	 * is contained.
	 *
	 * @author David McKain
	 * @version $Revision: 564 $
	...
	public static class IndexResolution {
		...
		@Override
		public String toString() {
			return getClass().getSimpleName() + "(sbIndex=" + scoreboardIndex + ", slice=" + slice
					+ ", indexInComponent=" + indexInComponent + ")";
		}
	}

	/**
	 * Marker interface that can be applied to classes that provide information for a {@link CharacterSource}.
	 */
	...
	/**
	 * This is a rather nasty method which resets the "freeze index" of this document back to the given index. Any
	 * {@link FrozenSlice}s that may previously have spanned this index are no longer valid as their contents may
	 * change.
	 * <p>
	 * This class does NOT attempt to enforce this in any way - it is up to the caller to make sure he/she understands
	 * the implications of doing this.
	 * <p>
	 * The main requirement for having this is when evaluating single-argument user-defined commands like
	 * <tt>\\udc \\alpha</tt> where the argument is implicitly taken to be the next valid token.
	 * 
	...
	public IndexResolution resolveIndex(final int index, final boolean fallLeftOnBoundaries) {
		...
		if (fallLeftOnBoundaries && index == 0) {
			/*
			 * If falling to left at index 0, then we miss the first component completely. This is not useful in
			 * practice!
			 */
			return null;
		}
		...
		/*
		 * Search for the slice containing this index. We'll search from the index of the last successful call to here
		 * (0 on first call) since most calls to this method are made on indices quite close together.
		 */
	...
	public CharacterSource substitute(final int startIndex, final int endIndex, final CharSequence replacement) {
		checkRange(startIndex, endIndex);
		...
		if (startIndex < freezeIndex) {
			throw new IllegalArgumentException("Cannot modify frozen part of document (startIndex=" + startIndex
					+ ",freezeIndex=" + freezeIndex + ",attemptedText="
					+ buffer.substring(freezeIndex, Math.min(freezeIndex + 20, length)) + ")");
		}
		...
		if (startResolution == null) {
			/*
			 * Special case - we are *appending* to the document, rather than substituting existing content. This case
			 * is somewhat simpler to deal with so we will do it all now.
			 */
		...
		/*
		 * If still here, then we're doing a genuine substitution (or insertion). The first thing we do is modify the
		 * StringBuilder.
		 */
		...
		/*
		 * Next up, we have to rebuild the scoreboard. This is much more complicated so is best demonstrated with some
		 * examples.
		 * 
		 * First note that the scoreboard will change from the slice containing to the startIndex all the way to the
		 * end.
		 * 
		 * Let's assume for convenience that there are 4 entries in the scoreboard and that the resolved startIndex lies
		 * within the entry with index 1 (i.e. the second entry).
		 * 
		 * [..............)[...............)[..................)[..............)
		 * 
		 * Expl 1: [XXXXXXXXX) is being replaced with [YYYYYYY) to give
		 * [..............)[YYYYYYY)[...)[..................)[..............) This stays repl left- next component
		 * similarly the same over has offset shifted
		 * 
		 ...
		 * [..............)[)[YYYYYYY)[.)[..................)[..............) Stays same cut repl left shift shift
		 * 
		 * Here we need to create a slice out of the second component before continuing in the same way.
		 * 
		 * Expl 3: [XXXXXXXXXXXXXXXXXXXXX) is being replaced with [Y) to give
		 * [..............)[Y)[..........)[..............) Stays same repl slice shift left
		 * 
		 * Here, the replacement spans more than 1 slice. The original 3rd slice needs to be cut down and shifted left.
		 * The 4th slice is just shifted left. Phew!!!
		 */

		/*
		 * Anyway, in all cases we start off by keeping the slices that lie before the one containing the start of the
		 * substitution.
		 */
		...
		/*
		 * Next up, we look at the slice containing the start of the substitution, making a new slice to reflect
		 * "the bit before" if it is required. The offset of this component does not change.
		 */
		Slice startSlice = startResolution.slice;
		if (startIndex > startSlice.startIndex) {
			Slice bitBefore = new Slice(startSlice.startIndex, startIndex, startSlice.resolvedComponent,
					startSlice.componentIndexOffset);
			newScoreBoard.add(bitBefore);
		}

		/*
		 * Now we create a slice corresponding to the substitution. The offset of this slice is set to ensure that
		 * currentIndex corresponds to index 0 in the substitution - i.e. shift left by currentIndex.
		 */
		...
		/*
		 * Next we need to find out where the end of the substitution is. All slices before this one will be removed
		 * from the scoreboard. From the current end slice, we will re-slice it if required to include whatever comes
		 * immediately after the substitution end point.
		 * 
		 * To calculate what happens to the offset, we look at 'endIndex' (before) and 'currentIndex' (after). Both must
		 * map to the same point in the original character source. Thus:
		 * 
		...
		if (endResolution != null) { /* (Will be null if endIndex==0, i.e. insertion at start) */
			...
			if (endIndex < endSlice.endIndex) {
				...
				Slice bitAfter = new Slice(currentIndex, currentIndex + bitAfterSize, endSlice.resolvedComponent,
						resultOffset);
		...
		/*
		 * Finally, we include shifted versions of all slices that followed the end index in the original scoreboard.
		 * The change in offset is calculated by looking at the left endpoint of the slice before and after shifted,
		 * which must map to the same point in the original character source. Thus we have:
		 * 
		...
		if (buffer.length() != currentIndex) {
			throw new SnuggleLogicException(
					"Failed sanity check: buffer length is " + buffer.length() + ", last board index=" + currentIndex);
		}
		length = currentIndex;
		return result;
	}
	...
	private void checkIndex(final int index, final String errorStart) {
		if (!(index >= 0 && index <= length)) {
			throw new IndexOutOfBoundsException(
					errorStart + " " + index + " is outwith the current bounds [0," + length + ")");
		}
	}
