...
public class Hart_AnalyzeSquareCongruences extends FactorAlgorithm {
	...
	public long findSingleFactor(long N) {
		...
		try {
			...
			LOG.error(getName() + ": Failed to factor N=" + N + ". Either it has factors < cbrt(N) needing trial division, or the arrays are too small.", e);
			return 1;
		}
		return 0;
	}
	...
	public static void main(String[] args) {
		...
		for (int rest=0; rest<MOD; rest++) {
			...
			LOG.info("(kN+1)%" + MOD + "=" + rest + ": adjust = [(kN+1)-a] % (2^" + bestAdjustExp + ") has sMin=" + maxSMin);
