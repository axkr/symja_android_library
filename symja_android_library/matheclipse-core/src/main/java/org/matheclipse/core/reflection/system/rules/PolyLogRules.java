...
public class PolyLogRules {
  ...
  final public static IAST RULES = List(IInit(PolyLog, SIZES),
      // PolyLog(2,-1)=-Pi^2/12
      ISet(PolyLog(C2, CN1), Times(QQ(-1L, 12L), Sqr(Pi)), true),
      // PolyLog(2,1)=Pi^2/6
      ISet(PolyLog(C2, C1), Times(QQ(1L, 6L), Sqr(Pi)), true),
      // PolyLog(2,1/2)=Pi^2/12-Log(2)^2/2
      ISet(PolyLog(C2, C1D2), Plus(Times(QQ(1L, 12L), Sqr(Pi)), Times(CN1D2, Sqr(Log(C2)))), true),
      // PolyLog(2,2)=Pi^2/4+(-1)*I*Pi*Log(2)
      ISet(PolyLog(C2, C2), Plus(Times(C1D4, Sqr(Pi)), Times(CNI, Pi, Log(C2))), true),
      // PolyLog(2,I)=I*Catalan-Pi^2/48
      ISet(PolyLog(C2, CI), Plus(Times(CI, Catalan), Times(QQ(-1L, 48L), Sqr(Pi))), true),
      // PolyLog(2,-I)=-I*Catalan-Pi^2/48
      ISet(PolyLog(C2, CNI), Plus(Times(CNI, Catalan), Times(QQ(-1L, 48L), Sqr(Pi))), true),
      ...
      ISet(PolyLog(C3, C1D2),
          Times(QQ(1L, 24L),
              Plus(Times(CN2, Sqr(Pi), Log(C2)), Times(C4, Power(Log(C2), C3)),
                  Times(ZZ(21L), Zeta(C3)))),
          true),
      // PolyLog(i_Integer,z_):=Module({n=-i},Sum(Sum((-1)^(k+1)*Binomial(n+1,-1+k)*(-k+m+1)^n,{k,1,m})*z^m,{m,1,n})/(1-z)^(n+1)/;i<0)
      ISetDelayed(PolyLog($p(i, Integer), z_), Module(list(Set(n, Negate(i))), Condition(Times(
          Power(Power(Subtract(C1, z), Plus(n, C1)), CN1),
          Sum(Times(Sum(Times(Power(-1, Plus(k, C1)), Binomial(Plus(n, C1), Plus(CN1, k)),
              Power(Plus(Negate(k), m, C1), n)), list(k, C1, m)), Power(z, m)), list(m, C1, n))),
          Less(i, C0)))),
      ...
      ISet(PolyLog(CN1, C2, C1D2), Log(C2), true));
}
