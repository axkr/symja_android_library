...
public class SumTest extends AbstractTestCase {
  ...
  public void testSum002() {
    ...
    check(
        ...
        "Cos(x)");
    check(
        "Sum(1/(i^d),{i,1,Infinity})", //
        "Zeta(d)");
    check(
        "Sum(1/(i^(-d)),{i,1,Infinity})", //
        "Zeta(-d)");
    check(
        "Sum(x^(2*i+1)/(2*i+1)!,{i,0,Infinity})", //
        "Sinh(x)");
    check(
        "Sum(x^(2*i)/(2*i)!,{i,0,Infinity})", //
        "Cosh(x)");
    check(
  ...
  public void testSum003() {
    check(
        "Sum(1/Binomial(2*i,i), {i,1,Infinity})", //
        "1/27*(9+2*Sqrt(3)*Pi)");
    check(
        "Sum(1/(i*Binomial(2*i,i)), {i,1,Infinity})", //
        "Pi/(3*Sqrt(3))");
    check(
        "Sum(1/(i^2*Binomial(2*i,i)), {i,1,Infinity})", //
        "Pi^2/18");
    check(
        "Sum((-1)^(i-1)/i, {i,1,Infinity})", //
        "Log(2)");
  ...
  public void testSum005() {
    ...
    check(
        ...
        "b+c+d+e");
    check(
        "Sum(a*f, {a, {b, c, d, e}}, {f, {g, h}})", //
        "b*g+c*g+d*g+e*g+b*h+c*h+d*h+e*h");

    check(
        "Sum(f(k,j),{k,0,-1+2}, {j,0,-1+k})", //
        "f(1,0)");
    check(
        "Sum(k, {k, 1, n})", //
        "1/2*n*(1+n)");
  ...
  public void testSum007() {
    ...
    check(
        "Simplify(c*n*(-i+n)+1/2*c*(i-n)*n*(1+i+n)+c*(1/2*n*(-i+n)+1/2*(-i+n)*n^2))", //
        "1/2*c*(2-i)*n*(-i+n)");
  ...
  public void testSum009() {
    ...
    check(
        "Sum(i0^3+p^2, {i0, 0, n0})", //
        "n0^2/4+n0^3/2+n0^4/4+(1+n0)*p^2");
    check(
        "Sum(Binomial(n0,i0), {i0, 0, n0})", //
        "2^n0");
    check(
        "sum(i0*binomial(n0,i0), {i0, 0, n0})", //
        "n0/2^(1-n0)");
    check(
        "sum(p, {i0, 1, n0})", //
        "n0*p");
