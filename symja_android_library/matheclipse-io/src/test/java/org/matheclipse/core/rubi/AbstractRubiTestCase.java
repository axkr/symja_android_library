...
public abstract class AbstractRubiTestCase extends TestCase {
  ...
  /**
   * Symbolic zero test for {@code D(result, x) - integrand}: {@code PossibleZeroQ} escalated
   * through {@code Together} and {@code Simplify}. Works when Symja can differentiate and simplify
   * cleanly.
   */
   ...
   * This avoids the symbolic derivative entirely, so it verifies antiderivatives (e.g.
   * {@code RootSum} or special-function forms) whose symbolic {@code D} Symja cannot simplify.
   */
