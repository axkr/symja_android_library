...
public class SortedList<T> extends ArrayList<T> {
  ...
  /**
   * In descending order the biggest elements according to Comparator or Comparable are sorted in at
   * the beginning.
   */
  private SortOrder sortOrder = null;

  /**
   * Complete constructor for a list sorted in ascending or descending order, where the comparison
   * is done by an explicit constructor or the comparable capability of list elements.
   *
   ...
   *
   * <p>The argument list should be sortable after the same criteria like this, using the same
  ...
  /**
   * Compares internal and external object and returns how the insort relation looks like (before,
   * equals, after).
   *
   * @param internalObj Internal object, already contained by this.
   * @param externalObj External object, to be sorted in
   * @return <0 / 0 / >0 if internal object shall be before / no matter where / behind the external
   *     object.
   */
    ...
    if ((internalObj != null) && (externalObj != null)) {
      ...
      if (this.explicitComparator != null) {
        ...
        throw new IllegalStateException(
            "SortedList.compare(): Sorting not possible, "
                + internalObj.getClass()
                + " is not Comparable and an external Comparator has not been given.");
      }
