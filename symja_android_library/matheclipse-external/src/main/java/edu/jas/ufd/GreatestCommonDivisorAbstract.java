...
public abstract class GreatestCommonDivisorAbstract<C extends GcdRingElem<C>>
  ...
  public List<GenPolynomial<GenPolynomial<C>>> recursivePrimitivePart(
    ...
    List<GenPolynomial<GenPolynomial<C>>> Pp =
        new ArrayList<GenPolynomial<GenPolynomial<C>>>(F.size());
   ...
   */
  @SuppressWarnings("unused")
  public GenPolynomial<GenPolynomial<C>> recursiveUnivariateResultant(
      GenPolynomial<GenPolynomial<C>> P, GenPolynomial<GenPolynomial<C>> S) {
    throw new UnsupportedOperationException("not implmented");
  }
   ...
   * @throws UnsupportedOperationException if there is no implementation in the sub-class.
   */
  public GenPolynomial<GenPolynomial<C>> recursiveResultant(
      GenPolynomial<GenPolynomial<C>> P, GenPolynomial<GenPolynomial<C>> S) {
    if (S == null || S.isZERO()) {
      return S;
    }
  ...
  /**
   * GenPolynomial resultant. The input polynomials are considered as univariate polynomials in the
   * main variable.
   *
   * @param P GenPolynomial.
   * @param S GenPolynomial.
  ...
  /**
   * Univariate GenPolynomial extended greatest common divisor. Uses sparse pseudoRemainder for
   * remainder.
   *
   * @param P univariate GenPolynomial.
   * @param S univariate GenPolynomial.
  ...
  /**
   * Univariate GenPolynomial half extended greatest comon divisor. Uses sparse pseudoRemainder for
   * remainder.
   *
   * @param S GenPolynomial.
   ...
   */
  @SuppressWarnings("unchecked")
  public GenPolynomial<C>[] baseGcdDiophant(
      GenPolynomial<C> P, GenPolynomial<C> S, GenPolynomial<C> c) {
   ...
   * @param P univariate GenPolynomial.
   * @param S univariate GenPolynomial.
   * @return [ A0, Ap, As ] with A/(P*S) = A0 + Ap/P + As/S with deg(Ap) < deg(P) and deg(As) <
   *     deg(S).
   */
  @SuppressWarnings("unchecked")
  public GenPolynomial<C>[] basePartialFraction(
      GenPolynomial<C> A, GenPolynomial<C> P, GenPolynomial<C> S) {
    GenPolynomial<C>[] ret = (GenPolynomial<C>[]) new GenPolynomial[3];
    ret[0] = null;
    ret[1] = null;
    ret[2] = null;
   ...
   * @return true if A/prod(D) = F0 + sum( Fi/Di ) with deg(Fi) < deg(Di), Fi in F, else false.
   */
  public boolean isBasePartialFraction(
      GenPolynomial<C> A, List<GenPolynomial<C>> D, List<GenPolynomial<C>> F) {
   ...
   * @param e exponent for P.
   * @param F list of univariate GenPolynomials from a partial fraction computation.
   * @return true if A/(P^e) = F0 + sum( Fi / P^i ) with deg(Fi) < deg(P), Fi in F, else false.
   */
  public boolean isBasePartialFraction(
      GenPolynomial<C> A, GenPolynomial<C> P, int e, List<GenPolynomial<C>> F) {
   ...
   */
  public GenPolynomial<C> basePartialFractionValue(
      GenPolynomial<C> P, int e, List<GenPolynomial<C>> F) {
